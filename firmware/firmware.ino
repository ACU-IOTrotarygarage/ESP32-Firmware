/*
  Firmware.ino - Firmware firmware for iTead Sonoff, Wemos, NodeMCU, ESP8266 and ESP32 hardwares
...
#endif // ESP32_STAGE
#include "include/Firmware_compat.h"
#include "include/Firmware_version.h"        // Firmware version information
#include "include/Firmware.h"                // Enumeration used in my_user_config.h
#include "my_user_config.h"                 // Fixed user configurable options
#ifdef USE_TLS
  #include <t_bearssl.h>                    // We need to include before "Firmware_globals.h" to take precedence over the BearSSL version in Arduino
#endif // USE_TLS
#include "include/Firmware_globals.h"        // Function prototypes and global configuration
#include "include/i18n.h"                   // Language support configured by my_user_config.h
#include "include/Firmware_template.h"       // Hardware configuration
...
// Structs
#include "include/Firmware_types.h"
...
struct FirmwareGlobal_t {
  ...
  uint8_t init_state;                       // Firmware init state
  ...
  uint8_t restart_flag;                     // Firmware restart flag
...
#else
  #define BACKLOG_EMPTY (FirmwareGlobal.backlog_pointer == FirmwareGlobal.backlog_index)
#endif
...
void setup(void) {
  ...
  memset(&FirmwareGlobal, 0, sizeof(FirmwareGlobal));
  FirmwareGlobal.baudrate = APP_BAUDRATE;
  FirmwareGlobal.seriallog_timer = SERIALLOG_TIMER;
  FirmwareGlobal.temperature_celsius = NAN;
  FirmwareGlobal.blinks = 201;
  FirmwareGlobal.wifi_state_flag = WIFI_RESTART;
  FirmwareGlobal.tele_period = 9999;
  FirmwareGlobal.active_device = 1;
  FirmwareGlobal.global_state.data = 0xF;  // Init global state (wifi_down, mqtt_down) to solve possible network issues
  FirmwareGlobal.enable_logging = 1;
  FirmwareGlobal.seriallog_level = LOG_LEVEL_INFO;  // Allow specific serial messages until config loaded
  ...
  if (RtcSettingsLoad(0)) {
    ...
    if (baudrate) { FirmwareGlobal.baudrate = baudrate; }
  }
  ...
  FirmwareGlobal.log_buffer = (char*)malloc(LOG_BUFFER_SIZE);    // Allocate in "new" 16k heap space
  ESP.resetHeap();
  if (FirmwareGlobal.log_buffer == nullptr) {
    FirmwareGlobal.log_buffer = (char*)malloc(LOG_BUFFER_SIZE);  // Allocate in "old" heap space as fallback
  }
  if (FirmwareGlobal.log_buffer != nullptr) {
    FirmwareGlobal.log_buffer[0] = '\0';
  }
#endif  // PIO_FRAMEWORK_ARDUINO_MMU_CACHE16_IRAM48_SECHEAP_SHARED
  if (Settings == nullptr) {
  ...
  Serial.begin(FirmwareGlobal.baudrate);
  Serial.println();
...
//  AddLog(LOG_LEVEL_INFO, PSTR("ADR: Settings %p, Log %p"), Settings, FirmwareGlobal.log_buffer);
#ifdef ESP32
  ...
  FirmwareGlobal.seriallog_level = Settings->seriallog_level;
  FirmwareGlobal.syslog_level = Settings->syslog_level;

  FirmwareGlobal.module_changed = (Settings->module != Settings->last_module);
  if (FirmwareGlobal.module_changed) {
  ...
  FirmwareGlobal.sleep = Settings->sleep;
...
//  AddLog(LOG_LEVEL_INFO, PSTR("DBG: FirmwareGlobal size %d, data %100_H"), sizeof(FirmwareGlobal), (uint8_t*)&FirmwareGlobal);
  ...
  if (Settings->param[P_BOOT_LOOP_OFFSET]) {         // SetOption36
    ...
    if (RtcReboot.fast_reboot_count > Settings->param[P_BOOT_LOOP_OFFSET]) {       // Restart twice
      ...
      if (RtcReboot.fast_reboot_count > Settings->param[P_BOOT_LOOP_OFFSET] +2) {  // Restarted 4 times
        ...
        FirmwareGlobal.no_autoexec = true;
      }
...
//        Settings->last_module = Settings->fallback_module;
  ...
  memcpy_P(FirmwareGlobal.version, VERSION_MARKER, 1);  // Dummy for compiler saving VERSION_MARKER

  snprintf_P(FirmwareGlobal.version, sizeof(FirmwareGlobal.version), PSTR("%d.%d.%d"), VERSION >> 24 & 0xff, VERSION >> 16 & 0xff, VERSION >> 8 & 0xff);  // Release version 6.3.0
  if (VERSION & 0xff) {  // Development or patched version 6.3.0.10
    snprintf_P(FirmwareGlobal.version, sizeof(FirmwareGlobal.version), PSTR("%s.%d"), FirmwareGlobal.version, VERSION & 0xff);
  }

  // Thehackbox inserts "release" or "commit number" before compiling using sed -i -e 's/PSTR("(%s)")/PSTR("(85cff52-%s)")/g' Firmware.ino
  snprintf_P(FirmwareGlobal.image_name, sizeof(FirmwareGlobal.image_name), PSTR("(%s)"), PSTR(CODE_IMAGE_STR));  // Results in (85cff52-Firmware) or (release-Firmware)

  Format(FirmwareGlobal.mqtt_client, SettingsText(SET_MQTT_CLIENT), sizeof(FirmwareGlobal.mqtt_client));
  Format(FirmwareGlobal.mqtt_topic, SettingsText(SET_MQTT_TOPIC), sizeof(FirmwareGlobal.mqtt_topic));
  if (strchr(SettingsText(SET_HOSTNAME), '%') != nullptr) {
    SettingsUpdateText(SET_HOSTNAME, WIFI_HOSTNAME);
    snprintf_P(FirmwareGlobal.hostname, sizeof(FirmwareGlobal.hostname)-1, SettingsText(SET_HOSTNAME), FirmwareGlobal.mqtt_topic, ESP_getChipId() & 0x1FFF);
  } else {
    snprintf_P(FirmwareGlobal.hostname, sizeof(FirmwareGlobal.hostname)-1, SettingsText(SET_HOSTNAME));
  }
  char *s = FirmwareGlobal.hostname;
  while (*s) {
    if (!(isalnum(*s) || ('.' == *s))) { *s = '-'; }                 // Valid hostname chars are A..Z, a..z, 0..9, . and -
    if ((s == FirmwareGlobal.hostname) && ('-' == *s)) { *s = 'x'; }  // First char cannot be a dash so replace by an x
    s++;
  }
  snprintf_P(FirmwareGlobal.mqtt_topic, sizeof(FirmwareGlobal.mqtt_topic), ResolveToken(FirmwareGlobal.mqtt_topic).c_str());
  ...
  FirmwareGlobal.init_state = INIT_GPIOS;
  ...
  AddLog(LOG_LEVEL_INFO, PSTR(D_PROJECT " %s - %s " D_VERSION " %s%s-" ARDUINO_CORE_RELEASE "(%s)"),
    PSTR(PROJECT), SettingsText(SET_DEVICENAME), FirmwareGlobal.version, FirmwareGlobal.image_name, GetBuildDateAndTime().c_str());
#ifdef FIRMWARE_MINIMAL
  ...
  FirmwareGlobal.rules_flag.system_init = 1;
}

void BacklogLoop(void) {
  if (TimeReached(FirmwareGlobal.backlog_timer)) {
    if (!BACKLOG_EMPTY && !FirmwareGlobal.backlog_mutex) {
      FirmwareGlobal.backlog_mutex = true;
      bool nodelay = false;
      ...
      do {
        ...
        cmd = backlog.shift();
#else
        cmd = FirmwareGlobal.backlog[FirmwareGlobal.backlog_pointer];
        FirmwareGlobal.backlog[FirmwareGlobal.backlog_pointer] = (const char*) nullptr;  // Force deallocation of the String internal memory
        FirmwareGlobal.backlog_pointer++;
        if (FirmwareGlobal.backlog_pointer >= MAX_BACKLOG) { FirmwareGlobal.backlog_pointer = 0; }
#endif
      ...
      if (nodelay || FirmwareGlobal.backlog_nodelay) {
        FirmwareGlobal.backlog_timer = millis();  // Reset backlog_timer which has been set by ExecuteCommand (CommandHandler)
      }
      FirmwareGlobal.backlog_mutex = false;
    }
    if (BACKLOG_EMPTY) {
      FirmwareGlobal.backlog_nodelay = false;
    }
  }
}

void SleepDelay(uint32_t mseconds) {
  if (!FirmwareGlobal.backlog_nodelay && mseconds) {
...
// MDNS.update() needs to be called in main loop
  ...
  if (!FirmwareGlobal.serial_local) { SerialInput(); }
#ifdef ESP32
...
void loop(void) {
  ...
  if (Settings->flag3.sleep_normal) {              // SetOption60 - Enable normal sleep instead of dynamic sleep
    ...
    SleepDelay(FirmwareGlobal.sleep);               // https://github.com/esp8266/Arduino/issues/2021
  } else {
    if (my_activity < (uint32_t)FirmwareGlobal.sleep) {
      SleepDelay((uint32_t)FirmwareGlobal.sleep - my_activity);  // Provide time for background tasks like wifi
    } else {
  ...
  uint32_t loop_delay = FirmwareGlobal.sleep;
  if (!loop_delay) { loop_delay++; }               // We cannot divide by 0
  uint32_t loops_per_second = 1000 / loop_delay;   // We need to keep track of this many loops per second
  uint32_t this_cycle_ratio = 100 * my_activity / loop_delay;
  FirmwareGlobal.loop_load_avg = FirmwareGlobal.loop_load_avg - (FirmwareGlobal.loop_load_avg / loops_per_second) + (this_cycle_ratio / loops_per_second); // Take away one loop average away and add the new one
}
