/*
  xlgt_05_sonoff_l1.ino - Sonoff L1 led support for Firmware
...
#define SONOFF_L1_START_DELAY                // Sync Nuvotron power state with Firmware on power up
...
bool SnfL1SerialInput(void) {
  if (FirmwareGlobal.serial_in_byte != 0x1B) {
    if (FirmwareGlobal.serial_in_byte_counter >= SONOFF_L1_BUFFER_SIZE) {
      FirmwareGlobal.serial_in_byte_counter = 0;
    }
    if (FirmwareGlobal.serial_in_byte_counter || (!FirmwareGlobal.serial_in_byte_counter && ('A' == FirmwareGlobal.serial_in_byte))) {  // A from AT
      FirmwareGlobal.serial_in_buffer[FirmwareGlobal.serial_in_byte_counter++] = FirmwareGlobal.serial_in_byte;
    }
  } else {
    FirmwareGlobal.serial_in_buffer[FirmwareGlobal.serial_in_byte_counter++] = 0x00;
...
#ifdef SONOFF_L1_DEBUG1
    AddLog(LOG_LEVEL_DEBUG, PSTR("SL1: Rcvd '%s'"), FirmwareGlobal.serial_in_buffer);
#endif
    if (!strncmp(FirmwareGlobal.serial_in_buffer +3, "RESULT", 6)) {
...
/*
      ...
      char *end_str;
      char *string = FirmwareGlobal.serial_in_buffer +10;
...
//      AddLog(LOG_LEVEL_DEBUG, PSTR("Sl1: ++Busy %d"), Snfl1.busy);
    ...
    else if (!strncmp(FirmwareGlobal.serial_in_buffer +3, "UPDATE", 6)) {
      char cmnd_dimmer[20];
      char cmnd_color[20];
      char *end_str;
      char *string = FirmwareGlobal.serial_in_buffer +10;
...
bool SnfL1ModuleSelected(void) {
  if (SONOFF_L1 == FirmwareGlobal.module_type) {
    if (PinUsed(GPIO_RXD) && PinUsed(GPIO_TXD)) {
      ...
      if (Snfl1.buffer) {
        ...
        FirmwareGlobal.light_driver = XLGT_05;
