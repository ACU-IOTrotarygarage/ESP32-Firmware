/*
  xlgt_03_sm16716.ino - sm16716 three channel led support for Firmware
 ...
 *
 * Source: https://community.home-assistant.io/t/cheap-uk-wifi-bulbs-with-Firmware-teardown-help-tywe3s/40508/27
\*********************************************************************************************/
...
/*
  ...
  for (uint32_t i = 3; i < Light.subtype; i++) {
    if (PinUsed(GPIO_PWM1, i-3)) {
      ...
      analogWrite(Pin(GPIO_PWM1, i-3), bitRead(FirmwareGlobal.pwm_inverted, i-3) ? Settings->pwm_range - cur_col_10bits[i] : cur_col_10bits[i]);
    }
  }
*/
...
void Sm16716ModuleSelected(void)
{
  if (PinUsed(GPIO_SM16716_CLK) && PinUsed(GPIO_SM16716_DAT)) {
    ...
    FirmwareGlobal.light_type += LST_RGB;    // Add RGB to be controlled by sm16716
    FirmwareGlobal.light_driver = XLGT_03;
