/*
  xlgt_02_my92x1.ino - led support for Firmware
...
void My92x1ModuleSelected(void)
{
  if (PinUsed(GPIO_DCKI) && PinUsed(GPIO_DI)) {
    ...
    My92x1.model = 2;
    FirmwareGlobal.light_type = LT_RGBW;                 // RGBW (2 chips) as used in Lohas
#ifdef ESP8266
    if (AILIGHT == FirmwareGlobal.module_type) {         // RGBW (1 chip) as used in Ailight
      My92x1.model = 0;
//      FirmwareGlobal.light_type = LT_RGBW;
    }
    else if (SONOFF_B1 == FirmwareGlobal.module_type) {  // RGBWC (2 chips) as used in Sonoff B1
      My92x1.model = 1;
    ...
    FirmwareGlobal.light_driver = XLGT_02;
