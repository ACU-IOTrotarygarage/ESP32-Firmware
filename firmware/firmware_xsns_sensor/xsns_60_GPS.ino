/*
  xsns_60_GPS.ino - GPS UBLOX support for Firmware

  Copyright (C) 2021  Theo Arends and Mahmoud Shams El-Din, Christian Baars and Adrian Scillato
...
/*********************************************************************************************\
  ...
  ---
  0.9.1.0 20191216  integrate - Added pin specifications from Firmware WEB UI. Minor tweaks.
  ---
  0.9.0.0 20190817  started   - further development by Christian Baars  - https://github.com/Staars/Sonoff-Firmware
                    forked    - from arendst/Firmware                    - https://github.com/arendst/Sonoff-Firmware
...
## Usage:
The serial pins are GPS_RX and GPS_TX, no further installation steps needed. To get more debug information compile it with option "DEBUG_Firmware_SENSOR".
...
+ sensor60 11
  force update of Firmware-system-UTC with every new GPS-time-message

+ sensor60 12
  do not update of Firmware-system-UTC with every new GPS-time-message
...
void UBXDetect(void)
{
  ...
  if (PinUsed(GPIO_GPS_RX) && PinUsed(GPIO_GPS_TX)) {
    UBXSerial = new FirmwareSerial(Pin(GPIO_GPS_RX), Pin(GPIO_GPS_TX), 1, 0, UBX_SERIAL_BUFFER_SIZE); // 64 byte buffer is NOT enough
...
void UBXsendHeader(void)
{
  ...
  Webserver->sendHeader(F("Content-Disposition"), F("attachment; filename=Firmware.gpx"));
  WSSend(200, CT_APP_STREAM, F(
    "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\" ?>\r\n"
    "<GPX version=\"1.1\" creator=\"Firmware\" xmlns=\"http://www.topografix.com/GPX/1/1\" \r\n"
...
void UBXSelectMode(uint16_t mode)
{
  ...
  switch(mode){
    ...
    case 8:
      ...
      break;
    case 9:
      if (!FirmwareGlobal.global_state.network_down && timeServer.beginListening()) {
...
void UBXLoop50msec(void)
{
  ...
  // handle NTP-server
  if(!FirmwareGlobal.global_state.network_down && UBX.mode.runningNTP){
...
//  const char UBX_GOOGLE_MAPS[] ="<iframe width='100%%' src='https://maps.google.com/maps?width=&amp;height=&amp;hl=en&amp;q=%s %s+(Firmware)&amp;ie=UTF8&amp;t=&amp;z=10&amp;iwloc=B&amp;output=embed' frameborder='0' scrolling='no' marginheight='0' marginwidth='0'></iframe>";
