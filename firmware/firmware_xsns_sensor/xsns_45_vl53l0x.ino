/*
  xsns_45_vl53l0x.ino - VL53L0X time of flight multiple sensors support for Firmware

  Copyright (C) 2021  Theo Arends and Mahmoud Shams El-Din, Gerhard Mutz and Adrian Scillato
 ...
 * in order to let Firmware change by software the I2C address of those and give them an unique address
 ...
 *
 * The default value of VL53LXX_MAX_SENSORS is set in the file Firmware.h
...
#ifdef USE_DOMOTICZ
  if (json && (0 == FirmwareGlobal.tele_period)){
    float distance = (float)Vl53l0x_data[0].distance / 10;  // cm
    DomoticzFloatSensor(DZ_ILLUMINANCE, distance);
  }
#endif  // USE_DOMOTICZ
