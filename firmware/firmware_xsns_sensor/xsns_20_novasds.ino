/*
  xsns_20_novasds.ino - Nova SDS011/SDS021 particle concentration sensor support for Firmware
...
void NovaSdsSecond(void)                 // Every second
{
  ...
  if(FirmwareGlobal.tele_period == Settings->tele_period -  Settings->novasds_startingoffset && !cont_mode)
  { //lets start fan and laser
    NovaSdsCommand(NOVA_SDS_SLEEP_AND_WORK, NOVA_SDS_SET_MODE, NOVA_SDS_WORK, NOVA_SDS_DEVICE_ID, nullptr);
  }
  if(FirmwareGlobal.tele_period >= Settings->tele_period-5 && FirmwareGlobal.tele_period <= Settings->tele_period-2)
  { //we are doing 4 measurements here
    if(!(NovaSdsReadData())) novasds_valid=0;
    pm100_sum += novasds_data.pm100;
    pm25_sum  += novasds_data.pm25;
  }
  if(FirmwareGlobal.tele_period == Settings->tele_period-1)
...
void NovaSdsInit(void)
{
  ...
  if (PinUsed(GPIO_SDS0X1_RX) && PinUsed(GPIO_SDS0X1_TX)) {
    NovaSdsSerial = new FirmwareSerial(Pin(GPIO_SDS0X1_RX), Pin(GPIO_SDS0X1_TX), 1);
