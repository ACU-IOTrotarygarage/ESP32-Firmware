/*
  xsns_35_Tx20.ino - La Crosse Tx20/Tx23 wind sensor support for Firmware

  Copyright (C) 2021  Thomas Eckerstorfer, Norbert Richter and Theo Arends and Mahmoud Shams El-Din
...
void IRAM_ATTR TX2xStartRead(void)
{
  ...
  if (0!=tx23_stage)
  {
    if ((2==tx23_stage) || (3==tx23_stage))
    {
      ...
      {
#endif
        tx2x_last_available = FirmwareGlobal.uptime;
...
{
  return ((FirmwareGlobal.uptime - tx2x_last_available) < TX2X_TIMEOUT);
}
...
void Tx2xResetStat(void)
{
  ...
  tx2x_last_uptime = FirmwareGlobal.uptime;
  Tx2xResetStatData();
}
...
void Tx2xRead(void)
{
  ...
  if ((FirmwareGlobal.uptime % TX23_READ_INTERVAL)==0) {
  ...
  } else if ((FirmwareGlobal.uptime % TX23_READ_INTERVAL)==1) {
  ...
  if (Tx2xAvailable()) {
    ...
    DEBUG_SENSOR_LOG(PSTR(D_TX2x_NAME ": dir stat - counter=%ld, actint=%ld, avgint=%ld, avg=%1_f (cosx=%1_f, siny=%1_f), min %d, max %d"),
      (FirmwareGlobal.uptime-tx2x_last_uptime),
      tx2x_wind_direction,
  ...
  if (0==Settings->tele_period && (FirmwareGlobal.uptime-tx2x_last_uptime)>=tx2x_avg_samples) {
    Tx2xResetStat();
  }
#endif  // USE_TX2X_WIND_SENSOR_NOSTATISTICS
}
