/*
  xsns_62_esp32_mi.ino - MI-BLE-sensors via ESP32 support for Firmware
  enabled by ESP32 && !USE_BLE_ESP32
  if (ESP32 && USE_BLE_ESP32) then xsns_62_esp32_mi_ble.ino is used

  Copyright (C) 2021  Christian Baars and Theo Arends and Mahmoud Shams El-Din
...
extern "C" {
  ...
  uint32_t MI32numOfRelays(){
    if(FirmwareGlobal.devices_present>0) MI32.HKinfoMsg = MI32_HAP_OUTLET_ADDED;
    return FirmwareGlobal.devices_present;
  }
...
#ifdef USE_MI_HOMEKIT
  if(FirmwareGlobal.devices_present>0){
    for(uint32_t i=0;i<FirmwareGlobal.devices_present;i++){
      power_t mask = 1 << i;
      // AddLog(LOG_LEVEL_DEBUG,PSTR("M32: Power masl: %u"), (FirmwareGlobal.power & mask));
      mi_homekit_update_value(MI32.outlet_hap_service[i],(FirmwareGlobal.power & mask)!=0,0xf0);
    }
  }
#endif //USE_MI_HOMEKIT
...
void MI32Show(bool json)
{
  if (json) {
    ...
    if(FirmwareGlobal.masterlog_level == LOG_LEVEL_DEBUG_MORE) return; // we want to announce sensors unlinked to the ESP, check for LOG_LEVEL_DEBUG_MORE is medium-safe
    MI32suspendScanTask();
    for (uint32_t i = 0; i < MIBLEsensors.size(); i++) {
      ...
      if((!MI32.mode.triggeredTele && !MI32.option.minimalSummary)||MI32.mode.triggeredTele){
        ...
        if (MIBLEsensors[i].feature.Btn){
          if(MIBLEsensors[i].eventType.Btn){
            MI32ShowContinuation(&commaflg);
            ResponseAppend_P(PSTR("\"Button%u\":%u"),MIBLEsensors[i].Btn,MIBLEsensors[i].BtnType + 1); //internal type is Xiaomi/Homekit 0,1,2 -> Firmware 1,2,3
          }
        }
