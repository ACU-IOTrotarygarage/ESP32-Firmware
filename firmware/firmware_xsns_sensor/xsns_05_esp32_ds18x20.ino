/*
  xsns_05_esp32_ds18x20.ino - DS18x20 temperature sensor support for ESP32 Firmware

  Copyright (C) 2021  Heiko Krupp, Theo Arends and Mahmoud Shams El-Din and md5sum-as (https://github.com/md5sum-as)
...
void Ds18x20Show(bool json) {
  ...
  for (uint32_t i = 0; i < DS18X20Data.sensors; i++) {
    ...
    if (Ds18x20Read(i, t)) {           // Check if read failed
      ...
      if (json) {
        if (Settings->flag5.ds18x20_mean) {
          if ((0 == FirmwareGlobal.tele_period) && ds18x20_sensor[i].numread) {
        ...
        dsxflg++;
#ifdef USE_DOMOTICZ
        if ((0 == FirmwareGlobal.tele_period) && (1 == dsxflg)) {
