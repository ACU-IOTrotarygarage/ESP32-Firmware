/*
  xsns_31_ccs811.ino - CCS811 gas and air quality sensor support for Firmware
    ...
    if (ccs.available()) {
      if (!ccs.readData()){
        ...
        if (FirmwareGlobal.global_update && (FirmwareGlobal.humidity > 0) && !isnan(FirmwareGlobal.temperature_celsius)) {
          ccs.setEnvironmentalData((uint8_t)FirmwareGlobal.humidity, FirmwareGlobal.temperature_celsius);
        }
        ecnt = 0;
      }
    } else {
...
#ifdef USE_DOMOTICZ
      if (0 == FirmwareGlobal.tele_period) DomoticzSensor(DZ_AIRQUALITY, eCO2);
#endif  // USE_DOMOTICZ
#ifdef USE_WEBSERVER
    } else {
