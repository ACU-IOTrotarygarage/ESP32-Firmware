/*
  xsns_75_prometheus.ino - Web based information for Firmware
...
void WritePromMetric(const char *name, uint8_t flags, const char *value, va_list labels) {
  PGM_P const prefix = PSTR("Firmware_");
  PGM_P tmp;
  String lval;
...
void HandleMetrics(void) {
  ...
  WritePromMetricInt32(PSTR("info"), kPromMetricGauge, 1,
    PSTR("version"), FirmwareGlobal.version,
    PSTR("image"), FirmwareGlobal.image_name,
  ...
  WritePromMetricInt32(PSTR("uptime_seconds"), kPromMetricGauge, FirmwareGlobal.uptime, nullptr);
  ...
  if (!isnan(FirmwareGlobal.temperature_celsius)) {
    WritePromMetricDec(PSTR("global_temperature_celsius"), kPromMetricGauge,
      FirmwareGlobal.temperature_celsius, Settings->flag2.temperature_resolution,
      nullptr);
  }

  if (FirmwareGlobal.humidity != 0) {
    WritePromMetricDec(PSTR("global_humidity_percentage"), kPromMetricGauge,
      FirmwareGlobal.humidity, Settings->flag2.humidity_resolution,
      nullptr);
  }

  if (FirmwareGlobal.pressure_hpa != 0) {
    WritePromMetricDec(PSTR("global_pressure_hpa"), kPromMetricGauge,
      FirmwareGlobal.pressure_hpa, Settings->flag2.pressure_resolution,
      nullptr);
  }
  ...
  for (uint32_t device = 0; device < FirmwareGlobal.devices_present; device++) {
    ...
    WritePromMetricInt32(namebuf, kPromMetricGauge,
      (FirmwareGlobal.power & mask), nullptr);
  }
