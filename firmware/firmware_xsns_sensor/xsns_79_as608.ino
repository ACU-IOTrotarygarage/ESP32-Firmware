/*
  xsns_79_as608.ino - AS608 and R503 fingerprint sensor support for Firmware

  Copyright (C) 2021  boaschti and Theo Arends and Mahmoud Shams El-Din
 ...
 *
 * Uses Adafruit-Fingerprint-sensor-library with FirmwareSerial
 *
 * Changes made to Adafruit_Fingerprint.h and Adafruit_Fingerprint.cpp:
 * - Replace SoftwareSerial with FirmwareSerial
...
void As608Init(void) {
  if (PinUsed(GPIO_AS608_RX) && PinUsed(GPIO_AS608_TX)) {
    As608Serial = new FirmwareSerial(Pin(GPIO_AS608_RX), Pin(GPIO_AS608_TX), 0);
