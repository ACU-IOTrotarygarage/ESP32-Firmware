/*
  xsns_21_sgp30.ino - SGP30 gas and air quality sensor support for Firmware
...
void Sgp30Update(void)  // Perform every second to ensure proper operation of the baseline compensation algorithm
{
  ...
  if (FirmwareGlobal.global_update && (FirmwareGlobal.humidity > 0) && !isnan(FirmwareGlobal.temperature_celsius)) {
    // abs hum in mg/m3
    sgp30_abshum = sgp30_AbsoluteHumidity(FirmwareGlobal.temperature_celsius, FirmwareGlobal.humidity);
...
void Sgp30Show(bool json)
{
  if (sgp30_ready) {
    ...
    if (json) {
      ...
      ResponseJsonEnd();
#ifdef USE_DOMOTICZ
      if (0 == FirmwareGlobal.tele_period) DomoticzSensor(DZ_AIRQUALITY, sgp.eCO2);
#endif  // USE_DOMOTICZ
#ifdef USE_WEBSERVER
    } else {
