/*
  xsns_90-hrg15.ino - Hydreon RG-15 support for Firmware
...
FirmwareSerial *HydreonSerial = nullptr;
...
void Rg15Init(void) {
  if (PinUsed(GPIO_HRG15_RX) && PinUsed(GPIO_HRG15_TX)) {
    // Max size message:
    // Acc 0.001 mm, EventAcc 0.002 mm, TotalAcc 0.003 mm, RInt 0.004 mmph, XTBTips 0, XTBAcc 0.01 mm, XTBEventAcc 0.02 mm, XTBTotalAcc 0.03 mm
    HydreonSerial = new FirmwareSerial(Pin(GPIO_HRG15_RX), Pin(GPIO_HRG15_TX), 2, 0, RG15_BUFFER_SIZE);
    if (HydreonSerial) {
...
void Rg15Poll(void) {
  ...
  if (publish && !FirmwareGlobal.global_state.mqtt_down) {
    MqttPublishSensor();
  }
