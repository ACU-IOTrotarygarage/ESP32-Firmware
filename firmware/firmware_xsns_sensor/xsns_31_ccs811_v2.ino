    ...
    if (pccsd->refresh_count >= EVERYNSECONDS) {
      ...
      if (pccsd->ccsinstance.available()) {
        if (!pccsd->ccsinstance.readData()){
          ...
          if ((FirmwareGlobal.global_update) &&
              (FirmwareGlobal.humidity > 0) &&
              (!isnan(FirmwareGlobal.temperature_celsius))) {
            pccsd->ccsinstance.setEnvironmentalData((uint8_t)FirmwareGlobal.humidity,
                                                    FirmwareGlobal.temperature_celsius);
          }
          pccsd->reset_count = 0;
        }
      } else {
