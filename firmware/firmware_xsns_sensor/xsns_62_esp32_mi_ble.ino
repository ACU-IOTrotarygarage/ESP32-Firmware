/*
  xsns_62_esp32_mi_ble.ino - MI-BLE-sensors via ESP32 support for Firmware
  enabled by ESP32 && USE_BLE_ESP32
  if (ESP32 && !USE_BLE_ESP32) then xsns_62_esp32_mi.ino is used - the older driver


  Copyright (C) 2020  Christian Baars and Theo Arends and Mahmoud Shams El-Din
  ...
  0.9.2.1 20210217  changed - make features alos depend on received data - i.e. 'unknown' devices will show what they send.
                              Add MI32Option6 1 to switch to tele/Firmware_ble/<somename> style MQTT independent of HASS discovery.
  -------
  ...
  0.9.0.0 20200413  started - initial development by Christian Baars
                    forked  - from arendst/Firmware            - https://github.com/arendst/Firmware
...
#define MI32_BLE_TOPIC "Firmware_ble"
#endif
...
struct mi_sensor_t{
  ...
  union {
    struct {
      uint32_t temp:1;
      uint32_t hum:1;
      uint32_t tempHum:1; //every hum sensor has temp too, easier to use Firmware dew point functions
      uint32_t lux:1;
      uint32_t moist:1;
      uint32_t fert:1;
      uint32_t bat:1;
...
//  "\">%s</a>{m} {e}";
const char HTTP_NEEDKEY[] PROGMEM = "{s}%s <a target=\"_blank\" href=\""
  "https://Firmware.github.io/ble_key_extractor?mac=%s&cb=http%%3A%%2F%%2F%s%%2Fmikey"
  "\">%s</a>{m} {e}";
...
void MI32DiscoveryOneMISensor(){
  ...
  if(Settings->flag.hass_discovery){
    ...
    /*
    {"availability":[],"device":{"identifiers":["FirmwareBLEa4c1387fc1e1"],"manufacturer":"simon","model":"someBLEsensor","name":"TASBLEa4c1387fc1e1","sw_version":"0.0.0"},"dev_cla":"temperature","json_attr_t":"tele/Firmware_esp32/SENSOR","name":"TASLYWSD037fc1e1Temp","state_topic":"tele/Firmware_esp32/SENSOR","uniq_id":"Firmwarea4c1387fc1e1temp","unit_of_meas":"°C","val_tpl":"{{ value_json.LYWSD037fc1e1.Temperature }}"}
    {"availability":[],"device":{"identifiers":["FirmwareBLEa4c1387fc1e1"],
    "name":"TASBLEa4c1387fc1e1"},"dev_cla":"temperature",
    "json_attr_t":"tele/Firmware_esp32/SENSOR",
    "name":"TASLYWSD037fc1e1Temp","state_topic":  "tele/Firmware_esp32/SENSOR",
    "uniq_id":"Firmwarea4c1387fc1e1temp","unit_of_meas":"°C",
