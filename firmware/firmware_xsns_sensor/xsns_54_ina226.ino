/*
  xsns_54_ina226.ino - INA226 Current Sensor support for Firmware

  Copyright (C) 2021  Stephen Rodgers and Theo Arends and Mahmoud Shams El-Din
...
*    c. Sensor54 2 saves the settings and restarts Firmware. The device should show up after the system boots again.
  ...
  if (p1 < 10 || p1 >= 50){
    ...
    switch (p1){
      ...
      case 2: // Save and restart
        ...
        Response_P(PSTR("{\"Sensor54-Command-Result\":{\"Restart_flag\":%d}}"),FirmwareGlobal.restart_flag);
        break;
...
/**
 * The callback function Xsns_57() interfaces Firmware with the sensor driver.
 *
 * It provides the Firmware callback IDs.
 *
 * @param   byte    callback_id  Firmware function ID.
...
bool Xsns54(uint32_t callback_id)
{
  ...
  // Check which callback ID is called by Firmware
  switch (callback_id) {
