/*
  xsns_98_sgp40.ino - SGP40 gas and air quality sensor support for Firmware
 ...
 * Adaption for Firmware: Jean-Pierre Deschamps
 *
 * I2C Address: 0x59
\*********************************************************************************************/
...
{
  sgp40_ready = false;
  if (FirmwareGlobal.global_update && (FirmwareGlobal.humidity > 0) && !isnan(FirmwareGlobal.temperature_celsius)) {
    // abs hum in mg/m3
    sgp40_abshum = sgp40_AbsoluteHumidity(FirmwareGlobal.temperature_celsius, FirmwareGlobal.humidity);
  }
  sgp40_ready = true;

  // these should normally be stored permanently and used for fast restart
  if (!(FirmwareGlobal.uptime%SAVE_PERIOD)) {
    // store settings every N seconds
    raw_base = sgp40.measureRaw(FirmwareGlobal.temperature_celsius, FirmwareGlobal.humidity);
    voc_index = sgp40.measureVocIndex(FirmwareGlobal.temperature_celsius, FirmwareGlobal.humidity);
  }
}
...
void Sgp40Show(bool json)
{
  if (sgp40_ready) {
    char abs_hum[33];
    bool ahum_available = FirmwareGlobal.global_update && (FirmwareGlobal.humidity > 0) && !isnan(FirmwareGlobal.temperature_celsius);
    if (ahum_available) {
    ...
    if (json) {
      ...
      ResponseJsonEnd();
#ifdef USE_DOMOTICZ
      if (0 == FirmwareGlobal.tele_period) DomoticzSensor(DZ_AIRQUALITY, raw_base);
#endif  // USE_DOMOTICZ
#ifdef USE_WEBSERVER
    } else {
