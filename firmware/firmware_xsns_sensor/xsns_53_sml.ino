/*
  xsns_53_sml.ino - SML,OBIS,EBUS,RAW,COUNTER interface for Firmware

  Created by Gerhard Mutz on 07.10.11.
  adapted for Firmware
...
#ifdef USE_DOMOTICZ
  if (json && !FirmwareGlobal.tele_period) {
    char str[16];
    ...
    if (mp->type == 'c') {
      ...
      if (mp->srcpin == TCP_MODE_FLG) {
        ...
        if (type == 'm' || type == 'M' || type == 'k' || type == 'p' || type == 'R' || type == 'v') {
          mp->meter_ss = new FirmwareSerial(mp->srcpin, mp->trxpin, 1, 0, mp->sibsiz);
        } else {
          mp->meter_ss = new FirmwareSerial(mp->srcpin, mp->trxpin, 1, 1, mp->sibsiz);
        }
#else
        mp->meter_ss = new FirmwareSerial(mp->srcpin, mp->trxpin, 1, 0, mp->sibsiz);
#endif  // SPECIAL_SS
