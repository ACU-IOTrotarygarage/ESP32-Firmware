/*
  xsns_52_ibeacon.ino - Support for HM17 BLE Module + ibeacon reader on Firmware
...
FirmwareSerial *IBEACON_Serial = nullptr;
...
void ESP32Init() {
  ...
  if (FirmwareGlobal.global_state.wifi_down) { return; }
...
// actually doesnt work reliably with software serial
  if (PinUsed(GPIO_IBEACON_RX) && PinUsed(GPIO_IBEACON_TX)) {
    IBEACON_Serial = new FirmwareSerial(Pin(GPIO_IBEACON_RX), Pin(GPIO_IBEACON_TX),1,0,TMSBSIZ52);
...
void hm17_every_second(void) {
  ...
  if (hm17_found) {
    if (IB_UPDATE_TIME && (FirmwareGlobal.uptime%IB_UPDATE_TIME==0)) {
      if (hm17_cmd!=99) {
        if (hm17_flag&2) {
          ib_sendbeep();
        } else {
