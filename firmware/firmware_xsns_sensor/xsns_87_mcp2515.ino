/*
  xsns_87_mcp2515.ino - MCP2515 CAN bus support for Firmware
...
void MCP2515_Init(void) {
  if (PinUsed(GPIO_MCP2515_CS, GPIO_ANY) && FirmwareGlobal.spi_enabled) {
...
void MCP2515_Read() {
  ...
  while (checkRcv && nCounter <= MCP2515_MAX_FRAMES) {
    ...
    if (mcp2515->readMessage(&canFrame) == MCP2515::ERROR_OK) {
      Mcp2515.lastFrameRecv = FirmwareGlobal.uptime;
#ifdef MCP2515_BMS_CLIENT
#ifdef MCP2515_BMS_FREEDWON
...
#ifdef MCP2515_BMS_FREEDWON
  if (!(FirmwareGlobal.uptime%CAN_KEEP_ALIVE_SECS) && FirmwareGlobal.uptime>60) {
...
void MCP2515_Show(bool Json) {
  if (Json) {
    if (Mcp2515.lastFrameRecv > 0 && FirmwareGlobal.uptime - Mcp2515.lastFrameRecv <= MCP2515_TIMEOUT) {
#ifdef MCP2515_BMS_CLIENT
