/*
  xsns_78_xezo.ino - EZO family I2C driver support for Firmware
...
struct EZOManager {
  void Command()
  {
    ...
    // PS: This should ideally be done through the Firmware mailbox
    if (at[0] == '-') {
  ...
  void EverySecond()
  {
    ...
    if (count < 0) {
      ...
      if (FirmwareGlobal.uptime >= next) {
        count++;

        if (count == -1) {
          DetectRequest();
          next = FirmwareGlobal.uptime + 1;
        } else if (count == 0) {
          ProcessDetection();
        }
      }
