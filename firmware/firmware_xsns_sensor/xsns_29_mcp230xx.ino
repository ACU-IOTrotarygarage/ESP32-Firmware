/*
  xsns_29_mcp230xx.ino - Support for I2C MCP23008/MCP23017 GPIO Expander on Firmware
...
{
#ifdef USE_MCP230xx_OUTPUT
  FirmwareGlobal.devices_present -= mcp230xx_outpincount;
  ...
  for (uint32_t mcp230xx_port = 0; mcp230xx_port < mcp230xx_type; mcp230xx_port++) {
    for (uint32_t idx = 0; idx < mcp230xx_outpincount; idx++) {
      if (mcp230xx_port ? mcp230xx_outpinmapping[idx] > 7 : mcp230xx_outpinmapping[idx] < 8) {
        uint8_t relay_no = FirmwareGlobal.devices_present - mcp230xx_outpincount + idx + 1;
...
bool MCP230xx_Command(void)
{
  ...
  if (validpin && (paramcount > 1)) {
    ...
    if (Settings->mcp230xx_config[pin].pinmode >= 5 && paramcount == 2) { // Changing output value
      ...
      relay_no = FirmwareGlobal.devices_present - mcp230xx_outpincount + relay_no +1;
...
void MCP230xx_SwitchRelay() {
  for (uint32_t i = FirmwareGlobal.devices_present - mcp230xx_outpincount; i < FirmwareGlobal.devices_present; i++) {
    uint8_t pin = mcp230xx_outpinmapping[i - (FirmwareGlobal.devices_present - mcp230xx_outpincount)];
    uint8_t pincmd = Settings->mcp230xx_config[pin].pinmode - 5;
  ...
  if (FUNC_INIT == function) {
    ...
    switch (function) {
      ...
      case FUNC_EVERY_SECOND:
        ...
        if (FirmwareGlobal.tele_period == 0) {
