/*
  xsns_38_az7798.ino - AZ_Instrument 7798 CO2/temperature/humidity meter support for Firmware
 ...
 * Firmware-6.3.0 by Arthur de Beun.
 *
...
void AzInit(void)
{
  ...
  if (PinUsed(GPIO_AZ_RXD) && PinUsed(GPIO_AZ_TXD)) {
    AzSerial = new FirmwareSerial(Pin(GPIO_AZ_RXD), Pin(GPIO_AZ_TXD), 1);
...
void AzShow(bool json)
{
  if (json) {
    ...
    ResponseJsonEnd();
#ifdef USE_DOMOTICZ
    if (0 == FirmwareGlobal.tele_period) DomoticzSensor(DZ_AIRQUALITY, az_co2);
#endif  // USE_DOMOTICZ
#ifdef USE_WEBSERVER
  } else {
