/*
  xsns_65_hdc1080.ino - Texas Instruments HDC1080 temperature and humidity sensor support for Firmware
...
void HdcEverySecond(void) {
  if (FirmwareGlobal.uptime &1) {  // Every 2 seconds
    if (!HdcTriggerRead()) {
...
/**
 * Firmware boilerplate for presenting the sensor data in the web UI, JSON for
 * the MQTT messages, and so on.
 *
 */
void HdcShow(bool json) {
  if (hdc_valid) {
    TempHumDewShow(json, (0 == FirmwareGlobal.tele_period), hdc_type_name, hdc_temperature, hdc_humidity);
  }
}
