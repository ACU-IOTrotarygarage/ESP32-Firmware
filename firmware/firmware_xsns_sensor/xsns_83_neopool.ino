/*
  xsns_83_neopool.ino - Sugar Valley NeoPool Control System Modbus support for Firmware
 ...
 *              0 - sync with Firmware local time
 *              1 - sync with Firmware utc time
...
void NeoPoolInit(void) {
  ...
  if (PinUsed(GPIO_NEOPOOL_RX) && PinUsed(GPIO_NEOPOOL_TX)) {
    NeoPoolModbus = new FirmwareModbus(Pin(GPIO_NEOPOOL_RX), Pin(GPIO_NEOPOOL_TX));
