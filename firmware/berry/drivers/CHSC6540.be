 ...
 class CHSC6540 : I2C_Driver
  ...
  def init()
    ...
    if !introspect.module("display")
      Firmware.log("I2C: can't start CHSC6540 without display enabled", 3)
      self.wire = nil
    end

    if self.tp_int < 0
      Firmware.log("I2C: can't start CHSC6540 without INTERRUPT-1 gpio configured", 3)
      self.wire = nil
    else
  ...
  def every_50ms()
    ...
    # Firmware.log("DEBUG> int="+str(self.tp_int)+" touched="+str(self.touched), 2)
    if self.touched
      import string
      ...
      # Firmware.log(string.format("I2C: screen pressed x=%i y=%i", self.x, self.y), 2)
