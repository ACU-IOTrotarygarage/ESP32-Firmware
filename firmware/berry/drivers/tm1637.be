#-
Simplified Firmware TM1637 driver written in Berry
...
Add custom commands to the native Firmware console:
...
def tm_add_custom_commands(dio, clk)
    ...
    tm.clear()
    tm.set_on(4)
    Firmware.add_cmd('tmprint', def(cmd, idx, payload)
        tm.print(payload) ? Firmware.resp_cmnd_done() : Firmware.resp_cmnd_failed()
    end)
    #  0-8 range, 0 to 'OFF'
    Firmware.add_cmd('tmbrightness', def(cmd, idx, payload)
        tm.set_on(int(payload)) ? Firmware.resp_cmnd_done() : Firmware.resp_cmnd_failed()
    end)
    log("Firmware custom commands registered: TmPrint, TmBrightness")
end
