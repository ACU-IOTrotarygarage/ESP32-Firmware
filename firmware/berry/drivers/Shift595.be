...
#   import Shift595
#   Firmware.add_driver(Shift595(srclk,ser,rclk,count,inverted))
# where
...
class Shift595
  ...
  def init(srclk, ser, rclk, count, inverted)
    ...
    self.first = Firmware.global.devices_present
    Firmware.global.devices_present += count
  ...
  def set_power_handler(cmd, idx)
    var new_state = Firmware.get_power()[self.first..self.first+self.count]
