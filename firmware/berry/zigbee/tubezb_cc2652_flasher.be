...
class tubezb_cc2652_flasher
  ...
  def dump_to_file(filename)
    import cc2652_flasher   # this stops zigbee and configures serial
    self.flasher = cc2652_flasher
    print("FLH: Dump started (takes 3 minutes during which Firmware is unresponsive)")
    self.flasher.start()
    self.flasher.ping()
  ...
  #################################################################################
  def _flash_pre()
    print("FLH: Flashing started (takes 5 minutes during which Firmware is unresponsive)")
    self.flasher.start()
    self.flasher.ping()
    # erase flash
  ...
  def _flash_post()
    ...
    # Firmware.log("FLH: Verification of HEX file OK", 2)
  end
  ...
  def _check_pre()
    ...
    # Firmware.log("FLH: Starting verification of HEX file", 2)
  end
