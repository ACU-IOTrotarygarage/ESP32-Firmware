...
# normal zigbee functions requires a Firmware restart.
#
...
#   Dumping the complete file is done as follows (it takes 3 minutes during which Firmware is unresponsive):
...
# This takes 3 minutes during which Firmware is unresponsive
#
import cc2652_flasher as cc
cc.start()
...
class cc2652_flasher
  ...
  def init(rx, tx, rst, bsl)
    ...
    # Firmware.log(string.format("FLH: cc2652_flasher rx=%i tx=%i rst=%i bsl=%i", self.rx, self.tx, self.rst, self.bsl), 3)
  ...
  def reset_bsl()
    ...
    Firmware.delay(10)               # wait 10ms
    gpio.digital_write(self.rst, 1)
    Firmware.delay(100)               # wait 100ms
  ...
  def recv_raw(timeout)
    var due = Firmware.millis() + timeout
    while !Firmware.time_reached(due)
      ...
      Firmware.delay(5)        # check every 5ms
    end
