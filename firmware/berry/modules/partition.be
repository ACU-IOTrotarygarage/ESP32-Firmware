...
class Partition_manager_UI
  static app_size_min = 896    # Min OTA size - let's set it at a safe 896KB for minimal Firmware32 with TLS
  ...
  def page_show_partitions(p)
    ...
    for slot: p.slots
      ...
      if ota_num != nil
        ...
        var flash_size = Firmware.memory()['flash'] * 1024
  ...
  def page_show_repartition_asym(p)
    ...
    if p.get_active() != 0
      ...
      var flash_size_kb = Firmware.memory()['flash']
  ...
  def page_part_ctl()
    ...
    try
      ...
      #---------------------------------------------------------------------#
        ...
        var spiffs_max_size = ((Firmware.memory()['flash'] - (spiffs_slot.start / 1024)) / 16) * 16
...
if Firmware
  ...
  Firmware.add_driver(partition_manager_ui)
