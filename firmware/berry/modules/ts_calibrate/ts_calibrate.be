...
ts_calibrate.init = def (m)
  ...
  #
    ...
    def start()
      ...
      if !self.m_line
        Firmware.log("TS : Abort, no touchscreen ':M' line present in 'display.ini'", 2)
        return
      end
      ...
      var display_rotate_ret = Firmware.cmd("DisplayRotate")
      if display_rotate_ret != nil && display_rotate_ret["DisplayRotate"] != 0
        Firmware.log("TS : Calibration requires 'DisplayRotate 0'")
        return
      end
      ...
      Firmware.set_timer(2000, /-> self.do_next_corner())
    end
    ...
    def track_touch_screen()
      ...
      if tracking.state
        ...
        if (self.raw_pts_weight >= 3)
          ...
          Firmware.set_timer(0, /-> self.do_next_corner())   # defer to next corner
    ...
    def finish()
      ...
      Firmware.log(string.format("TS : Calibration (%i,%i) - (%i,%i) - (%i,%i) - (%i,%i)",
      ...
      Firmware.set_timer(3000, /->self.cleanup(ok))
    end
    ...
    def load_m_line()
      try
        ...
        Firmware.log("TS : Couldn't open 'display.ini': "+str(e)+" '"+str(m)+"'")
      end
      return nil
    end
    ...
    def update_display_ini(m_line)
      try
        ...
        disp_f.close()
        Firmware.log("TS : Successfully updated 'display.ini'", 2)
        return true
      except .. as e, m
        Firmware.log("TS : Error updating 'display.ini': "+str(e)+" '"+str(m)+"'")
      end
      return false
    end
    ...
    def calc_geometry(p0x, p0y, p1x, p1y, p2x, p2y, p3x, p3y, hres, vres, padding)
      ...
      Firmware.log(string.format("TS : Geometry (%i,%i) (%i,%i) (%i,%i) (%i,%i) - %ix%i pad %i",
                                  int(p0x), int(p0y),
                                  int(p1x), int(p1y),
                                  int(p2x), int(p2y),
                                  int(p3x), int(p3y),
      ...
      Firmware.log("cos_th_01_02=" + str(cos_th_01_02), 4)

      if (cos_th_01_02 > 0.05)
        Firmware.log("TS : Wrong geometry - bad angle. Try again.", 2)
        return nil
      end
      ...
      Firmware.log("norm_delta_centers=" + str(norm_delta_centers), 4)

      if (norm_delta_centers > 0.02)
        Firmware.log("TS : Wrong geometry - bad center. Try again", 2)
        return nil
      end
      ...
      var ymin = (p0y + p1y) / 2
      var ymax = (p2y + p3y) / 2
      Firmware.log("raw xmin=" + str(xmin) + " xmax=" + str(xmax) + " ymin=" + str(ymin) + " ymax=" + str(ymax), 4)

      var range_x = xmax - xmin
      var range_y = ymax - ymin

      Firmware.log("range_x=" + str(range_x) + " range_y=" + str(range_y), 4)
      ...
      ymin -= extend_y
      ymax += extend_y
      Firmware.log("final xmin=" + str(xmin) + " xmax=" + str(xmax) + " ymin=" + str(ymin) + " ymax=" + str(ymax), 4)

      var M_string = string.format(":M,%i,%i,%i,%i", int(xmin), int(xmax), int(ymin), int(ymax))
      Firmware.log(string.format("TS : Add this to display.ini '%s'", M_string))
      return M_string
    end
  end
  return TS_Calibrate()
