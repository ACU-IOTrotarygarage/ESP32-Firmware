...
class Partition_wizard_UI
  static app_size_min = 832    # Min OTA size - let's set it at a safe 896KB for minimal Firmware32 with TLS
  static app_size_max = 3072   # Max OTA size - (4096 - 896 - 128)
  static _default_safeboot_URL = "http://ota.Firmware.com/Firmware32/release/Firmware32%s-safeboot.bin"
  ...
  def init()
    ...
    if persist.find("factory_migrate") == true
      ...
      def continue_after_5s()
        Firmware.remove_rule("parwiz_5s")      # first remove rule to avoid firing it again at Wifi reconnect
        Firmware.set_timer(5000, /-> self.do_safeboot_partitioning())  # delay by 5 s
      end
      Firmware.add_rule("Wifi#Connected=1", continue_after_5s, "parwiz_5s")
  ...
  def get_max_flash_size_k(p)
    var flash_size_k = Firmware.memory()['flash']
    var flash_size_real_k = Firmware.memory().find("flash_real", flash_size_k)
  ...
  def resize_max_flash_size_k(p)
    ...
    if (flash_size_k != flash_size_real_k) && flash_definition_sector != nil
      ...
      if flash_size_code != nil
        ...
        Firmware.log(string.format("UPL: changing flash definition from 0x02X to 0x%02X", old_def, flash_def[3]), 3)
      else
  ...
  static def copy_ota(from_addr, to_addr, sz)
    ...
    Firmware.log(string.format("UPL: Copy flash from 0x%06X to 0x%06X (size: %ikB)", from_addr, to_addr, sz / 1024), 2)
    while size_left > 0
      ...
      if ((offset-4096) / 102400) < (offset / 102400)
        Firmware.log(string.format("UPL: Progress %ikB", offset/1024), 3)
      end
    end
    Firmware.log("UPL: done", 2)
  end
  ...
  def do_step_1(p)
    ...
    Firmware.log("UPL: restarting on `app1`", 2)
    Firmware.cmd("Restart 1")
  end
  ...
  def do_step_2(p, safeboot_url)
    ...
    if safeboot_url == nil || safeboot_url == ""
      ...
      Firmware.log("UPL: no `safeboot` URL, defaulting to "+safeboot_url, 2)
    end
    ...
    Firmware.log(string.format("UPL: flashing `safeboot` from %s %ikB", safeboot_url, (safeboot_size / 1024) + 1), 2)
    var app0 = p.get_ota_slot(0)
  ...
  def show_migrate_to_factory(p)
    ...
    webserver.content_send("<p>Please see <a href='https://Firmware.github.io/docs/Safeboot/' target='_blank'>Safeboot layout documentation</a></p>")
    ...
    var ota_url = Firmware.cmd("OtaUrl").find("OtaUrl", "")
  ...
  def show_current_partitions(p)
    ...
    if self.has_factory_layout(p)
      webserver.content_send("<p>This device uses the <a href='https://Firmware.github.io/docs/Safeboot/' target='_blank'>Safeboot</a> layout</p>")
    end
  ...
  def page_part_mgr()
    ...
    webserver.content_send_style()                  #- send standard Firmware styles -#
    ...
    try
      ...
      #---------------------------------------------------------------------#
        ...
        if ota_url != nil && ota_url != ""
          Firmware.cmd("OtaUrl "+ota_url)
        end

        Firmware.set_timer(5000, /-> self.do_safeboot_partitioning())
      ...
      Firmware.log(string.format("BRY: Exception> '%s' - %s", e, m), 2)
      #- display error page -#
  ...
  def do_safeboot_partitioning()
    ...
    if step1_state == false
      import persist
      Firmware.log("UPL: Starting step 1", 2)
      try
        self.do_step_1(p)
      except .. as e, m
        Firmware.log(string.format("UPL: error (%s) %s", e, m), 2)
        return m
      end
    ...
    Firmware.log("UPL: Step 3 Done", 2)
  ...
  #- ---------------------------------------------------------------------- -#
  #- this is called at Firmware start-up, as soon as Wifi/Eth is up and web server running -#
  def web_add_handler()
    import webserver
...
#- create and register driver in Firmware -#
if Firmware
  import partition_core
  var partition_wizard_ui = partition_wizard.Partition_wizard_UI()
  Firmware.add_driver(partition_wizard_ui)
