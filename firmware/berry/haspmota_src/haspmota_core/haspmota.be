...
class lvh_obj
  ...
  def event_cb(event)
    ...
    # defer the actual action to the Firmware event loop
    ...
    if self.action != "" && code == lv.EVENT_CLICKED
      ...
      Firmware.set_timer(0, /-> oh.do_action(self, code))
    end
    ...
    if event_hasp != nil
      ...
      # print("val=",val)
      Firmware.set_timer(0, /-> Firmware.publish_rule(tas_event))
    end
  end
  ...
  def set_val_rule(t)
    ...
    if self._val_rule != nil
      Firmware.remove_rule(self._val_rule, self)
    end

    self._val_rule = str(t)
    Firmware.add_rule(self._val_rule, / val -> self.val_rule_matched(val), self)
  end
  def get_val_rule()
  ...
  def set_text_rule(t)
    ...
    if self._text_rule != nil
      Firmware.remove_rule(self._text_rule, self)
    end

    self._text_rule = str(t)
    Firmware.add_rule(self._text_rule, / val -> self.text_rule_matched(val), self)
  end
  def get_text_rule()
...
class lvh_page
  ...
  def show(anim, duration)
    ...
    Firmware.set_timer(0, /-> Firmware.publish_rule(event_str_in))
    var event_str_out = string.format('{"hasp":{"p%i":"in"}}', self._page_id)
    Firmware.set_timer(0, /-> Firmware.publish_rule(event_str_out))
