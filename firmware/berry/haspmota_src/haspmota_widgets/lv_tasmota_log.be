# lv_Firmware_log class

class lv_Firmware_log : lv.obj
  ...
  def init(parent)
    ...
    self.add_event_cb( / -> Firmware.remove_driver(self), lv.EVENT_DELETE, 0)
    
  	self.lines = []
  	self.line_len = 0
  	self.log_reader = Firmware_log_reader()
  	self.log_level = 2
    self._size_changed()
