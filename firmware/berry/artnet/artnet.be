    ...
    while (packet != nil)
      if size(packet) >= 18 &&
        ...
        if opcode == 0x5000 && protocol == 14 && universe >= universe_start && universe < universe_end
          # Firmware.log("DMX: received Art-Net packet :" + packet.tohex())
          ...
          # import string
          # Firmware.log(string.format("DMX: opcode=0x%04X protocol=%i seq=%i phy=%i universe=%i data_len=%i data=%s",
          #             opcode, protocol, seq, phy, universe, data_len, packet[18..-1].tohex()))
        end
      end
      packet = self.udp_server.read(self.packet)
      if packet == nil
        Firmware.delay_microseconds(20)   # wait 20 us just in case
        packet = self.udp_server.read(self.packet)
      end
    end
