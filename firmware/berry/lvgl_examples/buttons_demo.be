...
#- register buttons -#
var btns = []         # relay buttons are added to this list to match with Firmware relays
...
def btn_clicked_cb(obj, event)
  ...
  if btn_idx != nil
    ...
    Firmware.set_power(btn_idx, checked)                            # toggle the value
  end
end
...
def create_btn_relay(label)
  ...
  btn.add_event_cb(btn_clicked_cb, lv.EVENT_CLICKED, 0)       # set callback to update Firmware relays
...
def btns_update()
  var power_list = Firmware.get_power()                                            # get a list of booleans with status of each relay
  for b:btns
...
def btns_update_loop()
  btns_update()
  Firmware.set_timer(500, btns_update_loop)
end
