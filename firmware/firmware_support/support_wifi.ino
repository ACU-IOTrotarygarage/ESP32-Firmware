/*
  support_wifi.ino - wifi support for Firmware
...
void WifiSetMode(WiFiMode_t wifi_mode) {
  ...
  if (wifi_mode != WIFI_OFF) {
    WiFi.hostname(FirmwareGlobal.hostname);  // ESP32 needs this here (before WiFi.mode) for core 2.0.0
 ...
 * See https://github.com/arendst/Firmware/issues/2559
 */

// Sleep explanation: https://github.com/esp8266/Arduino/blob/3f0c601cfe81439ce17e9bd5d28994a7ed144482/libraries/ESP8266WiFi/src/ESP8266WiFiGeneric.cpp#L255
/*
  if (FirmwareGlobal.sleep && Settings->flag3.sleep_normal) {  // SetOption60 - Enable normal sleep instead of dynamic sleep
...
#endif
  if (0 == FirmwareGlobal.sleep || wifi_no_sleep) {
    if (!FirmwareGlobal.wifi_stay_asleep) {
...
//  WiFi.setAutoReconnect(true);
  ...
  WiFi.hostname(FirmwareGlobal.hostname);  // ESP8266 needs this here (after WiFi.mode)
  ...
  AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_WIFI D_CONNECTING_TO_AP "%d %s%s " D_IN_MODE " 11%c " D_AS " %s..."),
    Settings->sta_active +1, SettingsText(SET_STASSID1 + Settings->sta_active), stemp, pgm_read_byte(&kWifiPhyMode[WiFi.getPhyMode() & 0x3]), FirmwareGlobal.hostname);
  ...
  if (Settings->flag5.wait_for_wifi_result) {  // SetOption142 - (Wifi) Wait 1 second for wifi connection solving some FRITZ!Box modem issues (1)
    WiFi.waitForConnectResult(1000);  // https://github.com/arendst/Firmware/issues/14985
  }
}
...
{
  if (state == FirmwareGlobal.global_state.wifi_down) {
    if (state) {
      FirmwareGlobal.rules_flag.wifi_connected = 1;
      Wifi.link_count++;
      Wifi.downtime += UpTime() - Wifi.last_event;
    } else {
      FirmwareGlobal.rules_flag.wifi_disconnected = 1;
      Wifi.last_event = UpTime();
    }
  }
  FirmwareGlobal.global_state.wifi_down = state ^1;
  if (!FirmwareGlobal.global_state.wifi_down) {
    FirmwareGlobal.global_state.network_down = 0;
  }
}
...
//        SettingsSdkErase();  //  Disabled v6.1.0b due to possible bad wifi connects
      ...
      if ((WL_CONNECTED == WiFi.status()) && WifiHasIP() && !Wifi.config_type) {
        ...
        if (Settings->flag3.use_wifi_rescan) {  // SetOption57 - Scan wifi network every 44 minutes for configured AP's
          if (!(FirmwareGlobal.uptime % (60 * WIFI_RESCAN_MINUTES))) {
...
int WifiState(void)
{
  ...
  if (!FirmwareGlobal.global_state.wifi_down) { state = WIFI_RESTART; }
...
//#ifdef ESP8266
  // https://github.com/arendst/Firmware/issues/16061#issuecomment-1216970170
...
void WifiDisable(void) {
  ...
  FirmwareGlobal.global_state.wifi_down = 1;
}
...
void EspRestart(void)
{
  ...
  if (FirmwareGlobal.restart_halt) {
    while (1) {
...
void WifiPollNtp() {
  ...
  if (FirmwareGlobal.global_state.network_down || Rtc.user_time_entry) { return; }

  uint8_t uptime_minute = (FirmwareGlobal.uptime / 60) % 60;  // 0 .. 59
  ...
  uint8_t offset = (FirmwareGlobal.uptime < 30) ? RtcTime.second + ntp_run_time : (((ESP_getChipId() & 0xF) * 3) + 3) ;

  if ( (((offset == RtcTime.second) && ( (RtcTime.year < 2016) ||                  // Never synced
                                         (ntp_sync_minute == uptime_minute))) ||   // Re-sync every hour
       FirmwareGlobal.ntp_force_sync ) ) {                                          // Forced sync

    FirmwareGlobal.ntp_force_sync = false;
