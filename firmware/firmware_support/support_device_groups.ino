/*
  support_device_groups.ino - device groups support for Firmware
...
#define DEVICE_GROUP_MESSAGE        "Firmware_DGR"
...
{
  if (Settings->flag4.device_groups_enabled && !device_groups_up && !FirmwareGlobal.restart_flag) {
...
void SendReceiveDeviceGroupMessage(struct device_group * device_group, struct device_group_member * device_group_member, uint8_t * message, int message_length, bool received)
{
  ...
  if (received) {
    ...
    */
    ...
    if (flags & (DGR_FLAG_MORE_TO_COME | DGR_FLAG_DIRECT)) FirmwareGlobal.skip_light_fade = true;
        ...
        switch (item) {
          case DGR_ITEM_POWER:
            if (Settings->flag4.multiple_device_groups) {  // SetOption88 - Enable relays in separate device groups
              ...
              if (device && device <= FirmwareGlobal.devices_present) {
                bool on = (value & 1);
                if (on != ((FirmwareGlobal.power >> (device - 1)) & 1)) ExecuteCommandPower(device, (on ? POWER_ON : POWER_OFF), SRC_REMOTE);
              }
            }
            else if (XdrvMailbox.index & DGR_FLAG_LOCAL) {
              uint8_t mask_devices = value >> 24;
              if (mask_devices > FirmwareGlobal.devices_present) mask_devices = FirmwareGlobal.devices_present;
              for (uint32_t i = 0; i < mask_devices; i++) {
                uint32_t mask = 1 << i;
                bool on = (value & mask);
                if (on != (FirmwareGlobal.power & mask)) ExecuteCommandPower(i + 1, (on ? POWER_ON : POWER_OFF), SRC_REMOTE);
              }
            }
            break;
          case DGR_ITEM_NO_STATUS_SHARE:
            ...
            *message_ptr++ = value & 0xff;
            if (item > DGR_ITEM_MAX_16BIT) {
              ...
              *message_ptr++ = value & 0xff;
              ...
              if (item == DGR_ITEM_POWER && !value) value = (!Settings->flag4.multiple_device_groups && device_group_index == 0 && first_device_group_is_local ? FirmwareGlobal.devices_present : 1);
              *message_ptr++ = value;
            }
          }
        }
...
{
  if (!device_groups_up || FirmwareGlobal.restart_flag) return;
