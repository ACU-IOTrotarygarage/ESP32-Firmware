/*
  support_Firmware.ino - Core support for Firmware
...
String ResolveToken(const char* input) {
  ...
  resolved.replace(F("%hostname%"), FirmwareGlobal.hostname);
...
char* GetTopic_P(char *stopic, uint32_t prefix, const char *topic, const char* subtopic)
{
  ...
  */
  ...
  if (FirmwareGlobal.fallback_topic_flag || (prefix > 3)) {
    ...
    fulltopic += F("/");
    if (fallback) {
      fulltopic += FirmwareGlobal.mqtt_client;
...
void ZeroCrossMomentStart(void) {
  if (!FirmwareGlobal.zc_interval) { return; }

#ifdef DEBUG_ZEROCROSS
  uint32_t dbg_interval = FirmwareGlobal.zc_interval;
  uint32_t dbg_zctime = FirmwareGlobal.zc_time;
  uint32_t dbg_starttime = micros();
#endif

  uint32_t trigger_moment = FirmwareGlobal.zc_time + FirmwareGlobal.zc_interval - FirmwareGlobal.zc_offset - FirmwareGlobal.zc_code_offset;
  while (TimeReachedUsec(trigger_moment)) {    // Trigger moment already passed so try next
    trigger_moment += FirmwareGlobal.zc_interval;
  }
  ...
  FirmwareGlobal.zc_code_offset = micros();
}

void ZeroCrossMomentEnd(void) {
  if (!FirmwareGlobal.zc_interval) { return; }
  FirmwareGlobal.zc_code_offset = (micros() - FirmwareGlobal.zc_code_offset) / 2;

#ifdef DEBUG_ZEROCROSS
  AddLog(LOG_LEVEL_DEBUG, PSTR("ZCD: CodeExecTime %d"), FirmwareGlobal.zc_code_offset * 2);
#endif
}

void IRAM_ATTR ZeroCrossIsr(void) {
  uint32_t time = micros();
  FirmwareGlobal.zc_interval = ((int32_t) (time - FirmwareGlobal.zc_time));
  FirmwareGlobal.zc_time = time;
}

void ZeroCrossInit(uint32_t offset) {
  if (PinUsed(GPIO_ZEROCROSS)) {
    FirmwareGlobal.zc_offset = offset;
...
void SetLatchingRelay(power_t lpower, uint32_t state) {
  // FirmwareGlobal.power xx00 - toggle REL1 (Off) and REL3 (Off) - device 1 Off, device 2 Off
  // FirmwareGlobal.power xx01 - toggle REL2 (On)  and REL3 (Off) - device 1 On,  device 2 Off
  // FirmwareGlobal.power xx10 - toggle REL1 (Off) and REL4 (On)  - device 1 Off, device 2 On
  // FirmwareGlobal.power xx11 - toggle REL2 (On)  and REL4 (On)  - device 1 On,  device 2 On
  static power_t latching_power = 0;     // Power state at latching start

  if (state && !FirmwareGlobal.latching_relay_pulse) {  // Set latching relay to power if previous pulse has finished
    latching_power = lpower;
    FirmwareGlobal.latching_relay_pulse = 2;            // max 200mS (initiated by stateloop())
  }

  for (uint32_t i = 0; i < FirmwareGlobal.devices_present; i++) {
    uint32_t port = (i << 1) + ((latching_power >> i) &1);
    DigitalWrite(GPIO_REL1, port, bitRead(FirmwareGlobal.rel_inverted, port) ? !state : state);
  }
}

void SetDevicePower(power_t rpower, uint32_t source) {
  if (FirmwareGlobal.power_on_delay) {
    FirmwareGlobal.power_on_delay_state = rpower;
    return;
  }

  ShowSource(source);
  FirmwareGlobal.last_source = source;

  if (POWER_ALL_ALWAYS_ON == Settings->poweronstate) {  // All on and stay on
    FirmwareGlobal.power = POWER_MASK >> (POWER_SIZE - FirmwareGlobal.devices_present);
    rpower = FirmwareGlobal.power;
  }
  ...
  if (Settings->flag.interlock) {          // Allow only one or no relay set - CMND_INTERLOCK - Enable/disable interlock
    for (uint32_t i = 0; i < MAX_INTERLOCKS; i++) {
      power_t mask = 1;
      uint32_t count = 0;
      for (uint32_t j = 0; j < FirmwareGlobal.devices_present; j++) {
  ...
  if (rpower) {                           // Any power set
    FirmwareGlobal.last_power = rpower;
  }
  ...
  if (XdrvCall(FUNC_SET_DEVICE_POWER)) {  // Set power state and stop if serviced
...
#ifdef ESP8266
  else if ((SONOFF_DUAL == FirmwareGlobal.module_type) || (CH4 == FirmwareGlobal.module_type)) {
    Serial.write(0xA0);
    Serial.write(0x04);
  ...
  else if (EXS_RELAY == FirmwareGlobal.module_type) {
    ...
    for (uint32_t i = 0; i < FirmwareGlobal.devices_present; i++) {
      ...
      if (bitRead(FirmwareGlobal.rel_bistable, port)) {
    ...
    if (FirmwareGlobal.rel_bistable) {             // If bistable relays in the mix reset them after 40ms
      ...
      for (uint32_t i = 0; i < port; i++) {       // Reset up to detected amount of ports
        if (bitRead(FirmwareGlobal.rel_bistable, i)) {
          DigitalWrite(GPIO_REL1, i, bitRead(FirmwareGlobal.rel_inverted, i) ? 1 : 0);
        }
      }
    }
  }
...
{
  if (FirmwareGlobal.power != FirmwareGlobal.last_power) {
    FirmwareGlobal.power = FirmwareGlobal.last_power;
    SetDevicePower(FirmwareGlobal.power, source);
    if (publish_power) {
...
// state 16 = POWER_SHOW_STATE = Show power state
  ...
  if ((state >= POWER_OFF) && (state <= POWER_TOGGLE)) {
    power_t all_on = POWER_MASK >> (POWER_SIZE - FirmwareGlobal.devices_present);
    switch (state) {
    case POWER_OFF:
      FirmwareGlobal.power = 0;
      break;
    case POWER_ON:
      FirmwareGlobal.power = all_on;
      break;
    case POWER_TOGGLE:
      FirmwareGlobal.power ^= all_on;      // Complement current state
    }
    SetDevicePower(FirmwareGlobal.power, source);
  }
  if (publish_power) {
...
{
#ifdef ESP8266
  if (MOTOR == FirmwareGlobal.module_type) {
  ...
  if (POWER_ALL_ALWAYS_ON == Settings->poweronstate) {
    ...
    power_t devices_mask = POWER_MASK >> (POWER_SIZE - FirmwareGlobal.devices_present);
    if (ResetReasonPowerOn()) {
      switch (Settings->poweronstate) {
      case POWER_ALL_OFF:
      case POWER_ALL_OFF_PULSETIME_ON:
        FirmwareGlobal.power = 0;
        SetDevicePower(FirmwareGlobal.power, SRC_RESTART);
        break;
      case POWER_ALL_ON:  // All on
        FirmwareGlobal.power = devices_mask;
        SetDevicePower(FirmwareGlobal.power, SRC_RESTART);
        break;
      case POWER_ALL_SAVED_TOGGLE:
        FirmwareGlobal.power = (Settings->power & devices_mask) ^ POWER_MASK;
        if (Settings->flag.save_state) {  // SetOption0 - Save power state and use after restart
          SetDevicePower(FirmwareGlobal.power, SRC_RESTART);
        }
        break;
      case POWER_ALL_SAVED:
        FirmwareGlobal.power = Settings->power & devices_mask;
        if (Settings->flag.save_state) {  // SetOption0 - Save power state and use after restart
          SetDevicePower(FirmwareGlobal.power, SRC_RESTART);
        }
        break;
...
//  AddLog(LOG_LEVEL_DEBUG, PSTR("PWR: PowerOnState %d restored"), Settings->poweronstate);
  ...
  for (uint32_t i = 0; i < FirmwareGlobal.devices_present; i++) {
    ...
    if (!Settings->flag3.no_power_feedback &&  // SetOption63 - Don't scan relay power state at restart - #5594 and #5663
        !FirmwareGlobal.power_on_delay) {       // SetOption47 - Delay switching relays to reduce power surge at power on
      if ((port < MAX_RELAYS) && PinUsed(GPIO_REL1, port)) {
        if (bitRead(FirmwareGlobal.rel_bistable, port)) {
          port++;                              // Skip both bistable relays as always 0
        } else {
          bitWrite(FirmwareGlobal.power, i, digitalRead(Pin(GPIO_REL1, port)) ^ bitRead(FirmwareGlobal.rel_inverted, port));
        }
      }
      port++;
    }
#endif  // ESP8266
    if (bitRead(FirmwareGlobal.power, i) || (POWER_ALL_OFF_PULSETIME_ON == Settings->poweronstate)) {
      SetPulseTimer(i % MAX_PULSETIMERS, Settings->pulse_timer[i % MAX_PULSETIMERS]);
    }
  }

  FirmwareGlobal.blink_powersave = FirmwareGlobal.power;
#ifdef USE_RULES
  RulesEvery50ms();
#endif
}
...
{
	for (uint32_t i = 0; i < FirmwareGlobal.leds_present; i++) {
		SetLedPowerIdx(i, bitRead(FirmwareGlobal.led_power, i));
	}
}
...
*/
  if (PinUsed(GPIO_LED1, led)) {
    ...
    if (state) {
      state = 1;
      FirmwareGlobal.led_power |= mask;
    } else {
      FirmwareGlobal.led_power &= (0xFF ^ mask);
    }
    uint16_t pwm = 0;
...
#endif
        analogWrite(Pin(GPIO_LED1, led), bitRead(FirmwareGlobal.led_inverted, led) ? Settings->pwm_range - pwm : pwm);
    } else {
      DigitalWrite(GPIO_LED1, led, bitRead(FirmwareGlobal.led_inverted, led) ? !state : state);
    }
  }
#ifdef USE_BUZZER
  if (led == 0) {
...
*/
    ...
    power_t mask = 1;
    for (uint32_t i = 0; i < FirmwareGlobal.leds_present; i++) {  // Map leds to power
      bool tstate = (FirmwareGlobal.power & mask);
...
void SetLedLink(uint32_t state) {
  ...
  uint32_t led_inv = FirmwareGlobal.ledlnk_inverted;
  ...
  if (Settings->flag4.powered_off_led) FirmwareGlobal.restore_powered_off_led_counter = 3;
...
void DebugLed(uint32_t mode) {
  ...
  if (PinUsed(GPIO_LEDLNK)) {
    ...
    digitalWrite(Pin(GPIO_LEDLNK), (FirmwareGlobal.ledlnk_inverted) ? !mode : mode);
  }
}

void SetPulseTimer(uint32_t index, uint32_t time)
{
  FirmwareGlobal.pulse_timer[index] = (time > 111) ? millis() + (1000 * (time - 100)) : (time > 0) ? millis() + (100 * time) : 0L;
}

uint32_t GetPulseTimer(uint32_t index)
{
  long time = TimePassedSince(FirmwareGlobal.pulse_timer[index]);
  if (time < 0) {
    time *= -1;
...
// state 10 = POWER_DELAYED = button released delayed
  ...
  if (Settings->flag.mqtt_enabled && MqttIsConnected() && (strlen(key_topic) != 0) && strcmp(key_topic, "0")) {  // SetOption3 - Enable MQTT
    if (!key && (device > FirmwareGlobal.devices_present)) {
    ...
    if (CLEAR_RETAIN == state) {
      ...
      if ((Settings->flag3.button_switch_force_local ||      // SetOption61 - Force local operation when button/switch topic is set
           !strcmp(FirmwareGlobal.mqtt_topic, key_topic) ||
           !strcmp(SettingsText(SET_MQTT_GRP_TOPIC), key_topic)) &&
          (POWER_TOGGLE == state)) {
        state = ~(FirmwareGlobal.power >> (device -1)) &1;   // POWER_OFF or POWER_ON
      }
...
#ifdef USE_PWM_DIMMER
  if (PWM_DIMMER != FirmwareGlobal.module_type || (!result && !Settings->flag3.mqtt_buttons)) {
#endif  // USE_PWM_DIMMER
...
#ifdef USE_PWM_DIMMER
    if (PWM_DIMMER == FirmwareGlobal.module_type) result = true;
  }
#endif  // USE_PWM_DIMMER
  return result;
}
...
//  if (1049 == LANGUAGE_LCID) { return; }
  ...
  if (IsModuleIfan()) {
    FirmwareGlobal.blink_mask &= 1;   // No blinking on the fan relays
  ...
  if ((device < 1) || (device > FirmwareGlobal.devices_present)) {
    device = 1;
  }
  FirmwareGlobal.active_device = device;
  ...
  if (state <= POWER_TOGGLE) {
    if ((FirmwareGlobal.blink_mask & mask)) {
      FirmwareGlobal.blink_mask &= (POWER_MASK ^ mask);  // Clear device mask
      MqttPublishPowerBlinkState(device);
    }

    if (Settings->flag.interlock &&        // CMND_INTERLOCK - Enable/disable interlock
        !interlock_mutex &&
        ((POWER_ON == state) || ((POWER_TOGGLE == state) && !(FirmwareGlobal.power & mask)))
       ) {
      ...
      for (uint32_t i = 0; i < MAX_INTERLOCKS; i++) {
        if (Settings->interlock[i] & mask) {             // Find interlock group
          for (uint32_t j = 0; j < FirmwareGlobal.devices_present; j++) {
            power_t imask = 1 << j;
            if ((Settings->interlock[i] & imask) && (FirmwareGlobal.power & imask) && (mask != imask)) {
    ...
    power_t old_power = FirmwareGlobal.power;
#endif  // USE_DEVICE_GROUPS
    switch (state) {
    case POWER_OFF: {
      FirmwareGlobal.power &= (POWER_MASK ^ mask);
      break; }
    case POWER_ON:
      FirmwareGlobal.power |= mask;
      break;
...
#ifdef USE_DEVICE_GROUPS
    if (FirmwareGlobal.power != old_power && SRC_REMOTE != source && SRC_RETRY != source) {
      power_t dgr_power = FirmwareGlobal.power;
...
#endif  // USE_DOMOTICZ
#ifdef USE_KNX
    KnxUpdatePowerState(device, FirmwareGlobal.power);
#endif  // USE_KNX
    ...
    SetPulseTimer((device -1) % MAX_PULSETIMERS, (((POWER_ALL_OFF_PULSETIME_ON == Settings->poweronstate) ? ~FirmwareGlobal.power : FirmwareGlobal.power) & mask) ? Settings->pulse_timer[(device -1) % MAX_PULSETIMERS] : 0);
  }
  else if (POWER_BLINK == state) {
    if (!(FirmwareGlobal.blink_mask & mask)) {
      FirmwareGlobal.blink_powersave = (FirmwareGlobal.blink_powersave & (POWER_MASK ^ mask)) | (FirmwareGlobal.power & mask);  // Save state
      FirmwareGlobal.blink_power = (FirmwareGlobal.power >> (device -1))&1;  // Prep to Toggle
    }
    FirmwareGlobal.blink_timer = millis() + 100;
    FirmwareGlobal.blink_counter = ((!Settings->blinkcount) ? 64000 : (Settings->blinkcount *2)) +1;
    FirmwareGlobal.blink_mask |= mask;  // Set device mask
    MqttPublishPowerBlinkState(device);
    return;
  }
  else if (POWER_BLINK_STOP == state) {
    bool flag = (FirmwareGlobal.blink_mask & mask);
    FirmwareGlobal.blink_mask &= (POWER_MASK ^ mask);  // Clear device mask
    MqttPublishPowerBlinkState(device);
    if (flag) {
      ExecuteCommandPower(device, (FirmwareGlobal.blink_powersave >> (device -1))&1, SRC_IGNORE);  // Restore state
    }
    return;
  }
  if (publish_power) {
...
void StopAllPowerBlink(void)
{
  ...
  for (uint32_t i = 1; i <= FirmwareGlobal.devices_present; i++) {
    mask = 1 << (i -1);
    if (FirmwareGlobal.blink_mask & mask) {
      FirmwareGlobal.blink_mask &= (POWER_MASK ^ mask);  // Clear device mask
      MqttPublishPowerBlinkState(i);
      ExecuteCommandPower(i, (FirmwareGlobal.blink_powersave >> (i -1))&1, SRC_IGNORE);  // Restore state
    }
  }
}
...
void MqttShowState(void)
{
  ...
  ResponseAppend_P(PSTR(",\"" D_JSON_HEAPSIZE "\":%d,\"SleepMode\":\"%s\",\"Sleep\":%u,\"LoadAvg\":%u,\"MqttCount\":%u"),
    ...
    FirmwareGlobal.sleep, FirmwareGlobal.loop_load_avg, MqttConnectCount());
  ...
  for (uint32_t i = 1; i <= FirmwareGlobal.devices_present; i++) {
    ...
    if ((LightDevice()) && (i >= LightDevice())) {
      ...
      ResponseAppend_P(PSTR(",\"%s\":\"%s\""), GetPowerDevice(stemp1, i, sizeof(stemp1), Settings->flag.device_index_enable),  // SetOption26 - Switch between POWER or POWER1
                                               GetStateText(bitRead(FirmwareGlobal.power, i-1)));
#ifdef USE_SONOFF_IFAN
      if (IsModuleIfan()) {
  ...
  if (FirmwareGlobal.pwm_present) {
    ResponseAppend_P(PSTR(","));
    MqttShowPWMState();
  }

  if (!FirmwareGlobal.global_state.wifi_down) {
...
void GetSensorValues(void) {
  ...
  for (uint32_t type = 0; type < 3; type++) {
    if (!Settings->global_sensor_index[type] || FirmwareGlobal.user_globals[type]) { continue; }
...
//        AddLog(LOG_LEVEL_DEBUG, PSTR("DBG: %s value%d = %2_f"), key, idx, &new_value);
    ...
    if (value != -9999) {
      switch (type) {
        case 0:  // Temperature
          FirmwareGlobal.temperature_celsius = ConvertTempToCelsius(value);
          break;
        case 1:  // Humidity
          FirmwareGlobal.humidity = value;
          break;
        case 2:  // Pressure
          FirmwareGlobal.pressure_hpa = ConvertHgToHpa(value);
          break;
      }
      FirmwareGlobal.global_update = FirmwareGlobal.uptime;
    }
  }
}
...
bool MqttShowSensor(bool call_show_sensor) {
  ...
  if (FirmwareGlobal.global_update && Settings->flag.mqtt_add_global_info) {  // SetOption2 (MQTT) Add global temperature/humidity/pressure info to JSON sensor message
    if ((FirmwareGlobal.humidity > 0) || !isnan(FirmwareGlobal.temperature_celsius) || (FirmwareGlobal.pressure_hpa != 0)) {
      uint32_t add_comma = 0;
      ResponseAppend_P(PSTR(",\"Global\":{"));
      if (!isnan(FirmwareGlobal.temperature_celsius)) {
        float t = ConvertTempToFahrenheit(FirmwareGlobal.temperature_celsius);
        ResponseAppend_P(PSTR("\"" D_JSON_TEMPERATURE "\":%*_f"),
          Settings->flag2.temperature_resolution, &t);
        add_comma++;
      }
      if (FirmwareGlobal.humidity > 0) {
        ResponseAppend_P(PSTR("%s\"" D_JSON_HUMIDITY "\":%*_f"),
          (add_comma)?",":"", Settings->flag2.humidity_resolution, &FirmwareGlobal.humidity);
        add_comma++;
      }
      if (2 == add_comma) {
        float dewpoint = CalcTempHumToDew(FirmwareGlobal.temperature_celsius, FirmwareGlobal.humidity);
        ResponseAppend_P(PSTR("%s\"" D_JSON_DEWPOINT "\":%*_f"),
          (add_comma)?",":"", Settings->flag2.temperature_resolution, &dewpoint);
      }
      if (FirmwareGlobal.pressure_hpa != 0) {
        float p = ConvertPressure(FirmwareGlobal.pressure_hpa);
        float s = ConvertPressureForSeaLevel(FirmwareGlobal.pressure_hpa);
...
{
  FirmwareGlobal.uptime++;

  if (POWER_CYCLE_TIME == FirmwareGlobal.uptime) {
    UpdateQuickPowerCycle(false);
  }

  if (BOOT_LOOP_TIME == FirmwareGlobal.uptime) {
    RtcRebootReset();
  ...
  if (FirmwareGlobal.power_on_delay) {
    if (1 == Settings->param[P_POWER_ON_DELAY2]) {       // SetOption47 1
      ...
      if (!FirmwareGlobal.global_state.network_down) {
        FirmwareGlobal.power_on_delay = 0;
      }
    }
    else if (2 == Settings->param[P_POWER_ON_DELAY2]) {  // SetOption47 2
      // Allow relay power on once mqtt is available
      if (!FirmwareGlobal.global_state.mqtt_down) {
        FirmwareGlobal.power_on_delay = 0;
      }
    }
    else {                                               // SetOption47 3..255
      // Allow relay power on after x seconds
      FirmwareGlobal.power_on_delay--;
    }
    if (!FirmwareGlobal.power_on_delay && FirmwareGlobal.power_on_delay_state) {
      // Set relays according to last SetDevicePower() request
      SetDevicePower(FirmwareGlobal.power_on_delay_state, SRC_SO47);
    }
  }

  if (FirmwareGlobal.mqtt_cmnd_blocked_reset) {
    FirmwareGlobal.mqtt_cmnd_blocked_reset--;
    if (!FirmwareGlobal.mqtt_cmnd_blocked_reset) {
      FirmwareGlobal.mqtt_cmnd_blocked = 0;             // Clean up MQTT cmnd loop block
    }
  }

  if (FirmwareGlobal.seriallog_timer) {
    FirmwareGlobal.seriallog_timer--;
    if (!FirmwareGlobal.seriallog_timer) {
      if (FirmwareGlobal.seriallog_level) {
        AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_APPLICATION D_SERIAL_LOGGING_DISABLED));
      }
      FirmwareGlobal.seriallog_level = 0;
    }
  }

  if (FirmwareGlobal.syslog_timer) {  // Restore syslog level
    FirmwareGlobal.syslog_timer--;
    if (!FirmwareGlobal.syslog_timer) {
      FirmwareGlobal.syslog_level = Settings->syslog_level;
  ...
  if ((FirmwareGlobal.init_state >= INIT_GPIOS) && PinUsed(GPIO_HEARTBEAT)) {
    digitalWrite(Pin(GPIO_HEARTBEAT), ~FirmwareGlobal.heartbeat_inverted &1);
    delayMicroseconds(50);
    digitalWrite(Pin(GPIO_HEARTBEAT), FirmwareGlobal.heartbeat_inverted);
  }

  // Teleperiod
  if (Settings->tele_period || (3601 == FirmwareGlobal.tele_period)) {
    if (FirmwareGlobal.tele_period >= 9999) {
      if (!FirmwareGlobal.global_state.network_down) {
        FirmwareGlobal.tele_period = 0;  // Allow teleperiod once wifi is connected
      }
    } else {
      FirmwareGlobal.tele_period++;
      if (FirmwareGlobal.tele_period >= Settings->tele_period) {
        FirmwareGlobal.tele_period = 0;
        ...
        if (!(FirmwareGlobal.tele_period % 10)) {
          for (uint32_t type = 0; type < 3; type++) {
            if (!Settings->global_sensor_index[type] || FirmwareGlobal.user_globals[type]) { continue; }
            GetSensorValues();
            break;
...
#ifdef ESP32
  if (11 == FirmwareGlobal.uptime) {  // Perform one-time ESP32 houskeeping
...
void Every100mSeconds(void)
{
  ...
  if (FirmwareGlobal.latching_relay_pulse) {
    FirmwareGlobal.latching_relay_pulse--;
    if (!FirmwareGlobal.latching_relay_pulse) {
#ifdef ESP8266
      if (EXS_RELAY == FirmwareGlobal.module_type) {
  ...
  for (uint32_t i = 0; i < MAX_PULSETIMERS; i++) {
    if (FirmwareGlobal.pulse_timer[i] != 0L) {           // Timer active?
      if (TimeReached(FirmwareGlobal.pulse_timer[i])) {  // Timer finished?
        FirmwareGlobal.pulse_timer[i] = 0L;              // Turn off this timer
        for (uint32_t j = 0; (i + j) < FirmwareGlobal.devices_present; j = j +MAX_PULSETIMERS) {
  ...
  if (FirmwareGlobal.blink_mask) {
    if (TimeReached(FirmwareGlobal.blink_timer)) {
      SetNextTimeInterval(FirmwareGlobal.blink_timer, 100 * Settings->blinktime);
      FirmwareGlobal.blink_counter--;
      if (!FirmwareGlobal.blink_counter) {
        StopAllPowerBlink();
      } else {
        FirmwareGlobal.blink_power ^= 1;
        power_now = (FirmwareGlobal.power & (POWER_MASK ^ FirmwareGlobal.blink_mask)) | ((FirmwareGlobal.blink_power) ? FirmwareGlobal.blink_mask : 0);
...
// As the max amount of sleep = 250 mSec this loop should always be taken...
  ...
  FirmwareGlobal.state_250mS++;
  FirmwareGlobal.state_250mS &= 0x3;

  FirmwareGlobal.global_state.network_down = (FirmwareGlobal.global_state.wifi_down && FirmwareGlobal.global_state.eth_down) ? 1 : 0;

  if (!Settings->flag.global_state) {                      // SetOption31 - Control link led blinking
    if (FirmwareGlobal.global_state.data &0x03) {                        // Network or MQTT problem
      if (FirmwareGlobal.global_state.mqtt_down) { blinkinterval = 7; }  // MQTT problem so blink every 2 seconds (slowest)
      if (FirmwareGlobal.global_state.network_down) { blinkinterval = 3; }  // Network problem so blink every second (slow)
      FirmwareGlobal.blinks = 201;                         // Allow only a single blink in case the problem is solved
    }
  }
  if (FirmwareGlobal.blinks || FirmwareGlobal.restart_flag || FirmwareGlobal.ota_state_flag) {
    if (FirmwareGlobal.restart_flag || FirmwareGlobal.ota_state_flag) {  // Overrule blinks and keep led lit
      FirmwareGlobal.blinkstate = true;                                  // Stay lit
    } else {
      blinkspeed--;
      if (!blinkspeed) {
        blinkspeed = blinkinterval;                       // Set interval to 0.2 (default), 1 or 2 seconds
        FirmwareGlobal.blinkstate ^= 1;                                  // Blink
      }
    }
    if ((!(Settings->ledstate &0x08)) && ((Settings->ledstate &0x06) || (FirmwareGlobal.blinks > 200) || (FirmwareGlobal.blinkstate))) {
      SetLedLink(FirmwareGlobal.blinkstate);                            // Set led on or off
    }
    if (!FirmwareGlobal.blinkstate) {
      FirmwareGlobal.blinks--;
      if (200 == FirmwareGlobal.blinks) { FirmwareGlobal.blinks = 0; }  // Disable blink
    }
  }
  if (Settings->ledstate &1 && (PinUsed(GPIO_LEDLNK) || !(FirmwareGlobal.blinks || FirmwareGlobal.restart_flag || FirmwareGlobal.ota_state_flag)) ) {
    bool tstate = FirmwareGlobal.power & Settings->ledmask;
#ifdef ESP8266
    if ((SONOFF_TOUCH == FirmwareGlobal.module_type) || (SONOFF_T11 == FirmwareGlobal.module_type) || (SONOFF_T12 == FirmwareGlobal.module_type) || (SONOFF_T13 == FirmwareGlobal.module_type)) {
      tstate = (!FirmwareGlobal.power) ? 1 : 0;           // As requested invert signal for Touch devices to find them in the dark
    }
#endif  // ESP8266
    SetLedPower(tstate);
  }
...
\*-------------------------------------------------------------------------------------------*/
  ...
  switch (FirmwareGlobal.state_250mS) {
  case 0:                                                 // Every x.0 second
    if (FirmwareGlobal.ota_state_flag && CommandsReady()) {
      FirmwareGlobal.ota_state_flag--;
      if (2 == FirmwareGlobal.ota_state_flag) {
...
//        if (Settings->webserver) StopWebserver();       // 20230107 No more need for disabling webserver during OTA
        ...
        if (ota_retry_counter) {
          ...
          if (RtcSettings.ota_loader) {
            ...
            // Replace Firmware                                         with Firmware-minimal
            // Replace Firmware-DE                                      with Firmware-minimal
            // Replace Firmware.bin                                     with Firmware-minimal.bin
            // Replace Firmware.xyz                                     with Firmware-minimal.xyz
            // Replace Firmware.bin.gz                                  with Firmware-minimal.bin.gz
            // Replace Firmware.xyz.gz                                  with Firmware-minimal.xyz.gz
            // Replace Firmware.ino.bin                                 with Firmware-minimal.ino.bin
            // Replace Firmware.ino.bin.gz                              with Firmware-minimal.ino.bin.gz
            // Replace http://domus1:80/api/arduino/Firmware.bin        with http://domus1:80/api/arduino/Firmware-minimal.bin
            // Replace http://domus1:80/api/arduino/Firmware.bin.gz     with http://domus1:80/api/arduino/Firmware-minimal.bin.gz
            // Replace http://domus1:80/api/arduino/Firmware-DE.bin.gz  with http://domus1:80/api/arduino/Firmware-minimal.bin.gz
            // Replace http://domus1:80/api/ard-uino/Firmware-DE.bin.gz with http://domus1:80/api/ard-uino/Firmware-minimal.bin.gz
            // Replace http://192.168.2.17:80/api/arduino/Firmware.bin  with http://192.168.2.17:80/api/arduino/Firmware-minimal.bin
            // Replace http://192.168.2.17/api/arduino/Firmware.bin.gz  with http://192.168.2.17/api/arduino/Firmware-minimal.bin.gz
...
//AddLog(LOG_LEVEL_DEBUG, PSTR("OTA: File type [%s]"), ech);
            ...
            char *pch = strrchr(bch, '-');                             // Find last dash (-) and ignore remainder - handles Firmware-DE
            if (pch == nullptr) { pch = ech; }                         // No dash so ignore filetype
            *pch = '\0';                                               // full_ota_url = http://domus1:80/api/arduino/Firmware
...
//#ifdef CONFIG_IDF_TARGET_ESP32C3
          ...
          char version[50];
          snprintf_P(version, sizeof(version), PSTR("%s%s"), FirmwareGlobal.version, FirmwareGlobal.image_name);
          ...
          if (!OTAclient.begin(full_ota_url)) {
            ...
            httpUpdateLight.setFactory(FirmwareGlobal.ota_factory);
...
#endif  // ESP8266
#endif  // FIRMWARE_MINIMAL
            FirmwareGlobal.ota_state_flag = 2;             // Upgrade failed - retry
          }
        }
      }
      if (90 == FirmwareGlobal.ota_state_flag) {           // Allow MQTT to reconnect
        FirmwareGlobal.ota_state_flag = 0;
        Response_P(PSTR("{\"" D_CMND_UPGRADE "\":\""));
        if (ota_result) {
          ResponseAppend_P(PSTR(D_JSON_SUCCESSFUL ". " D_JSON_RESTARTING));
          FirmwareGlobal.restart_flag = 2;
        } else {
          ResponseAppend_P(PSTR(D_JSON_FAILED " %s"), ESPhttpUpdate.getLastErrorString().c_str());
        }
        ResponseAppend_P(PSTR("\"}"));
//        FirmwareGlobal.restart_flag = 2;                   // Restart anyway to keep memory clean webserver
  ...
  case 1:                                                 // Every x.25 second
    ...
    if (FirmwareGlobal.save_data_counter && CommandsReady()) {
      FirmwareGlobal.save_data_counter--;
      if (FirmwareGlobal.save_data_counter <= 0) {
        if (Settings->flag.save_state) {                   // SetOption0 - Save power state and use after restart
          ...
          if (!((Settings->power &mask) == (FirmwareGlobal.power &mask))) {
            Settings->power = FirmwareGlobal.power;
          }
        } else {
          Settings->power = 0;
        }
        if (!FirmwareGlobal.restart_flag) { SettingsSave(0); }
        FirmwareGlobal.save_data_counter = Settings->save_data;
      }
    }

    if (FirmwareGlobal.restart_flag && CommandsReady()) {
      if ((214 == FirmwareGlobal.restart_flag) ||          // Reset 4
          (215 == FirmwareGlobal.restart_flag) ||          // Reset 5
          (216 == FirmwareGlobal.restart_flag)) {          // Reset 6
...
//        if (216 == FirmwareGlobal.restart_flag) {
          // Backup mqtt host, port, client, username and password
//        }
        if ((215 == FirmwareGlobal.restart_flag) ||        // Reset 5
            (216 == FirmwareGlobal.restart_flag)) {        // Reset 6
        ...
        if (216 == FirmwareGlobal.restart_flag) {          // Reset 6
        ...
        FirmwareGlobal.restart_flag = 3;                   // Finish backlog then Restart 1
      }
      else if (213 == FirmwareGlobal.restart_flag) {       // Reset 3
        SettingsSdkErase();  // Erase flash SDK parameters
        FirmwareGlobal.restart_flag = 2;                   // Restart 1
      }
      else if (212 == FirmwareGlobal.restart_flag) {       // Reset 2
        SettingsErase(0);    // Erase all flash from program end to end of physical flash
        FirmwareGlobal.restart_flag = 211;                 // Reset 1
      }

      if (211 == FirmwareGlobal.restart_flag) {            // Reset 1
        SettingsDefault();
        FirmwareGlobal.restart_flag = 3;                   // Finish backlog then Restart 1
      }

      if (2 == FirmwareGlobal.restart_flag) {              // Restart 1
        SettingsSaveAll();
      }

      FirmwareGlobal.restart_flag--;
      if (FirmwareGlobal.restart_flag <= 0) {
        AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_APPLICATION "%s"), (FirmwareGlobal.restart_halt) ? PSTR("Halted") : PSTR(D_RESTARTING));
        EspRestart();
      }
    }
    break;
  case 2:                                                 // Every x.5 second
    if (Settings->flag4.network_wifi) {
      WifiCheck(FirmwareGlobal.wifi_state_flag);
      FirmwareGlobal.wifi_state_flag = WIFI_RESTART;
    } else {
      WifiDisable();
    }
    break;
...
//#ifdef CONFIG_IDF_TARGET_ESP32C3
        ...
        if (OtaFactoryRead()) {
          OtaFactoryWrite(false);
          FirmwareGlobal.ota_state_flag = 3;
...
void ArduinoOTAInit(void)
{
  ...
  {
    if ((LOG_LEVEL_DEBUG <= FirmwareGlobal.seriallog_level)) {
  ...
  ArduinoOTA.onError([](ota_error_t error)
  {
    ...
    */
    ...
    if ((LOG_LEVEL_DEBUG <= FirmwareGlobal.seriallog_level) && arduino_ota_progress_dot_count) { TasConsole.println(); }
    switch (error) {
  ...
  ArduinoOTA.onEnd([]()
  {
    if ((LOG_LEVEL_DEBUG <= FirmwareGlobal.seriallog_level)) { TasConsole.println(); }
...
//    yield();
    delay(0);
    FirmwareGlobal.serial_in_byte = Serial.read();

    if (0 == FirmwareGlobal.serial_in_byte_counter) {
      serial_buffer_overrun = false;
    }
    else if ((FirmwareGlobal.serial_in_byte_counter == INPUT_BUFFER_SIZE)
#ifdef ESP8266
             || Serial.hasOverrun()
#endif
                                                             ) {
...
\*-------------------------------------------------------------------------------------------*/
    if ((SONOFF_DUAL == FirmwareGlobal.module_type) || (CH4 == FirmwareGlobal.module_type)) {
      FirmwareGlobal.serial_in_byte = ButtonSerial(FirmwareGlobal.serial_in_byte);
    }
#endif  // ESP8266
/*-------------------------------------------------------------------------------------------*/

    if (XdrvCall(FUNC_SERIAL)) {
      FirmwareGlobal.serial_in_byte_counter = 0;
      Serial.flush();
      return;
    }
...
/*-------------------------------------------------------------------------------------------*/
    ...
    if (FirmwareGlobal.serial_in_byte > 127 && !Settings->flag.mqtt_serial_raw) {   // Discard binary data above 127 if no raw reception allowed - CMND_SERIALSEND3
      FirmwareGlobal.serial_in_byte_counter = 0;
      Serial.flush();
      return;
    }
    if (!Settings->flag.mqtt_serial) {                                             // SerialSend active - CMND_SERIALSEND and CMND_SERIALLOG
      if (isprint(FirmwareGlobal.serial_in_byte)) {                                 // Any char between 32 and 127
        if (FirmwareGlobal.serial_in_byte_counter < INPUT_BUFFER_SIZE -1) {         // Add char to string if it still fits
          FirmwareGlobal.serial_in_buffer[FirmwareGlobal.serial_in_byte_counter++] = FirmwareGlobal.serial_in_byte;
        } else {
          serial_buffer_overrun = true;                                            // Signal overrun but continue reading input to flush until '\n' (EOL)
        }
      }
    } else {
      if (FirmwareGlobal.serial_in_byte || Settings->flag.mqtt_serial_raw) {        // Any char between 1 and 127 or any char (0 - 255) - CMND_SERIALSEND3
        bool in_byte_is_delimiter =                                                // Char is delimiter when...
          (((Settings->serial_delimiter < 128) && (FirmwareGlobal.serial_in_byte == Settings->serial_delimiter)) || // Any char between 1 and 127 and being delimiter
          ((Settings->serial_delimiter == 128) && !isprint(FirmwareGlobal.serial_in_byte))) &&   // Any char not between 32 and 127
          !Settings->flag.mqtt_serial_raw;                                         // In raw mode (CMND_SERIALSEND3) there is never a delimiter

        if ((FirmwareGlobal.serial_in_byte_counter < INPUT_BUFFER_SIZE -1) &&       // Add char to string if it still fits and ...
            !in_byte_is_delimiter) {                                               // Char is not a delimiter
          FirmwareGlobal.serial_in_buffer[FirmwareGlobal.serial_in_byte_counter++] = FirmwareGlobal.serial_in_byte;
        }

        if ((FirmwareGlobal.serial_in_byte_counter >= INPUT_BUFFER_SIZE -1) ||      // Send message when buffer is full or ...
...
\*-------------------------------------------------------------------------------------------*/
    if (SONOFF_SC == FirmwareGlobal.module_type) {
      if (FirmwareGlobal.serial_in_byte == '\x1B') {                                // Sonoff SC status from ATMEGA328P
        FirmwareGlobal.serial_in_buffer[FirmwareGlobal.serial_in_byte_counter] = 0;  // Serial data completed
        SonoffScSerialInput(FirmwareGlobal.serial_in_buffer);
        FirmwareGlobal.serial_in_byte_counter = 0;
        Serial.flush();
        return;
      }
...
/*-------------------------------------------------------------------------------------------*/
    ...
    if (!Settings->flag.mqtt_serial && (FirmwareGlobal.serial_in_byte == '\n')) {   // CMND_SERIALSEND and CMND_SERIALLOG
      FirmwareGlobal.serial_in_buffer[FirmwareGlobal.serial_in_byte_counter] = 0;    // Serial data completed
      FirmwareGlobal.seriallog_level = (Settings->seriallog_level < LOG_LEVEL_INFO) ? (uint8_t)LOG_LEVEL_INFO : Settings->seriallog_level;
      if (serial_buffer_overrun) {
        AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_COMMAND "Serial buffer overrun"));
      } else {
        AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_COMMAND "%s"), FirmwareGlobal.serial_in_buffer);
        ExecuteCommand(FirmwareGlobal.serial_in_buffer, SRC_SERIAL);
      }
      FirmwareGlobal.serial_in_byte_counter = 0;
  ...
  if (Settings->flag.mqtt_serial && FirmwareGlobal.serial_in_byte_counter && (millis() > (serial_polling_window + SERIAL_POLLING))) {  // CMND_SERIALSEND and CMND_SERIALLOG
    FirmwareGlobal.serial_in_buffer[FirmwareGlobal.serial_in_byte_counter] = 0;      // Serial data completed
    bool assume_json = (!Settings->flag.mqtt_serial_raw && (FirmwareGlobal.serial_in_buffer[0] == '{'));
    ...
    if (assume_json) {
      ResponseAppend_P(FirmwareGlobal.serial_in_buffer);
    } else {
      ResponseAppend_P(PSTR("\""));
      if (Settings->flag.mqtt_serial_raw) {
        ResponseAppend_P(PSTR("%*_H"), FirmwareGlobal.serial_in_byte_counter, FirmwareGlobal.serial_in_buffer);
      } else {
        ResponseAppend_P(EscapeJSONString(FirmwareGlobal.serial_in_buffer).c_str());
      }
      ResponseAppend_P(PSTR("\""));
    }
    ResponseJsonEnd();
...
//  AddLog(LOG_LEVEL_DEBUG, PSTR("DBG: Used GPIOs %d"), GPIO_SENSOR_END);
  ...
  for (uint32_t i = 0; i < nitems(Settings->my_gp.io); i++) {
    if ((Settings->my_gp.io[i] >= AGPIO(GPIO_SENSOR_END)) && (Settings->my_gp.io[i] < AGPIO(GPIO_USER))) {
      ...
      FirmwareGlobal.my_module.io[i] = Settings->my_gp.io[i];       // Set User selected Module sensors
    }
    if ((template_gp.io[i] > GPIO_NONE) && (template_gp.io[i] < AGPIO(GPIO_USER))) {
      FirmwareGlobal.my_module.io[i] = template_gp.io[i];               // Force Template override
    }
  }

  for (uint32_t i = 0; i < nitems(FirmwareGlobal.my_module.io); i++) {
    uint32_t mpin = ValidPin(i, FirmwareGlobal.my_module.io[i]);
    ...
    if (mpin) {                                     // Above GPIO_NONE
      ...
      if ((mpin >= AGPIO(GPIO_OPTION_A)) && (mpin < (AGPIO(GPIO_OPTION_A) + MAX_OPTIONS_A))) {
        bitSet(FirmwareGlobal.gpio_optiona.data, mpin - AGPIO(GPIO_OPTION_A));
        mpin = GPIO_NONE;
      }
#ifdef ROTARY_V1
        ...
        FirmwareGlobal.emulated_module_type = pgm_read_byte(kModuleEmulationList + (mpin - AGPIO(GPIO_OPTION_E)));
        SetModuleType();
        mpin = GPIO_NONE;
      }
...
//  AddLog(LOG_LEVEL_DEBUG, PSTR("DBG: FirmwareGlobal.gpio_pin %*_V"), nitems(FirmwareGlobal.gpio_pin), (uint8_t*)FirmwareGlobal.gpio_pin);

  if (ResetReasonPowerOn()) {
    FirmwareGlobal.power_on_delay = Settings->param[P_POWER_ON_DELAY2];  // SetOption47 - Delay switching relays to reduce power surge at power on
    if (FirmwareGlobal.power_on_delay) {
      ...
      for (uint32_t i = 0; i < MAX_RELAYS; i++) {
        if (PinUsed(GPIO_REL1, i)) {
          DigitalWrite(GPIO_REL1, i, bitRead(FirmwareGlobal.rel_inverted, i) ? 1 : 0);  // Off
        }
      }
...
#ifdef ESP8266
  if ((2 == Pin(GPIO_TXD)) || (H801 == FirmwareGlobal.module_type)) { Serial.set_tx(2); }
  SetSerialSwap();
#endif
  ...
  FirmwareGlobal.soft_spi_enabled = sspi_mosi + sspi_miso;
  AddLogSpi(0, Pin(GPIO_SSPI_SCLK), Pin(GPIO_SSPI_MOSI), Pin(GPIO_SSPI_MISO));

#ifdef USE_SPI
#ifdef ESP8266
  if (!FirmwareGlobal.soft_spi_enabled) {
    ...
    FirmwareGlobal.spi_enabled = (valid_cs) ? SPI_MOSI_MISO : SPI_NONE;
    if (FirmwareGlobal.spi_enabled) {
      FirmwareGlobal.my_module.io[12] = AGPIO(GPIO_SPI_MISO);
      SetPin(12, AGPIO(GPIO_SPI_MISO));
      FirmwareGlobal.my_module.io[13] = AGPIO(GPIO_SPI_MOSI);
      SetPin(13, AGPIO(GPIO_SPI_MOSI));
      FirmwareGlobal.my_module.io[14] = AGPIO(GPIO_SPI_CLK);
...
/*
  if (PinUsed(GPIO_SPI_CS) ||
    ...
    FirmwareGlobal.spi_enabled = spi_mosi + spi_miso;
  }
*/
  uint32_t spi_mosi = (PinUsed(GPIO_SPI_CLK) && PinUsed(GPIO_SPI_MOSI)) ? SPI_MOSI : SPI_NONE;
  uint32_t spi_miso = (PinUsed(GPIO_SPI_CLK) && PinUsed(GPIO_SPI_MISO)) ? SPI_MISO : SPI_NONE;
...
*/
  ...
  DigitalWrite(GPIO_HEARTBEAT, 0, FirmwareGlobal.heartbeat_inverted);
...
#ifdef USE_I2C
  FirmwareGlobal.i2c_enabled = (PinUsed(GPIO_I2C_SCL) && PinUsed(GPIO_I2C_SDA));
  if (FirmwareGlobal.i2c_enabled) {
    FirmwareGlobal.i2c_enabled = I2cBegin(Pin(GPIO_I2C_SDA), Pin(GPIO_I2C_SCL));
  }
#ifdef ESP32
  FirmwareGlobal.i2c_enabled_2 = (PinUsed(GPIO_I2C_SCL, 1) && PinUsed(GPIO_I2C_SDA, 1));
  if (FirmwareGlobal.i2c_enabled_2) {
    FirmwareGlobal.i2c_enabled_2 = I2c2Begin(Pin(GPIO_I2C_SDA, 1), Pin(GPIO_I2C_SCL, 1));
  }
#endif
#endif  // USE_I2C

  FirmwareGlobal.devices_present = 0;
  uint32_t bi_device = 0;
  for (uint32_t i = 0; i < MAX_RELAYS; i++) {
    if (PinUsed(GPIO_REL1, i)) {
      FirmwareGlobal.devices_present++;
#ifdef ESP8266
      if (EXS_RELAY == FirmwareGlobal.module_type) {
        if (i &1) { FirmwareGlobal.devices_present--; }
      }
#endif  // ESP8266
      if (bitRead(FirmwareGlobal.rel_bistable, i)) {
        if (bi_device &1) { FirmwareGlobal.devices_present--; }
        bi_device++;
      }
    }
  }
  ...
  FirmwareGlobal.light_type = LT_BASIC;                     // Use basic PWM control if SetOption15 = 0

  if (XdrvCall(FUNC_MODULE_INIT)) {                        // Init and claim single module (like tuya, armtronix, ifan, light)
    // Serviced
  }
#ifdef ESP8266
  else if (YTF_IR_BRIDGE == FirmwareGlobal.module_type) {
    ClaimSerial();  // Stop serial loopback mode
  }
  else if (SONOFF_DUAL == FirmwareGlobal.module_type) {
    UpdateDevicesPresent(2);
    SetSerial(19200, TS_SERIAL_8N1);
  }
  else if (CH4 == FirmwareGlobal.module_type) {
...
#ifdef USE_ARILUX_RF
      if ((3 == i) && (FirmwareGlobal.leds_present < 2) && !PinUsed(GPIO_ARIRFSEL)) {
        SetPin(Pin(GPIO_LED1, i), AGPIO(GPIO_ARIRFSEL));  // Legacy support where LED4 was Arilux RF enable
      } else {
#endif
        FirmwareGlobal.leds_present++;
        DigitalWrite(GPIO_LED1, i, bitRead(FirmwareGlobal.led_inverted, i));
#ifdef USE_ARILUX_RF
      }
#endif
    }
  }
  DigitalWrite(GPIO_LEDLNK, 0, FirmwareGlobal.ledlnk_inverted);

#ifdef USE_PWM_DIMMER
  if (PWM_DIMMER == FirmwareGlobal.module_type && PinUsed(GPIO_REL1)) { FirmwareGlobal.devices_present--; }
#endif  // USE_PWM_DIMMER
