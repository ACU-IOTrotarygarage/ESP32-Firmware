/*
  support_esp.ino - ESP specific code for Firmware

  Copyright (C) 2021  Theo Arends and Mahmoud Shams El-Din / Jörg Schüler-Maroldt
 ...
 *
 * At the end the common Firmware calls are provided
\*********************************************************************************************/
...
// Add below line to Firmware_globals.h
...
void SettingsErase(uint8_t type) {
  // SDK and Firmware data is held in default NVS partition
  // Firmware data is held also in file /.settings on default filesystem
  // cal_data - SDK PHY calibration data as documented in esp_phy_init.h
  // qpc      - Firmware Quick Power Cycle state
  // main     - Firmware Settings data
  int32_t r1, r2, r3 = 0;
  switch (type) {
...
//      nvs_flash_erase();  // Erase RTC, PHY, sta.mac, ap.sndchan, ap.mac, Firmware etc.
      r1 = NvmErase("qpc");
      r2 = NvmErase("main");
#ifdef USE_UFILESYS
      r3 = TfsDeleteFile(TASM_FILE_SETTINGS);
#endif
      AddLog(LOG_LEVEL_DEBUG, PSTR(D_LOG_APPLICATION D_ERASE " Firmware data (%d,%d,%d)"), r1, r2, r3);
      break;
...
//      r1 = NvmErase("cal_data");
      ...
      break;
    case 3:               // QPC Reached = QPC, Firmware and SDK parameter area (0x0F3xxx - 0x0FFFFF)
//      nvs_flash_erase();  // Erase RTC, PHY, sta.mac, ap.sndchan, ap.mac, Firmware etc.
      r1 = NvmErase("qpc");
      r2 = NvmErase("main");
//      r3 = esp_phy_erase_cal_data_in_nvs();
//      r3 = NvmErase("cal_data");
//      AddLog(LOG_LEVEL_DEBUG, PSTR(D_LOG_APPLICATION D_ERASE " Firmware (%d,%d) and PHY data (%d)"), r1, r2, r3);
#ifdef USE_UFILESYS
      r3 = TfsDeleteFile(TASM_FILE_SETTINGS);
#endif
      AddLog(LOG_LEVEL_DEBUG, PSTR(D_LOG_APPLICATION D_ERASE " Firmware data (%d,%d,%d)"), r1, r2, r3);
...
uint32_t EspFlashBaseAddress(void) {
  if (EspSingleOtaPartition()) {       // Only one partition so start at end of sketch
    ...
    return partition->address;  // For Firmware 0x00010000 or 0x00200000
  }
}
...
uint32_t EspFlashBaseEndAddress(void) {
  ...
  if (!partition) { return 0; }
  return partition->address + partition->size;  // For Firmware 0x00200000 or 0x003F0000
}
...
//    const esp_partition_t* partition = esp_ota_get_next_update_partition(nullptr);
    if (!partition) { return 0; }
    err = esp_partition_mmap(partition, 0, 4 * SPI_FLASH_MMU_PAGE_SIZE, SPI_FLASH_MMAP_DATA, (const void **)&FirmwareGlobal_mmap_data, &handle);

    AddLog(LOG_LEVEL_DEBUG, PSTR("DBG: Partition start 0x%08X, Partition end 0x%08X, Mmap data 0x%08X"), partition->address, partition->size, FirmwareGlobal_mmap_data);
 ...
 * Standard Firmware 32 do not include those patches.
