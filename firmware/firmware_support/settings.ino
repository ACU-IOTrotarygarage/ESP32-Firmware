/*
  settings.ino - user settings for Firmware
...
/*********************************************************************************************\
 * ESP8266 Firmware Flash usage offset from 0x40200000
 *
 * Firmware 1M  Firmware 2M  Firmware 4M - Flash usage
 ...
 * 0x000F3000  0x000F6000  0x000F6000 - 4k Firmware Quick Power Cycle counter (SETTINGS_LOCATION - CFG_ROTATES) - First four bytes only
 * 0x000F3FFF  0x000F6FFF  0x000F6FFF
 * 0x000F4000  0x000F7000  0x000F7000 - 4k First Firmware rotating settings page
 *    ::::
 * 0x000FA000  0x000FD000  0x000FD000 - 4k Last Firmware rotating settings page = Last page used by Core OTA (SETTINGS_LOCATION)
 ...
 *
 * 0x000FB000  0x001FB000  0x003FB000 - 4k Core EEPROM = Firmware settings page during OTA and when no flash rotation is active (EEPROM_LOCATION)
 ...
 * 0x000FF000  0x001FF000  0x0031F000 - 4k SDK - Uses at least first 32 bytes of this page - Firmware Zigbee persistence from 0x000FF800 to 0x000FFFFF
...
String SettingsConfigFilename(void) {
  char filename[TOPSZ];
  char hostname[sizeof(FirmwareGlobal.hostname)];
  snprintf_P(filename, sizeof(filename), PSTR("Config_%s_%s.dmp"), NoAlNumToUnderscore(hostname, FirmwareGlobal.hostname), FirmwareGlobal.version);
 ...
 */
  ...
  if ((GetSettingsCrc32() != settings_crc32) || rotate) {
    if (1 == rotate) {                                 // Use eeprom flash slot only and disable flash rotate from now on (upgrade)
      FirmwareGlobal.stop_flash_rotate = 1;
    }

    if (FirmwareGlobal.stop_flash_rotate || (2 == rotate)) {  // Use eeprom flash slot and erase next flash slots if stop_flash_rotate is off (default)
    ...
    if (!FirmwareGlobal.stop_flash_rotate && rotate) {  // SetOption12 - (Settings) Switch between dynamic (0) or fixed (1) slot flash save location
...
void SettingsErase(uint8_t type) {
  /*
    ...
    3 = Erase Firmware and SDK parameter area (0x0F3xxx - 0x0FFFFF)
...
*/
  ...
  else if (3 == type) {    // QPC Reached = QPC and Firmware and SDK parameter area (0x0F3xxx - 0x0FFFFF)
#ifdef USE_UFILESYS
