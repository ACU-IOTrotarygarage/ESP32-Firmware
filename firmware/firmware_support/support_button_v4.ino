/*
  support_button.ino - button support for Firmware

  Copyright (C) 2022  Federico Leoni and Theo Arends and Mahmoud Shams El-Din
...
void ButtonProbe(void) {
  if (Button.probe_mutex || (FirmwareGlobal.uptime < 4)) { return; }  // Block GPIO for 4 seconds after poweron to workaround Wemos D1 / Obi RTS circuit
...
#ifdef ESP8266
    if ((0 == i) && ((SONOFF_DUAL == FirmwareGlobal.module_type) || (CH4 == FirmwareGlobal.module_type))) {
...
void ButtonHandler(void) {
  if (FirmwareGlobal.uptime < 4) { return; }                    // Block GPIO for 4 seconds after poweron to workaround Wemos D1 / Obi RTS circuit
    ...
    if (XdrvCall(FUNC_BUTTON_PRESSED)) {
...
#ifdef ESP8266
    else if (SONOFF_4CHPRO == FirmwareGlobal.module_type) {
      ...
      if (!Settings->flag.button_single) {                   // SetOption13 (0) - Allow multi-press
        if (Button.window_timer[button_index]) {
          ...
          if (!FirmwareGlobal.restart_flag && !Button.hold_timer[button_index] && (Button.press_counter[button_index] > 0) && (Button.press_counter[button_index] < 7)) {

            bool single_press = false;
            if (Button.press_counter[button_index] < 3) {    // Single or Double press
#ifdef ESP8266
              if ((SONOFF_DUAL_R2 == FirmwareGlobal.module_type) || (SONOFF_DUAL == FirmwareGlobal.module_type) || (CH4 == FirmwareGlobal.module_type)) {
                single_press = true;
              } else
#endif  // ESP8266
              {
                single_press = (Settings->flag.button_swap +1 == Button.press_counter[button_index]);  // SetOption11 (0)
                if ((1 == Button.used) && (2 == FirmwareGlobal.devices_present)) {  // Single Button with two devices only
                  ...
                  if (!Settings->flag3.mqtt_buttons) {       // SetOption73 - Detach buttons from relays and enable MQTT action state for multipress
                    if (Button.press_counter[button_index] == 1) {  // By default first press always send a TOGGLE (2)
                      ...
                      if (0 == button_index) {               // BUTTON1 can toggle up to 5 relays if present. If a relay is not present will send out the key value (2,11,12,13 and 14) for rules
                        uint32_t max_device = (FirmwareGlobal.devices_present < MAX_RELAY_BUTTON1) ? FirmwareGlobal.devices_present : MAX_RELAY_BUTTON1;
