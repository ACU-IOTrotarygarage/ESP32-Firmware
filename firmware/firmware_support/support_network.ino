/*
  support_network.ino - Network support for Firmware
...
void StartMdns(void) {
  if (Settings->flag3.mdns_enabled) {  // SetOption55 - Control mDNS service
    if (!Mdns.begun) {
      ...
      Mdns.begun = (uint8_t)MDNS.begin(FirmwareGlobal.hostname);
      AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_MDNS "%s '%s.local'"), (Mdns.begun) ? PSTR(D_INITIALIZED) : PSTR(D_FAILED), FirmwareGlobal.hostname);
    }
  }
}
...
void MdnsAddServiceHttp(void) {
  if (1 == Mdns.begun) {
    ...
    MDNS.addServiceTxt("http", "tcp", "devicetype", "Firmware");
  }
}
...
#endif
#endif
  return FirmwareGlobal.hostname;
}
