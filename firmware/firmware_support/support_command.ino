/*
  support_command.ino - command support for Firmware
...
const char kFirmwareCommands[] PROGMEM = "|"  // No prefix
  // SetOptions synonyms
  D_SO_WIFINOSLEEP "|"
  // Other commands
...
SO_SYNONYMS(kFirmwareSynonyms,
  127,
);

void (* const FirmwareCommand[])(void) PROGMEM = {
...
{
  // Test WIFI Connection to Router if Firmware is in AP mode since in AP mode, a STA connection can be established
    ...
    if (Wifi.wifiTest != WIFI_TESTING) { // Start Test
      ...
      if (pos != nullptr) {
        ...
        AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_WIFI D_CONNECTING_TO_AP " %s " D_AS " %s ..."),
          SettingsText(Wifi.wifi_Test_Save_SSID2 ? SET_STASSID2 : SET_STASSID1), FirmwareGlobal.hostname);
...
void ExecuteCommand(const char *cmnd, uint32_t source)
{
  ...
  ShowSource(source);
  FirmwareGlobal.last_command_source = source;
  ...
  while (*pos && (isalpha(*pos) || isdigit(*pos) || '_' == *pos || '/' == *pos)) {
    if ('/' == *pos) {            // Skip possible cmnd/Firmware/ preamble
      start = pos + 1;
    }
    pos++;
  }
...
// topicBuf:        cmnd/Firmware/power1  dataBuf: toggle  = Mqtt command using topic
// topicBuf:       cmnd/Firmwares/power1  dataBuf: toggle  = Mqtt command using a group topic
...
void CommandHandler(char* topicBuf, char* dataBuf, uint32_t data_len) {
  ...
  FirmwareGlobal.fallback_topic_flag = (!strncmp(topicBuf, stemp1, strlen(stemp1)));
  ...
  if (type != nullptr) {
    ...
    if (Settings->ledstate &0x02) { FirmwareGlobal.blinks++; }

//    FirmwareGlobal.backlog_timer = millis() + (100 * MIN_BACKLOG_DELAY);
    FirmwareGlobal.backlog_timer = millis() + Settings->param[P_BACKLOG_DELAY];  // SetOption34
  ...
  // allow overwrite Firmware cmds
    if (!Script_SubCmd()) {
      if (!DecodeCommand(kFirmwareCommands, FirmwareCommand, kFirmwareSynonyms)) {
  ...
  FirmwareGlobal.fallback_topic_flag = false;
}
...
void CmndBacklog(void) {
  ...
  if (XdrvMailbox.data_len) {
    if (0 == XdrvMailbox.index) {
      FirmwareGlobal.backlog_nodelay = true;
    }
...
#else
    uint32_t bl_pointer = (!FirmwareGlobal.backlog_pointer) ? MAX_BACKLOG -1 : FirmwareGlobal.backlog_pointer;
    bl_pointer--;
    while ((blcommand != nullptr) && (FirmwareGlobal.backlog_index != bl_pointer))
#endif
    {
...
#else
        FirmwareGlobal.backlog[FirmwareGlobal.backlog_index] = blcommand;
        FirmwareGlobal.backlog_index++;
        if (FirmwareGlobal.backlog_index >= MAX_BACKLOG) {
          FirmwareGlobal.backlog_index = 0;
        }
#endif
      }
...
//    ResponseCmndChar(D_JSON_APPENDED);
    ResponseClear();
    FirmwareGlobal.backlog_timer = millis();
  } else {
    bool blflag = BACKLOG_EMPTY;
#ifdef SUPPORT_IF_STATEMENT
    backlog.clear();
#else
    FirmwareGlobal.backlog_pointer = FirmwareGlobal.backlog_index;
#endif
...
void CmndDelay(void)
{
  if ((XdrvMailbox.payload >= (MIN_BACKLOG_DELAY / 100)) && (XdrvMailbox.payload <= 3600)) {
    FirmwareGlobal.backlog_timer = millis() + (100 * XdrvMailbox.payload);
  }
  uint32_t bl_delay = 0;
  long bl_delta = TimePassedSince(FirmwareGlobal.backlog_timer);
...
void CmndPower(void)
{
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= FirmwareGlobal.devices_present)) {
...
void CmndStatus(void)
{
  ...
  if (!FirmwareGlobal.energy_driver && (9 == payload)) { return; }
  ...
  if ((0 == payload) || (-99 == payload)) {
    ...
    Response_P(PSTR("{\"" D_CMND_STATUS "\":{\"" D_CMND_MODULE "\":%d,\"" D_CMND_DEVICENAME "\":\"%s\",\"" D_CMND_FRIENDLYNAME "\":[%s],\"" D_CMND_TOPIC "\":\"%s\",\""
                          ...
                          ModuleNr(), EscapeJSONString(SettingsText(SET_DEVICENAME)).c_str(), stemp, FirmwareGlobal.mqtt_topic,
                          SettingsText(SET_MQTT_BUTTON_TOPIC), FirmwareGlobal.power, Settings->poweronstate, Settings->ledstate,
...
#endif
                          "}}"),
                          FirmwareGlobal.baudrate, GetSerialConfig().c_str(), SettingsText(SET_MQTT_GRP_TOPIC), SettingsText(SET_OTAURL),
  ...
  if ((0 == payload) || (2 == payload)) {
    Response_P(PSTR("{\"" D_CMND_STATUS D_STATUS2_FIRMWARE "\":{\"" D_JSON_VERSION "\":\"%s%s%s\",\"" D_JSON_BUILDDATETIME "\":\"%s\""
                          ...
                          "%s}}"),
                          FirmwareGlobal.version, FirmwareGlobal.image_name, GetCodeCores().c_str(), GetBuildDateAndTime().c_str()
#ifdef ESP8266
                          , ESP.getBootVersion()
#endif
                          , ESP.getSdkVersion(),
  ...
  if (((0 == payload) || (6 == payload)) && Settings->flag.mqtt_enabled) {  // SetOption3 - Enable MQTT
    Response_P(PSTR("{\"" D_CMND_STATUS D_STATUS6_MQTT "\":{\"" D_CMND_MQTTHOST "\":\"%s\",\"" D_CMND_MQTTPORT "\":%d,\"" D_CMND_MQTTCLIENT D_JSON_MASK "\":\"%s\",\""
                          ...
                          FirmwareGlobal.mqtt_client, EscapeJSONString(SettingsText(SET_MQTT_USER)).c_str(), MqttConnectCount(), MQTT_MAX_PACKET_SIZE, Settings->mqtt_keepalive, Settings->mqtt_socket_timeout);
  ...
  if (FirmwareGlobal.energy_driver) {
...
void CmndGlobalTemp(void) {
  if (2 == XdrvMailbox.index) {
    if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload < 251)) {
      ...
      FirmwareGlobal.user_globals[0] = 0;
    }
    ...
    if (XdrvMailbox.data_len > 0) {
      ...
      if ((temperature >= -50.0f) && (temperature <= 100.0f)) {
        FirmwareGlobal.temperature_celsius = temperature;
        FirmwareGlobal.global_update = FirmwareGlobal.uptime;
        FirmwareGlobal.user_globals[0] = 1;
      }
    }
    ResponseCmndFloat(FirmwareGlobal.temperature_celsius, 1);
  }
}
...
void CmndGlobalHum(void) {
  if (2 == XdrvMailbox.index) {
    if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload < 251)) {
      ...
      FirmwareGlobal.user_globals[1] = 0;
    }
    ...
    if (XdrvMailbox.data_len > 0) {
      ...
      if ((humidity >= 0.0f) && (humidity <= 100.0f)) {
        FirmwareGlobal.humidity = humidity;
        FirmwareGlobal.global_update = FirmwareGlobal.uptime;
        FirmwareGlobal.user_globals[1] = 1;
      }
    }
    ResponseCmndFloat(FirmwareGlobal.humidity, 1);
  }
}
...
void CmndGlobalPress(void) {
  if (2 == XdrvMailbox.index) {
    if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload < 251)) {
      ...
      FirmwareGlobal.user_globals[2] = 0;
    }
    ...
    if (XdrvMailbox.data_len > 0) {
      ...
      if ((pressure >= 0.0f) && (pressure <= 1200.0f)) {
        FirmwareGlobal.pressure_hpa = pressure;
        FirmwareGlobal.global_update = FirmwareGlobal.uptime;
        FirmwareGlobal.user_globals[2] = 1;
      }
    }
    ResponseCmndFloat(FirmwareGlobal.pressure_hpa, 1);
  }
}
...
void CmndSleep(void)
{
  if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload < 251)) {
    ...
    FirmwareGlobal.sleep = XdrvMailbox.payload;
    WiFiSetSleepMode();
  }
  Response_P(S_JSON_COMMAND_NVALUE_ACTIVE_NVALUE, XdrvMailbox.command, Settings->sleep, FirmwareGlobal.sleep);
...
#ifdef ESP32
    FirmwareGlobal.ota_factory = false;  // Reset in case of failed safeboot upgrade
#endif  // ESP32 and WEBCLIENT_HTTPS
    FirmwareGlobal.ota_state_flag = 3;
    char stemp1[TOPSZ];
    Response_P(PSTR("{\"%s\":\"" D_JSON_VERSION " %s " D_JSON_FROM " %s\"}"), XdrvMailbox.command, FirmwareGlobal.version, GetOtaUrl(stemp1, sizeof(stemp1)));
  }
#if defined(ESP32) && defined(USE_WEBCLIENT_HTTPS)
  else if (EspSingleOtaPartition() && !EspRunningFactoryPartition() && (1 == XdrvMailbox.data_len) && (2 == XdrvMailbox.payload)) {
    FirmwareGlobal.ota_factory = true;
    FirmwareGlobal.ota_state_flag = 3;
    ResponseCmndChar(PSTR("Safeboot"));
  }
#endif  // ESP32 and WEBCLIENT_HTTPS
  else {
    Response_P(PSTR("{\"%s\":\"" D_JSON_ONE_OR_GT "\"}"), XdrvMailbox.command, FirmwareGlobal.version);
  }
}
...
void CmndSeriallog(void)
{
  ...
  Response_P(S_JSON_COMMAND_NVALUE_ACTIVE_NVALUE, XdrvMailbox.command, Settings->seriallog_level, FirmwareGlobal.seriallog_level);
}
...
void CmndRestart(void)
{
  ...
  case 2:
    FirmwareGlobal.restart_flag = 2;
    FirmwareGlobal.restart_halt = true;
...
{
#ifdef ESP8266
  if (FirmwareGlobal.module_type != MOTOR)
#endif  // ESP8266
  {
...
void CmndBlinktime(void)
{
  if ((XdrvMailbox.payload > 1) && (XdrvMailbox.payload <= 3600)) {
    ...
    if (FirmwareGlobal.blink_timer > 0) {
      FirmwareGlobal.blink_timer = millis() + (100 * XdrvMailbox.payload);
    }
  }
...
void CmndBlinkcount(void)
{
  if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= 32000)) {
    ...
    if (FirmwareGlobal.blink_counter) { FirmwareGlobal.blink_counter = Settings->blinkcount *2; }
  }
      ...
      if (1 == ptype) {                    // SetOption32 .. 49
        ...
        if ((XdrvMailbox.payload >= param_low) && (XdrvMailbox.payload <= param_high)) {
          ...
          if (P_RGB_REMAP == pindex) {
            ...
            FirmwareGlobal.restart_flag = 2;  // SetOption37 needs a reboot in most cases
          }
#endif
        ...
        if (XdrvMailbox.payload <= 1) {
          if (2 == ptype) {                // SetOption0 .. 31
            ...
            if (12 == pindex) {            // stop_flash_rotate
              FirmwareGlobal.stop_flash_rotate = XdrvMailbox.payload;
              SettingsSave(2);
            }
#ifdef USE_HOME_ASSISTANT
...
#endif  // USE_HOME_ASSISTANT
#ifdef USE_Firmware_DISCOVERY
            if (19 == pindex) {
              TasRediscover();
            }
#endif  // USE_Firmware_DISCOVERY
          }
            ...
            switch (pindex) {
              case 5:                      // SetOption55
                if (0 == XdrvMailbox.payload) {
                  FirmwareGlobal.restart_flag = 2;  // Disable mDNS needs restart
                }
                break;
            ...
            switch (pindex) {
              ...
              case 5:                      // SetOption87 - (PWM Dimmer) Turn red LED on (1) when powered off
                FirmwareGlobal.restore_powered_off_led_counter = 1;
                break;
  ...
  if (strchr(XdrvMailbox.data, '{') == nullptr) {  // If no JSON it must be parameter
    if ((XdrvMailbox.payload > 0) && (XdrvMailbox.payload <= MAXMODULE)) {
      ...
      if (ValidTemplateModule(XdrvMailbox.payload)) {
        ...
        if (USER_MODULE == Settings->module) { FirmwareGlobal.restart_flag = 2; }
      }
    }
      ...
      for (uint32_t i = 0; i < nitems(Settings->user_template.gp.io); i++) {
        ...
        if (8 == i) { j = 12; }
#endif
        if (FirmwareGlobal.my_module.io[j] > GPIO_NONE) {
          Settings->user_template.gp.io[i] = FirmwareGlobal.my_module.io[j];
        }
        j++;
      }
    }
...
#ifndef FIRMWARE_MINIMAL      // if Firmware-minimal, `Template` is read-only
...
void CmndBaudrate(void)
{
  if (XdrvMailbox.payload >= 300) {
    ...
    FirmwareGlobal.baudrate = (XdrvMailbox.payload & 0xFFFF) * 300;
    SetSerialBaudrate(FirmwareGlobal.baudrate);
  }
  ResponseCmndNumber(FirmwareGlobal.baudrate);
}

void CmndSerialConfig(void)
{
  // See FirmwareSerialConfig for possible options
...
void CmndSyslog(void)
{
  ...
  Response_P(S_JSON_COMMAND_NVALUE_ACTIVE_NVALUE, XdrvMailbox.command, Settings->syslog_level, FirmwareGlobal.syslog_level);
}
...
void CmndNtpServer(void)
{
  ...
  //        FirmwareGlobal.restart_flag = 2;  // Issue #3890
        FirmwareGlobal.ntp_force_sync = true;
      }
...
void CmndWifiConfig(void)
{
  if ((XdrvMailbox.payload >= WIFI_RESTART) && (XdrvMailbox.payload < MAX_WIFI_OPTION)) {
    ...
    FirmwareGlobal.wifi_state_flag = Settings->sta_config;
...
void CmndInterlock(void)
{
  ...
  uint32_t max_relays = FirmwareGlobal.devices_present;
  if (FirmwareGlobal.light_type) { max_relays--; }
    ...
    if (XdrvMailbox.data_len > 0) {
      if (strchr(XdrvMailbox.data, ',') != nullptr) {           // Interlock entry
        ...
        if (Settings->flag.interlock) {
          SetDevicePower(FirmwareGlobal.power, SRC_IGNORE);                    // Remove multiple relays if set
        }
      }
#ifdef USE_SHUTTER
...
void CmndTeleperiod(void)
{
  ...
  FirmwareGlobal.tele_period = (Settings->tele_period) ? Settings->tele_period : 3601;  // Show teleperiod data also on empty command
...
void CmndReset(void)
{
  switch (XdrvMailbox.payload) {
  case 1:
    FirmwareGlobal.restart_flag = 211;
    ResponseCmndChar(PSTR(D_JSON_RESET_AND_RESTARTING));
    break;
  case 2 ... 6:
    FirmwareGlobal.restart_flag = 210 + XdrvMailbox.payload;
...
void CmndLedPower(void) {
  // If GPIO_LEDLINK (used for network status) then allow up to 4 GPIO_LEDx control using FirmwareGlobal.led_power
  ...
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= MAX_LEDS)) {
    ...
    if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= 2)) {
      ...
      FirmwareGlobal.blinks = 0;
      if (!PinUsed(GPIO_LEDLNK)) {
        SetLedPower(Settings->ledstate &8);
      } else {
        SetLedPowerIdx(XdrvMailbox.index -1, (FirmwareGlobal.led_power & mask));
      }
    }
    bool state = bitRead(FirmwareGlobal.led_power, XdrvMailbox.index -1);
    if (!PinUsed(GPIO_LEDLNK)) {
...
{
  if ((1 == XdrvMailbox.index) && (FirmwareGlobal.i2c_enabled)) {
    I2cScan();
  }
#ifdef ESP32
  if ((2 == XdrvMailbox.index) && (FirmwareGlobal.i2c_enabled_2)) {
    I2cScan(1);
  }
#endif
}
...
void CmndSetSensor(void)
{
  if (XdrvMailbox.index < MAX_XSNS_DRIVERS) {
    if (XdrvMailbox.payload >= 0) {
      ...
      if (1 == XdrvMailbox.payload) {
        FirmwareGlobal.restart_flag = 2;  // To safely re-enable a sensor currently most sensor need to follow complete restart init cycle
      }
    }
