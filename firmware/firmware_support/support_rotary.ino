/*
  support_rotary.ino - rotary switch support for Firmware
...
bool RotaryButtonPressed(uint32_t button_index) {
  ...
  for (uint32_t index = 0; index < MAX_ROTARIES; index++) {
    ...
    bool powered_on = (FirmwareGlobal.power);
#ifdef USE_LIGHT
...
void IRAM_ATTR RotaryIsrArg(void *arg) {
  ...
  // Theo Arends and Mahmoud Shams El-Din
...
void RotaryInit(void) {
  ...
  Rotary.model = !FirmwareGlobal.gpio_optiona.rotary_mi_desk;  // Option_A5
#ifdef ESP8266
  if (MI_DESK_LAMP == FirmwareGlobal.module_type) {
    Rotary.model = 0;
  }
#endif  // ESP8266
...
void RotaryHandler(void) {
  ...
  for (uint32_t index = 0; index < MAX_ROTARIES; index++) {
    ...
    if (Settings->save_data && (FirmwareGlobal.save_data_counter < 2)) {
      FirmwareGlobal.save_data_counter = 3;                   // Postpone flash writes while rotary is turned
    }
    ...
    if (!Settings->flag4.rotary_uses_rules) {   // SetOption98 - Use rules instead of light control
      ...
      if (0 == index) {                        // Rotary1
        if (button_pressed) {
          ...
          if (!Settings->flag4.rotary_poweron_dimlow || FirmwareGlobal.power) {  // SetOption113 - On rotary dial after power off set dimmer low
