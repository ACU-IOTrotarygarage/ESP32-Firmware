/*
  support_switch.ino - switch support for Firmware
...
void SwitchProbe(void) {
  if (Switch.probe_mutex || (FirmwareGlobal.uptime < 4)) { return; }  // Block GPIO for 4 seconds after poweron to workaround Wemos D1 / Obi RTS circuit
    ...
    if (switchflag <= POWER_TOGGLE) {
      if (!Settings->flag5.mqtt_switches) {                    // SetOption114 (0) - Detach Switches from relays and enable MQTT action state for all the SwitchModes
        if (!SendKey(KEY_SWITCH, i +1, switchflag)) {         // Execute command via MQTT
          ExecuteCommandPower(i +1, switchflag, SRC_SWITCH);  // Execute command internally (if i < FirmwareGlobal.devices_present)
        }
