/*
  support_a_i2c.ino - I2C support for Firmware
...
bool I2cValidRead(uint8_t addr, uint8_t reg, uint8_t size, uint8_t bus = 0) {
  i2c_buffer = 0;
#ifdef ESP32
  if (bus && !FirmwareGlobal.i2c_enabled_2) { return false; }  // Error
...
#ifdef ESP32
  if (bus && !FirmwareGlobal.i2c_enabled_2) { return true; }  // Error
  TwoWire & myWire = (bus == 0) ? Wire : Wire1;
#else
...
#ifdef ESP32
  if (bus && !FirmwareGlobal.i2c_enabled_2) { return 1; }  // Error
  TwoWire & myWire = (bus == 0) ? Wire : Wire1;
#else
...
#ifdef ESP32
  if (bus && !FirmwareGlobal.i2c_enabled_2) { return; }
  TwoWire & myWire = (bus == 0) ? Wire : Wire1;
...
#ifdef ESP32
  if (bus && !FirmwareGlobal.i2c_enabled_2) { return false; }  // If no second bus report as not present;
  TwoWire & myWire = (bus == 0) ? Wire : Wire1;
#else
  bus = 0;
  TwoWire & myWire = Wire;
