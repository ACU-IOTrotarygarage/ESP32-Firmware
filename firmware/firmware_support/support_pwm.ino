/*
  support_pwm.ino - command support for Firmware

  Copyright (C) 2021  Theo Arends and Mahmoud Shams El-Din & Stephan Hadinger
...
void PwmRearmChanges(void) {
  for (uint32_t i = 0; i < MAX_PWMS; i++) {
    ...
    FirmwareGlobal.pwm_value[i] = -1;          // no change wanted
    FirmwareGlobal.pwm_phase[i] = -1;          // no change wanted
  }
}
...
//       FirmwareGlobal.pwm_cur_value[i] = Settings->pwm_value[i];    // retrieve in Legacy pool for 0..4
//     } else {
//       FirmwareGlobal.pwm_cur_value[i] = Settings->pwm_value_ext[i - MAX_PWMS_LEGACY];    // retrieve in Legacy pool for 5..15
//     }
//     FirmwareGlobal.pwm_cur_phase[i] = 0;       // no phase shift for now, will be recomputed at first push to GPIOs
//   }
...
void PwmSaveToSettings(void) {
  for (uint32_t i = 0; i < MAX_PWMS; i++) {
    if (i < MAX_PWMS_LEGACY) {
      Settings->pwm_value[i] = FirmwareGlobal.pwm_cur_value[i];    // store in Legacy pool for 0..4
    } else {
      Settings->pwm_value_ext[i - MAX_PWMS_LEGACY] = FirmwareGlobal.pwm_cur_value[i];    // retrieve in Legacy pool for 5..15
    }
  }
}
...
// As input, `FirmwareGlobal.pwm_value[]` and `FirmwareGlobal.pwm_phase[]` contain the new expected values
// or `-1` if no change.
...
void PwmApplyGPIO(bool force_update_all) {
  ...
  for (uint32_t i = 0; i < MAX_PWMS; i++) {
    ...
    uint32_t pwm_val = FirmwareGlobal.pwm_cur_value[i];      // logical value of PWM, 0..1023
    uint32_t pwm_phase = FirmwareGlobal.pwm_cur_phase[i];    // pwm_phase is the logical phase of the active pulse, ignoring inverted
    ...
    if (PinUsed(GPIO_PWM1, i)) {
      ...
      if (FirmwareGlobal.pwm_value[i] >= 0) { pwm_val = FirmwareGlobal.pwm_value[i]; }    // new value explicitly specified
      if (pwm_val > range) { pwm_val = range; } // prevent overflow

      // compute phase
      if (FirmwareGlobal.pwm_phase[i] >= 0) {
        pwm_phase = FirmwareGlobal.pwm_phase[i]; // if explicit set explicitly, 
      ...
      // AddLog(LOG_LEVEL_INFO, "PWM: i=%i used=%i pwm_val=%03X vs %03X pwm_phase=%03X vs %03X", i, PinUsed(GPIO_PWM1, i), pwm_val, FirmwareGlobal.pwm_cur_value[i], pwm_phase, FirmwareGlobal.pwm_cur_phase[i]);

      if (force_update_all || (pwm_val != FirmwareGlobal.pwm_cur_value[i]) || (pwm_phase != FirmwareGlobal.pwm_cur_phase[i])) {
    ...
    FirmwareGlobal.pwm_cur_value[i] = pwm_val;
    FirmwareGlobal.pwm_cur_phase[i] = pwm_phase;
  }
  // AddLog(LOG_LEVEL_INFO, "PWM: Val=%03X-%03X-%03X-%03X-%03X Phase=%03X-%03X-%03X-%03X-%03X Range=%03X",
  //                         FirmwareGlobal.pwm_cur_value[0], FirmwareGlobal.pwm_cur_value[1], FirmwareGlobal.pwm_cur_value[2], FirmwareGlobal.pwm_cur_value[3],
  //                         FirmwareGlobal.pwm_cur_value[4],
  //                         FirmwareGlobal.pwm_cur_phase[0], FirmwareGlobal.pwm_cur_phase[1], FirmwareGlobal.pwm_cur_phase[2], FirmwareGlobal.pwm_cur_phase[3],
  //                         FirmwareGlobal.pwm_cur_phase[4],
  //                         Settings->pwm_range
  //                         );
...
void CmndPwm(void)
{
  if (FirmwareGlobal.pwm_present && (XdrvMailbox.index > 0) && (XdrvMailbox.index <= MAX_PWMS)) {
    if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= Settings->pwm_range) && PinUsed(GPIO_PWM1, XdrvMailbox.index -1)) {
      FirmwareGlobal.pwm_value[XdrvMailbox.index - 1] = XdrvMailbox.payload;
      PwmApplyGPIO(false);
    }
    Response_P(PSTR("{"));
...
void GpioInitPwm(void) {
  ...
  for (uint32_t i = 0; i < MAX_PWMS; i++) {     // Basic PWM control only
    if (PinUsed(GPIO_PWM1, i)) {
      analogAttach(Pin(GPIO_PWM1, i), bitRead(FirmwareGlobal.pwm_inverted, i));

      if (i < FirmwareGlobal.light_type) {
        // force PWM GPIOs to black
        FirmwareGlobal.pwm_value[i] = 0;
      } else {
        FirmwareGlobal.pwm_present = true;
        if (i < MAX_PWMS_LEGACY) {
          FirmwareGlobal.pwm_value[i] = Settings->pwm_value[i];
        } else {
          FirmwareGlobal.pwm_value[i] = Settings->pwm_value_ext[i - MAX_PWMS_LEGACY];
        }
      }
    }
  }
...
void CmndPwm(void)
{
  if (FirmwareGlobal.pwm_present && (XdrvMailbox.index > 0) && (XdrvMailbox.index <= MAX_PWMS)) {
    if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= Settings->pwm_range) && PinUsed(GPIO_PWM1, XdrvMailbox.index -1)) {
      ...
      analogWrite(Pin(GPIO_PWM1, pwm_index), bitRead(FirmwareGlobal.pwm_inverted, pwm_index) ? Settings->pwm_range - XdrvMailbox.payload : XdrvMailbox.payload);
    }
    Response_P(PSTR("{"));
    MqttShowPWMState();  // Render the PWM status to MQTT
    ResponseJsonEnd();
  ...
  for (uint32_t i = 0; i < MAX_PWMS; i++) {     // Basic PWM control only
    if (PinUsed(GPIO_PWM1, i)) {
      ...
      if (i < FirmwareGlobal.light_type) {
        ...
        analogWrite(Pin(GPIO_PWM1, i), bitRead(FirmwareGlobal.pwm_inverted, i) ? Settings->pwm_range : 0);
      } else {
        FirmwareGlobal.pwm_present = true;
        if (i < MAX_PWMS_LEGACY) {
          analogWrite(Pin(GPIO_PWM1, i), bitRead(FirmwareGlobal.pwm_inverted, i) ? Settings->pwm_range - Settings->pwm_value[i] : Settings->pwm_value[i]);
        } else {
          analogWrite(Pin(GPIO_PWM1, i), bitRead(FirmwareGlobal.pwm_inverted, i) ? Settings->pwm_range - Settings->pwm_value_ext[i] : Settings->pwm_value_ext[i]);
        }
      }
    }
  }
