/*
  support_rtc.ino - Real Time Clock support for Firmware
...
void RtcSecond(void) {
  ...
  if (Rtc.time_synced) {
    ...
    if (Rtc.restart_time == 0) {
      Rtc.restart_time = Rtc.utc_time - FirmwareGlobal.uptime;  // save first synced time as restart time
    }
    ...
    if (Rtc.local_time < START_VALID_TIME) {  // 2016-01-01
      FirmwareGlobal.rules_flag.time_init = 1;
    } else {
      FirmwareGlobal.rules_flag.time_set = 1;
    }
  } else {
    if (Rtc.last_synced) {
