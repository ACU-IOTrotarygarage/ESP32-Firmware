/*
  xdrv_62_improv.ino - IMPROV support for Firmware

  SPDX-FileCopyrightText: 2022 Theo Arends and Mahmoud Shams El-Din
...
void ImprovReceived(void) {
  uint32_t command = FirmwareGlobal.serial_in_buffer[9];
  switch (command) {
...
//      } else {
        ...
        uint32_t ssid_length = FirmwareGlobal.serial_in_buffer[11];
        uint32_t ssid_end = 12 + ssid_length;
        uint32_t pass_length = FirmwareGlobal.serial_in_buffer[ssid_end];
        uint32_t pass_start = ssid_end + 1;
        uint32_t pass_end = pass_start + pass_length;
        FirmwareGlobal.serial_in_buffer[ssid_end] = '\0';
        char* ssid = &FirmwareGlobal.serial_in_buffer[12];
        FirmwareGlobal.serial_in_buffer[pass_end] = '\0';
        char* password = &FirmwareGlobal.serial_in_buffer[pass_start];
#ifdef IMPROV_DEBUG
...
//      }
    ...
    case IMPROV_GET_DEVICE_INFO: {                             // 0x03
      // Firmware Zbbridge 11.0.0.7 ESP8266EX Wemos4
      // Firmware Sensors 11.0.0.7 ESP8266EX Wemos4
      // Firmware DE 11.0.0.7 ESP8266EX Wemos4
      char image_name[33];
      ...
      if (!strcmp_P(image_name, PSTR("EN")) &&                 // English
           strcasecmp_P("Firmware", PSTR(CODE_IMAGE_STR))) {    // Not Firmware
      ...
      uint32_t len = snprintf_P(data, sizeof(data), PSTR("01\nFirmware %s\n%s\n%s\n%s\n"),
                                image_name, FirmwareGlobal.version, GetDeviceHardware().c_str(), SettingsText(SET_DEVICENAME));
      data[0] = command;
...
bool ImprovSerialInput(void) {
  ...
  if (6 == FirmwareGlobal.serial_in_byte_counter) {
    FirmwareGlobal.serial_in_buffer[FirmwareGlobal.serial_in_byte_counter] = 0;
    if (!strcmp_P(FirmwareGlobal.serial_in_buffer, PSTR("IMPROV"))) {
      if (IMPROV_SERIAL_VERSION == FirmwareGlobal.serial_in_byte) {
        Improv.seriallog_level = FirmwareGlobal.seriallog_level;
        FirmwareGlobal.seriallog_level = 0;                     // Disable seriallogging interfering with IMPROV
  ...
  if (IMPROV_SERIAL_VERSION == Improv.version) {
    ...
    if (FirmwareGlobal.serial_in_byte_counter > 8) {            // Wait for length
      uint32_t data_len = FirmwareGlobal.serial_in_buffer[8];
      if (FirmwareGlobal.serial_in_byte_counter > 10 + data_len) {  // Receive including '\n'

        AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("IMP: Rcvd '%*_H'"), FirmwareGlobal.serial_in_byte_counter, FirmwareGlobal.serial_in_buffer);

        uint32_t checksum_pos = FirmwareGlobal.serial_in_byte_counter -2;
        uint8_t checksum = 0x00;
        for (uint32_t i = 0; i < checksum_pos; i++) {
          checksum += FirmwareGlobal.serial_in_buffer[i];
        }
        if (checksum != FirmwareGlobal.serial_in_buffer[checksum_pos]) {
          ImprovSendError(IMPROV_ERROR_INVALID_RPC);           // 0x01 - CRC error
        }
        else if (IMPROV_TYPE_RPC == FirmwareGlobal.serial_in_buffer[7]) {
          uint32_t data_length = FirmwareGlobal.serial_in_buffer[10];
          if (data_length == data_len - 2) {
            ImprovReceived();
          }
        }
        Improv.version = 0;                                    // Done
        FirmwareGlobal.seriallog_level = Improv.seriallog_level;  // Restore seriallogging
        return true;
      }
    }
...
void ImprovEverySecond(void) {
  if (Improv.wifi_timeout) {
    Improv.wifi_timeout--;
    if (Improv.wifi_timeout < IMPROV_WIFI_TIMEOUT -3) {        // Firmware restarts after ssid or password change
      if (WifiHasIP()) {
