/*
  xdrv_87_esp32_sonoff_tm1621.ino - Sonoff POWR3xxD and THR3xxD display support for Firmware
...
void TM1621Show(void) {
  ...
  if (TM1621_THR316D == Tm1621.device) {
    if (!isnan(FirmwareGlobal.temperature_celsius)) {
      float temperature = ConvertTempToFahrenheit(FirmwareGlobal.temperature_celsius);
      if (FirmwareGlobal.humidity == 0.0f) {  // No humidity so check for more temperature sensors
        if (0 == Tm1621.temp_sensors) {
    ...
    if (FirmwareGlobal.humidity > 0.0f) {
      Tm1621.humidity = true;
      ext_snprintf_P(Tm1621.row[1], sizeof(Tm1621.row[1]), PSTR("%1_f"), &FirmwareGlobal.humidity);
    }
  }
  TM1621SendRows();
}
