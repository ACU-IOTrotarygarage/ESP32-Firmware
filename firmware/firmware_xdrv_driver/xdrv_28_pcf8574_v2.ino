/*
  xdrv_28_pcf8574.ino - PCF8574 I2C support for Firmware

  Copyright (C) 2021  Stefan Bode and Theo Arends and Mahmoud Shams El-Din
 ...
 *
 * Mode 2:
 * Allows easy configuration by using a sequential list of pins configured as Firmware
...
/*********************************************************************************************\
 * PCF8574 Mode 2 - Theo Arends and Mahmoud Shams El-Din
\*********************************************************************************************/
...
void Pcf8574Power(void) {
  ...
  uint32_t relay_max = FirmwareGlobal.devices_present;
  if (!Pcf8574.base) {
...
void Pcf8574SwitchRelay(void) {
  for (uint32_t i = 0; i < FirmwareGlobal.devices_present; i++) {
    ...
    if (Pcf8574.max_devices > 0 && Pcf8574_pin[i] < 99) {
      ...
      uint8_t _val = bitRead(FirmwareGlobal.rel_inverted, i) ? !relay_state : relay_state;
...
void Pcf8574ModuleInitMode1(void) {
  ...
  for (uint32_t idx = 0; idx < Pcf8574.max_devices; idx++) { // suport up to 8 boards PCF8574
    ...
    for (uint32_t i = 0; i < 8; i++, gpio>>=1) {
      ...
      //AddLog(LOG_LEVEL_DEBUG, PSTR("PCF: I2C shift i %d: %d. Powerstate: %d, FirmwareGlobal.devices_present: %d"), i,_result, Settings->power>>i&1, FirmwareGlobal.devices_present);
      if (_result > 0) {
        Pcf8574_pin[FirmwareGlobal.devices_present] = i + 8 * idx;
        bitWrite(FirmwareGlobal.rel_inverted, FirmwareGlobal.devices_present, Settings->flag3.pcf8574_ports_inverted);  // SetOption81 - Invert all ports on PCF8574 devices
        if (!Settings->flag.save_state && !Settings->flag3.no_power_feedback) {  // SetOption63 - Don't scan relay power state at restart - #5594 and #5663
          ...
          bitWrite(FirmwareGlobal.power, FirmwareGlobal.devices_present, power_state);
          bitWrite(Settings->power, FirmwareGlobal.devices_present, power_state);
        }
  ...
  //AddLog(LOG_LEVEL_DEBUG, PSTR("PCF: Settings->power=0x%08X, FirmwareGlobal.power=0x%08X"), Settings->power, FirmwareGlobal.power);
...
void Pcf8574SaveSettings(void) {
  ...
  for (byte idx = 0; idx < Pcf8574.max_devices; idx++) {
    ...
    if (count <= FirmwareGlobal.devices_present) {
  ...
  if (Pcf8574.mode) {
    ...
    if (Pcf8574_pin) {
      ...
      if (Pcf8574LoadTemplate()) {
        ...
        Pcf8574.relay_offset = FirmwareGlobal.devices_present;
