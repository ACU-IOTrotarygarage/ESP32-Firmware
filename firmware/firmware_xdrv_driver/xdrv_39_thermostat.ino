/*
  xdrv_39_thermostat.ino - Thermostat controller for Firmware
...
{
  return (uint8_t)bitRead(FirmwareGlobal.power, (output_switch - 1));
}
...
void ThermostatSignalPreProcessingSlow(uint8_t ctr_output)
{
  ...
  if ((FirmwareGlobal.uptime - Thermostat[ctr_output].timestamp_temp_measured_update) > ((uint32_t)Thermostat[ctr_output].time_sens_lost * 60)) {
    Thermostat[ctr_output].status.sensor_alive = IFACE_OFF;
...
void ThermostatSignalProcessingFast(uint8_t ctr_output)
{
  ...
  if (Thermostat[ctr_output].status.status_input == IFACE_ON) {
    Thermostat[ctr_output].timestamp_input_on = FirmwareGlobal.uptime;
  }
  ...
  switch (Thermostat[ctr_output].status.controller_mode) {
    ...
    case CTR_RAMP_UP:
      ...
      if ((Thermostat[ctr_output].time_ctr_checkpoint != 0) &&
          (FirmwareGlobal.uptime >= Thermostat[ctr_output].time_ctr_checkpoint)) {
        // Reset times
        Thermostat[ctr_output].time_ctr_checkpoint = 0;
        Thermostat[ctr_output].timestamp_rampup_start = FirmwareGlobal.uptime;
      }
      break;
#ifdef USE_PI_AUTOTUNING
    // PI autotune
  ...
  if (Thermostat[ctr_output].status.controller_mode == CTR_HYBRID) {
    switch (Thermostat[ctr_output].status.phase_hybrid_ctr) {
      ...
      case CTR_HYBRID_PI:
          ...
          // then go to ramp-up
          if (((FirmwareGlobal.uptime - Thermostat[ctr_output].timestamp_output_off) > (60 * (uint32_t)Thermostat[ctr_output].time_allow_rampup))
...
bool ThermostatStateManualToAuto(uint8_t ctr_output)
{
  ...
  if ((Thermostat[ctr_output].status.status_input == IFACE_OFF)
    ...
    && ((FirmwareGlobal.uptime - Thermostat[ctr_output].timestamp_input_on) > ((uint32_t)Thermostat[ctr_output].time_manual_to_auto * 60))) {
    change_state = true;
  }
  return change_state;
}
...
//#endif // DEBUG_THERMOSTAT
    Thermostat[ctr_output].timestamp_output_off = FirmwareGlobal.uptime;
    Thermostat[ctr_output].status.status_output = IFACE_OFF;
#ifdef DEBUG_THERMOSTAT
    ThermostatVirtualSwitch(ctr_output);
#endif // DEBUG_THERMOSTAT
...
void ThermostatCalculatePI(uint8_t ctr_output)
{
  ...
  Thermostat[ctr_output].time_ctr_changepoint = FirmwareGlobal.uptime + (uint32_t)Thermostat[ctr_output].time_total_pi;
  // Adjust next cycle point
  Thermostat[ctr_output].time_ctr_checkpoint = FirmwareGlobal.uptime + ((uint32_t)Thermostat[ctr_output].time_pi_cycle * 60);
}

void ThermostatWorkAutomaticPI(uint8_t ctr_output)
{
  bool flag_heating = (Thermostat[ctr_output].status.climate_mode == CLIMATE_HEATING);
  if ( (FirmwareGlobal.uptime >= Thermostat[ctr_output].time_ctr_checkpoint)
    || (Thermostat[ctr_output].temp_target_level != Thermostat[ctr_output].temp_target_level_ctr)
    || (  (( (Thermostat[ctr_output].temp_measured < Thermostat[ctr_output].temp_target_level)
          && (Thermostat[ctr_output].temp_measured_gradient < 0)
          && (flag_heating))
  ...
  if (FirmwareGlobal.uptime < Thermostat[ctr_output].time_ctr_changepoint) {
...
void ThermostatWorkAutomaticRampUp(uint8_t ctr_output)
{
  ...
  time_in_rampup = FirmwareGlobal.uptime - Thermostat[ctr_output].timestamp_rampup_start;
  ...
  if ((time_in_rampup <= (60 * (uint32_t)Thermostat[ctr_output].time_rampup_max))
    ...
    if ( (abs(temp_delta_rampup) >= Thermostat[ctr_output].temp_rampup_delta_out)
      ...
      Thermostat[ctr_output].time_rampup_nextcycle = FirmwareGlobal.uptime + ((uint32_t)Thermostat[ctr_output].time_rampup_cycle * 60);
      // Set auxiliary variables
      Thermostat[ctr_output].temp_rampup_cycle = Thermostat[ctr_output].temp_measured;
      Thermostat[ctr_output].time_ctr_changepoint = FirmwareGlobal.uptime + (60 * (uint32_t)Thermostat[ctr_output].time_rampup_max);
    ...
    // Gradient calculation every time_rampup_cycle
    else if ((Thermostat[ctr_output].time_rampup_deadtime > 0) && (FirmwareGlobal.uptime >= Thermostat[ctr_output].time_rampup_nextcycle)) {
      ...
      if (   ((Thermostat[ctr_output].temp_rampup_meas_gradient > 0)
        ...
        Thermostat[ctr_output].temp_rampup_output_off = (int16_t)(((int32_t)temp_delta_rampup * (int32_t)(Thermostat[ctr_output].time_ctr_changepoint - (FirmwareGlobal.uptime - (time_total_rampup)))) / (int32_t)(time_total_rampup * Thermostat[ctr_output].counter_rampup_cycles)) + Thermostat[ctr_output].temp_rampup_cycle;
        // Set auxiliary variables
        Thermostat[ctr_output].time_rampup_nextcycle = FirmwareGlobal.uptime + ((uint32_t)Thermostat[ctr_output].time_rampup_cycle * 60);
        Thermostat[ctr_output].temp_rampup_cycle = Thermostat[ctr_output].temp_measured;
        ...
        Thermostat[ctr_output].time_ctr_changepoint = FirmwareGlobal.uptime + (60 * (uint32_t)Thermostat[ctr_output].time_rampup_max) - time_in_rampup;
        Thermostat[ctr_output].temp_rampup_output_off =  Thermostat[ctr_output].temp_target_level_ctr;
      }
    ...
    if ((Thermostat[ctr_output].time_rampup_deadtime == 0)
      || (Thermostat[ctr_output].time_ctr_checkpoint == 0)
      || (FirmwareGlobal.uptime < Thermostat[ctr_output].time_ctr_changepoint)
    ...
    Thermostat[ctr_output].time_ctr_checkpoint = FirmwareGlobal.uptime;
    // Switch Off output
    Thermostat[ctr_output].status.command_output = IFACE_OFF;
  }
}
...
void ThermostatPeakDetectorInit(uint8_t ctr_output)
{
  ...
  Thermostat[ctr_output].time_ctr_checkpoint = FirmwareGlobal.uptime + THERMOSTAT_TIME_MAX_AUTOTUNE;
}
  ...
  if (peak_num < THERMOSTAT_PEAKNUMBER_AUTOTUNE) {
    ...
    if ((peak_num % 2) == 0) {
      ...
      if ( (cond_peak_2)
        ...
        Thermostat[ctr_output].time_peak_timestamps_atune[peak_num] = (FirmwareGlobal.uptime / 60);
...
void ThermostatWorkAutomaticPIAutotune(uint8_t ctr_output)
{
  ...
  if ((FirmwareGlobal.uptime < Thermostat[ctr_output].time_ctr_checkpoint)
    ...
    if (FirmwareGlobal.uptime >= Thermostat[ctr_output].time_ctr_checkpoint) {
      ...
      Thermostat[ctr_output].time_ctr_changepoint = FirmwareGlobal.uptime + (((uint32_t)Thermostat[ctr_output].time_pi_cycle * (uint32_t)Thermostat[ctr_output].dutycycle_step_autotune) / (uint32_t)100);
      // Reset cycle active
      Thermostat[ctr_output].status.status_cycle_active = CYCLE_OFF;
    }
...
void ThermostatDebug(uint8_t ctr_output)
{
  ...
  dtostrfd(FirmwareGlobal.uptime, 0, result_chr);
  AddLog(LOG_LEVEL_DEBUG, PSTR("uptime: %s"), result_chr);
  dtostrfd(FirmwareGlobal.power, 0, result_chr);
  ...
  if (root) {
    ...
    if (value_token.isNum()) {
      ...
      if ( (value >= -1000)
        ...
        uint32_t timestamp = FirmwareGlobal.uptime;
