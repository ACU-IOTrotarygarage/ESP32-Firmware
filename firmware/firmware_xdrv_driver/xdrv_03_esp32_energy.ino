/*
  xdrv_03_esp32_energy.ino - Energy sensor support for Firmware

  Copyright (C) 2023  Theo Arends and Mahmoud Shams El-Din
...
void Energy200ms(void) {
  Energy->power_on = (FirmwareGlobal.power != 0) | Settings->flag.no_power_on_check;  // SetOption21 - Show voltage even if powered off
...
void EnergyMarginCheck(void) {
  if (!Energy->phase_count || (FirmwareGlobal.uptime < 8)) { return; }
...
//    AddLog(LOG_LEVEL_DEBUG, PSTR("NRG: APower %d, HPower0 %d, HPower1 %d, HPower2 %d"), active_power, Energy->power_history[phase][0], Energy->power_history[phase][1], Energy->power_history[phase][2]);
  ...
  if (Settings->energy_max_power_limit) {
    ...
    else if (FirmwareGlobal.power && (energy_power_u <= Settings->energy_max_power_limit)) {
      Energy->mplh_counter = 0;
      Energy->mplr_counter = 0;
      Energy->mplw_counter = 0;
    }
    if (!FirmwareGlobal.power) {
...
void EnergyEverySecond(void) {
  // Overtemp check
  if (Energy->use_overtemp && FirmwareGlobal.global_update) {
    if (FirmwareGlobal.power && !isnan(FirmwareGlobal.temperature_celsius) && (FirmwareGlobal.temperature_celsius > (float)Settings->param[P_OVER_TEMP])) {  // SetOption42 Device overtemp, turn off relays

      AddLog(LOG_LEVEL_DEBUG, PSTR("NRG: Temperature %1_f"), &FirmwareGlobal.temperature_celsius);
  ...
  // Invalid data reset
  if (FirmwareGlobal.uptime > ENERGY_WATCHDOG) {
...
//  Energy->use_overtemp = false;
  ...
  FirmwareGlobal.energy_driver = ENERGY_NONE;
  XnrgCall(FUNC_PRE_INIT);             // Find first energy driver
  if (FirmwareGlobal.energy_driver) {
    AddLog(LOG_LEVEL_INFO, PSTR("NRG: Init driver %d"), FirmwareGlobal.energy_driver);
  }
}
...
//    Energy->energy_today_offset_kWh[i] = 0;   // Reset by EnergyDrvInit()
      // 20220805 - Change from https://github.com/arendst/Firmware/issues/16118
...
void EnergyShow(bool json) {
  ...
  if (json) {
    bool show_energy_period = (0 == FirmwareGlobal.tele_period);
...
bool Xdrv03(uint32_t function)
{
  ...
  else if (FirmwareGlobal.energy_driver) {
    switch (function) {
      case FUNC_LOOP:
