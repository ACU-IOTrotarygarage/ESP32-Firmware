...
FirmwareSerial *ZigbeeSerial = nullptr;
...
void ZigbeeInputLoop(void) {
  ...
  while (ZigbeeSerial->available()) {
    ...
    uint8_t zigbee_in_byte = ZigbeeSerial->read();
      ...
      // in this case the first bit (lsb) is missed and Firmware receives 0xFF instead of 0xFE
...
void ZigbeeInitSerial(void)
{
  ...
  if (PinUsed(GPIO_ZIGBEE_RX) && PinUsed(GPIO_ZIGBEE_TX)) {
    ...
    // if FirmwareGlobal.seriallog_level is 0, we allow GPIO 13/15 to switch to Hardware Serial
    ZigbeeSerial = new FirmwareSerial(Pin(GPIO_ZIGBEE_RX), Pin(GPIO_ZIGBEE_TX), FirmwareGlobal.seriallog_level ? 1 : 2, 0, 256);   // set a receive buffer of 256 bytes
...
void ZigbeeProcessInputRaw(SBuffer &buf) {
  ...
  if (control_byte & 0x80) {  // non DATA frame
    ...
    if (frame_type == 0x80) {
      ...
      FirmwareGlobal.restart_flag = 2;  // there is nothing more we can do except restart
    } else {
