/*
  xdrv_57_tasmesh.ino - Mesh support for Firmware using ESP-Now

  Copyright (C) 2021  Christian Baars, Federico Leoni and Theo Arends and Mahmoud Shams El-Din
...
/*
  ...
  0.9.5.1 20210622  integrate  Expand number of chunks to satisfy larger MQTT messages
                               Refactor to latest Firmware standards
  ---
  ...
  switch (_recvPacket->type) {
    case PACKET_TYPE_TIME:
      ...
      if (MESH.flags.nodeGotTime == 0) {
        RtcSync("Mesh");
        FirmwareGlobal.rules_flag.system_boot  = 1; // for now we consider the node booted and let trigger system#boot on RULES
      }
...
void MESHregisterNode(uint8_t mode){
  ...
  strcpy((char*)MESH.sendPacket.payload +6, FirmwareGlobal.mqtt_topic);  // Remaining bytes -> topic of node
  AddLog(LOG_LEVEL_DEBUG, PSTR("MSH: Register node with topic '%s'"), (char*)MESH.sendPacket.payload +6);
  MESH.sendPacket.TTL = 2;
  MESH.sendPacket.chunks = 1;
  MESH.sendPacket.chunk = 0;
  MESH.sendPacket.chunkSize = strlen(FirmwareGlobal.mqtt_topic) + 1 + 6;
