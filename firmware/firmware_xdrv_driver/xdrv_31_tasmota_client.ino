/*
  xdrv_31_Firmware_client.ino - Support for external microcontroller on serial

  Copyright (C) 2021  Andre Thomas and Theo Arends and Mahmoud Shams El-Din
...
#ifndef USE_Firmware_CLIENT_FLASH_SPEED
#define USE_Firmware_CLIENT_FLASH_SPEED  57600     // Usually 57600 for 3.3V variants and 115200 for 5V variants
#endif
#ifndef USE_Firmware_CLIENT_SERIAL_SPEED
#define USE_Firmware_CLIENT_SERIAL_SPEED 57600     // Depends on the sketch that is running on the Uno/Pro Mini
#endif

#define Firmware_CLIENT_LIB_VERSION      20191129
#define Firmware_CLIENT_TIMEOUT          250       // mSeconds
...
/*************************************************\
 * Firmware Client Specific Commands
\*************************************************/
...
FirmwareSerial *FirmwareClient_Serial;

void FirmwareClient_Reset(void) {
  if (TClient.SerialEnabled) {
    digitalWrite(Pin(GPIO_FirmwareCLIENT_RST), !TClient.inverted);
    delay(1);
    digitalWrite(Pin(GPIO_FirmwareCLIENT_RST), TClient.inverted);
    delay(1);
    digitalWrite(Pin(GPIO_FirmwareCLIENT_RST), !TClient.inverted);
...
uint8_t FirmwareClient_waitForSerialData(int dataCount, int timeout) {
  int timer = 0;
  while (timer < timeout) {
    if (FirmwareClient_Serial->available() >= dataCount) {
      return 1;
    }
    delay(1);
    timer++;
...
uint8_t FirmwareClient_receiveData(char* buffer, int size) {
  uint8_t index = 255;
  int timer = 0;
  while (timer < Firmware_CLIENT_TIMEOUT) {
    int data = FirmwareClient_Serial->read();
    if (data >= 0) {
...
uint8_t FirmwareClient_sendBytes(uint8_t* bytes, int count) {
//  AddLog(LOG_LEVEL_DEBUG, PSTR("TCL: SendBytes %*_H"), count, (uint8_t*)&bytes);

  FirmwareClient_Serial->write(bytes, count);
  FirmwareClient_waitForSerialData(2, Firmware_CLIENT_TIMEOUT);
  uint8_t sync = FirmwareClient_Serial->read();
  uint8_t ok = FirmwareClient_Serial->read();
...
uint8_t FirmwareClient_execCmd(uint8_t cmd) {
  uint8_t bytes[] = { cmd, CONST_STK_CRC_EOP };
  return FirmwareClient_sendBytes(bytes, 2);
}

uint8_t FirmwareClient_execParam(uint8_t cmd, uint8_t* params, int count) {
  uint8_t bytes[32];
  bytes[0] = cmd;
  int i = 0;
  while (i < count) {
  ...
  return FirmwareClient_sendBytes(bytes, i + 2);
}

uint32_t FirmwareClient_Flash(uint8_t* data, size_t size) {
/*
  // Don't do this as there is no re-init configured
  FirmwareClient_Serial->end();
  delay(10);

  FirmwareClient_Serial->begin(USE_Firmware_CLIENT_FLASH_SPEED);
  if (FirmwareClient_Serial->hardwareSerial()) {
    ClaimSerial();
  }
*/
  FirmwareClient_Reset();

  uint8_t timeout = 0;
  while (timeout <= 50) {
    if (FirmwareClient_execCmd(CMND_STK_GET_SYNC)) {
      break;
    }
    timeout++;
    delay(1);
  ...
  if (!FirmwareClient_execParam(CMND_STK_SET_DEVICE, ProgParams, sizeof(ProgParams))) {
    return 2;                                // Error: Could not configure device for programming (1)
  }

  uint8_t ExtProgParams[] = {0x05, 0x04, 0xd7, 0xc2, 0x00};
  if (!FirmwareClient_execParam(CMND_STK_SET_DEVICE_EXT, ExtProgParams, sizeof(ExtProgParams))) {
    return 3;                                // Error: Could not configure device for programming (2)
  }

  if (!FirmwareClient_execCmd(CMND_STK_ENTER_PROGMODE)) {
      ...
      if ((processed <= size) && (!SHParse.EndOfFile)) {
        ...
        if (':' == flash_buffer[ca]) {
          ...
          if (SHParse.FlashPageIdx == sizeof(SHParse.FlashPage)) {
            ...
            FirmwareClient_execParam(CMND_STK_LOAD_ADDRESS, params, sizeof(params));

            FirmwareClient_Serial->write(header, sizeof(header));

            for (uint32_t i = 0; i < sizeof(SHParse.FlashPage); i++) {
              FirmwareClient_Serial->write(SHParse.FlashPage[i]);
            }
            FirmwareClient_Serial->write(CONST_STK_CRC_EOP);

            FirmwareClient_waitForSerialData(2, Firmware_CLIENT_TIMEOUT);
            FirmwareClient_Serial->read();
            FirmwareClient_Serial->read();
...
//            AddLog(LOG_LEVEL_DEBUG, PSTR("DBG: Size %d, Processed %d"), size, processed);
  ...
  FirmwareClient_execCmd(CMND_STK_LEAVE_PROGMODE);

  return error;                              // Error or Flash done!
}

void FirmwareClient_Init(void) {
  if (TClient.type) {
    return;
  }
  ...
  if (!TClient.SerialEnabled) {
    if (PinUsed(GPIO_FirmwareCLIENT_RXD) && PinUsed(GPIO_FirmwareCLIENT_TXD) &&
        (PinUsed(GPIO_FirmwareCLIENT_RST) || PinUsed(GPIO_FirmwareCLIENT_RST_INV))) {
      FirmwareClient_Serial = new FirmwareSerial(Pin(GPIO_FirmwareCLIENT_RXD), Pin(GPIO_FirmwareCLIENT_TXD), 1, 0, 200);
      if (FirmwareClient_Serial->begin(USE_Firmware_CLIENT_SERIAL_SPEED)) {
        if (FirmwareClient_Serial->hardwareSerial()) {
          ClaimSerial();
        }
#ifdef ESP32
        AddLog(LOG_LEVEL_DEBUG, PSTR("TCL: Serial UART%d"), FirmwareClient_Serial->getUart());
#endif
        if (PinUsed(GPIO_FirmwareCLIENT_RST_INV)) {
          SetPin(Pin(GPIO_FirmwareCLIENT_RST_INV), AGPIO(GPIO_FirmwareCLIENT_RST));
          TClient.inverted = HIGH;
        }
        pinMode(Pin(GPIO_FirmwareCLIENT_RST), OUTPUT);
  ...
  if (TClient.SerialEnabled) {  // All go for hardware now we need to detect features if there are any
    FirmwareClient_sendCmnd(CMND_FEATURES, 0);

    char buffer[sizeof(TClientSettings)];
    uint8_t len = FirmwareClient_receiveData(buffer, sizeof(buffer));  // 99 17 34 01 02 00 00 00
    if (len == sizeof(TClientSettings)) {
      memcpy(&TClientSettings, &buffer, sizeof(TClientSettings));
      if (Firmware_CLIENT_LIB_VERSION == TClientSettings.features_version) {
        TClient.type = true;
...
bool FirmwareClient_Available(void) {
  return TClient.SerialEnabled;
}

void FirmwareClient_Show(void) {
  if ((TClient.type) && (TClientSettings.features.func_json_append)) {
    FirmwareClient_sendCmnd(CMND_GET_JSON, 0);

    char buffer[250];  // Keep size below 255 to stay within 8-bits index and len
    uint8_t len = FirmwareClient_receiveData(buffer, sizeof(buffer) -1);

    buffer[len] = '\0';
    ResponseAppend_P(PSTR(",\"FirmwareClient\":%s"), buffer);
  }
}

void FirmwareClient_sendCmnd(uint8_t cmnd, uint8_t param) {
...
//  AddLog(LOG_LEVEL_DEBUG, PSTR("TCL: SendCmnd %*_H"), sizeof(buffer), (uint8_t*)&buffer);
  ...
  for (uint32_t ca = 0; ca < sizeof(buffer); ca++) {
    FirmwareClient_Serial->write(buffer[ca]);
  }
}
...
const char kFirmwareClientCommands[] PROGMEM = D_PRFX_CLIENT "|"
  D_CMND_CLIENT_RESET "|" D_CMND_CLIENT_SEND;

void (* const FirmwareClientCommand[])(void) PROGMEM = {
...
void CmndClientSend(void) {
  if (TClient.SerialEnabled) {
    if (0 < XdrvMailbox.data_len) {
      FirmwareClient_sendCmnd(CMND_CLIENT_SEND, XdrvMailbox.data_len);
      FirmwareClient_Serial->write(char(PARAM_DATA_START));
      for (uint32_t idx = 0; idx < XdrvMailbox.data_len; idx++) {
        FirmwareClient_Serial->write(XdrvMailbox.data[idx]);
      }
      FirmwareClient_Serial->write(char(PARAM_DATA_END));
    }
    ResponseCmndDone();
  }
}

void FirmwareClient_ProcessIn(void) {
  uint8_t cmnd = FirmwareClient_Serial->read();
  if (CMND_START == cmnd) {
    FirmwareClient_waitForSerialData(sizeof(TClientCommand), 50);
    uint8_t buffer[sizeof(TClientCommand)];
    for (uint32_t idx = 0; idx < sizeof(TClientCommand); idx++) {
      buffer[idx] = FirmwareClient_Serial->read();
    }
    FirmwareClient_Serial->read(); // read trailing byte of command
    memcpy(&TClientCommand, &buffer, sizeof(TClientCommand));
    char inbuf[TClientCommand.parameter+1];
    FirmwareClient_waitForSerialData(TClientCommand.parameter, 50);
    FirmwareClient_Serial->read(); // Read leading byte
    for (uint32_t idx = 0; idx < TClientCommand.parameter; idx++) {
      inbuf[idx] = FirmwareClient_Serial->read();
    }
    FirmwareClient_Serial->read(); // Read trailing byte
    ...
    if (CMND_PUBLISH_TELE == TClientCommand.command) { // We need to publish stat/ with incoming stream as content
      Response_P(PSTR("{\"FirmwareClient\":"));
      ResponseAppend_P("%s", inbuf);
      ResponseJsonEnd();
      MqttPublishPrefixTopicRulesProcess_P(RESULT_OR_TELE, PSTR("FirmwareClient"));
    }
...
bool Xdrv31(uint32_t function) {
  ...
  switch (function) {
    case FUNC_EVERY_100_MSECOND:
      if (TClient.type) {
        if (FirmwareClient_Serial->available()) {
          FirmwareClient_ProcessIn();
        }
        if (TClientSettings.features.func_every_100_msecond) {
          FirmwareClient_sendCmnd(CMND_FUNC_EVERY_100_MSECOND, 0);
        }
      }
      break;
    case FUNC_EVERY_SECOND:
      if ((TClient.type) && (TClientSettings.features.func_every_second)) {
        FirmwareClient_sendCmnd(CMND_FUNC_EVERY_SECOND, 0);
      }
      FirmwareClient_Init();
      break;
    case FUNC_JSON_APPEND:
      ...
      break;
    case FUNC_COMMAND:
      result = DecodeCommand(kFirmwareClientCommands, FirmwareClientCommand);
      break;
  }
  return result;
}
