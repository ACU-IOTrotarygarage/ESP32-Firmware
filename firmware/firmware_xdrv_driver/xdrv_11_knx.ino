/*
  xdrv_11_knx.ino - KNX IP Protocol support for Firmware
...
bool KNX_CONFIG_NOT_MATCH(void)
{
  ...
  // Check for invalid or erroneous configuration (Firmware flashed without clearing the memory)
...
void KNX_INIT(void)
{
  ...
  */
  ...
  if ( FirmwareGlobal.energy_driver != ENERGY_NONE ) {
...
bool Xdrv11(uint32_t function)
{
  bool result = false;
    switch (function) {
      case FUNC_LOOP:
        if (!FirmwareGlobal.global_state.network_down) { knx.loop(); }  // Process knx events
        break;
