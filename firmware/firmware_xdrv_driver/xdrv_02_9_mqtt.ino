/*
  xdrv_02_9_mqtt.ino - mqtt support for Firmware
...
void MqttDisableLogging(bool state) {
  ...
  FirmwareGlobal.masterlog_level = (Mqtt.disable_logging) ? LOG_LEVEL_DEBUG_MORE : LOG_LEVEL_NONE;
}
 ...
 * #define MQTT_MAX_PACKET_SIZE 1200     // Firmware v8.1.0.8
\*********************************************************************************************/
...
void MqttInit(void) {
  ...
  if (Mqtt.mqtt_tls) {
    ...
    if (!Settings->flag5.tls_use_fingerprint) {
      tlsClient->setTrustAnchor(Firmware_TA, nitems(Firmware_TA));
    }
  ...
  void ProvisionAzureDPS(){
    ...
    httpsClient.addHeader("User-Agent", "Firmware");
...
bool MqttPublishLib(const char* topic, const uint8_t* payload, unsigned int plength, bool retained) {
  ...
  if (!strcmp(SettingsText(SET_MQTTPREFIX1), SettingsText(SET_MQTTPREFIX2))) {
    ...
    if (str == topic) {
      FirmwareGlobal.mqtt_cmnd_blocked_reset = 4;  // Allow up to four seconds before resetting residual cmnd blocks
      FirmwareGlobal.mqtt_cmnd_blocked++;
    }
  }
...
void MqttDataHandler(char* mqtt_topic, uint8_t* mqtt_data, unsigned int data_len) {
  ...
  if (!strcmp(SettingsText(SET_MQTTPREFIX1), SettingsText(SET_MQTTPREFIX2))) {
    ...
    if ((str == mqtt_topic) && FirmwareGlobal.mqtt_cmnd_blocked) {
      FirmwareGlobal.mqtt_cmnd_blocked--;
      return;
    }
  }
...
//  AddLog(LOG_LEVEL_DEBUG, PSTR(D_LOG_MQTT "BufferSize %d, Topic |%s|, Length %d, data_len %d"), MqttClient.getBufferSize(), mqtt_topic, strlen(mqtt_topic), data_len);
  ...
  if (Mqtt.disable_logging) {
    FirmwareGlobal.masterlog_level = LOG_LEVEL_DEBUG_MORE;  // Hide logging
  }
  ...
  if (Mqtt.disable_logging) {
    FirmwareGlobal.masterlog_level = LOG_LEVEL_NONE;  // Enable logging
  }
}
...
void MqttPublishLoggingAsync(bool refresh) {
  ...
  while (GetLog(Settings->mqttlog_level, &index, &line, &len)) {
    char stopic[TOPSZ];
    GetTopic_P(stopic, STAT, FirmwareGlobal.mqtt_topic, PSTR("LOGGING"));
...
void MqttPublishPayload(const char* topic, const char* payload, uint32_t binary_length, bool retained) {
...
#endif  // USE_TASMESH
    log_data_topic += topic;                             // stat/Firmware/STATUS2
  } else {
  ...
  AddLogData(LOG_LEVEL_INFO, log_data_topic.c_str(), log_data_payload, log_data_retained);  // MQT: stat/Firmware/STATUS2 = {"StatusFWR":{"Version":...
...
*/
  ...
  prefix &= 3;
  char stopic[TOPSZ];
  GetTopic_P(stopic, prefix, FirmwareGlobal.mqtt_topic, romram);
...
void MqttPublishPowerState(uint32_t device) {
  ...
  if ((device < 1) || (device > FirmwareGlobal.devices_present)) { device = 1; }
  ...
  if (IsModuleIfan() && (device > 1)) {
    if (GetFanspeed() < MaxFanspeed()) {  // 4 occurs when fanspeed is 3 and RC button 2 is pressed
      ...
      GetTopic_P(stopic, STAT, FirmwareGlobal.mqtt_topic, (Settings->flag.mqtt_response) ? scommand : S_RSLT_RESULT);  // SetOption4 - Switch between MQTT RESULT or COMMAND
    ...
    GetTopic_P(stopic, STAT, FirmwareGlobal.mqtt_topic, (Settings->flag.mqtt_response) ? scommand : S_RSLT_RESULT);  // SetOption4 - Switch between MQTT RESULT or COMMAND
    Response_P(S_JSON_COMMAND_SVALUE, scommand, GetStateText(bitRead(FirmwareGlobal.power, device -1)));
    MqttPublish(stopic);
    ...
    if (!Settings->flag4.only_json_message) {  // SetOption90 - Disable non-json MQTT response
      ...
      Response_P(GetStateText(bitRead(FirmwareGlobal.power, device -1)));
...
void MqttPublishPowerBlinkState(uint32_t device) {
  ...
  Response_P(PSTR("{\"%s\":\"" D_JSON_BLINK " %s\"}"),
    GetPowerDevice(scommand, device, sizeof(scommand), Settings->flag.device_index_enable), GetStateText(bitRead(FirmwareGlobal.blink_mask, device -1)));  // SetOption26 - Switch between POWER or POWER1
...
void MqttDisconnected(int state) {
  ...
  Firmware MQTT_DNS_DISCONNECTED       -5
  ...
  */
  ...
  FirmwareGlobal.rules_flag.mqtt_disconnected = 1;
}
...
void MqttConnected(void) {
  ...
  if (Mqtt.allowed) {
    ...
    GetTopic_P(stopic, TELE, FirmwareGlobal.mqtt_topic, S_LWT);
    ...
    GetTopic_P(stopic, CMND, FirmwareGlobal.mqtt_topic, PSTR("#"));
  ...
  if (Mqtt.initial_connection_state) {
    if (ResetReason() != REASON_DEEP_SLEEP_AWAKE) {
      ...
      Response_P(PSTR("{\"Info1\":{\"" D_CMND_MODULE "\":\"%s\",\"" D_JSON_VERSION "\":\"%s%s%s\",\"" D_JSON_FALLBACKTOPIC "\":\"%s\",\"" D_CMND_GROUPTOPIC "\":\"%s\"}}"),
        ModuleName().c_str(), FirmwareGlobal.version, FirmwareGlobal.image_name, GetCodeCores().c_str(), GetFallbackTopic_P(stopic, ""), GetGroupTopic_P(stopic2, "", SET_MQTT_GRP_TOPIC));
      ...
      if (Settings->webserver) {
        ...
        if (static_cast<uint32_t>(WiFi.localIP()) != 0) {
          ResponseAppend_P(PSTR(",\"" D_CMND_HOSTNAME "\":\"%s\",\"" D_CMND_IPADDRESS "\":\"%_I\""),
            FirmwareGlobal.hostname, (uint32_t)WiFi.localIP());
#ifdef USE_IPV6
    ...
    if (Settings->tele_period) {
      FirmwareGlobal.tele_period = Settings->tele_period -5;  // Enable TelePeriod in 5 seconds
    }
    FirmwareGlobal.rules_flag.system_boot = 1;
    XdrvCall(FUNC_MQTT_INIT);
  }
  Mqtt.initial_connection_state = 0;

  FirmwareGlobal.global_state.mqtt_down = 0;
  if (Settings->flag.mqtt_enabled) {  // SetOption3 - Enable MQTT
    FirmwareGlobal.rules_flag.mqtt_connected = 1;
  }
}

void MqttReconnect(void) {
  Mqtt.allowed = Settings->flag.mqtt_enabled && (FirmwareGlobal.restart_flag == 0);  // SetOption3 - Enable MQTT, and don't connect if restart in process
  if (Mqtt.allowed) {
#if defined(USE_MQTT_AZURE_DPS_SCOPEID) && defined(USE_MQTT_AZURE_DPS_PRESHAREDKEY)
  ProvisionAzureDPS();
#endif
  ...
  Mqtt.connected = false;
  Mqtt.retry_counter = Settings->mqtt_retry * Mqtt.retry_counter_delay;
  FirmwareGlobal.global_state.mqtt_down = 1;
  ...
  if (MqttClient.connect(FirmwareGlobal.mqtt_client,
                         mqtt_user,
                         mqtt_pwd,
                         stopic,                                         // Will topic
                         1,                                              // Will QoS
...
void CmndPublish(void) {
  ...
  // publish cmnd/theo#arends/power 2 ==> publish cmnd/Theo Arends and Mahmoud Shams El-Din/power 2
  if (XdrvMailbox.data_len > 0) {
    char *payload_part;
...
void CmndGroupTopic(void) {
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= MAX_GROUP_TOPICS)) {
    if (XdrvMailbox.data_len > 0) {
      ...
      MakeValidMqtt(0, XdrvMailbox.data);
      if (!strcmp(XdrvMailbox.data, FirmwareGlobal.mqtt_topic)) {
...
void CmndButtonTopic(void) {
  if (!XdrvMailbox.grpflg && (XdrvMailbox.data_len > 0)) {
    ...
    switch (Shortcut()) {
      ...
      case SC_DEFAULT: SettingsUpdateText(SET_MQTT_BUTTON_TOPIC, FirmwareGlobal.mqtt_topic); break;
...
void CmndSwitchTopic(void) {
  if (!XdrvMailbox.grpflg && (XdrvMailbox.data_len > 0)) {
    ...
    switch (Shortcut()) {
      ...
      case SC_DEFAULT: SettingsUpdateText(SET_MQTT_SWITCH_TOPIC, FirmwareGlobal.mqtt_topic); break;
...
void CmndPowerRetain(void) {
  if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= 1)) {
    if (!XdrvMailbox.payload) {
      char stemp1[TOPSZ];
      char scommand[CMDSZ];
      for (uint32_t i = 1; i <= FirmwareGlobal.devices_present; i++) {  // Clear MQTT retain in broker
        GetTopic_P(stemp1, STAT, FirmwareGlobal.mqtt_topic, GetPowerDevice(scommand, i, sizeof(scommand), Settings->flag.device_index_enable));  // SetOption26 - Switch between POWER or POWER1
        ResponseClear();
