/*
  xdrv_23_zigbee_converters.ino - zigbee support for Firmware
...
// - Firmware_cmd: the human-readable name of the command as entered or displayed, use '|' to split into multiple commands when displayed
    ...
    if (conv_cluster == cluster) {
      ...
      if ((0xFF == conv_cmd) || (cmd == conv_cmd)) {
        ...
        if ((direction && (conv_direction & 0x02)) || (!direction && (conv_direction & 0x01))) {
          ...
          if (match) {
            command_name = Z_strings + pgm_read_word(&conv->Firmware_cmd_offset);
...
const char * zigbeeFindCommand(const char *command, uint16_t *cluster, uint16_t *cmd) {
  ...
  for (uint32_t i = 0; i < sizeof(Z_Commands) / sizeof(Z_Commands[0]); i++) {
    ...
    if ((conv_direction & 0x01) && (0 == strcasecmp_P(command, Z_strings + pgm_read_word(&conv->Firmware_cmd_offset)))) {
