/*
  xdrv_12_discovery.ino - MQTT Discovery support for Firmware
...
void TasRediscover(void) {
  FirmwareGlobal.discovery_counter = 1;                         // Delayed discovery or clear retained messages
}

void TasDiscoverInit(void) {
  if (ResetReason() != REASON_DEEP_SLEEP_AWAKE) {
    Settings->flag.hass_discovery = 0;                         // SetOption19 - Enable Firmware discovery and Disable legacy Hass discovery
...
void CmndTasDiscover(void) {
  if (XdrvMailbox.payload >= 0) {
    Settings->flag.hass_discovery = !(XdrvMailbox.payload & 1);  // SetOption19 - Firmware discovery (0)
    TasRediscover();
  }
