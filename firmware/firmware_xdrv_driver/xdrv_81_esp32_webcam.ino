/*
  xdrv_81_esp32_webcam.ino - ESP32 webcam support for Firmware
...
//    }
  ...
  if (!PinUsed(GPIO_WEBCAM_XCLK) || !PinUsed(GPIO_WEBCAM_PCLK) ||
      ...
      ((!PinUsed(GPIO_WEBCAM_SIOD) || !PinUsed(GPIO_WEBCAM_SIOC)) && !FirmwareGlobal.i2c_enabled_2)    // preferred option is to reuse and share I2Cbus 2
      ) {
        pin_used = false;
  }
  return pin_used;
...
//esp_log_level_set("*", ESP_LOG_VERBOSE);
  ...
  if (WcPinUsed()) {
    ...
    if(FirmwareGlobal.i2c_enabled_2){              // configure SIOD and SIOC as SDA,2 and SCL,2
...
uint32_t WcSetStreamserver(uint32_t flag) {
  if (FirmwareGlobal.global_state.network_down) { return 0; }
...
#ifdef ENABLE_RTSPSERVER
    if (Settings->webcam_config.rtsp && !FirmwareGlobal.global_state.wifi_down && Wc.up) {
      if (!Wc.rtsp_start) {
