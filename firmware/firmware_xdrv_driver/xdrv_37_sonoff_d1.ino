/*
  xdrv_37_sonoff_d1.ino - sonoff D1 dimmer support for Firmware
...
{
  if (FirmwareGlobal.serial_in_byte_counter < 8) { return; }  // Received ack from Rf chip (aa 55 01 04 00 00 05)

  uint8_t action = FirmwareGlobal.serial_in_buffer[6] & 1;
...
//    AddLog(LOG_LEVEL_DEBUG, PSTR("SD1: Remote power (%d, %d)"), SnfD1.power, SnfD1.dimmer);
  ...
  uint8_t dimmer = FirmwareGlobal.serial_in_buffer[7];
...
bool SonoffD1SerialInput(void)
{
  ...
  if (SnfD1.receive_len) {
    ...
    if (6 == FirmwareGlobal.serial_in_byte_counter) {
      SnfD1.receive_len += FirmwareGlobal.serial_in_byte;  // 8 or 17
    }
    if (FirmwareGlobal.serial_in_byte_counter == SnfD1.receive_len) {
      ...
      if (crc == FirmwareGlobal.serial_in_buffer[SnfD1.receive_len -1]) {
        SonoffD1Received();
        SnfD1.receive_len = 0;
        return true;
      }
...
//    AddLog(LOG_LEVEL_DEBUG, PSTR("SD1: Firmware power (%d, %d)"), SnfD1.power, SnfD1.dimmer);
...
//    AddLog(LOG_LEVEL_DEBUG, PSTR("SD1: Firmware dimmer (%d, %d)"), SnfD1.power, SnfD1.dimmer);
...
bool SonoffD1ModuleSelected(void)
{
  ...
  FirmwareGlobal.light_type = LT_SERIAL1;
...
bool Xdrv37(uint32_t function)
{
  ...
  if (SONOFF_D1 == FirmwareGlobal.module_type) {
    switch (function) {
      case FUNC_SERIAL:
