/*
  xdrv_04_light.ino - PWM, WS2812 and sonoff led support for Firmware
...
class LightStateClass {
  private:
    ...
    bool     _power = false;    // power indicator, used only in virtual lights, Firmware tracks power separately
  ...
  public:
    ...
    // power accessors, for virtual lights only (has no effect on Firmware lights)
    ...
    // reachable accessors, for virtual lights only (has no effect on Firmware lights)
...
public:
  ...
  void loadSettings() {
    ...
    AddLog(LOG_LEVEL_DEBUG_MORE, "LightControllerClass::loadSettings light_type/sub (%d %d)",
      FirmwareGlobal.light_type, Light.subtype);
#endif
    if (_pwm_multi_channels) {
...
bool LightModuleInit(void)
{
  ...
  if (Settings->flag.pwm_control) {                     // SetOption15 - Switch between commands PWM or COLOR/DIMMER/CT/CHANNEL
    for (uint32_t i = 0; i < LST_MAX; i++) {
      if (PinUsed(GPIO_PWM1, i)) { FirmwareGlobal.light_type++; }  // Use Dimmer/Color control for all PWM as SetOption15 = 1
    }
  }

  FirmwareGlobal.light_driver = 0;
  if (XlgtCall(FUNC_MODULE_INIT)) {
    // serviced
  }
#ifdef ESP8266
  else if (SONOFF_BN == FirmwareGlobal.module_type) {   // PWM Single color led (White)
    FirmwareGlobal.light_type = LT_PWM1;
  }
  else if (SONOFF_LED == FirmwareGlobal.module_type) {  // PWM Dual color led (White warm and cold)
    if (!FirmwareGlobal.my_module.io[4]) {              // Fix Sonoff Led instabilities
      pinMode(4, OUTPUT);                              // Stop floating outputs
      digitalWrite(4, LOW);
    }
    if (!FirmwareGlobal.my_module.io[5]) {
      pinMode(5, OUTPUT);                              // Stop floating outputs
      digitalWrite(5, LOW);
    }
    if (!FirmwareGlobal.my_module.io[14]) {
      pinMode(14, OUTPUT);                             // Stop floating outputs
      digitalWrite(14, LOW);
    }
    FirmwareGlobal.light_type = LT_PWM2;
  }
#endif  // ESP8266
#ifdef USE_PWM_DIMMER
  else if (PWM_DIMMER == FirmwareGlobal.module_type) {
    FirmwareGlobal.light_type = Settings->pwm_dimmer_cfg.pwm_count + 1;
  }
#endif  // USE_PWM_DIMMER

  if (FirmwareGlobal.light_type > LT_BASIC) {
  ...
  if (Settings->flag3.pwm_multi_channels) {  // SetOption68 - Enable multi-channels PWM instead of Color PWM
    ...
    if ((Settings->flag4.virtual_ct) && (LST_RGB <= pwm_channels)) {
      ...
      FirmwareGlobal.light_type += 5 - pwm_channels;               // pretend it is a 5 channels bulb
    }
#endif // USE_LIGHT_VIRTUAL_CT
  }

  return (FirmwareGlobal.light_type > LT_BASIC);
}
...
void LightInit(void)
{
  ...
  Light.device = FirmwareGlobal.devices_present;
  Light.subtype = (FirmwareGlobal.light_type & 7) > LST_MAX ? LST_MAX : (FirmwareGlobal.light_type & 7); // Always 0 - LST_MAX (5)
  ...
  if ((LST_SINGLE <= Light.subtype) && Light.pwm_multi_channels) {
    ...
    Light.device = FirmwareGlobal.devices_present - Light.subtype + 1; // adjust if we also have relays
  ...
  LightCalcPWMRange();
#ifdef DEBUG_LIGHT
  AddLog(LOG_LEVEL_DEBUG_MORE, "LightInit Light.pwm_multi_channels=%d Light.subtype=%d Light.device=%d FirmwareGlobal.devices_present=%d",
    Light.pwm_multi_channels, Light.subtype, Light.device, FirmwareGlobal.devices_present);
#endif
  ...
  if (FirmwareGlobal.light_type < LT_PWM6) {           // PWM
    for (uint32_t i = 0; i < FirmwareGlobal.light_type; i++) {
...
uint8_t LightGetBri(uint8_t device) {
  ...
  if (Light.pwm_multi_channels) {
    if ((device >= Light.device) && (device < Light.device + LST_MAX) && (device <= FirmwareGlobal.devices_present)) {
...
#ifdef DEBUG_LIGHT
  AddLog(LOG_LEVEL_DEBUG, "LightPreparePower power=%d Light.power=%d", FirmwareGlobal.power, Light.power);
#endif
...
#ifdef DEBUG_LIGHT
  AddLog(LOG_LEVEL_DEBUG, "LightPreparePower End power=%d Light.power=%d", FirmwareGlobal.power, Light.power);
#endif
  Light.power = FirmwareGlobal.power >> (Light.device - 1);  // reset next state, works also with unlinked RGB/CT
...
void LightListenDDP()
{
  ...
  if (FirmwareGlobal.restart_flag || FirmwareGlobal.global_state.network_down) {
...
//  Light.power = XdrvMailbox.index;
  ...
  // Ex: 3 Relays and 4 PWM - FirmwareGlobal.devices_present = 7, Light.device = 4, Light.subtype = 4
  // Result: mask = 0b00001111 = 0x0F, shift = 3.
  // Power bits we consider are: 0b01111000 = 0x78
  // If regular situation: FirmwareGlobal.devices_present == Light.subtype
...
void LightAnimate(void)
{
  ...
  if (Light.power || Light.fade_running) {
    if (FirmwareGlobal.sleep > PWM_MAX_SLEEP) {
      sleep_previous = FirmwareGlobal.sleep;     // save previous value of sleep
      FirmwareGlobal.sleep = PWM_MAX_SLEEP;      // set a maximum value (in milliseconds) to sleep to ensure that animations are smooth
    }
  } else {
    if (sleep_previous > 0) {
      FirmwareGlobal.sleep = sleep_previous;
  ...
  if ((Settings->light_scheme < LS_MAX) || power_off) {  // exclude WS281X Neopixel schemes
    ...
    if (Light.update) {
      ...
      if (!LightGetFadeSetting() || FirmwareGlobal.skip_light_fade || power_off || (!Light.fade_initialized)) { // no fade
    ...
    if (FirmwareGlobal.gpio_optiona.pwm1_input && !Light.power && !Light.fade_running) {  // GPIO Option_A1
...
#ifdef ESP8266
  if ((PHILIPS == FirmwareGlobal.module_type) || (Settings->flag4.pwm_ct_mode)) {
#else
...
void LightStopFade(void) {
  ...
  if (PWM_DIMMER == FirmwareGlobal.module_type && !Light.power) PWMDimmerSetPower();
#endif  // USE_PWM_DIMMER
}
  ...
  if (0 == Light.fade_duration) {
    ...
    if (distance > 0) {
      ...
      if (Settings->save_data) {
        ...
        // AddLog(LOG_LEVEL_INFO, PSTR("delay_seconds %d, save_data_counter %d"), delay_seconds, FirmwareGlobal.save_data_counter);
        if (FirmwareGlobal.save_data_counter < delay_seconds) {
          FirmwareGlobal.save_data_counter = delay_seconds;      // pospone
        }
      }
    } else {
...
void LightSetOutputs(const uint16_t *cur_col_10) {
  ...
  if (FirmwareGlobal.light_type < LT_PWM6) {   // only for direct PWM lights, not for Tuya, Armtronix...

    int32_t channel_ct = ChannelCT();  // Channel for PWM CT or -1 if no CT or regular CT
#ifdef USE_PWM_DIMMER
    uint16_t max_col = 0;
#ifdef USE_I2C
    if (FirmwareGlobal.gpio_optiona.linkind_support) {  // Option_A6
...
#ifdef ESP32
          FirmwareGlobal.pwm_value[i] = ac_zero_cross_power(cur_col);   // mark the new expected value
          // AddLog(LOG_LEVEL_DEBUG_MORE, "analogWrite-%i 0x%03X", i, cur_col);
#else // ESP32
          analogWrite(Pin(GPIO_PWM1, i), bitRead(FirmwareGlobal.pwm_inverted, i) ? Settings->pwm_range - ac_zero_cross_power(cur_col) : ac_zero_cross_power(cur_col));
          // AddLog(LOG_LEVEL_DEBUG_MORE, "analogWrite-%i 0x%03X", bitRead(FirmwareGlobal.pwm_inverted, i) ? Settings->pwm_range - cur_col : cur_col);
#endif // ESP32
        }
      }
    }
    ...
    if (PWM_DIMMER == FirmwareGlobal.module_type) {
      FirmwareGlobal.pwm_dimmer_led_bri = change10to8(max_col);
...
bool calcGammaBulbs(uint16_t cur_col_10[5]) {
  ...
  if ((LST_COLDWARM == Light.subtype) || (LST_RGBCW == Light.subtype)) {
#ifdef ESP8266
    if ((PHILIPS == FirmwareGlobal.module_type) || (Settings->flag4.pwm_ct_mode)) {   // channel 1 is the color tone, mapped to cold channel (0..255)
#else
...
void LightHandleDevGroupItem(void)
{
  ...
  switch (XdrvMailbox.command_code) {
    case DGR_ITEM_EOL:
      ...
      if (more_to_come) {
        FirmwareGlobal.skip_light_fade = true;
      }
      ...
      FirmwareGlobal.skip_light_fade = false;
...
void CmndDimmer(void)
{
  ...
  if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= 100)) {
    ...
    Light.update = true;
    if (FirmwareGlobal.skip_light_fade) LightAnimate();
  } else {
...
bool Xdrv04(uint32_t function)
{
  ...
  else if (FirmwareGlobal.light_type) {
    switch (function) {
      case FUNC_SERIAL:
