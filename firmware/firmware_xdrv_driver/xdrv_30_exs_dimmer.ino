/*
  xdrv_30_exs_dimmer.ino - ex-store dimmer support for Firmware
...
FirmwareSerial *ExsSerial = nullptr;
...
bool ExsSetPower(void)
{
  AddLog(LOG_LEVEL_INFO, PSTR("EXS: Set Power, Device %d, Power 0x%02x"),
            FirmwareGlobal.active_device, XdrvMailbox.index);
...
void ExsInit(void)
{
  ...
  {
    ExsSerial = new FirmwareSerial(Pin(GPIO_RXD), Pin(GPIO_TXD), 2);
...
bool Xdrv30(uint32_t function)
{
  ...
  if (EXS_DIMMER == FirmwareGlobal.module_type)
  {
    switch (function)
    {
    case FUNC_LOOP:
