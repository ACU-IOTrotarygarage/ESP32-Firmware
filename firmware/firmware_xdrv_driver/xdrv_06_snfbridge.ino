/*
  xdrv_06_snfbridge.ino - sonoff RF bridge 433 support for Firmware

  Copyright (C) 2021  Theo Arends and Mahmoud Shams El-Din and Erik Andrén Zachrisson (fw update)
 ...
 * Based on source by Erik Andrén Zachrisson (https://github.com/arendst/Firmware/pull/2886)
\*********************************************************************************************/
...
void SonoffBridgeReceivedRaw(void)
{
  ...
  if (0xB1 == FirmwareGlobal.serial_in_buffer[1]) { buckets = FirmwareGlobal.serial_in_buffer[2] << 1; }  // Bucket sniffing

  ResponseTime_P(PSTR(",\"" D_CMND_RFRAW "\":{\"" D_JSON_DATA "\":\""));
  for (uint32_t i = 0; i < FirmwareGlobal.serial_in_byte_counter; i++) {
    ResponseAppend_P(PSTR("%02X"), FirmwareGlobal.serial_in_buffer[i]);
    if (0xB1 == FirmwareGlobal.serial_in_buffer[1]) {
      if ((i > 3) && buckets) { buckets--; }
      if ((i < 3) || (buckets % 2) || (i == FirmwareGlobal.serial_in_byte_counter -2)) {
...
void SonoffBridgeReceived(void)
{
  ...
  if (0xA2 == FirmwareGlobal.serial_in_buffer[0]) {       // Learn timeout
    SonoffBridgeLearnFailed();
  }
  else if (0xA3 == FirmwareGlobal.serial_in_buffer[0]) {  // Learned A3 20 F8 01 18 03 3E 2E 1A 22 55
    SnfBridge.learn_active = 0;
    low_time = FirmwareGlobal.serial_in_buffer[3] << 8 | FirmwareGlobal.serial_in_buffer[4];   // Low time in uSec
    high_time = FirmwareGlobal.serial_in_buffer[5] << 8 | FirmwareGlobal.serial_in_buffer[6];  // High time in uSec
    if (low_time && high_time) {
      for (uint32_t i = 0; i < 9; i++) {
        Settings->rf_code[SnfBridge.learn_key][i] = FirmwareGlobal.serial_in_buffer[i +1];
      }
  ...
  else if (0xA4 == FirmwareGlobal.serial_in_buffer[0]) {  // Received RF data A4 20 EE 01 18 03 3E 2E 1A 22 55
    if (SnfBridge.learn_active) {
      SonoffBridgeLearnFailed();
    } else {
      sync_time = FirmwareGlobal.serial_in_buffer[1] << 8 | FirmwareGlobal.serial_in_buffer[2];  // Sync time in uSec
      low_time = FirmwareGlobal.serial_in_buffer[3] << 8 | FirmwareGlobal.serial_in_buffer[4];   // Low time in uSec
      high_time = FirmwareGlobal.serial_in_buffer[5] << 8 | FirmwareGlobal.serial_in_buffer[6];  // High time in uSec
      received_id = FirmwareGlobal.serial_in_buffer[7] << 16 | FirmwareGlobal.serial_in_buffer[8] << 8 | FirmwareGlobal.serial_in_buffer[9];
...
bool SonoffBridgeSerialInput(void)
{
  ...
  if (SnfBridge.receive_flag) {
    if (SnfBridge.receive_raw_flag) {
      if (!FirmwareGlobal.serial_in_byte_counter) {
        FirmwareGlobal.serial_in_buffer[FirmwareGlobal.serial_in_byte_counter++] = 0xAA;
      }
      FirmwareGlobal.serial_in_buffer[FirmwareGlobal.serial_in_byte_counter++] = FirmwareGlobal.serial_in_byte;
      if (FirmwareGlobal.serial_in_byte_counter == 3) {
        if ((0xA6 == FirmwareGlobal.serial_in_buffer[1]) || (0xAB == FirmwareGlobal.serial_in_buffer[1])) {  // AA A6 06 023908010155 55 - 06 is receive_len
          receive_len = FirmwareGlobal.serial_in_buffer[2] + 4;  // Get at least receive_len bytes
        }
      }
      if ((!receive_len && (0x55 == FirmwareGlobal.serial_in_byte)) || (receive_len && (FirmwareGlobal.serial_in_byte_counter == receive_len))) {  // 0x55 - End of text
        SonoffBridgeReceivedRaw();
        SnfBridge.receive_flag = 0;
        return 1;
      }
    }
    else if (!((0 == FirmwareGlobal.serial_in_byte_counter) && (0 == FirmwareGlobal.serial_in_byte))) {  // Skip leading 0
      if (0 == FirmwareGlobal.serial_in_byte_counter) {
        SnfBridge.expected_bytes = 2;     // 0xA0, 0xA1, 0xA2
        if (FirmwareGlobal.serial_in_byte >= 0xA3) {
          SnfBridge.expected_bytes = 11;  // 0xA3, 0xA4, 0xA5
        }
        if (FirmwareGlobal.serial_in_byte == 0xA6) {
      ...
      FirmwareGlobal.serial_in_buffer[FirmwareGlobal.serial_in_byte_counter++] = FirmwareGlobal.serial_in_byte;
      if ((SnfBridge.expected_bytes == FirmwareGlobal.serial_in_byte_counter) && (0x55 == FirmwareGlobal.serial_in_byte)) {  // 0x55 - End of text
        SonoffBridgeReceived();
        SnfBridge.receive_flag = 0;
        return 1;
      }
    }
    FirmwareGlobal.serial_in_byte = 0;
  }
  if (0xAA == FirmwareGlobal.serial_in_byte) {               // 0xAA - Start of text
...
#ifdef ESP8266
  if (SONOFF_BRIDGE == FirmwareGlobal.module_type) {
    switch (function) {
      case FUNC_SERIAL:
