/*
  xdrv_23_zigbee_2a_devices_impl.ino - zigbee support for Firmware
  ...
  if (Settings->flag4.zigbee_distinct_topics) {
    ...
    if (Settings->flag4.zb_topic_fname && local_friendfly_name && strlen(local_friendfly_name)) {
      ...
      if (Settings->flag5.zigbee_hide_bridge_topic) {
        ...
        snprintf_P(subtopic, sizeof(subtopic), PSTR("%s/%s"), FirmwareGlobal.mqtt_topic, stemp);
      }
    } else {
      if (Settings->flag5.zigbee_hide_bridge_topic) {
        snprintf_P(subtopic, sizeof(subtopic), PSTR("%04X"), shortaddr);
      } else {
        snprintf_P(subtopic, sizeof(subtopic), PSTR("%s/%04X"), FirmwareGlobal.mqtt_topic, shortaddr);
      }
    }
