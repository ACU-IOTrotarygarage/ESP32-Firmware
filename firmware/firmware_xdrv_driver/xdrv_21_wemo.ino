/*
  xdrv_21_wemo.ino - wemo support for Firmware
...
//  AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("\n%s"), event);
  ...
  if (strstr_P(event, PSTR("SetBinaryState")) != nullptr) {
    ...
    if (power != POWER_TOGGLE) {
      uint8_t device = (FirmwareGlobal.light_type) ? FirmwareGlobal.devices_present : 1;  // Select either a configured light or relay1
      ExecuteCommandPower(device, power, SRC_WEMO);
    }
  }

#ifdef USE_UNISHOX_COMPRESSION
  snprintf_P(event, sizeof(event), Decompress(WEMO_RESPONSE_STATE_SOAP, WEMO_RESPONSE_STATE_SOAP_SIZE).c_str(), state, bitRead(FirmwareGlobal.power, FirmwareGlobal.devices_present -1), state);
#else
  snprintf_P(event, sizeof(event), WEMO_RESPONSE_STATE_SOAP, state, bitRead(FirmwareGlobal.power, FirmwareGlobal.devices_present -1), state);
#endif
...
bool Xdrv21(uint32_t function)
{
  ...
  if (FirmwareGlobal.devices_present && (EMUL_WEMO == Settings->flag2.emulation)) {
    switch (function) {
