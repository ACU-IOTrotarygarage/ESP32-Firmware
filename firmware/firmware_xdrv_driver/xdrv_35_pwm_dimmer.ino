/*
  xdrv_35_pwm_dimmer.ino - PWM Dimmer Switch support for Firmware
...
//  if (!FirmwareGlobal.power && PinUsed(GPIO_REL1)) digitalWrite(Pin(GPIO_REL1), bitRead(FirmwareGlobal.rel_inverted, 0) ? 1 : 0);
...
void PWMDimmerSetBrightnessLeds(int32_t bri)
{
  ...
  uint32_t mask = 1;
  int32_t led;
  for (led = 0; led < FirmwareGlobal.leds_present; led++) {
  ...
  if (leds) {
    ...
    if (bri < 0) {
      bri = ((bri == -2 && Settings->flag4.led_timeout) || !Light.power ? 0 : FirmwareGlobal.pwm_dimmer_led_bri);
    ...
    int32_t level = 0;
    if (FirmwareGlobal.gpio_optiona.linkind_support) {
      step = 256 / leds;
      level = -step;
    }
    led = -1;
    ...
    for (uint32_t count = 0; count < leds; count++) {
      ...
      if (FirmwareGlobal.gpio_optiona.linkind_support) {
        ...
        analogWrite(Pin(GPIO_LED1, led), bitRead(FirmwareGlobal.led_inverted, led) ? Settings->pwm_range - pwm_led_bri : pwm_led_bri);
      }
    }
  }
}
...
void PWMDimmerSetPoweredOffLed(void)
{
  ...
  if (PinUsed(GPIO_LEDLNK)) {
    bool power_off_led_on = !FirmwareGlobal.power && Settings->flag4.powered_off_led;
    if (FirmwareGlobal.ledlnk_inverted) power_off_led_on ^= 1;
...
{
  DigitalWrite(GPIO_REL1, 0, bitRead(FirmwareGlobal.rel_inverted, 0) ? !FirmwareGlobal.power : FirmwareGlobal.power);
  PWMDimmerSetPoweredOffLed();
}
...
#ifdef USE_PWM_DIMMER_REMOTE
  bool power_is_on = (active_remote_pwm_dimmer ? active_remote_pwm_dimmer->power_on : FirmwareGlobal.power);
  bool is_power_button = (button_index == power_button_index);
  bool is_down_button = (button_index == down_button_index);
#else // USE_PWM_DIMMER_REMOTE
  bool power_is_on = FirmwareGlobal.power;
...
#endif  // USE_PWM_DIMMER_REMOTE
      new_power = FirmwareGlobal.power ^ 1;
    if (new_power)
...
bool Xdrv35(uint32_t function)
{
  ...
  if (PWM_DIMMER != FirmwareGlobal.module_type) return result;
  ...
  switch (function) {
    case FUNC_EVERY_SECOND:
      ...
      if (FirmwareGlobal.restore_powered_off_led_counter && !--FirmwareGlobal.restore_powered_off_led_counter) PWMDimmerSetPoweredOffLed();
      break;
        ...
        if (!XdrvMailbox.payload) {
          ...
          if (!button_pressed[button_index]) {
            ...
            if (!buttons_pressed && Settings->flag4.multiple_device_groups) {
              ...
              down_button_index = (is_two_button ? 99 : Pin(GPIO_KEY1, power_button_index) == 15 ? FirmwareGlobal.gpio_pin[1] : FirmwareGlobal.gpio_pin[15]) - 32;
