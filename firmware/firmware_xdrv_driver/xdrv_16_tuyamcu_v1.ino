/*
  xdrv_16_tuyamcu.ino - Tuya MCU support for Firmware

  Copyright (C) 2021  Federico Leoni, digiblur, Joel Stein and Theo Arends and Mahmoud Shams El-Din
...
FirmwareSerial *TuyaSerial = nullptr;

struct TUYA {
  uint16_t Levels[5] = {0,0,0,0,0};       // Array to store the values of TuyaMCU channels
  uint16_t Snapshot[5] = {0,0,0,0,0};     // Array to store a snapshot of Firmware actual channel values
...
//  if (SK03_TUYA == FirmwareGlobal.module_type) {
//    is_tuya = true;
//  }
//#endif
  return is_tuya;
...
{
  return ((FirmwareGlobal.light_type > LT_RGB) && TuyaGetDpId(TUYA_MCU_FUNC_MODESET) != 0);
}
  ...
  for (uint8_t i = 0; i < MAX_TUYA_FUNCTIONS; i++) {
    ...
    if (fnId > TUYA_MCU_FUNC_NONE && Settings->tuya_fnid_map[i].dpid > 0) {
      ...
      if (fnId >= TUYA_MCU_FUNC_REL1 && fnId <= TUYA_MCU_FUNC_REL8) { //Relay
        bitClear(FirmwareGlobal.rel_inverted, fnId - TUYA_MCU_FUNC_REL1);
      } else if (fnId >= TUYA_MCU_FUNC_REL1_INV && fnId <= TUYA_MCU_FUNC_REL8_INV) { // Inverted Relay
        bitSet(FirmwareGlobal.rel_inverted, fnId - TUYA_MCU_FUNC_REL1_INV);
      }
    }
  }
}
...
bool TuyaSetPower(void)
{
  ...
  uint8_t dpid = TuyaGetDpId(TUYA_MCU_FUNC_REL1 + FirmwareGlobal.active_device - 1);
  if (dpid == 0) dpid = TuyaGetDpId(TUYA_MCU_FUNC_REL1_INV + FirmwareGlobal.active_device - 1);

  if (source != SRC_SWITCH && TuyaSerial) {  // ignore to prevent loop from pushing state from faceplate interaction
    TuyaSendBool(dpid, bitRead(rpower, FirmwareGlobal.active_device-1) ^ bitRead(FirmwareGlobal.rel_inverted, FirmwareGlobal.active_device-1));
...
bool TuyaSetChannels(void)
{
  ...
  if (LT_SERIAL1 == FirmwareGlobal.light_type) {
    Tuya.Snapshot[0] = light_state.getDimmer();
  }
  if (LT_SERIAL2 == FirmwareGlobal.light_type || LT_RGBWC == FirmwareGlobal.light_type) {
    idx = 1;
    if (LT_SERIAL2 == FirmwareGlobal.light_type && Settings->flag3.pwm_multi_channels && (TuyaGetDpId(TUYA_MCU_FUNC_DIMMER2) != 0)) {
  ...
  if (LT_RGBW == FirmwareGlobal.light_type) {
    idx = 1;
  ...
  if (FirmwareGlobal.light_type > LT_BASIC) {
    ...
    if (LT_RGB != FirmwareGlobal.light_type) {
    ...
    if (FirmwareGlobal.light_type >= LT_RGB) {
      ...
      if ((RGBType > 1 && (StrCmpNoCase(scolor, Tuya.RGBColor) != 0)) || (RGBType <= 1 && ((hue != Tuya.Snapshot[2]) ||
        ...
        if ((LightMode && Tuya.ModeSet) || LT_RGB == FirmwareGlobal.light_type) {
  ...
  if (TuyaIdx > 0 && TuyaIdx <= 2) {
    ...
    if (duty > 0 && !Tuya.ignore_dim && TuyaSerial && dpid > 0) {
      ...
      if (duty < Settings->dimmer_hw_min) { duty = Settings->dimmer_hw_min; }  // dimming acts odd below 25(10%) - this mirrors the threshold set on the faceplate itself
        ...
        if (Tuya.ModeSet && (TuyaGetDpId(TUYA_MCU_FUNC_MODESET) != 0) && FirmwareGlobal.light_type > LT_RGB) {
...
void TuyaProcessStatePacket(void) {
  ...
  uint16_t dpDataLen;
  bool PowerOff = false;
  bool tuya_energy_enabled = (XNRG_32 == FirmwareGlobal.energy_driver);
  ...
  while (dpidStart + 4 < Tuya.byte_counter) {
    ...
    if (Tuya.buffer[dpidStart + 1] == 0) {
        ...
        if (fnId >= TUYA_MCU_FUNC_REL1 && fnId <= TUYA_MCU_FUNC_REL8) {
          AddLog(LOG_LEVEL_DEBUG, PSTR("TYA: RX Relay-%d --> MCU State: %s Current State:%s"), fnId - TUYA_MCU_FUNC_REL1 + 1, Tuya.buffer[dpidStart + 4]?"On":"Off",bitRead(FirmwareGlobal.power, fnId - TUYA_MCU_FUNC_REL1)?"On":"Off");
          if (Tuya.buffer[dpidStart + 4] != bitRead(FirmwareGlobal.power, fnId - TUYA_MCU_FUNC_REL1)) {
          ...
          AddLog(LOG_LEVEL_DEBUG, PSTR("TYA: RX Relay-%d-Inverted --> MCU State: %s Current State:%s"), fnId - TUYA_MCU_FUNC_REL1_INV + 1, Tuya.buffer[dpidStart + 4]?"Off":"On",bitRead(FirmwareGlobal.power, fnId - TUYA_MCU_FUNC_REL1_INV) ^ 1?"Off":"On");
          if (Tuya.buffer[dpidStart + 4] != bitRead(FirmwareGlobal.power, fnId - TUYA_MCU_FUNC_REL1_INV) ^ 1) {
        ...
        if (SnsUpdate) {
          ...
          if (FirmwareGlobal.uptime < 8) { // delay to avoid multiple topics at the same time at boot time
            return;
          } else {
        ...
        if ((fnId == TUYA_MCU_FUNC_DIMMER) || (fnId == TUYA_MCU_FUNC_REPORT1) ||
          ...
          if (Tuya.ignore_dimmer_cmd_timeout < millis()) {
            if ((FirmwareGlobal.power || Settings->flag3.tuya_apply_o20) && ((Tuya.Levels[dimIndex] > 0 || Settings->flag5.tuya_allow_dimmer_0) && (Tuya.Levels[dimIndex] != Tuya.Snapshot[dimIndex]))) { // SetOption54 - Apply SetOption20 settings to Tuya device / SetOption131 Allow save dimmer = 0 receved by MCU
...
#ifdef ESP8266
  if (TUYA_DIMMER != FirmwareGlobal.module_type) { return false; }
  ...
  if (TuyaGetDpId(TUYA_MCU_FUNC_DIMMER) != 0) {
    if (TuyaGetDpId(TUYA_MCU_FUNC_RGB) != 0) {
      if (TuyaGetDpId(TUYA_MCU_FUNC_CT) != 0) {
        FirmwareGlobal.light_type = LT_RGBWC;
      } else if (TuyaGetDpId(TUYA_MCU_FUNC_WHITE) != 0) {
        FirmwareGlobal.light_type = LT_RGBW;
      } else { FirmwareGlobal.light_type = LT_RGB; }
    } else if (TuyaGetDpId(TUYA_MCU_FUNC_CT) != 0 || TuyaGetDpId(TUYA_MCU_FUNC_DIMMER2) != 0) {
      if (TuyaGetDpId(TUYA_MCU_FUNC_RGB) != 0) {
        FirmwareGlobal.light_type = LT_RGBWC;
      } else { FirmwareGlobal.light_type = LT_SERIAL2; }
    } else { FirmwareGlobal.light_type = LT_SERIAL1; }
  } else {
    FirmwareGlobal.light_type = LT_BASIC;
  }
...
void TuyaInit(void) {
  ...
  if (Tuya.buffer != nullptr) {
    TuyaSerial = new FirmwareSerial(Pin(GPIO_TUYA_RX), Pin(GPIO_TUYA_TX), 2);
...
void TuyaAddButton(void) {
  if (AsModuleTuyaMS()) {
    ...
    WSContentSend_P(HTTP_DEVICE_CONTROL, 26, FirmwareGlobal.devices_present + 1,
      (strlen(SettingsText(SET_BUTTON1 + FirmwareGlobal.devices_present))) ? SettingsText(SET_BUTTON1 + FirmwareGlobal.devices_present) : stemp, "");
  ...
  if (Tuya.active) {
    if (FUNC_PRE_INIT == function) {
      if (TuyaGetDpId(TUYA_MCU_FUNC_POWER) != 0 || TuyaGetDpId(TUYA_MCU_FUNC_POWER_COMBINED) != 0) {
        ...
        FirmwareGlobal.energy_driver = XNRG_32;
      }
    }
  }
  return result;
