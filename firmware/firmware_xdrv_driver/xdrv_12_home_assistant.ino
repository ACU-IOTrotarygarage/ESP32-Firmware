/*
  xdrv_12_home_assistant.ino - home assistant support for Firmware

  Copyright (C) 2021  Erik Montnemery, Federico Leoni and Theo Arends and Mahmoud Shams El-Din
...
#undef USE_Firmware_DISCOVERY
/*********************************************************************************************\
 * Legacy Home Assistant discovery now replaced by Firmware Discovery and haFirmware in HASS
\*********************************************************************************************/
...
/*********************************************************************************************\
 * New discovery direct copy of Firmware discovery
 *
 * Supported by latest versions of Home Assistant (with haFirmware) and TasmoManager.
 *
...
void HassDiscoverMessage(void) {
  ...
  char* hostname = FirmwareGlobal.hostname;
  ...
  bool iFanMod = false;
#ifdef ESP8266
  iFanMod = ((SONOFF_IFAN02 == FirmwareGlobal.module_type) || (SONOFF_IFAN03 == FirmwareGlobal.module_type));
#endif  // ESP8266
  ...
  ResponseAppend_P(PSTR("],"                                   // Friendly Names (end)
                   ...
                   FirmwareGlobal.version,
                   FirmwareGlobal.mqtt_topic,
  ...
  if (light_subtype > LST_NONE) {
    ...
    if (!light_controller_isCTRGBLinked) {                     // One or two lights present
      light_idx = FirmwareGlobal.devices_present - 2;
    } else {
      light_idx = FirmwareGlobal.devices_present - 1;
    }
  }

  if ((Light.device > 0) && Settings->flag3.pwm_multi_channels) {  // How many relays are light devices?
    light_idx = FirmwareGlobal.devices_present - light_subtype;
  }
#endif  // USE_LIGHT
  ...
  for (uint32_t i = 0; i < MAX_RELAYS_SET; i++) {
    if (i < FirmwareGlobal.devices_present) {

#ifdef USE_SHUTTER
      if (Settings->flag3.shutter_mode) {
        for (uint32_t k = 0; k < FirmwareGlobal.shutters_present; k++) {
      ...
      if (Shutter[i] != 0) {                                   // Check if there are shutters present
        ...
        if (i >= light_idx || (iFanMod && (0 == i))) {          // First relay on Ifan controls the light
          ...
          if (!iFanMod) {                                      // Relays 2-4 for ifan are controlled by FANSPEED and don't need to be present if FirmwareGlobal.module_type = SONOFF_IFAN02 or SONOFF_IFAN03
...
#ifdef ESP8266
    SerialButton = ((0 == i) && (SONOFF_DUAL == FirmwareGlobal.module_type ));
#endif  // ESP8266
  ...
  for (uint32_t i = 0; i < tmax(FirmwareGlobal.shutters_present, MAX_SHUTTERS); i++) {
#ifdef USE_SHUTTER
...
void NewHAssDiscovery(void) {
  FirmwareGlobal.masterlog_level = LOG_LEVEL_DEBUG_MORE;        // Hide topic on clean and remove use weblog 4 to show it
  ...
  char stopic[TOPSZ];
  snprintf_P(stopic, sizeof(stopic), PSTR("Firmware/discovery/%s/config"), NetworkUniqueId().c_str());
  ...
  snprintf_P(stopic, sizeof(stopic), PSTR("Firmware/discovery/%s/sensors"), NetworkUniqueId().c_str());
  MqttPublish(stopic, true);

  FirmwareGlobal.masterlog_level = LOG_LEVEL_NONE;              // Restore WebLog state
}
...
void TryResponseAppend_P(const char *format, ...) {
  ...
  {
    va_start(args, format);
    vsnprintf_P(FirmwareGlobal.mqtt_data + mlen, slen, format, args);
  }
  va_end(args);
#endif
}
...
#ifdef ESP8266
  PwmMod = (PWM_DIMMER == FirmwareGlobal.module_type);
  FanMod = (SONOFF_IFAN02 == FirmwareGlobal.module_type || SONOFF_IFAN03 == FirmwareGlobal.module_type);
  if (SONOFF_DUAL == FirmwareGlobal.module_type) { valid_relay = 2; }
#endif //ESP8266
#ifdef USE_TUYA_MCU
  TuyaMod = IsModuleTuya();
#endif
...
#ifdef USE_TUYA_MCU
  TuyaRel = TuyaGetDpId((TUYA_MCU_FUNC_REL1+ i-1) + FirmwareGlobal.active_device - 1);
  TuyaRelInv = TuyaGetDpId((TUYA_MCU_FUNC_REL1_INV+ i-1) + FirmwareGlobal.active_device - 1);
  TuyaDim = TuyaGetDpId((TUYA_MCU_FUNC_DIMMER) + FirmwareGlobal.active_device - 1);
#endif //USE_TUYA_MCU

    FirmwareGlobal.masterlog_level = ShowTopic = 4; // Hide topic on clean and remove use weblog 4 to see it
    ...
    is_topic_light = Settings->flag.hass_light && RelayX || FirmwareGlobal.light_type && !RelayX || PwmMod || (TuyaDim > 0 && TuyaMod); // SetOption30 - Enforce HAss autodiscovery as light
    ResponseClear();  // Clear retained message
    ...
    if (bitRead(shutter_mask, i-1)) {
      ...
      {                    // SetOption19 - Control Home Assistant automatic discovery (See SetOption59)
          ...
          GetTopic_P(command_topic, CMND, FirmwareGlobal.mqtt_topic, value_template);
          GetTopic_P(state_topic, TELE, FirmwareGlobal.mqtt_topic, D_RSLT_STATE);
          GetTopic_P(availability_topic, TELE, FirmwareGlobal.mqtt_topic, S_LWT);
        ...
        if (i >= Light.device) {
          if (!RelayX || PwmMod || (TuyaDim > 0 && TuyaMod)) {
            ...
            GetTopic_P(brightness_command_topic, CMND, FirmwareGlobal.mqtt_topic, channel_num);
          ...
          if ((ind_light && !PwmMulti) || LightControl) {
            ...
            if (Light.subtype >= LST_RGB) {
              ...
              GetTopic_P(clr_command_topic, CMND, FirmwareGlobal.mqtt_topic, D_CMND_HSBCOLOR);
              TryResponseAppend_P(HASS_DISCOVER_LIGHT_HS_COLOR, clr_command_topic, state_topic);

              char *effect_command_topic = stemp1;
              GetTopic_P(effect_command_topic, CMND, FirmwareGlobal.mqtt_topic, D_CMND_SCHEME);
          ...
          if ((!ind_light && ct_light) || (LST_COLDWARM == Light.subtype &&
              ...
              GetTopic_P(color_temp_command_topic, CMND, FirmwareGlobal.mqtt_topic, D_CMND_COLORTEMPERATURE);
          ...
          if (LST_RGBW == Light.subtype && !PwmMulti && LightControl) {
              ...
              GetTopic_P(white_temp_command_topic, CMND, FirmwareGlobal.mqtt_topic, D_CMND_WHITE);
    ...
    FirmwareGlobal.masterlog_level = ShowTopic;
    MqttPublish(stopic, true);
  }
}
...
void HAssAnnouncerTriggers(uint8_t device, uint8_t present, uint8_t key, uint8_t toggle, uint8_t hold, uint8_t single, uint8_t trg_start, uint8_t trg_end)
{
  ...
  for (uint8_t i = trg_start; i <= trg_end; i++) {
    ...
    if (Settings->flag.hass_discovery && present) {                // SetOption19 - Control Home Assistantautomatic discovery (See SetOption59)
      ...
      GetTopic_P(state_topic, STAT, FirmwareGlobal.mqtt_topic, jsoname);
      GetTopic_P(availability_topic, TELE, FirmwareGlobal.mqtt_topic, S_LWT);
...
void HAssAnnouncerBinSensors(uint8_t device, uint8_t present, uint8_t dual, uint8_t toggle, uint8_t pir)
{
  ...
  ResponseClear();   // Clear retained message
  FirmwareGlobal.masterlog_level = 4; // Hide topic on clean and remove use weblog 4 to see it
...
void HAssAnnounceSwitches(void)
{
  for (uint32_t switch_index = 0; switch_index < MAX_SWITCHES_SET; switch_index++)
  {
    ...
    if (KeyTopicActive(1) && strcmp(SettingsText(SET_MQTT_SWITCH_TOPIC), FirmwareGlobal.mqtt_topic))   // Enable Discovery for Switches only if SwitchTopic is set to a custom name
    {
...
#ifdef ESP8266
    if (!button_index && ((SONOFF_DUAL == FirmwareGlobal.module_type) || (CH4 == FirmwareGlobal.module_type)))
    {
      button_present = 1;
    } else
#endif // ESP8266
...
void HAssAnnounceSensor(const char *sensorname, const char *subsensortype, const char *MultiSubName, uint8_t subqty, bool nested, const char* SubKey)
{
  ...
  {                     // SetOption19 - Control Home Assistantautomatic discovery (See SetOption59)
    ...
    char *state_topic = stemp1;
    char *availability_topic = stemp2;
     FirmwareGlobal.masterlog_level = 0; // Show the new generated topic

    GetTopic_P(state_topic, TELE, FirmwareGlobal.mqtt_topic, PSTR(D_RSLT_SENSOR));
...
void HAssAnnounceSensors(void)
{
  ...
  {
    ResponseClear();
    int tele_period_save = FirmwareGlobal.tele_period;
    FirmwareGlobal.tele_period = 2;                                 // Do not allow HA updates during next function call
    XsnsNextCall(FUNC_JSON_APPEND, hass_xsns_index); // ,"INA219":{"Voltage":4.494,"Current":0.020,"Power":0.089}
    FirmwareGlobal.tele_period = tele_period_save;
...
void HAssAnnounceShutters(void)
{
  ...
  for (uint32_t i = 0; i < MAX_SHUTTERS; i++) {
    ...
    if (Settings->flag.hass_discovery && Settings->flag3.shutter_mode && Settings->shutter_startrelay[i] > 0) {
      ...
      GetTopic_P(stemp1, TELE, FirmwareGlobal.mqtt_topic, S_LWT);
      TryResponseAppend_P(HASS_DISCOVER_SENSOR_LWT, stemp1);

      GetTopic_P(stemp1, CMND, FirmwareGlobal.mqtt_topic, PSTR("Backlog"));
      TryResponseAppend_P(HASS_DISCOVER_SHUTTER_BASE, stemp1, i + 1, i + 1, i + 1);

      GetTopic_P(stemp1, STAT, FirmwareGlobal.mqtt_topic, PSTR("SHUTTER"));
      GetTopic_P(stemp2, CMND, FirmwareGlobal.mqtt_topic, PSTR("ShutterPosition"));
      ...
      if (Settings->shutter_tilt_config[3][i] != Settings->shutter_tilt_config[4][i]) {
        GetTopic_P(stemp1, CMND, FirmwareGlobal.mqtt_topic, PSTR("ShutterTilt"));
...
void HAssAnnounceDeviceInfoAndStatusSensor(void)
{
  ...
  {                    // SetOption19 - Control Home Assistantautomatic discovery (See SetOption59)
    ...
    GetTopic_P(state_topic, TELE, FirmwareGlobal.mqtt_topic, PSTR(D_RSLT_HASS_STATE));
    GetTopic_P(availability_topic, TELE, FirmwareGlobal.mqtt_topic, S_LWT);
    ...
    TryResponseAppend_P(HASS_DISCOVER_DEVICE_INFO, unique_id, ESP_getChipId(), SettingsText(SET_DEVICENAME),
                        ModuleName().c_str(), FirmwareGlobal.version, FirmwareGlobal.image_name);
    TryResponseAppend_P(PSTR("}"));
  }
  FirmwareGlobal.masterlog_level = ShowTopic;
  MqttPublish(stopic, true);
  ...
  if (!Settings->flag.hass_discovery) {
    FirmwareGlobal.masterlog_level = 0;
...
void HAssPublishStatus(void)
{
  Response_P(PSTR("{\"" D_JSON_VERSION "\":\"%s%s\",\"" D_JSON_BUILDDATETIME "\":\"%s\",\"" D_CMND_MODULE " or " D_CMND_TEMPLATE"\":\"%s\","
             ...
             FirmwareGlobal.version, FirmwareGlobal.image_name, GetBuildDateAndTime().c_str(), ModuleName().c_str(), GetResetReason().c_str(),
             GetUptime().c_str(), FirmwareGlobal.hostname, (uint32_t)WiFi.localIP(), WifiGetRssiAsQuality(WiFi.RSSI()),
             WiFi.RSSI(), WifiLinkCount(), WifiDowntime().c_str(), MqttConnectCount(), FirmwareGlobal.loop_load_avg);
  MqttPublishPrefixTopic_P(TELE, PSTR(D_RSLT_HASS_STATE));
}

void HAssDiscovery(void)
{
  // Configure Firmware for default Home Assistant parameters to keep discovery message as short as possible
  if (Settings->flag.hass_discovery)
  {                                           // SetOption19 - Control Home Assistant automatic discovery (See SetOption59)
  ...
  { // SetOption19 - Control Home Assistantautomatic discovery (See SetOption59)
    ...
    FirmwareGlobal.masterlog_level = 0; // Restores weblog level
...
void HAssDiscover(void)
{
  ...
  FirmwareGlobal.discovery_counter = 1; // Delayed discovery
}
...
void HAssAnyKey(void)
{
  ...
  GetTopic_P(stopic, STAT, FirmwareGlobal.mqtt_topic, scommand);
...
bool Xdrv12(uint32_t function)
{
  ...
  { // SetOption3 - Enable MQTT
    ...
    case FUNC_EVERY_SECOND:
      if (FirmwareGlobal.discovery_counter)
      {
        FirmwareGlobal.discovery_counter--;
        if (!FirmwareGlobal.discovery_counter)
        {
...
/*
    case FUNC_MQTT_INIT:
      ...
      FirmwareGlobal.discovery_counter = 10; // Delayed discovery
      // if (!Settings->flag.hass_discovery) {
      //   NewHAssDiscovery();
      // }
      break;
*/
    case FUNC_MQTT_SUBSCRIBE:
      ...
      hass_mode = 0;      // Discovery only if Settings->flag.hass_discovery is set
      FirmwareGlobal.discovery_counter = (0 == Mqtt.initial_connection_state) ? 1 : 10; // Delayed discovery
      break;
    case FUNC_MQTT_DATA:
      result = HAssMqttLWT();
      break;
