/*
  xdrv_86_esp32_sonoff_spm.ino - Sonoff SPM support for Firmware
 ...
 * Firmware support is based on Sonoff SPM v1.0.0 ARM firmware.
 * Energy history cannot be guaranteed using either SD-Card or internal flash. As a solution Firmware stores the total energy and yesterday energy just after midnight.
 *
 * Firmware features:
 ...
 * - Blue led equals Firmware WiFi status.
 ...
 *
 * Nice to have:
 * Support for all 32 SPM-4Relay units equals 128 relays (restricted due to internal Firmware register use)
 *
 * GPIO's used:
...
void SSPMAddModule(void) {
  if (Sspm->module_max < SSPM_MAX_MODULES) {
    ...
    if (Settings->save_data) {
      FirmwareGlobal.save_data_counter = Settings->save_data +2;            // Postpone flash write until all modules are scanned
    }
  }
}
      ...
      case SSPM_FUNC_GET_MODULE_STATE:
        ...
        */
        if (!status && (0x06 == expected_bytes)) {
          ...
          FirmwareGlobal.power |= current_state;

          Sspm->old_power = FirmwareGlobal.power;
        ...
        */
        if (expected_bytes > 24) {
          ...
          if ((0 == Sspm->Settings.energy_total[module][channel]) && energy_total) {
            ...
            if (Settings->save_data) {
              FirmwareGlobal.save_data_counter = Settings->save_data +2;            // Postpone flash write until all relays are updated
            }
          }
...
//          if (Sspm->Settings.simulate_count) { module = Sspm->simulated_module; }  // Won't work as this is initiated from device
      ...
      case SSPM_FUNC_SCAN_START:
        ...
        Cu - Current
        Su - Current Surge or Inrush (See https://github.com/arendst/Firmware/discussions/15695#discussioncomment-3747975)
        Vo - Voltage
        Po - Power
        Ot - Overtemp
        */
...
void SSPMInit(void) {
  ...
  SspmSerial = new FirmwareSerial(Pin(GPIO_RXD), Pin(GPIO_TXD), 1, 0, SSPM_SERIAL_BUFFER_SIZE);
  ...
  Sspm->old_power = FirmwareGlobal.power;
  Sspm->power_on_state = FirmwareGlobal.power;
...
*/
    ...
    case SPM_SCAN_COMPLETE:
      ...
      if (!Sspm->Settings.simulate_count) {
        ...
        if (Sspm->relay_version < SSPM_VERSION_1_2_0) {
          // Set relay power on state based on Firmware global setting
          if (Sspm->power_on_state) {
            FirmwareGlobal.power = Sspm->power_on_state;
...
#endif  // SSPM_SIMULATE
      FirmwareGlobal.discovery_counter = 1;     // Force TasDiscovery()
    ...
    case SPM_STALL_MIDNIGHT:
      // Get totals for ALL relays after midnight updating Firmwares total and yesterday energy
    ...
    case SPM_GET_ENERGY_TOTALS:
      ...
      if (Sspm->allow_updates) {
        ...
        if (Sspm->get_energy_relay >= FirmwareGlobal.devices_present) {
          Sspm->get_energy_relay = FirmwareGlobal.devices_present;
    ...
    case SPM_UPDATE_CHANNELS:
      ...
      if (Sspm->allow_updates) {
        ...
        if (time > 86370) {                    // Stall updates after 23:59:31 to satisfy ARM firmware
          ...
          if ((SPM_DISPLAY_TABS == Sspm->Settings.flag.display) && (FirmwareGlobal.devices_present > 8) && (0 == Sspm->get_totals)) {
            if ((Sspm->get_energy_relay % 4) == 0) {  // Every fourth relay
              uint32_t next = Sspm->rotate +4;
              if (next >= FirmwareGlobal.devices_present) { next = 0; }
...
//          AddLog(LOG_LEVEL_DEBUG, PSTR("SPM: Totals %d, Relay %d"), Sspm->get_totals, Sspm->get_energy_relay);
          ...
          power_t powered_on = FirmwareGlobal.power >> Sspm->get_energy_relay;
          if (powered_on &1) {
            SSPMSetLock(4);
            if (Sspm->get_totals) {
              SSPMSendGetEnergyTotal(Sspm->get_energy_relay);
...
void SSPMEnergyShow(bool json) {
  if (!FirmwareGlobal.devices_present) { return; }  // Not ready yet
  ...
  if (json) {
    ...
    power_t power = FirmwareGlobal.power;
    for (uint32_t i = 0; i < FirmwareGlobal.devices_present; i++) {
...
void CmndSpmEnergyTotal(void) {
  ...
  if (Sspm->module_max) {
    ...
    if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= FirmwareGlobal.devices_present) && XdrvMailbox.data_len) {
...
void CmndSSPMOverload(void) {
  ...
  // SspmOverload<relay> 10,0,0,0,235.2                     - Enable overload detection after 10 seconds for MaxVoltage
  if (Sspm->module_max) {
    if ((XdrvMailbox.index < 1) || (XdrvMailbox.index > FirmwareGlobal.devices_present)) { XdrvMailbox.index = 1; }
...
void CmndSSPMIamHere(void) {
  ...
  // SspmIamHere 6
  if (Sspm->module_max) {
    if ((XdrvMailbox.payload < 1) || (XdrvMailbox.payload > FirmwareGlobal.devices_present)) { XdrvMailbox.payload = 1; }
  ...
  if (Sspm->main_version > SSPM_VERSION_1_0_0) {
    ...
    if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= max_index)) {
      ...
      if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= 2)) {
        if (XdrvMailbox.payload < 2) { XdrvMailbox.payload = !XdrvMailbox.payload; }  // Swap Firmware power off (0) with Sonoff (1)
      ...
      for (uint32_t module = 0; module < Sspm->module_max; module++) {
        for (uint32_t relay = 0; relay < 4; relay++) {
          ...
          if (poweron_state < 2) { poweron_state = !poweron_state; }  // Swap Sonoff power off (1) with Firmware (0)
