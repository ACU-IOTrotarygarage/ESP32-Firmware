/*
  xdrv_49_pid.ino - PID algorithm plugin for Sonoff-Firmware
 ...
 * In user_config_override.h include code as follows:
 ...
 #define USE_PID         // include the pid feature (+4.3k)
   ...
   #define PID_SHUTTER                   1       // if using the PID to control a 3-way valve, create Firmware Shutter and define the
...
void PIDShowSensor() {
  ...
  if (!isnan(FirmwareGlobal.temperature_celsius)) {
    const float temperature = FirmwareGlobal.temperature_celsius;
