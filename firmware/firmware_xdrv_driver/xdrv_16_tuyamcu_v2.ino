...
//  uint8_t toSet[TUYA_MAX_STORED_DPs];
  ...
  uint16_t Snapshot[5];     // Array to store a snapshot of Firmware actual channel values
...
bool TuyaSetPower(void)
{
  ...
  uint8_t dev = FirmwareGlobal.active_device-1;
  uint8_t value = bitRead(rpower, dev) ^ bitRead(FirmwareGlobal.rel_inverted, dev);
...
bool TuyaSetChannels(void)
{
  ...
  if ((SRC_SWITCH == FirmwareGlobal.last_source) || (SRC_SWITCH == FirmwareGlobal.last_command_source)) {
  ...
  if (LT_SERIAL2 == FirmwareGlobal.light_type || LT_RGBWC == FirmwareGlobal.light_type) {
    idx = 1;
    if (LT_SERIAL2 == FirmwareGlobal.light_type &&
    ...
    if (FirmwareGlobal.light_type >= LT_RGB) {
      ...
      if ((RGBType > 1 && (StrCmpNoCase(scolor, pTuya->RGBColor) != 0)) ||
        ...
        if ((LightMode && pTuya->ModeSet) ||
            LT_RGB == FirmwareGlobal.light_type) {
...
//    }
    ...
    if (duty > 0 && !pTuya->ignore_dim && TuyaSerial && dpid > 0) {
      ...
      if (pTuya->ModeSet &&
          ...
          FirmwareGlobal.light_type > LT_RGB) {
...
/*          uint16_t tmpVol = pTuya->buffer[dpidStart + 4] << 8 | pTuya->buffer[dpidStart + 5];
    ...
    case TUYA_TYPE_BOOL: {  // Data Type 1
        if (fnId >= TUYA_MCU_FUNC_REL1 && fnId <= TUYA_MCU_FUNC_REL8) {
          AddLog(LOG_LEVEL_DEBUG, PSTR("T:fn%d Relay%d-->M%s T%s"), fnId, fnId - TUYA_MCU_FUNC_REL1 + 1, value?"On":"Off",bitRead(FirmwareGlobal.power, fnId - TUYA_MCU_FUNC_REL1)?"On":"Off");
          if (value != bitRead(FirmwareGlobal.power, fnId - TUYA_MCU_FUNC_REL1)) {
          ...
          AddLog(LOG_LEVEL_DEBUG, PSTR("T:fn%d Relay%d-Inv-->M%s T%s"), fnId, fnId - TUYA_MCU_FUNC_REL1_INV + 1, value?"Off":"On",bitRead(FirmwareGlobal.power, fnId - TUYA_MCU_FUNC_REL1_INV) ^ 1?"Off":"On");
          if (value != bitRead(FirmwareGlobal.power, fnId - TUYA_MCU_FUNC_REL1_INV) ^ 1) {
    ...
    case TUYA_TYPE_VALUE: {  // Data Type 2
      ...
      AddLog(LOG_LEVEL_DEBUG, PSTR("TYA: fn%d value %d dp%d "), fnId, packetValue, dpid);
        ...
        if (1) {//pTuya->ignore_dimmer_cmd_timeout < millis()) {
          if ((FirmwareGlobal.power || Settings->flag3.tuya_apply_o20) &&
