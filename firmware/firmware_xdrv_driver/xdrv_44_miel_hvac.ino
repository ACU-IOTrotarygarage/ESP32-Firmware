/*
  xdrv_44_miel_hvac.ino - Mitsubishi Electric HVAC support for Firmware
...
{
	FirmwareSerial *serial = sc->sc_serial;
	uint8_t cksum = 0;
	size_t i;
    ...
    const struct miel_hvac_data *d)
{
	...
	if (bitRead(FirmwareGlobal.power, sc->sc_device) != !!state)
...
miel_hvac_pre_init(void)
{
	...
	sc->sc_serial = new FirmwareSerial(Pin(GPIO_MIEL_HVAC_RX),
