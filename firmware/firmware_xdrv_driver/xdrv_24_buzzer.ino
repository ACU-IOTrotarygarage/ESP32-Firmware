/*
  xdrv_24_Buzzer.ino - buzzer support for Firmware
...
struct BUZZER {
  ...
  uint8_t tune_size = 0;
  uint8_t size = 0;
  uint8_t sleep;                   // Current copy of FirmwareGlobal.sleep
} Buzzer;
...
void BuzzerBeep(uint32_t count, uint32_t on, uint32_t off, uint32_t tune, uint32_t mode) {
  ...
  if (Buzzer.enable) {
    Buzzer.sleep = FirmwareGlobal.sleep;
    if (Settings->sleep > PWM_MAX_SLEEP) {
      FirmwareGlobal.sleep = PWM_MAX_SLEEP;   // Set a maxumum value of 10 milliseconds to ensure that buzzer periods are a bit more accurate
    } else {
      FirmwareGlobal.sleep = Settings->sleep;  // Or keep the current sleep if it's lower than 10
    }
  } else {
    FirmwareGlobal.sleep = Buzzer.sleep;      // Restore original sleep
    BuzzerSet(0);
  }
}
