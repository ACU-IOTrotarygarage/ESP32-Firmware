/*
  xdrv_18_armtronix_dimmers.ino - Armtronix dimmers support for Firmware

  Copyright (C) 2021  wvdv2002 and Theo Arends and Mahmoud Shams El-Din
 ...
 * Checkout the Firmware Wiki for information on how to flash this Atmega328 with the firmware
...
FirmwareSerial *ArmtronixSerial = nullptr;
...
bool ArmtronixModuleSelected(void)
{
  ...
  FirmwareGlobal.light_type = LT_SERIAL2;
  return true;
}
...
void ArmtronixInit(void)
{
  ...
  ArmtronixSerial = new FirmwareSerial(Pin(GPIO_RXD), Pin(GPIO_TXD), 2);
...
bool Xdrv18(uint32_t function)
{
  ...
  if (ARMTRONIX_DIMMERS == FirmwareGlobal.module_type) {
    switch (function) {
      case FUNC_LOOP:
