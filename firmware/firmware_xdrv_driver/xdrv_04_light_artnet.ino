/*
  ...
  Copyright (C) 2020  Stephan Hadinger & Theo Arends and Mahmoud Shams El-Din
...
bool ArtNetStart(void) {
  ArtNetLoadSettings();
  if (!artnet_udp_connected && !FirmwareGlobal.restart_flag) {
    ...
    if (ArtNetUdp->beginMulticast(IPAddress(USE_LIGHT_ARTNET_MCAST), artnet_conf.port)) {
      ...
      if (FirmwareGlobal.sleep > WS2812_ARTNET_MAX_SLEEP) {
        FirmwareGlobal.sleep = WS2812_ARTNET_MAX_SLEEP;
      }
...
//    Settings->light_scheme = LS_POWER;         // restore default scheme
    FirmwareGlobal.sleep = Settings->sleep;     // Restore sleep value
