...
\*********************************************************************************************/
extern "C" {
  FirmwareSerial * b_serial_get(struct bvm *vm) {
    be_getmember(vm, 1, ".p");
    FirmwareSerial * ow = (FirmwareSerial *) be_tocomptr(vm, -1);
    be_pop(vm, 1);
    return ow;
  }
  ...
  int32_t b_serial_init(struct bvm *vm) {
    ...
    if (argc >= 4 && be_isint(vm, 2) && be_isint(vm, 3) && be_isint(vm, 4)) {
      ...
      FirmwareSerial * ser = new FirmwareSerial(rx, tx);
  ...
  int32_t b_serial_deinit(struct bvm *vm) {
    be_getmember(vm, 1, ".p");
    FirmwareSerial * ser = (FirmwareSerial *) be_tocomptr(vm, -1);
    if (ser) {
      delete ser;
      // clear
