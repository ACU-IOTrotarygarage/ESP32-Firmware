/*
  xdrv_23_zigbee.ino - zigbee support for Firmware

  Copyright (C) 2021  Theo Arends and Mahmoud Shams El-Din and Stephan Hadinger
...
void ZbSendSend(class JsonParserToken val_cmd, ZCLFrame & zcl) {
  ...
  if (val_cmd.isObject()) {
    ...
    if (cmd_size > 1) {
      ...
      const char * Firmware_cmd = zigbeeFindCommand(key.getStr(), &local_cluster_id, &cmd_var);
      if (Firmware_cmd == nullptr) {    // did we find the command?
      ...
      zigbeeCmdAddParams(zcl.payload, Firmware_cmd, x, y, z);   // fill in parameters
    } else {
...
// ZbSend {"Device":"0x0000","Endpoint":1,"Write":{"Manufacturer":"Firmware","ModelId":"Firmware Z2T Router"}}
...
void CmndZbAttrDump(void) {
  ...
  for (uint32_t i = 0; i < sizeof(Z_Commands) / sizeof(Z_Commands[0]); i++) {
    ...
    const char * alias = Z_strings + pgm_read_word(&conv->Firmware_cmd_offset);
...
#ifdef ESP32
    if (analogAttach(led_pin, FirmwareGlobal.ledlnk_inverted) >= 0) {
      analogWritePhase(led_pin, led_power, 0);
    }
#else
    analogWrite(led_pin, FirmwareGlobal.ledlnk_inverted ? 1023 - led_power : led_power);
#endif
  }
}
...
void CmndZbConfig(void) {
  ...
  if (strlen(XdrvMailbox.data) > 0) {
    ...
    if ( (zb_channel      != Settings->zb_channel) ||
      ...
      FirmwareGlobal.restart_flag = 2;    // save and reboot
    }
  }
...
bool Xdrv23(uint32_t function) {
  if (FirmwareGlobal.gpio_optiona.enable_ccloader) { return false; }
