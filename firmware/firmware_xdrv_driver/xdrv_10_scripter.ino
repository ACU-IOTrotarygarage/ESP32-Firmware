/*
  xdrv_10_scripter.ino - script support for Firmware

  Copyright (C) 2021  Gerhard Mutz and Theo Arends and Mahmoud Shams El-Din
...
void Script_Init_UDP() {
  if (FirmwareGlobal.global_state.network_down) return;
  if (!glob_script_mem.udp_flags.udp_used) return;
...
// convert Firmware time stamp to ul seconds
...
// convert seconds to Firmware time stamp
    ...
    switch (vname[0]) {
      ...
      case 'a':
        ...
        if (!strncmp_XP(lp, XPSTR("ap("), 3)) {
          //FirmwareGlobal.restart_flag = 216;
      ...
      case 'b':
        if (!strncmp_XP(vname, XPSTR("boot"), 4)) {
          if (FirmwareGlobal.rules_flag.system_boot) {
            FirmwareGlobal.rules_flag.system_boot = 0;
            fvar = 1;
          }
          goto exit;
        }
...
//#define DEBUG_FS
      ...
      case 'g':
        if (!strncmp_XP(vname, XPSTR("gtmp"), 4)) {
          fvar = FirmwareGlobal.temperature_celsius;
          goto exit;
        }
        if (!strncmp_XP(vname, XPSTR("ghum"), 4)) {
          fvar = FirmwareGlobal.humidity;
          goto exit;
        }
        if (!strncmp_XP(vname, XPSTR("gprs"), 4)) {
          fvar = FirmwareGlobal.pressure_hpa;
          goto exit;
        }
  ...
  exit_settable:
      ...
      case 'm':
        ...
        if (!strncmp_XP(vname, XPSTR("mqttc"), 5)) {
          if (FirmwareGlobal.rules_flag.mqtt_connected) {
            FirmwareGlobal.rules_flag.mqtt_connected = 0;
            fvar = 1;
          }
          goto exit;
        }
        if (!strncmp_XP(vname, XPSTR("mqttd"), 5)) {
          if (FirmwareGlobal.rules_flag.mqtt_disconnected) {
            FirmwareGlobal.rules_flag.mqtt_disconnected = 0;
            fvar = 1;
          }
          goto exit;
        }
        if (!strncmp_XP(vname, XPSTR("mqtts"), 5)) {
          fvar = !FirmwareGlobal.global_state.mqtt_down;
          goto exit;
        }
      ...
      case 'n':
        if (!strncmp_XP(vname, XPSTR("npwr"), 4)) {
          fvar = FirmwareGlobal.devices_present;
          goto exit;
        }
        break;
      ...
      case 'p':
        ...
        if (!strncmp_XP(lp, XPSTR("pd["), 3)) {
          ...
          if ((gpiopin < nitems(FirmwareGlobal.gpio_pin)) && (FirmwareGlobal.gpio_pin[gpiopin] > 0)) {
            fvar = FirmwareGlobal.gpio_pin[gpiopin];
            // skip ] bracket
            len++;
            goto exit;
          }
        ...
        if (!strncmp_XP(lp, XPSTR("pwr["), 4)) {
          GetNumericArgument(lp + 4, OPER_EQU, &fvar, gv);
          uint8_t index = fvar;
          if (index <= FirmwareGlobal.devices_present) {
            fvar = bitRead(FirmwareGlobal.power, index - 1);
          } else {
            fvar = -1;
          }
          len += 1;
...
*/
      ...
      case 's':
        ...
        if (!strncmp_XP(lp, XPSTR("sht["), 4)) {
          GetNumericArgument(lp + 4, OPER_EQU, &fvar, gv);
          uint8_t index = fvar;
          if (index <= FirmwareGlobal.shutters_present) {
...
extern char *SML_GetSVal(uint32_t index);
        ...
        if (!strncmp_XP(lp, XPSTR("so("), 3)) {
          ...
          if (glob_script_mem.sp) {
            ...
            glob_script_mem.sp = new FirmwareSerial(rxpin, txpin, 1, 0, rxbsiz);
...
#define SCRIPT_HSTR_LEN 32
          ...
          switch (sel) {
            case 0:
              ...
              if (glob_script_mem.spi.sclk < 0) {
                // attach to existing Firmware SPI
                lp = GetNumericArgument(lp , OPER_EQU, &fvar, 0);
                fvar *= 1000000;
      ...
      case 't':
        ...
        if (!strncmp_XP(vname, XPSTR("tinit"), 5)) {
          fvar = FirmwareGlobal.rules_flag.time_init;
          goto exit;
        }
        if (!strncmp_XP(vname, XPSTR("tset"), 4)) {
          fvar = FirmwareGlobal.rules_flag.time_set;
          goto exit;
        }
        ...
        if (!strncmp_XP(vname, XPSTR("topic"), 5)) {
          if (sp) strlcpy(sp, FirmwareGlobal.mqtt_topic, glob_script_mem.max_ssize);
          goto strexit;
        }
#ifdef USE_SCRIPT_TIMER
...
*/
        ...
        if (!strncmp_XP(vname, XPSTR("wific"), 5)) {
          if (FirmwareGlobal.rules_flag.wifi_connected) {
            FirmwareGlobal.rules_flag.wifi_connected = 0;
            fvar = 1;
          }
          goto exit;
        }
        if (!strncmp_XP(vname, XPSTR("wifid"), 5)) {
          if (FirmwareGlobal.rules_flag.wifi_disconnected) {
            FirmwareGlobal.rules_flag.wifi_disconnected = 0;
            fvar = 1;
          }
          goto exit;
        }
        if (!strncmp_XP(vname, XPSTR("wifis"), 5)) {
          fvar = !FirmwareGlobal.global_state.wifi_down;
          goto exit;
        }
...
bool Is_gpio_used(uint8_t gpiopin) {
  if (gpiopin >= 0 && (gpiopin < nitems(FirmwareGlobal.gpio_pin)) && (FirmwareGlobal.gpio_pin[gpiopin] > 0)) {
    return true;
  }
  return false;
}
...
void ScripterEvery100ms(void) {
  ...
  if (bitRead(Settings->rule_enabled, 0) && (FirmwareGlobal.uptime > 4)) {
    ResponseClear();
    uint16_t script_tele_period_save = FirmwareGlobal.tele_period;
    FirmwareGlobal.tele_period = 2;
    XsnsNextCall(FUNC_JSON_APPEND, xsns_index);
    FirmwareGlobal.tele_period = script_tele_period_save;
    if (ResponseLength()) {
      ResponseJsonStart();
      ResponseJsonEnd();
    ...
    if (*lp!=';') {
      ...
      *cp = 0;
      ...
      if (response) {
        ...
        *response += String(EncodeLightId(hue_devs + FirmwareGlobal.devices_present + 1))+"\":";
...
void Script_Handle_Hue(String path) {
  ...
  uint8_t index = device - FirmwareGlobal.devices_present - 1;
...
bool ScriptCommand(void) {
  ...
  if (-1 == command_code) {
    ...
    Response_P(PSTR("{\"%s\":\"%s\",\"StopOnError\":\"%s\",\"Free\":%d}"), command, GetStateText(bitRead(Settings->rule_enabled, 0)), GetStateText(bitRead(Settings->rule_stop, 0)),glob_script_mem.script_size - strlen(glob_script_mem.script_ram));
      ...
      XdrvMailbox.topic = (char*)"stat/Firmware/SENSOR";
      ScriptMqttData();
      serviced = true;
#endif
#endif //SUPPORT_MQTT_EVENT
...
int32_t http_req(char *host, char *request) {
  ...
  FirmwareGlobal.mqtt_data = http.getString();
#else
  strlcpy(FirmwareGlobal.mqtt_data, http.getString().c_str(), ResponseSize());
#endif
...
//  FirmwareGlobal.mqtt_data = wd_jstr;
...
uint32_t call2https(const char *host, const char *path) {
  //if (FirmwareGlobal.global_state.wifi_down) return 1;
  uint32_t status = 0;
