/*
  xdrv_22_sonoff_ifan.ino - sonoff iFan02 and iFan03 support for Firmware
...
{
  return ((SONOFF_IFAN02 == FirmwareGlobal.module_type) || (SONOFF_IFAN03 == FirmwareGlobal.module_type));
}
...
uint8_t GetFanspeed(void)
{
  if (ifan_fanspeed_timer) {
    ...
    */
    uint8_t fanspeed = (uint8_t)(FirmwareGlobal.power &0xF) >> 1;
...
void SonoffIFanSetFanspeed(uint8_t fanspeed, bool sequence)
{
  ...
  if (SONOFF_IFAN03 == FirmwareGlobal.module_type) {
    if (sequence) {
...
void SonoffIfanReceived(void)
{
  ...
  uint8_t mode = FirmwareGlobal.serial_in_buffer[3];
  uint8_t action = FirmwareGlobal.serial_in_buffer[6];
  ...
  FirmwareGlobal.serial_in_buffer[5] = 0;                      // Ack
  FirmwareGlobal.serial_in_buffer[6] = 0;                      // Crc
  for (uint32_t i = 0; i < 7; i++) {
    if ((i > 1) && (i < 6)) { FirmwareGlobal.serial_in_buffer[6] += FirmwareGlobal.serial_in_buffer[i]; }
    Serial.write(FirmwareGlobal.serial_in_buffer[i]);
  }
}

bool SonoffIfanSerialInput(void)
{
  if (SONOFF_IFAN03 != FirmwareGlobal.module_type) { return false; }
  ...
  if (ifan_receive_flag) {
    ...
    if (FirmwareGlobal.serial_in_byte_counter == 8) {
      ...
      for (uint32_t i = 2; i < 7; i++) {
        crc += FirmwareGlobal.serial_in_buffer[i];
      }
      if (crc == FirmwareGlobal.serial_in_buffer[7]) {
        SonoffIfanReceived();
...
void SonoffIfanUpdate(void)
{
  ...
  if (ifan_restart_flag && (4 == FirmwareGlobal.uptime) && (SONOFF_IFAN02 == FirmwareGlobal.module_type)) {  // Microcontroller needs 3 seconds before accepting commands
    ...
    SetDevicePower(FirmwareGlobal.power, SRC_RETRY);  // Set required power on state
  }
}
