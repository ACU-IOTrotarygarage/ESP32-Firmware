/*
  xdrv_10_rules.ino - rule support for Firmware

  Copyright (C) 2021  ESP Easy Group and Theo Arends and Mahmoud Shams El-Din
...
//      if (!ucommand.startsWith("BACKLOG")) { commands = "backlog " + commands; }  // Always use Backlog to prevent power race exception
      ...
      RulesVarReplace(commands, F("%TOPIC%"), FirmwareGlobal.mqtt_topic);
...
void RulesInit(void)
{
  ...
  FirmwareGlobal.rules_flag.data = 0;
  for (uint32_t i = 0; i < MAX_RULE_SETS; i++) {
    if (0 == GetRuleLen(i)) {
...
void RulesEvery50ms(void)
{
  if ((Settings->rule_enabled || BERRY_RULES) && !Rules.busy) {  // Any rule enabled
    ...
    if (-1 == Rules.new_power) { Rules.new_power = FirmwareGlobal.power; }
...
void RulesEvery100ms(void) {
  ...
  if ((Settings->rule_enabled || BERRY_RULES) && !Rules.busy && (FirmwareGlobal.uptime > 4)) {  // Any rule enabled and allow 4 seconds start-up time for sensors (#3811)
    ResponseClear();
...
void RulesEverySecond(void)
{
  ...
  if ((Settings->rule_enabled || BERRY_RULES) && !Rules.busy) {  // Any rule enabled
    if (RtcTime.valid) {
      if ((FirmwareGlobal.uptime > 60) && (RtcTime.minute != Rules.last_minute)) {  // Execute from one minute after restart every minute only once
...
/*
    if (sCurrentCommand.length() > 0
      && backlog.size() < MAX_BACKLOG && !FirmwareGlobal.backlog_mutex)
    {
      //Insert into backlog
