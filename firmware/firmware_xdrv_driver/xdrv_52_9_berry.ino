...
// call the event dispatcher from Firmware object
...
// Simplified version of event loop. Just call `Firmware.fast_loop()`
void callBerryFastLoop(void) {
  bvm *vm = berry.vm;

  if (nullptr == vm) { return; }

  if (be_getglobal(vm, "Firmware")) {
...
void BrLoad(const char * script_name) {
  if (berry.vm == nullptr || FirmwareGlobal.no_autoexec) { return; }   // abort is berry is not running, or bootloop prevention kicked in
...
const char HTTP_BERRY_FORM_CMND[] PROGMEM =
  ...
  "<div contenteditable='false' class='br0 bro' readonly id='t1' cols='340' wrap='off'>"
    "<div class='br1'>Welcome to the Berry Scripting console. "
      "Check the <a href='#' target='_blank'>documentation</a>."
    "</div>"
  "</div>"
...
bool Xdrv52(uint32_t function)
{
  ...
  switch (function) {
    case FUNC_LOOP:
      ...
      if (FirmwareGlobal.berry_fast_loop_enabled) {    // call only if enabled at global level
        callBerryFastLoop();      // call `Firmware.fast_loop()` optimized for minimal performance impact
      }
      break;
