/*
  xdrv_27_Shutter.ino - Shutter/Blind support for Firmware
  ...
  switch (Shutter[i].switch_mode) {
    case SHT_SWITCH:
      for (int8_t k=0;k<2;k++) {
        if ((1 << (Settings->shutter_startrelay[i]+k-1)) & FirmwareGlobal.power) {
    ...
    case SHT_PULSE:
      ...
      if ((SRC_PULSETIMER == FirmwareGlobal.last_source || SRC_SHUTTER == FirmwareGlobal.last_source || SRC_WEBGUI == FirmwareGlobal.last_source)) {
        ...
        if (((1 << (Settings->shutter_startrelay[i])) & FirmwareGlobal.power)  && Settings->shutter_startrelay[i]+1 != cur_relay) {
...
void ShutterRelayChanged(void)
{
  ...
  char stemp1[10];
	...
	for (uint32_t i = 0; i < FirmwareGlobal.shutters_present; i++) {
		power_t powerstate_local = (FirmwareGlobal.power >> (Settings->shutter_startrelay[i] -1)) & 3;
    // SRC_IGNORE added because INTERLOCK function bite causes this as last source for changing the relay.
		//uint8   manual_relays_changed = ((ShutterGlobal.RelayCurrentMask >> (Settings->shutter_startrelay[i] -1)) & 3) && SRC_IGNORE != FirmwareGlobal.last_source && SRC_SHUTTER != FirmwareGlobal.last_source && SRC_PULSETIMER != FirmwareGlobal.last_source ;
    uint8   manual_relays_changed = ((ShutterGlobal.RelayCurrentMask >> (Settings->shutter_startrelay[i] -1)) & 3) && SRC_SHUTTER != FirmwareGlobal.last_source && SRC_PULSETIMER != FirmwareGlobal.last_source ;
...
bool ShutterButtonHandler(void)
{
  ...
  if (!Settings->flag.button_single) {                    // SetOption13 (0) - Allow multi-press
    if (Button.window_timer[button_index]) {
      ...
      if (!FirmwareGlobal.restart_flag && !Button.hold_timer[button_index] && (Button.press_counter[button_index] > 0)) {
        if (Button.press_counter[button_index]<99) {
...
void CmndShutterPosition(void)
{
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= FirmwareGlobal.shutters_present)) {
    if (!(Settings->shutter_options[XdrvMailbox.index-1] & 2)) {
      ...
      target_pos_percent = ((Settings->shutter_options[index] & 1) && (SRC_WEBGUI != FirmwareGlobal.last_source)) ? 100 - target_pos_percent : target_pos_percent;
      ...
      if ( (target_pos_percent >= 0) && (target_pos_percent <= 100) &&
        ...
        if (Shutter[index].direction != new_shutterdirection) {
          ...
          switch (ShutterGlobal.position_mode) {
            ...
            case SHT_TIME:
              if (!ShutterGlobal.skip_relay_change) {
                if ( (FirmwareGlobal.power >> (Settings->shutter_startrelay[index] -1)) & 3 > 0 ) {
  ...
  if (Settings->flag3.shutter_mode) {  // SetOption80 - Enable shutter support
    ...
    switch (function) {
      ...
      case FUNC_SET_POWER:
        ...
        save_powermatrix = FirmwareGlobal.power;
