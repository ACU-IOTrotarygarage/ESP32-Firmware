...
struct DINGTIAN_DATA {
  ...
  uint8_t     first;            // index of 1st Firmware relay assigned to 1st Dingtian relays
  ...
  if (PinUsed(GPIO_DINGTIAN_CLK, GPIO_ANY) && PinUsed(GPIO_DINGTIAN_SDI) && PinUsed(GPIO_DINGTIAN_Q7)
   && PinUsed(GPIO_DINGTIAN_PL) && PinUsed(GPIO_DINGTIAN_RCK)) {
    ...
    if (Dingtian) {
      ...
      Dingtian->first = FirmwareGlobal.devices_present;
      Dingtian->key_offset = -1;
      UpdateDevicesPresent(Dingtian->count);
      AddLog(LOG_LEVEL_DEBUG, PSTR("DNGT: Dingtian relays: POWER%d to POWER%d"), Dingtian->first + 1, FirmwareGlobal.devices_present);
    }
  }
}
