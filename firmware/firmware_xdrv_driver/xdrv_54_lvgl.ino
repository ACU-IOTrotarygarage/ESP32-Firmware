...
#include <renderer.h>
#include "lvgl.h"
#include "Firmware_lvgl_assets.h"    // force compilation of assets
...
bool Xdrv54(uint32_t function)
{
  ...
  switch (function) {
    ...
    case FUNC_LOOP:
      if (glue) {
        if (FirmwareGlobal.sleep > USE_LVGL_MAX_SLEEP) {
          FirmwareGlobal.sleep = USE_LVGL_MAX_SLEEP;   // sleep is max 10ms
        }
        lv_task_handler();
      }
      break;
