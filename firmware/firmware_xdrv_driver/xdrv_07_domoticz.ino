/*
  xdrv_07_domoticz.ino - domoticz support for Firmware
...
void MqttPublishDomoticzPowerState(uint8_t device) {
  if (Settings->flag.mqtt_enabled) {  // SetOption3 - Enable MQTT
    ...
    if ((device > FirmwareGlobal.devices_present) || (device > MAX_DOMOTICZ_IDX)) { return; }
    if (Settings->domoticz_relay_idx[device -1]) {
      ...
      if (IsModuleIfan() && (device > 1)) {
        ...
        Response_P(DOMOTICZ_MESSAGE, (int)Settings->domoticz_relay_idx[device -1], (FirmwareGlobal.power & (1 << (device -1))) ? 1 : 0, (FirmwareGlobal.light_type) ? svalue : "", DomoticzBatteryQuality(), DomoticzRssiQuality());
        MqttPublish(domoticz_in_topic);
#ifdef USE_SONOFF_IFAN
      }
#endif // USE_SONOFF_IFAN
...
void DomoticzMqttSubscribe(void) {
  uint8_t maxdev = (FirmwareGlobal.devices_present > MAX_DOMOTICZ_IDX) ? MAX_DOMOTICZ_IDX : FirmwareGlobal.devices_present;
...
int32_t DomoticzIdx2Relay(uint32_t idx) {
  ...
  uint32_t maxdev = (FirmwareGlobal.devices_present > MAX_DOMOTICZ_IDX) ? MAX_DOMOTICZ_IDX : FirmwareGlobal.devices_present;
  for (uint32_t i = 0; i < maxdev; i++) {
...
*/
  ...
  if (iscolordimmer && 10 == nvalue) {  // Color_SetColor
    ...
    if (FirmwareGlobal.light_type && (Settings->light_dimmer == nvalue) && ((FirmwareGlobal.power >> relay_index) &1)) {
  ...
  if (1 == nvalue || 0 == nvalue) {
    if (((FirmwareGlobal.power >> relay_index) &1) == (power_t)nvalue) {
      return true;  // Stop loop
    }
    char stemp1[10];
    snprintf_P(XdrvMailbox.topic, XdrvMailbox.index, PSTR("/" D_CMND_POWER "%s"), (FirmwareGlobal.devices_present > 1) ? itoa(relay_index +1, stemp1, 10) : "");
    snprintf_P(XdrvMailbox.data, XdrvMailbox.data_len, PSTR("%d"), nvalue);
  } else {
