/*
  xdrv_27_esp32_shutter.ino - Shutter/Blind support for Firmware
...
void ExecuteCommandPowerShutter(uint32_t device, uint32_t state, uint32_t source)
{
  ...
  if (device <= FirmwareGlobal.devices_present) ExecuteCommandPower(device,state,source);
}
...
void ShutterRtc50mS(void)
{
  ...
  for (uint8_t i = 0; i < FirmwareGlobal.shutters_present; i++) {
      ...
      switch (ShutterGlobal.position_mode) {
        ...
        case SHT_COUNTER:
          if (Shutter[i].accelerator) {
            ...
            FirmwareGlobal.pwm_value[i] = 512;
            pwm_apply = true;
  #endif  // ESP32
          }
        break;
...
{
  FirmwareGlobal.shutters_present = 0;
  ShutterGlobal.RelayShutterMask = 0;
  //Initialize to get relay that changed
  ShutterGlobal.RelayOldMask = FirmwareGlobal.power;
  ...
  for (uint32_t i = 0; i < MAX_SHUTTERS_ESP32; i++) {
    ...
    if (ShutterSettings.shutter_startrelay[i] && (ShutterSettings.shutter_startrelay[i] <= 32 )) {
      ...
      FirmwareGlobal.shutters_present++;
...
void ShutterReportPosition(bool always, uint32_t index)
{
  Response_P(PSTR("{"));
  uint32_t i = 0;
  uint32_t n = FirmwareGlobal.shutters_present;
  ...
  //AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: rules_flag.shutter_moving: %d, moved %d"), FirmwareGlobal.rules_flag.shutter_moving, FirmwareGlobal.rules_flag.shutter_moved);
}
...
void ShutterPowerOff(uint8_t i)
{
  ...
  FirmwareGlobal.rules_flag.shutter_moved = 1;
  switch (Shutter[i].switch_mode) {
    case SHT_SWITCH:
      for (int8_t k=0;k<2;k++) {
        if ((1 << (ShutterSettings.shutter_startrelay[i]+k-1)) & FirmwareGlobal.power) {
    ...
    case SHT_PULSE:
      ...
      if ((SRC_PULSETIMER == FirmwareGlobal.last_source || SRC_SHUTTER == FirmwareGlobal.last_source || SRC_WEBGUI == FirmwareGlobal.last_source)) {
        ExecuteCommandPowerShutter(cur_relay, 1, SRC_SHUTTER);
        // switch off direction relay to make it power less
        if (((1 << (ShutterSettings.shutter_startrelay[i])) & FirmwareGlobal.power)  && ShutterSettings.shutter_startrelay[i]+1 != cur_relay) {
          ExecuteCommandPowerShutter(ShutterSettings.shutter_startrelay[i]+1, 0, SRC_SHUTTER);
        }
      } else {
        FirmwareGlobal.last_source = SRC_SHUTTER;
      }
    break;
  }
...
void ShutterAllowPreStartProcedure(uint8_t i) {
  ...
  uptime_Local = FirmwareGlobal.uptime;
  while (uptime_Local+10 > FirmwareGlobal.uptime && (String)rules_vars[i] == "99") {
    loop();
  }
...
void ShutterStartInit(uint32_t i, int32_t direction, int32_t target_pos)
{
  ...
  if ( ( ( (1 == direction) && ((Shutter[i].open_max - Shutter[i].real_position)  <= Shutter[i].min_realPositionChange))
    ...
    if (Settings->save_data) {
      ...
      FirmwareGlobal.save_data_counter = Settings->save_data + (move_duration / 10) +1;
    }
  }
...
void ShutterRelayChanged(void)
{
  ...
  char stemp1[10];
	...
	for (uint32_t i = 0; i < FirmwareGlobal.shutters_present; i++) {
		power_t powerstate_local = (FirmwareGlobal.power >> (ShutterSettings.shutter_startrelay[i] -1)) & 3;
    // SRC_IGNORE added because INTERLOCK function bite causes this as last source for changing the relay.
		//uint8   manual_relays_changed = ((ShutterGlobal.RelayCurrentMask >> (ShutterSettings.shutter_startrelay[i] -1)) & 3) && SRC_IGNORE != FirmwareGlobal.last_source && SRC_SHUTTER != FirmwareGlobal.last_source && SRC_PULSETIMER != FirmwareGlobal.last_source ;
    uint8   manual_relays_changed = ((ShutterGlobal.RelayCurrentMask >> (ShutterSettings.shutter_startrelay[i] -1)) & 3) && SRC_SHUTTER != FirmwareGlobal.last_source && SRC_PULSETIMER != FirmwareGlobal.last_source ;
    //AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: Shtr%d, Source %s, Powerstate %ld, RelayMask %d, ManualChange %d"),
    //  i+1, GetTextIndexed(stemp1, sizeof(stemp1), FirmwareGlobal.last_source, kCommandSource), powerstate_local,ShutterGlobal.RelayCurrentMask,manual_relays_changed);
        ...
        case SHT_PULSE:
          if (Shutter[i].direction != 0 && powerstate_local) {
            ...
            powerstate_local = 0;
              ...
              i+1, Shutter[i].target_position, GetTextIndexed(stemp1, sizeof(stemp1), FirmwareGlobal.last_source, kCommandSource), powerstate_local,ShutterGlobal.RelayCurrentMask,manual_relays_changed);
          }
        break;
        default:
          FirmwareGlobal.last_source = SRC_SHUTTER; // avoid switch off in the next loop
...
bool ShutterButtonHandlerMulti(void)
{
  ...
  FirmwareGlobal.last_source = SRC_BUTTON;
...
void ShutterToggle(bool dir)
{
  ...
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= FirmwareGlobal.shutters_present)) {
    ...
    XdrvMailbox.data_len = 0;
    FirmwareGlobal.last_source = SRC_WEBGUI;
    CmndShutterPosition();
  }
}
...
{
  //AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: Change in: payload %s (%d), payload %d, idx %d, src %d"), XdrvMailbox.data , XdrvMailbox.data_len, XdrvMailbox.payload , XdrvMailbox.index, FirmwareGlobal.last_source );
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= FirmwareGlobal.shutters_present)) {
...
void CmndShutterPosition(void)
{
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= FirmwareGlobal.shutters_present)) {
    if (!(ShutterSettings.shutter_options[XdrvMailbox.index-1] & 2)) {
      uint32_t index = XdrvMailbox.index-1;
      //limit the payload
      AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: Pos. payload <%s> (%d), payload %d, idx %d (%d), src %d"), XdrvMailbox.data , XdrvMailbox.data_len, XdrvMailbox.payload , XdrvMailbox.index, XdrvMailbox.usridx, FirmwareGlobal.last_source );
      ...
      // STOP will come with payload 0 because predefined value in Firmware
      ...
      target_pos_percent = ((ShutterSettings.shutter_options[index] & 1) && (SRC_WEBGUI != FirmwareGlobal.last_source)) ? 100 - target_pos_percent : target_pos_percent;
      if (XdrvMailbox.payload != -99) {
        ...
        AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: lastsource %d:, real %d, target %d, tiltreal: %d, tilttarget: %d, payload %d"), FirmwareGlobal.last_source, Shutter[index].real_position ,Shutter[index].target_position,Shutter[index].tilt_real_pos, Shutter[index].tilt_target_pos,target_pos_percent);
      }
      if ( (target_pos_percent >= 0) && (target_pos_percent <= 100) &&
        ...
        if (Shutter[index].direction != new_shutterdirection) {
          ...
          switch (ShutterGlobal.position_mode) {
            ...
            case SHT_TIME:
              if (!ShutterGlobal.skip_relay_change) {
                if ( (FirmwareGlobal.power >> (ShutterSettings.shutter_startrelay[index] -1)) & 3 > 0 ) {
...
void CmndShutterRelay(void)
{
  ...
  uint32_t end   = (!XdrvMailbox.usridx && !XdrvMailbox.data_len)?FirmwareGlobal.shutters_present:XdrvMailbox.index;
...
void CmndShutterFrequency(void)
{
  if ((XdrvMailbox.payload > 0) && (XdrvMailbox.payload <= 20000)) {
    ...
    if (FirmwareGlobal.shutters_present < 4) {
...
{
  //AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: Change in: payload %s (%d), payload %d, idx %d, src %d"), XdrvMailbox.data , XdrvMailbox.data_len, XdrvMailbox.payload , XdrvMailbox.index, FirmwareGlobal.last_source );
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= FirmwareGlobal.shutters_present) && XdrvMailbox.data_len > 0) {
...
bool Xdrv27(uint32_t function)
{
  ...
  if (Settings->flag3.shutter_mode) {  // SetOption80 - Enable shutter support
    ...
    uint8_t  counterend = XdrvMailbox.index==0?FirmwareGlobal.shutters_present:XdrvMailbox.index;
    ...
    switch (function) {
...
#ifdef USE_DOMOTICZ
          if ((0 == FirmwareGlobal.tele_period) && (0 == i)) {
      ...
      case FUNC_SET_POWER:
        ...
        AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: FUNC_SET_POWER Relaymask %d SwitchedRelay:%d by %s, payload %d, powermask %d"), ShutterGlobal.RelayOldMask, ShutterGlobal.LastChangedRelay,GetTextIndexed(stemp1, sizeof(stemp1), FirmwareGlobal.last_source, kCommandSource),XdrvMailbox.payload, FirmwareGlobal.power);
        save_powermatrix = FirmwareGlobal.power; // can be changed in ShutterRelayChanged
        if (!ShutterGlobal.LastChangedRelay) {
          ...
          FirmwareGlobal.power = save_powermatrix;
        }
        //AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: FUNC_SET_POWER end. powermask %d"), FirmwareGlobal.power);
      break;
      case FUNC_SET_DEVICE_POWER:
        ...
        if (ShutterGlobal.skip_relay_change ) {
          ...
          if (ShutterGlobal.LastChangedRelay) ShutterGlobal.RelayOldMask = FirmwareGlobal.power ^=  1<<(ShutterGlobal.LastChangedRelay-1);
        }
        AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: FUNC_SET_DEVICE_POWER end. powermask %ld, current rel: %ld"), FirmwareGlobal.power, ShutterGlobal.RelayOldMask);
      break;
