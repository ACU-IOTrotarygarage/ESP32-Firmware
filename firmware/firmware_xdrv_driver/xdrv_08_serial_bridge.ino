/*
  xdrv_08_serial_bridge.ino - serial bridge support for Firmware

  Copyright (C) 2021  Theo Arends and Mahmoud Shams El-Din and Dániel Zoltán Tolnai
...
/*********************************************************************************************\
 * Serial Bridge using Software Serial library (FirmwareSerial)
\*********************************************************************************************/
...
FirmwareSerial *SerialBridgeSerial = nullptr;
...
void SerialBridgeInput(void) {
  ...
  if (serial_bridge_in_byte_counter && (millis() > (serial_bridge_polling_window + SERIAL_POLLING))) {
    ...
    FirmwareGlobal.serial_skip++;                       // SetOption35  Skip number of serial messages received (default 0)
    if (FirmwareGlobal.serial_skip > Settings->param[P_SERIAL_SKIP]) {  // Handle intermediate changes to SetOption35
      FirmwareGlobal.serial_skip = 0;
      ...
      if (Settings->flag6.mqtt_disable_sserialrec ) {  // SetOption147  If it is activated, Firmware will not publish SSerialReceived MQTT messages, but it will proccess event trigger rules
        XdrvRulesProcess(0);
      } else {
...
//    SerialBridgeSerial = new FirmwareSerial(Pin(GPIO_SBR_RX), Pin(GPIO_SBR_TX), HARDWARE_FALLBACK);  // Default TM_SERIAL_BUFFER_SIZE (=64) size
    SerialBridgeSerial = new FirmwareSerial(Pin(GPIO_SBR_RX), Pin(GPIO_SBR_TX), HARDWARE_FALLBACK, 0, MIN_INPUT_BUFFER_SIZE);  // 256
    if (SetSSerialBegin()) {
      if (SerialBridgeSerial->hardwareSerial()) {
        ClaimSerial();
        serial_bridge_buffer = FirmwareGlobal.serial_in_buffer;  // Use idle serial buffer to save RAM
      } else {
