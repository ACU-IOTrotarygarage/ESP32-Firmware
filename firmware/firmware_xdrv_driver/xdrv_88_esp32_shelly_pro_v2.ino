/*
  xdrv_88_esp32_shelly_pro.ino - Shelly pro family support for Firmware

  Copyright (C) 2022  Theo Arends and Mahmoud Shams El-Din
...
//  AddLog(LOG_LEVEL_DEBUG, PSTR("SHP: Input from %04X to %04X"), SPro.input_state, input_state);
  ...
  if (FirmwareGlobal.uptime < 3) { return; }         // Flush interrupt for 3 seconds after poweron
...
void ShellyProPreInit(void) {
  if ((SPI_MOSI_MISO == FirmwareGlobal.spi_enabled) &&
      PinUsed(GPIO_SPI_CS) &&                       // 74HC595 rclk / MCP23S17
      FirmwareGlobal.gpio_optiona.shelly_pro) {      // Option_A7
...
void ShellyProLedLink(void) {
  ...
  if (SPro.detected != 4) {
    ...
    */
    SPro.last_update = FirmwareGlobal.uptime;
    uint32_t ledlink = 0x1C;                        // All leds off
    if (XdrvMailbox.index) {
      ledlink &= 0xFB;                              // Blue blinks if wifi/mqtt lost
    }
    else if (!FirmwareGlobal.global_state.wifi_down) {
...
void ShellyProLedLinkWifiOff(void) {
  ...
  if (SPro.detected != 4) {
    ...
    - Green light indicator will be on if in STA mode and connected to a Wi-Fi network.
    */
    if (SPro.last_update +1 < FirmwareGlobal.uptime) {
      ShellyProUpdateLedLink((FirmwareGlobal.global_state.wifi_down) ? 0x1C : 0x14);  // Green off if wifi OFF
    }
  }
}
