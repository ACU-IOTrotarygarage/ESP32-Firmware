/*
  xdrv_50_filesystem.ino - unified file system for Firmware

  Copyright (C) 2020  Gerhard Mutz and Theo Arends and Mahmoud Shams El-Din
...
// Called from Firmware.ino at restart. This inits flash file only
void UfsInit(void) {
...
void UfsCheckSDCardInit(void) {
  ...
  if (FirmwareGlobal.spi_enabled && PinUsed(GPIO_SDCARD_CS)) {
...
void download_task(void *path) {
  ...
  //Webserver->sendHeader(F("X-Firmware-Debug"), attachment);
  snprintf_P(attachment, sizeof(attachment), PSTR("attachment; filename=%s"), cp);
  Webserver->sendHeader(F("Content-Disposition"), attachment);
  WSSend(200, CT_APP_STREAM, "");
