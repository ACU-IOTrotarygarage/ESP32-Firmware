...
FirmwareSerial *TCPSerial = nullptr;
...
void TCPInit(void) {
  if (PinUsed(GPIO_TCP_RX) && PinUsed(GPIO_TCP_TX)) {
    ...
    TCPSerial = new FirmwareSerial(Pin(GPIO_TCP_RX), Pin(GPIO_TCP_TX), FirmwareGlobal.seriallog_level ? 1 : 2, 0, TCP_BRIDGE_BUF_SIZE);   // set a receive buffer of 256 bytes
