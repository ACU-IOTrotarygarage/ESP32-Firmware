/*
  xdrv_13_display.ino - Display support for Firmware
            ...
            if (*cp == 'e' || *cp == 'd') {
              ...
              if (buttons[num]) {
                ...
                if (!dis) {
                  ...
                  else buttons[num]->xdrawButton(bitRead(FirmwareGlobal.power,num));
                }
              }
              break;
            }
            ...
            if (renderer) {
              ...
              if (buttons[num]) {
                if (!sbt) {
                  ...
                  if (!bflags) {
                    // power button
                    if (dflg) buttons[num]->xdrawButton(bitRead(FirmwareGlobal.power, num));
...
void DisplayLogBufferInit(void)
{
  if (Settings->display_mode) {
    ...
    char buffer[40];
    snprintf_P(buffer, sizeof(buffer), PSTR(D_VERSION " %s%s"), FirmwareGlobal.version, FirmwareGlobal.image_name);
...
bool DisplayMqttData(void)
{
  if (disp_subscribed) {
    ...
    if (tp) {                                                // tele/Firmware/SENSOR
      if (Settings->display_mode &0x04) {
        tp = tp + strlen(stopic);                              // Firmware/SENSOR
        char *topic = strtok(tp, "/");                         // Firmware
...
{
  if ((Settings->display_mode &0x02) && (0 == FirmwareGlobal.tele_period)) {
    char no_topic[1] = { 0 };
//    DisplayAnalyzeJson(FirmwareGlobal.mqtt_topic, ResponseData());  // Add local topic
...
//    ApplyDisplayDimmer();  // Not allowed here. Way too early in init sequence. Global power state has not been set at this point in time
    ...
    if (!PinUsed(GPIO_BACKLIGHT)) {
      if (FirmwareGlobal.light_type && (4 == Settings->display_model)) {
        UpdateDevicesPresent(-1);  // Assume PWM channel is used for backlight
      }
    }
    disp_device = FirmwareGlobal.devices_present;
...
void CmndDisplayModel(void) {
  if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload < DISPLAY_MAX_DRIVERS)) {
    ...
    if (XdspCall(FUNC_DISPLAY_MODEL)) {
      FirmwareGlobal.restart_flag = 2;  // Restart to re-init interface and add/Remove MQTT subscribe
    } else {
...
void CmndDisplayWidth(void) {
  if (XdrvMailbox.payload > 0) {
    if (XdrvMailbox.payload != Settings->display_width) {
      ...
      FirmwareGlobal.restart_flag = 2;  // Restart to re-init width
    }
  }
...
void CmndDisplayHeight(void) {
  if (XdrvMailbox.payload > 0) {
    if (XdrvMailbox.payload != Settings->display_height) {
      ...
      FirmwareGlobal.restart_flag = 2;  // Restart to re-init height
    }
  }
...
*/
  if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= 5)) {
    ...
    if (disp_subscribed != (Settings->display_mode &0x04)) {
      FirmwareGlobal.restart_flag = 2;  // Restart to Add/Remove MQTT subscribe
    } else {
