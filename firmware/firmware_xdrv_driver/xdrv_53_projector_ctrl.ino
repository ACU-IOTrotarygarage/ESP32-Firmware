/*
  xdrv_53_projector_ctrl.ino - LCD/DLP Projector Serial Control support for Firmware
...
projector_ctrl_pre_init(void)
{
	...
	sc->sc_serial = new FirmwareSerial(Pin(GPIO_PROJECTOR_CTRL_RX),
  ...
  UpdateDevicesPresent(1); /* claim a POWER device slot */
	sc->sc_device = FirmwareGlobal.devices_present;
...
projector_ctrl_tick(struct projector_ctrl_softc_s *sc)
{
	if(sc->sc_ser_state==PROJECTOR_CTRL_S_IDLE){
		switch (FirmwareGlobal.uptime&0xf) {
			case 0:
				projector_ctrl_request(sc,PROJECTOR_CTRL_S_QRY_PWR);
				break;
			case 8:
...
bool Xdrv53(uint32_t function) {
	...
	case FUNC_EVERY_SECOND:
		...
		else if ((FirmwareGlobal.uptime&0x7)==0) //each 8 seconds
