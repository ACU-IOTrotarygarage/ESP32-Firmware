/*
  xdrv_01_webserver.ino - webserver for Firmware

  Copyright (C) 2021  Theo Arends and Mahmoud Shams El-Din and Adrian Scillato
...
#define RESTART_AFTER_INITIAL_WIFI_CONFIG         true   // Restart Firmware after initial Wifi Config of a blank device
#endif                                                   //   If disabled, Firmware will keep both the wifi AP and the wifi connection to the router
...
#define AFTER_INITIAL_WIFI_CONFIG_GO_TO_NEW_IP    true   //   the user will be redirected to the new IP of Firmware (in the new Network).
...
#endif
*/
//  "<img style ='width: 300px;height: 200px;background-position: center;' src='upload.wikimedia.org/wikipedia/commons/4/4f/ACU_Logo.png' alt='ACU Logo' />";
  "<h3>%s</h3>"    // Module name
  "<h2>%s</h2>";   // Device name
...
const char HTTP_END[] PROGMEM =
  "<div style='text-align:right;font-size:11px;'><hr/><a href='https://github.com/MahmoudsEl-Din/' target='_blank' style='color:#aaa;'>graduation project %s " D_BY " Mahmoud Shams El-Din</a></div>"
  "</div>"
  "</body>"
  "</html>";
...
//      Webserver->on(F("/u2"), HTTP_POST, HandleUploadDone, HandleUploadLoop);  // this call requires 2 functions so we keep a direct call
  ...
  if (Web.state != type) {
    ...
    FirmwareGlobal.rules_flag.http_init = 1;
    Web.state = type;
  }
}
...
void WifiManagerBegin(bool reset_only)
{
  ...
  WiFi.softAP(FirmwareGlobal.hostname, WIFI_AP_PASSPHRASE, channel, 0, 1);
...
bool HttpCheckPriviledgedAccess(bool autorequestauth = true)
{
  ...
  if (!Settings->flag5.disable_referer_chk && !WifiIsInManagerMode()) {
    ...
    if (referer.length()) {
      referer.toUpperCase();
      String hostname = FirmwareGlobal.hostname;
...
{
  char server[32];
  snprintf_P(server, sizeof(server), PSTR("Firmware/%s (%s)"), FirmwareGlobal.version, GetDeviceHardware().c_str());
...
void WSContentSendStyle_P(const char* formatP, ...) {
  ...
  if ( Settings->flag3.gui_hostname_ip || ( (WiFi.getMode() == WIFI_AP_STA) && (!Web.initial_config) )  ) {
    ...
    bool eip = false;
    if (lip || sip) {
      WSContentSend_P(PSTR("<h4>%s%s (%s%s%s)"),    // Firmware.local (192.168.2.12, 192.168.4.1)
        FirmwareGlobal.hostname,
    ...
    eip = EthernetHasIP();
    if (eip) {
      WSContentSend_P(PSTR("%s%s%s (%s)"),          // Firmware-eth.local (192.168.2.13)
...
void WSContentStop(void) {
  ...
  WSContentSend_P(HTTP_END, FirmwareGlobal.version);
  WSContentEnd();
}
...
uint32_t WebDeviceColumns(void) {
  ...
  uint32_t rows = FirmwareGlobal.devices_present / max_columns;
  if (FirmwareGlobal.devices_present % max_columns) { rows++; }
  uint32_t cols = FirmwareGlobal.devices_present / rows;
  if (FirmwareGlobal.devices_present % rows) { cols++; }
  return cols;
}
...
void HandleRoot(void)
{
  ...
  if (FirmwareGlobal.devices_present) {
#ifdef USE_LIGHT
    if (FirmwareGlobal.light_type) {
      uint8_t light_subtype = FirmwareGlobal.light_type &7;
      if (!Settings->flag3.pwm_multi_channels) {  // SetOption68 0 - Enable multi-channels PWM instead of Color PWM
        bool split_white = ((LST_RGBW <= light_subtype) && (FirmwareGlobal.devices_present > 1));  // Only on RGBW or RGBCW and SetOption37 128
    ...
    if (Settings->flag3.shutter_mode) {  // SetOption80 - Enable shutter support
      for (uint32_t i = 0; i < FirmwareGlobal.shutters_present; i++) {
    ...
    if (IsModuleIfan()) {
      ...
      for (uint32_t idx = 1; idx <= FirmwareGlobal.devices_present; idx++) {
        ...
        if (ShutterWebButton = IsShutterWebButton(idx)) {
          ...
          WSContentSend_P(HTTP_DEVICE_CONTROL, 100 / cols, idx,
            ...
            (set_button) ? "" : (FirmwareGlobal.devices_present > 1) ? stemp : "");
#ifdef USE_SHUTTER
        }
#endif  // USE_SHUTTER
...
bool HandleRootStatusRefresh(void)
{
...
#ifdef USE_TUYA_MCU
    if (IsModuleTuya()) {
      if (device <= FirmwareGlobal.devices_present) {
        ExecuteCommandPower(device, POWER_TOGGLE, SRC_IGNORE);
      } else {
        if (AsModuleTuyaMS() && device == FirmwareGlobal.devices_present + 1) {
  ...
  uint32_t pwm_channels = (FirmwareGlobal.light_type & 7) > LST_MAX ? LST_MAX : (FirmwareGlobal.light_type & 7);
...
#endif  // USE_LIGHT
#ifdef USE_SHUTTER
  for (uint32_t j = 1; j <= FirmwareGlobal.shutters_present; j++) {
...
#ifdef USE_SONOFF_IFAN
    if (IsModuleIfan()) {
      WSContentSend_P(HTTP_DEVICE_STATE, 36, (bitRead(FirmwareGlobal.power, 0)) ? PSTR("bold") : PSTR("normal"), 54, GetStateText(bitRead(FirmwareGlobal.power, 0)));
      ...
      for (uint32_t idx = 1; idx <= FirmwareGlobal.devices_present; idx++) {
        snprintf_P(svalue, sizeof(svalue), PSTR("%d"), bitRead(FirmwareGlobal.power, idx -1));
        WSContentSend_P(HTTP_DEVICE_STATE, 100 / cols, (bitRead(FirmwareGlobal.power, idx -1)) ? PSTR("bold") : PSTR("normal"), fontsize,
          (cols < 5) ? GetStateText(bitRead(FirmwareGlobal.power, idx -1)) : svalue);
        if (0 == idx % cols) { WSContentSend_P(PSTR("</tr><tr>")); }
      }
#ifdef USE_SONOFF_IFAN
    }
...
void HandleModuleConfiguration(void) {
  ...
  for (uint32_t i = 0; i < nitems(template_gp.io); i++) {
    if (ValidGPIO(i, template_gp.io[i])) {
      WSContentSend_P(PSTR("sk(%d,%d);"), FirmwareGlobal.my_module.io[i], i);  // g0 - g17
    }
  }
  ...
  for (uint32_t i = 0; i < nitems(template_gp.io); i++) {
    if (ValidGPIO(i, template_gp.io[i])) {
      ...
      WSContentSend_P(PSTR("<tr><td style='width:116px'>%s <b>" D_GPIO "%d</b></td><td style='width:146px'><select id='g%d' onchange='ot(%d,this.value)'></select></td>"),
        (WEMOS==FirmwareGlobal.module_type)?stemp:"", i, i, i);
      WSContentSend_P(PSTR("<td style='width:54px'><select id='h%d'></select></td></tr>"), i);
    }
  }
  WSContentSend_P(PSTR("</table>"));
...
void HandleWifiConfiguration(void) {
  ...
  if (Webserver->hasArg(F("save")) && HTTP_MANAGER_RESET_ONLY != Web.state) {
    if ( WifiIsInManagerMode() ) {
      ...
      // As Firmware is in this case in AP mode, a STA connection can be established too at the same time

      if (WIFI_NOT_TESTING == Wifi.wifiTest) {
        if (MAX_WIFI_OPTION == Wifi.old_wificonfig) { Wifi.old_wificonfig = Settings->sta_config; }
        FirmwareGlobal.wifi_state_flag = Settings->sta_config = WIFI_MANAGER;
        Wifi.save_data_counter = FirmwareGlobal.save_data_counter;
      }

      Wifi.wifi_test_counter = 9;   // seconds to test user's proposed AP
      Wifi.wifiTest = WIFI_TESTING;
      FirmwareGlobal.save_data_counter = 0;               // Stop auto saving data - Updating Settings
      Settings->save_data = 0;
      FirmwareGlobal.sleep = 0;                           // Disable sleep
      FirmwareGlobal.restart_flag = 0;                    // No restart
      FirmwareGlobal.ota_state_flag = 0;                  // No OTA
//      FirmwareGlobal.blinks = 0;                          // Disable blinks initiated by WifiManager
      ...
      AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_WIFI D_CONNECTING_TO_AP " %s " D_AS " %s ..."),
        SettingsText(SET_STASSID1), FirmwareGlobal.hostname);
...
void HandleOtherConfiguration(void) {
  ...
  char stemp[32];
  uint32_t maxfn = (FirmwareGlobal.devices_present > MAX_FRIENDLYNAMES) ? MAX_FRIENDLYNAMES : (!FirmwareGlobal.devices_present) ? 1 : FirmwareGlobal.devices_present;
#ifdef USE_SONOFF_IFAN
...
void HandleInformation(void)
{
  ...
  WSContentSend_P(PSTR(D_PROGRAM_VERSION "    Graduation Project v0.6.0"));
  ...
  uint32_t maxfn = (FirmwareGlobal.devices_present > MAX_FRIENDLYNAMES) ? MAX_FRIENDLYNAMES : FirmwareGlobal.devices_present;
#ifdef USE_SONOFF_IFAN
  if (IsModuleIfan()) { maxfn = 1; }
#endif  // USE_SONOFF_IFAN
  for (uint32_t i = 0; i < maxfn; i++) {
  ...
  if (Settings->flag4.network_wifi) {
    ...
    WSContentSend_P(PSTR("}1" D_HOSTNAME "}2%s%s"), FirmwareGlobal.hostname, (Mdns.begun) ? PSTR(".local") : "");
#ifdef USE_IPV6
  ...
  if (!FirmwareGlobal.global_state.eth_down) {
  ...
  if (Settings->flag.mqtt_enabled) {  // SetOption3 - Enable MQTT
    ...
    WSContentSend_P(PSTR("}1" D_MQTT_CLIENT "}2%s"), FirmwareGlobal.mqtt_client);
    ...
    WSContentSend_P(PSTR("}1" D_MQTT_FULL_TOPIC "}2%s"), GetTopic_P(stopic, CMND, FirmwareGlobal.mqtt_topic, ""));
...
#if defined(USE_ZIGBEE_EZSP) || defined(USE_Firmware_CLIENT) || defined(SHELLY_FW_UPGRADE) || defined(USE_RF_FLASH) || defined(USE_CCLOADER)
...
void HandleUpgradeFirmware(void) {
  ...
  Web.upload_file_type = UPL_Firmware;
}
...
void HandleUploadDone(void) {
  ...
  WSContentStart_P(PSTR(D_INFORMATION));
  if (!Web.upload_error) {
    WSContentSend_P(HTTP_SCRIPT_RELOAD_TIME, (UPL_Firmware == Web.upload_file_type) ? HTTP_OTA_RESTART_RECONNECT_TIME : HTTP_RESTART_RECONNECT_TIME);  // Refesh main web ui after OTA upgrade
  }
  WSContentSendStyle();
  ...
  if (Web.upload_error) {
    ...
    FirmwareGlobal.stop_flash_rotate = Settings->flag.stop_flash_rotate;  // SetOption12 - Switch between dynamic or fixed slot flash save location
    Web.upload_error = 0;
  } else {
    WSContentSend_P(PSTR("%06x'>" D_SUCCESSFUL "</font></b><br>"), WebColor(COL_TEXT_SUCCESS));
    FirmwareGlobal.restart_flag = 2;  // Always restart to re-enable disabled features during update
    WSContentSend_P(HTTP_MSG_RSTRT);
    ShowWebSource(SRC_WEBGUI);
  }
  SettingsBufferFree();
...
void HandleUploadLoop(void) {
  ...
  if (Web.upload_error) {
    if (!upload_error_signalled) {
      if (UPL_Firmware == Web.upload_file_type) { Update.end(); }
      UploadServices(1);
...
#ifdef USE_WEB_FW_UPGRADE
#ifdef USE_RF_FLASH
      else if ((SONOFF_BRIDGE == FirmwareGlobal.module_type) && (':' == upload.buf[0])) {  // Check if this is a RF bridge FW file
        BUploadInit(UPL_EFM8BB1);
      }
#endif  // USE_RF_FLASH
#ifdef USE_Firmware_CLIENT
      else if (FirmwareClient_Available() && (':' == upload.buf[0])) {  // Check if this is a ARDUINO CLIENT hex file
        BUploadInit(UPL_FirmwareCLIENT);
      }
#endif  // USE_Firmware_CLIENT
...
#ifdef ESP8266
      else if ((SONOFF_ZB_BRIDGE == FirmwareGlobal.module_type) && (0xEB == upload.buf[0])) {  // Check if this is a Zigbee bridge FW file
#endif  // ESP8266
#ifdef ESP32
...
#endif  // USE_ZIGBEE_EZSP
#endif  // USE_WEB_FW_UPGRADE
      else if (UPL_Firmware == Web.upload_file_type) {
...
#endif  // USE_RF_FLASH
#ifdef USE_Firmware_CLIENT
      if (UPL_FirmwareCLIENT == Web.upload_file_type) {
        error = FirmwareClient_Flash(data, BUpload.spi_hex_size);
      }
#endif  // USE_Firmware_CLIENT
#ifdef SHELLY_FW_UPGRADE
...
void HandleHttpCommand(void)
{
  ...
  if (!WebAuthenticate()) {
    ...
    if (!(!strcmp(tmp1, WEB_USERNAME) && !strcmp(tmp2, SettingsText(SET_WEBPWD)))) {
      ...
      // https://github.com/arendst/Firmware/discussions/15420
  ...
  if (svalue.length() && (svalue.length() < MQTT_MAX_PACKET_SIZE)) {
    uint32_t curridx = FirmwareGlobal.log_buffer_pointer;
    FirmwareGlobal.templog_level = LOG_LEVEL_INFO;
    ...
    char* line;
    size_t len;
    while (GetLog(FirmwareGlobal.templog_level, &index, &line, &len)) {
    ...
    WSContentSend_P(PSTR("}"));
    FirmwareGlobal.templog_level = 0;
  } else {
...
void HandleConsoleRefresh(void)
{
  ...
  WSContentBegin(200, CT_PLAIN);
  WSContentSend_P(PSTR("%d}1%d}1"), FirmwareGlobal.log_buffer_pointer, Web.reset_web_log_flag);
...
{
  // [Firmware] POWER1 ON                                               --> Sends http://Firmware/cm?cmnd=POWER1 ON
  // [192.168.178.86:80,admin:joker] POWER1 ON                        --> Sends http://hostname:80/cm?user=admin&password=joker&cmnd=POWER1 ON
  // [Firmware] /any/link/starting/with/a/slash.php?log=123             --> Sends http://Firmware/any/link/starting/with/a/slash.php?log=123
  // [Firmware,admin:joker] /any/link/starting/with/a/slash.php?log=123 --> Sends http://Firmware/any/link/starting/with/a/slash.php?log=123
...
void CmndWebGetConfig(void) {
  // WebGetConfig http://myserver:8000/Firmware/conf/%id%.dmp where %id% is expanded to device mac address
  // WebGetConfig http://myserver:8000/Firmware/conf/Config_demo_9.5.0.8.dmp
  if (XdrvMailbox.data_len > 0) {
...
#ifndef FIRMWARE_MINIMAL      // if Firmware-minimal, read only and don't parse JSON
...
//          FirmwareGlobal.blinks = 255;                    // Signal wifi connection with blinks
          if (MAX_WIFI_OPTION != Wifi.old_wificonfig) {
            FirmwareGlobal.wifi_state_flag = Settings->sta_config = Wifi.old_wificonfig;
          }
          FirmwareGlobal.save_data_counter = Wifi.save_data_counter;
          Settings->save_data = Wifi.save_data_counter;
          SettingsSaveAll();

          if ( Wifi.wifi_Test_Restart ) { FirmwareGlobal.restart_flag = 2; }
          ...
          switch (WiFi.status()) {
            ...
            default:  // WL_IDLE_STATUS and WL_DISCONNECTED - SSId in range but no answer from the router
              ...
              // If this error occurs twice, Firmware will connect directly to the router without testing credentials.
              ...
              //   Firmware will restart and try to connect in STA mode only (11b/g/n).
              //
              //   If it fails again, depending on the WIFICONFIG settings, the user will need to wait or will need to
              //   push 6 times the button to enable Firmware AP mode again.
              if (Wifi.wifi_test_AP_TIMEOUT) {
                Wifi.wifiTest = WIFI_TEST_FINISHED;
