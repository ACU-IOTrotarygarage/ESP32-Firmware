/*
  xdrv_09_timers.ino - timer support for Firmware
...
// Warning this formula works only from 2000 to 2099, after 2100 we get 1 day off per century. If ever Firmware survives until then.
...
void TimerEverySecond(void)
{
  if (RtcTime.valid) {
    ...
    if (Settings->flag3.timers_enable &&                            // CMND_TIMERS
        (FirmwareGlobal.uptime > 60) && (RtcTime.minute != timer_last_minute)) {  // Execute from one minute after restart every minute only once
...
#endif  // USE_RULES
                if (FirmwareGlobal.devices_present) { ExecuteCommandPower(xtimer.device +1, xtimer.power, SRC_TIMER); }
            }
          }
        }
      }
        ...
        if (FirmwareGlobal.devices_present) {
          ...
          if (!root) {
            ...
            if (val) {
              ...
              Settings->timer[index].device = (device < FirmwareGlobal.devices_present) ? device : 0;
            }
            val = root[PSTR(D_JSON_TIMER_ACTION)];
            if (val) {
              uint8_t action = val.getUInt() & 0x03;
              Settings->timer[index].power = (FirmwareGlobal.devices_present) ? action : 3;  // If no devices than only allow rules
            }
...
#endif
    "if(%d>0){"                                                   // FirmwareGlobal.devices_present
...
const char HTTP_TIMER_SCRIPT5[] PROGMEM =
  "function it(){"                                                // Initialize elements and select first tab
    ...
    "if(%d>0){"                                                   // Create Output and Action drop down boxes (FirmwareGlobal.devices_present)
...
const char HTTP_TIMER_SCRIPT6[] PROGMEM =
    ...
    "o=qs('#d1');for(i=0;i<%d;i++){ce(i+1,o);}"                   // Create outputs ((FirmwareGlobal.devices_present > 16) ? 16 : FirmwareGlobal.devices_present)
    "var a='" D_DAY3LIST "';"
...
#endif  // USE_SUNRISE
  WSContentSend_P(HTTP_TIMER_SCRIPT3, FirmwareGlobal.devices_present);
  WSContentSend_P(HTTP_TIMER_SCRIPT4, WebColor(COL_TIMER_TAB_BACKGROUND), WebColor(COL_TIMER_TAB_TEXT), WebColor(COL_FORM), WebColor(COL_TEXT), FirmwareGlobal.devices_present);
  WSContentSend_P(HTTP_TIMER_SCRIPT5, MAX_TIMERS, FirmwareGlobal.devices_present);
  WSContentSend_P(HTTP_TIMER_SCRIPT6, (FirmwareGlobal.devices_present > 16) ? 16 : FirmwareGlobal.devices_present);  // Power field is 4-bit allowing 0 to 15 devices
...
#else
      if (FirmwareGlobal.devices_present) { WSContentSend_P(HTTP_BTN_MENU_TIMER); }
#endif  // USE_RULES
      break;
