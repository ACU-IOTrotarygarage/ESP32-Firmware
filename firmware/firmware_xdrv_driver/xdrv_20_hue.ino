/*
  xdrv_20_hue.ino - Philips Hue support for Firmware

  Copyright (C) 2021  Heiko Krupp and Theo Arends and Mahmoud Shams El-Din
...
// Use the tool at https://Firmware.hadinger.fr/util and choose "Compress Strings template with Unishox"
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// ++++++++++++++++++++ DO NOT EDIT BELOW ++++++++++++++++++++
// ++++++++++++++++++++vvvvvvvvvvvvvvvvvvv++++++++++++++++++++
enum {
...
void HueLightStatus1(uint8_t device, String *response)
{
  ...
  snprintf_P(buf, buf_size, PSTR("{\"on\":%s,"), (FirmwareGlobal.power & (1 << (device-1))) ? PSTR("true") : PSTR("false"));
...
void HueLightStatus2(uint8_t device, String *response)
{
  ...
  char* buf = HueLightStatus2Generic(fname, Settings->user_template_name, PSTR("Firmware"), GetHueDeviceId(device).c_str());
  *response += buf;
  free(buf);
}
#endif // USE_LIGHT
...
void CheckHue(String * response, bool &appending) {
  uint8_t maxhue = (FirmwareGlobal.devices_present > MAX_HUE_DEVICES) ? MAX_HUE_DEVICES : FirmwareGlobal.devices_present;
...
void HueLightsCommand(uint8_t device, uint32_t device_id, String &response) {
  ...
  if (Webserver->args()) {
    ...
    if (FirmwareGlobal.light_type && (local_light_subtype >= LST_SINGLE)) {
      if (!Settings->flag3.pwm_multi_channels) {  // SetOption68 - Enable multi-channels PWM instead of Color PWM
        light_state.getHSB(nullptr, &sat, nullptr);
        hue16 = light_state.getHue16();
...
#endif
      if (FirmwareGlobal.light_type && (local_light_subtype > LST_NONE)) {   // not relay
        if (!Settings->flag3.pwm_multi_channels) {  // SetOption68 - Enable multi-channels PWM instead of Color PWM
          if (g_gotct) {
...
{
/*
 * http://Firmware/api/username/lights/1/state?1={"on":true,"hue":56100,"sat":254,"bri":254,"alert":"none","transitiontime":40}
 */
  String response;
  int32_t code = 200;
  uint8_t device = 1;
...
#ifdef USE_SCRIPT_HUE
    if (device > FirmwareGlobal.devices_present) {
      Script_HueStatus(&response, device-FirmwareGlobal.devices_present - 1);
      goto exit;
    }
#endif
...
{
/*
 * http://Firmware/api/username/groups?1={"name":"Woonkamer","lights":[],"type":"Room","class":"Living room"})
 */
  String response(F("{}"));
  uint8_t maxhue = (FirmwareGlobal.devices_present > MAX_HUE_DEVICES) ? MAX_HUE_DEVICES : FirmwareGlobal.devices_present;
...
void HandleHueApi(String *path)
{
   ...
   *
   * Hue URL
   * http://Firmware/api/username/lights/1/state with post data {"on":true,"hue":56100,"sat":254,"bri":254,"alert":"none","transitiontime":40}
   * is converted by webserver to
   * http://Firmware/api/username/lights/1/state with arg plain={"on":true,"hue":56100,"sat":254,"bri":254,"alert":"none","transitiontime":40}
   */
...
#else
  if (FirmwareGlobal.devices_present && (EMUL_HUE == Settings->flag2.emulation)) {
#endif
    switch (function) {
