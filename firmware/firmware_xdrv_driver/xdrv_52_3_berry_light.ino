...
extern "C" {
  ...
  void push_getlight(bvm *vm, uint32_t light_num) {
    ...
    if (Light.device > 0) {
      ...
      // be_map_insert_int(vm, "_devices_present", FirmwareGlobal.devices_present);
      ...
      if (!Light.pwm_multi_channels) {
        ...
        if (data_present) {
          ...
          be_map_insert_bool(vm, "power", bitRead(FirmwareGlobal.power, light_num + Light.device - 1));
  ...
  int32_t l_setlight(bvm *vm) {
    ...
    if (top >= 1 && be_isinstance(vm, 1) && (top != 2 || be_isint(vm, 2))) {
      ...
      // read all arguments first and clear stack when calling Firmware APIs
      ...
      // power
      if (has_power) {
        bool current_power = bitRead(FirmwareGlobal.power, idx + Light.device - 1);
