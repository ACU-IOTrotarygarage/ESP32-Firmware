/*
  xdrv_21_wemo_multi.ino - multiple wemo support for Firmware

  Copyright (C) 2021  Magic73, Heiko Krupp and Theo Arends and Mahmoud Shams El-Din
...
private:
  ...
  void HandleUpnpEvent() {
    ...
    snprintf_P(event, sizeof(event), setup_xml.c_str(), state, bitRead(FirmwareGlobal.power, _deviceId - 1), state);
...
#ifdef USE_EMULATION_WEMO_DEBUG
        AddLog(LOG_LEVEL_DEBUG, PSTR("WMO: Adding handlers for %d devices"), FirmwareGlobal.devices_present);
#endif
