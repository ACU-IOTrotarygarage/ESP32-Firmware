/*
  xdrv_85_esp32_ble_eq3_trv.ino - EQ3 radiator valve sense and control via BLE_ESP32 support for Firmware
...
trv <mac> settime - set time to Firmware time (untested)
...
char *topicPrefix(int prefix, const uint8_t *addr, int useAlias){
  ...
  if (!EQ3TopicStyle){
    GetTopic_P(stopic, prefix, FirmwareGlobal.mqtt_topic, PSTR(""));
...
int EQ3SendResult(char *requested, const char *result){
  ...
  static char stopic[TOPSZ];
  GetTopic_P(stopic, STAT, FirmwareGlobal.mqtt_topic, PSTR(""));
  strcat(stopic, PSTR("EQ3/"));
...
void EQ3DiscoveryOneEQ3(){
  ...
  if(Settings->flag.hass_discovery){
    ...
    /*
    {"availability":[],"device":{"identifiers":["FirmwareBLEa4c1387fc1e1"],"manufacturer":"simon","model":"someBLEsensor","name":"TASBLEa4c1387fc1e1","sw_version":"0.0.0"},"dev_cla":"temperature","json_attr_t":"stat/Firmware_esp32/SENSOR","name":"TASLYWSD037fc1e1Temp","state_topic":"tele/Firmware_esp32/SENSOR","uniq_id":"Firmwarea4c1387fc1e1temp","unit_of_meas":"°C","val_tpl":"{{ value_json.LYWSD037fc1e1.Temperature }}"}
  ...
  //AddLog_P(LOG_LEVEL_DEBUG,PSTR("M32: %s: show some %d %s"),D_CMND_MI32, MI32.mqttCurrentSlot, FirmwareGlobal.mqtt_data);
