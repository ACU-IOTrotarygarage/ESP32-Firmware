/*
  xdrv_63_modbus_bridge.ino - modbus bridge support for Firmware

  Copyright (C) 2021  Theo Arends and Mahmoud Shams El-Din and Jeroenst
...
/*********************************************************************************************\
 * Modbus Bridge using Modbus library (FirmwareModbus)
 *
...
FirmwareModbus *modbusBridgeModbus = nullptr;
...
/********************************************************************************************/
//
// Handles data received from Firmware modbus wrapper and send this to (TCP or) MQTT client
//
...
/********************************************************************************************/
//
// Inits the Firmware modbus driver, sets serialport and if TCP enabled allocates a TCP buffer
//
void ModbusBridgeInit(void)
{
  if (PinUsed(GPIO_MBR_RX) && PinUsed(GPIO_MBR_TX))
  {
    modbusBridgeModbus = new FirmwareModbus(Pin(GPIO_MBR_RX), Pin(GPIO_MBR_TX), Pin(GPIO_MBR_TX_ENA));
    ModbusBridgeBegin();
#ifdef USE_MODBUS_BRIDGE_TCP
...
{
  // See FirmwareModusConfig for possible options
