...
void ZigbeeUploadSetSoftwareBootloader() {
  // https://github.com/arendst/Firmware/issues/8583#issuecomment-663967883
  SBuffer buf(4);
    ...
    case ZBU_PROMPT: {                   // *** Wait for prompt and select option upload ebl
      if (millis() > XModem.timeout) {
        ...
        if (ZigbeeUploadBootloaderPrompt()) {
          ...
          if (FirmwareGlobal.sleep > 0) {
            FirmwareGlobal.sleep = 1;     // Speed up loop used for xmodem upload
          }
...
//        AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("XMD: Rcvd2 0x%02X"), xmodem_sync);
    ...
    case ZBU_DONE: {                     // *** Clean up and restart to disable bootloader and use new firmware
      ...
      if (1 == FirmwareGlobal.sleep) {
        FirmwareGlobal.sleep = Settings->sleep;         // Restore loop sleep
      }
//      FirmwareGlobal.restart_flag = 2;    // Restart to disable bootloader and use new firmware
