/*
  xdrv_127_debug.ino - debug support for Firmware
...
00:00:08 MQTT: tele/Firmware/INFO3 = {"Started":"Fatal exception:28 flag:2 (EXCEPTION) epc1:0x4000bf64 epc2:0x00000000 epc3:0x00000000 excvaddr:0x00000007 depc:0x00000000"}
*/
  if (1 == type) {
    char svalue[10];
...
void CpuLoadLoop(void)
{
  ...
  if (CPU_load_check && CPU_last_millis) {
    ...
    if ((CPU_last_millis + (CPU_load_check *1000)) <= CPU_last_loop_time) {
      ...
      int CPU_load = 100 - ( (CPU_loops*(1 + 30*FirmwareGlobal.sleep)) / (CPU_load_check *800) );
...
#else
      int CPU_load = 100 - ( (CPU_loops*(1 + 30*FirmwareGlobal.sleep)) / (CPU_load_check *400) );
      CPU_loops = CPU_loops / CPU_load_check;
...
void DebugRtcDump(char* parms)
{
  ...
  // 100 - 2FF: User
  //  280 - 283: Firmware RtcReboot   (Offset 100 (x 4bytes) - sizeof(RTCRBT) (x 4bytes))
  //  290 - 2EB: Firmware RtcSettings (Offset 100 (x 4bytes))
...
{
#ifdef ESP8266
  if (FirmwareGlobal.i2c_enabled && (XdrvMailbox.payload > 0)) {
...
//      FirmwareGlobal.restart_flag = 2;  // Activate some SetOptions
    }
