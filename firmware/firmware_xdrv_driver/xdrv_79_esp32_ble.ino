/*
  xdrv_79_esp32_ble.ino - BLE via ESP32 support for Firmware

  Copyright (C) 2020  Christian Baars and Theo Arends and Mahmoud Shams El-Din and Simon Hailes
...
12:45:12 MQT: tele/Firmware_esp32/BLE = {"BLEOperation":{"opid":"11","stat":"7","state":"DONENOTIFIED","MAC":"4C65A8DAF43A","svc":"00001530-1212-efde-1523-785feabcd123","char":"00001531-1212-efde-1523-785feabcd123","notifychar":"00001531-1212-efde-1523-785feabcd123","write":"00","notify":"100003"}}
...
const char *BLERestartFirmwareReason = BLE_RESTART_TEAMOTA_REASON_UNKNOWN;
...
void initSafeLog(){
  FirmwareMainTask = xTaskGetCurrentTaskHandle();
...
int SafeAddLog_P(uint32_t loglevel, PGM_P formatP, ...) {
  ...
  if ((loglevel > Settings->weblog_level) &&
      (loglevel > FirmwareGlobal.seriallog_level) &&
      (loglevel > Settings->mqttlog_level) &&
      (loglevel > FirmwareGlobal.syslog_level)){
    return added;
  }
...
//  if (FirmwareGlobal.ota_state_flag) return;

  TasAutoMutex localmutex(&BLEOperationsRecursiveMutex, "BLEPostAdd");
  if (BLEAdvertismentDetailsJsonSet){

//    strncpy(FirmwareGlobal.mqtt_data, BLEAdvertismentDetailsJson, sizeof(FirmwareGlobal.mqtt_data));
//    FirmwareGlobal.mqtt_data[sizeof(FirmwareGlobal.mqtt_data)-1] = 0;
...
static void BLEInit(void) {
  ...
  if (FirmwareGlobal.global_state.wifi_down && FirmwareGlobal.global_state.eth_down) { return; }

  if (!FirmwareGlobal.global_state.wifi_down) {
    FirmwareGlobal.wifi_stay_asleep = true;
...
static void BLETaskRunTaskDoneOperation(BLE_ESP32::generic_sensor_t** op, NimBLEClient **ppClient){
  if ((*ppClient)->isConnected()){
    ...
    do {
      ...
      if (waits == 60){
        AddLog(LOG_LEVEL_ERROR,PSTR("BLE: >60s waiting -> BLE Failed, restart Firmware %d"), waits);
        BLEStop = 1;
        BLEStopAt = esp_timer_get_time();

        BLERestartFirmware = 10;
        BLERestartFirmwareReason = BLE_RESTART_TEAMOTA_REASON_BLE_DISCONNECT_FAIL;
        break;
      }
...
static void BLEOperationTask(void *pvParameters){
  ...
  for(;;){
    ...
    if (BLERestartNimBLE){
      BLERestartNimBLE = 0;
      BLERestartFirmware = 10;
      BLERestartFirmwareReason = BLE_RESTART_TEAMOTA_REASON_RESTARTING_BLE_TIMEOUT;
      AddLog(LOG_LEVEL_ERROR,PSTR("BLE: BLETask: Restart NimBLE - restart Firmware in 10 if not complt"));
      BLE_ESP32::BLETaskStopStartNimBLE(&pClient);
      BLERestartFirmwareReason = BLE_RESTART_TEAMOTA_REASON_UNKNOWN;
      BLERestartFirmware = 0;
      BLEResets ++;
    }
  }
...
/***********************************************************************\
 * Regular Firmware called functions
 *
\***********************************************************************/
...
static void BLEEverySecond(bool restart){
  ...
  if (BLERestartFirmware){
    BLERestartFirmware--;
    // 2 seconds to go, post to BLE topic on MQTT our reason
    if (BLERestartFirmware == 2){
      if (!BLERestartFirmwareReason) BLERestartFirmwareReason = BLE_RESTART_TEAMOTA_REASON_UNKNOWN;
      Response_P(PSTR("{\"reboot\":\"%s\"}"), BLERestartFirmwareReason);
      MqttPublishPrefixTopicRulesProcess_P(TELE, PSTR("BLE"), Settings->flag.mqtt_sensor_retain);
      AddLog(LOG_LEVEL_ERROR,PSTR("BLE: Failure! Restarting Firmware in %d seconds because %s"), BLERestartFirmware, BLERestartFirmwareReason);
    }

    if (!BLERestartFirmware){
      AddLog(LOG_LEVEL_ERROR,PSTR("BLE: Failure! Restarting Firmware because %s"), BLERestartFirmwareReason);
      // just a normal restart
      FirmwareGlobal.restart_flag = 1;
    }
  }
...
// details look like:
// MQT: tele/Firmware_esp32/BLE = {"details":{"mac":"001A22092C9A","p":"0C0943432D52542D4D2D424C450CFF0000000000000000000000"}}
...
#else
  memset(FirmwareGlobal.mqtt_data, 0, sizeof(FirmwareGlobal.mqtt_data));
  int timelen = ResponseTime_P(PSTR(""));
  char *dest = FirmwareGlobal.mqtt_data + timelen;
  int maxlen = ResponseSize() -20 -timelen;

//  if (!FirmwareGlobal.ota_state_flag){
  do {
    remains = getSeenDevicesToJson(dest, maxlen);
    // no retain - this is present devices, not historic
    MqttPublishPrefixTopicRulesProcess_P((1== type) ? TELE : STAT, PSTR("BLE"));
...
static void mainThreadBLETimeouts() {
  ...
  if (BLEStop == 1){
    if (BLEStopAt + 30L*1000L*1000L < now){ // if asked to stop > 30s ago...
      AddLog(LOG_LEVEL_ERROR,PSTR("BLE: Stop Timeout - restart Firmware"));
      BLERestartFirmware = 2;
      BLEStopAt = now;
    }
  ...
  // restart Firmware completely. (belt and braces)
  uint64_t bleLoopTimeout = ((uint64_t)BLEMaxTaskLoopTime)*1000L*1000L;
  if (BLELastLoopTime + bleLoopTimeout < now){
    BLELastLoopTime = now; // initialise the time of the last advertisment
    BLERestartFirmware = 10;
    AddLog(LOG_LEVEL_DEBUG,PSTR("BLE: BLETask stall > 120s, restart Firmware in 10s"));
    BLERestartFirmwareReason = BLE_RESTART_TEAMOTA_REASON_BLE_LOOP_STALLED;
  }
}
