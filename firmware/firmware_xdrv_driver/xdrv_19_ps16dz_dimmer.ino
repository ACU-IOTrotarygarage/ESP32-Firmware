/*
  xdrv_19_ps16dz.dimmer.ino - PS_16_DZ dimmer support for Firmware

  Copyright (C) 2021  Joel Stein and Theo Arends and Mahmoud Shams El-Din
...
FirmwareSerial *PS16DZSerial = nullptr;
...
void PS16DZSerialSendUpdateCommand(void)
{
  ...
  snprintf_P(tx_buffer, sizeof(tx_buffer), PSTR("AT+UPDATE=\"sequence\":\"%d%03d\",\"switch\":\"%s\",\"bright\":%d"),
    LocalTime(), millis()%1000, FirmwareGlobal.power?"on":"off", light_state_dimmer);
...
//            AddLog(LOG_LEVEL_DEBUG, PSTR("PSZ: Switch %d"), switch_state);
            ...
            is_switch_change = (switch_state != FirmwareGlobal.power);
            if (is_switch_change) {
...
//            AddLog(LOG_LEVEL_DEBUG, PSTR("PSZ: Brightness %d"), Ps16dz.dimmer);
            ...
            if (FirmwareGlobal.power && (Ps16dz.dimmer > 0) && is_brightness_change) {
...
void PS16DZInit(void)
{
  ...
  if (Ps16dz.rx_buffer != nullptr) {
    PS16DZSerial = new FirmwareSerial(Pin(GPIO_RXD), Pin(GPIO_TXD), 2);
...
bool Xdrv19(uint32_t function)
{
  ...
  if (PS_16_DZ == FirmwareGlobal.module_type) {
    switch (function) {
      case FUNC_LOOP:
