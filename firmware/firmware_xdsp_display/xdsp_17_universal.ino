/*
  xdsp_17_universal.ino -  universal display driver support for Firmware
...
int8_t cs;
  ...
  if (FirmwareGlobal.gpio_optiona.udisplay_driver || desc) {
...
#ifdef USE_UFILESYS
    if (ffsp  && !FirmwareGlobal.no_autoexec && !ddesc) {
      File fp;
    ...
    // now replace Firmware vars before passing to driver
