/*
  xnrg_05_pzem_ac.ino - PZEM-014,016 Modbus AC energy sensor support for Firmware
...
void PzemAcEverySecond(void)
{
    ...
    if (error) {
      ...
      if (10 == registers) {
        ...
        if (PzemAc.phase == Energy->phase_count -1) {
          if (FirmwareGlobal.uptime > (PZEM_AC_STABILIZE * ENERGY_MAX_PHASES)) {
            EnergyUpdateTotal();
          }
        }
      }
  ...
  if (0 == PzemAc.send_retry || data_ready) {
    ...
    PzemAc.send_retry--;
    if ((Energy->phase_count > 1) && (0 == PzemAc.send_retry) && (FirmwareGlobal.uptime < (PZEM_AC_STABILIZE * ENERGY_MAX_PHASES))) {
...
{
  PzemAcModbus = new FirmwareModbus(Pin(GPIO_PZEM016_RX), Pin(GPIO_PZEM0XX_TX), Pin(GPIO_NRG_MBS_TX_ENA));
...
void PzemAcDrvInit(void)
{
  if (PinUsed(GPIO_PZEM016_RX) && PinUsed(GPIO_PZEM0XX_TX)) {
    FirmwareGlobal.energy_driver = XNRG_05;
  }
}
...
bool Xnrg05(uint32_t function)
{
  ...
  switch (function) {
    case FUNC_ENERGY_EVERY_SECOND:
      if (FirmwareGlobal.uptime > 4) { PzemAcEverySecond(); }  // Fix start up issue #5875
      break;
    case FUNC_COMMAND:
