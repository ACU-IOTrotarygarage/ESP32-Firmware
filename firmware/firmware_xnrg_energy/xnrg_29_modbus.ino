/*
  xnrg_29_modbus.ino - Generic Modbus energy meter support for Firmware
...
void EnergyModbusLoop(void) {
#ifdef ENERGY_MODBUS_TICKER
  if (NrgMbsParam.mutex || FirmwareGlobal.ota_state_flag) { return; }
#else
  ...
  if (data_ready) {
    ...
    if (error) {
      /* Return codes from FirmwareModbus.h:
      * 0 = No error
      * 1 = Illegal Function,
...
void EnergyModbusSnsInit(void) {
  if (EnergyModbusRegisters()) {
    EnergyModbus = new FirmwareModbus(Pin(GPIO_NRG_MBS_RX), Pin(GPIO_NRG_MBS_TX), Pin(GPIO_NRG_MBS_TX_ENA));
...
void EnergyModbusDrvInit(void) {
  if (PinUsed(GPIO_NRG_MBS_RX) && PinUsed(GPIO_NRG_MBS_TX)) {
    FirmwareGlobal.energy_driver = XNRG_29;
  }
}
...
//    case FUNC_EVERY_200_MSECOND:     // Energy ticker interrupt
    case FUNC_EVERY_250_MSECOND:     // Firmware dispatcher
      EnergyModbusLoop();
      break;
