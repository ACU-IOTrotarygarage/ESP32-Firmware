/*
  xnrg_12_solaxX1.ino - Solax X1 inverter RS485 support for Firmware
...
void solaxX1_SnsInit(void)
{
  ...
  solaxX1Serial = new FirmwareSerial(Pin(GPIO_SOLAXX1_RX), Pin(GPIO_SOLAXX1_TX), 1);
...
void solaxX1_DrvInit(void)
{
  if (PinUsed(GPIO_SOLAXX1_RX) && PinUsed(GPIO_SOLAXX1_TX)) {
    FirmwareGlobal.energy_driver = XNRG_12;
  }
}
...
void solaxX1_Show(bool json)
{
  ...
  if (json) {
    ...
    if (0 == FirmwareGlobal.tele_period && solaxX1.temperature < 100) { DomoticzSensor(DZ_TEMP, solaxX1.temperature); }
#endif // USE_DOMOTICZ
