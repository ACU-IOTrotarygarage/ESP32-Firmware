/*
  xnrg_30_dummy.ino - Dummy energy sensor support for Firmware
...
void NrgDummyEverySecond(void) {
  if (Energy->power_on) {  // Powered on
    for (uint32_t channel = 0; channel < Energy->phase_count; channel++) {
      ...
      if (bitRead(FirmwareGlobal.power, channel)) {        // Emulate power read only if device is powered on
...
void NrgDummyDrvInit(void) {
  if (FirmwareGlobal.gpio_optiona.dummy_energy && FirmwareGlobal.devices_present) {
    Energy->phase_count = (FirmwareGlobal.devices_present < ENERGY_MAX_PHASES) ? FirmwareGlobal.devices_present : ENERGY_MAX_PHASES;
    ...
    FirmwareGlobal.energy_driver = XNRG_30;
  }
}
