/*
  xnrg_03_pzem004t.ino - PZEM004T energy sensor support for Firmware
...
FirmwareSerial *PzemSerial = nullptr;
...
void PzemSend(uint8_t cmd)
{
  ...
  pzem.command = cmd;
  pzem.addr[0] = 192;    // Address 192.168.1.1 for Firmware legacy reason
  pzem.addr[1] = 168;
  pzem.addr[2] = 1;
      ...
      switch (Pzem.read_state) {
        ...
        case 4:  // Total energy as 99999Wh
          ...
          if (Pzem.phase == Energy->phase_count -1) {
            if (FirmwareGlobal.uptime > (PZEM_STABILIZE * ENERGY_MAX_PHASES)) {
              EnergyUpdateTotal();
            }
          }
          break;
...
//      AddLog(LOG_LEVEL_DEBUG, PSTR("PZM: Probing address %d, Max phases %d"), Pzem.phase +1, Energy->phase_count);
    ...
    Pzem.send_retry--;
    if ((Energy->phase_count > 1) && (0 == Pzem.send_retry) && (FirmwareGlobal.uptime < (PZEM_STABILIZE * ENERGY_MAX_PHASES))) {
      ...
      if (FirmwareGlobal.discovery_counter) {
        FirmwareGlobal.discovery_counter += (PZEM_RETRY / 4) + 1;  // Don't send Discovery yet, delay by 5 * 250ms + 1s
      }
    }
  }
}
...
void PzemSnsInit(void)
{
  ...
  PzemSerial = new FirmwareSerial(Pin(GPIO_PZEM004_RX), Pin(GPIO_PZEM0XX_TX), 1);
...
void PzemDrvInit(void)
{
  if (PinUsed(GPIO_PZEM004_RX) && PinUsed(GPIO_PZEM0XX_TX)) {  // Any device with a Pzem004T
    FirmwareGlobal.energy_driver = XNRG_03;
  }
}
