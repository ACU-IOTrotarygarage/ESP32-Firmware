...
{
  BioPduModbus = new FirmwareModbus(Pin(GPIO_BIOPDU_PZEM016_RX), Pin(GPIO_BIOPDU_PZEM0XX_TX));
...
void BioPduDrvInit(void)
{
  ...
  {
    FirmwareGlobal.energy_driver = XNRG_24;
...
bool Xnrg24(uint32_t function)
{
  ...
  case FUNC_ENERGY_EVERY_SECOND:
    if (FirmwareGlobal.uptime > (BIOPDU_STABILIZE * BIOPDU_MAX_PHASES))
    {
      BioPduEverySecond();
    }
    break;
