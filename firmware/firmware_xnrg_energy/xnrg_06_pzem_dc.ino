/*
  xnrg_06_pzem_dc.ino - PZEM-003,017 Modbus DC energy sensor support for Firmware
...
void PzemDcEverySecond(void)
{
    ...
    if (error) {
      ...
      if (8 == registers) {
        ...
        if (PzemDc.channel == Energy->phase_count -1) {
          if (FirmwareGlobal.uptime > (PZEM_DC_STABILIZE * ENERGY_MAX_PHASES)) {
            EnergyUpdateTotal();
          }
        }
      }
  ...
  if (0 == PzemDc.send_retry || data_ready) {
    ...
    PzemDc.send_retry--;
    if ((Energy->phase_count > 1) && (0 == PzemDc.send_retry) && (FirmwareGlobal.uptime < (PZEM_DC_STABILIZE * ENERGY_MAX_PHASES))) {
      ...
      if (FirmwareGlobal.discovery_counter) {
        FirmwareGlobal.discovery_counter += ENERGY_WATCHDOG + 1;  // Don't send Discovery yet, delay by 4s + 1s
      }
    }
  }
}
...
{
  PzemDcModbus = new FirmwareModbus(Pin(GPIO_PZEM017_RX), Pin(GPIO_PZEM0XX_TX), Pin(GPIO_NRG_MBS_TX_ENA));
...
void PzemDcDrvInit(void)
{
  if (PinUsed(GPIO_PZEM017_RX) && PinUsed(GPIO_PZEM0XX_TX)) {
    FirmwareGlobal.energy_driver = XNRG_06;
  }
}
...
bool Xnrg06(uint32_t function)
{
  ...
  switch (function) {
    case FUNC_ENERGY_EVERY_SECOND:
      if (FirmwareGlobal.uptime > 4) { PzemDcEverySecond(); }  // Fix start up issue #5875
      break;
    case FUNC_COMMAND:
