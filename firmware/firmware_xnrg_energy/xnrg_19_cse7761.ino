/*
  xnrg_19_cse7761.ino - CSE7761 energy sensor support for Firmware
 ...
 * See https://github.com/arendst/Firmware/discussions/10793
...
FirmwareSerial *Cse7761Serial = nullptr;
...
//    Cse7761Write(CSE7761_REG_EMUCON | 0x80, 0x1003);
    Cse7761Write(CSE7761_REG_EMUCON | 0x80, 0x1183);  // Firmware enable zero cross detection on both positive and negative signal
...
/*
    ...
    5      WaveEN             Waveform data, instantaneous data output enable signal
                              =1, turn on the waveform data output function (Firmware add frequency)
    ...
    2      ZxEN               Zero-crossing detection, phase angle, voltage frequency measurement enable signal
                              =1, turn on the zero-crossing detection, phase angle, and voltage frequency measurement functions (Firmware add frequency)
...
#else
    Cse7761Write(CSE7761_REG_EMUCON2 | 0x80, 0x0FE5);  // Firmware add Frequency
...
/*
    ...
    1001   Voltage channel zero-crossing signal output (Firmware add zero-cross detection)
...
void Cse7761SnsInit(void) {
  ...
  Cse7761Serial = new FirmwareSerial(Pin(GPIO_CSE7761_RX), Pin(GPIO_CSE7761_TX), 1);
...
void Cse7761DrvInit(void) {
  if (PinUsed(GPIO_CSE7761_RX) && PinUsed(GPIO_CSE7761_TX)) {
    ...
    FirmwareGlobal.energy_driver = XNRG_19;
  }
}
