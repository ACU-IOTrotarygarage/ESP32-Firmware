/*
  xnrg_04_mcp39f501.ino - MCP39F501 energy sensor support for Firmware
...
FirmwareSerial *McpSerial = nullptr;
...
void McpSnsInit(void)
{
  ...
  McpSerial = new FirmwareSerial(Pin(GPIO_MCP39F5_RX), Pin(GPIO_MCP39F5_TX), 1);
  if (McpSerial->begin(MCP_BAUDRATE)) {
    if (McpSerial->hardwareSerial()) {
      ClaimSerial();
      mcp_buffer = FirmwareGlobal.serial_in_buffer;  // Use idle serial buffer to save RAM
    } else {
...
void McpDrvInit(void)
{
  if (PinUsed(GPIO_MCP39F5_RX) && PinUsed(GPIO_MCP39F5_TX)) {
    ...
    FirmwareGlobal.energy_driver = XNRG_04;
  }
}
