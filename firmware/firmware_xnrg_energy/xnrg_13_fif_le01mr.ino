/*
  xnrg_13_fif_le01mr.ino - F&F LE-01MR energy meter with Modbus interface - support for Firmware
 ...
 *                        counters values directly from the meter (without Firmware calculation,
 *                        energy used calculated by Firmware is in Total/Today fields ).
...
{
  FifLEModbus = new FirmwareModbus(Pin(GPIO_LE01MR_RX), Pin(GPIO_LE01MR_TX), Pin(GPIO_NRG_MBS_TX_ENA));
...
void FifLEDrvInit(void)
{
  if (PinUsed(GPIO_LE01MR_RX) && PinUsed(GPIO_LE01MR_TX)) {
    FirmwareGlobal.energy_driver = XNRG_13;
  }
}
