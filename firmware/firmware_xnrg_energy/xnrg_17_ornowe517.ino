/*
  xnrg_17_ornowe512.ino - Orno WE517-Modbus energy meter support for Firmware

  Copyright (C) 2021  Maxime Vincent - based on the work of Gennaro Tortone and Theo Arends and Mahmoud Shams El-Din
 ...
 * (as apposed to software energy totals kept in Firmware flash memory)
\*********************************************************************************************/
...
void We517SnsInit(void) {
  We517Modbus = new FirmwareModbus(Pin(GPIO_WE517_RX), Pin(GPIO_WE517_TX), Pin(GPIO_NRG_MBS_TX_ENA));
...
void We517DrvInit(void)
{
  if (PinUsed(GPIO_WE517_RX) && PinUsed(GPIO_WE517_TX)) {
    FirmwareGlobal.energy_driver = XNRG_17;
  }
}
