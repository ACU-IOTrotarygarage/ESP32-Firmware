...
The device comes pre-flashed with EZSP 6.9.1_2 but there are unresolved issues with Firmware and we currently only support 6.7.x. You need to downgrade the flash firmware to 6.7.9. Simply use the **Firmware Upggrade** feature of Firmware, it will automatically detect that the firmware is for the EFR32 and not for Firmware.
