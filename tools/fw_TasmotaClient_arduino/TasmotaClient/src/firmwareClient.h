/*
  FirmwareClient.h - Library for microcontrollers enslaved by Firmware
...
#ifndef __FirmwareCLIENT_H__
#define __FirmwareCLIENT_H__

#include <Arduino.h>

/*************************************************\
 * FirmwareClient Configuration Defaults
\*************************************************/

#define Firmware_CLIENT_LIB_VERSION      20191129

/*************************************************\
 * FirmwareClient Command definitions
\*************************************************/
...
/*************************************************\
 * FirmwareClient Parameter defintions
\*************************************************/
...
class FirmwareClient {
    public:
     ...
     // Constructor
     FirmwareClient(HardwareSerial *device = nullptr);
     // Sends configured features back to Firmware so it knows which callbacks are supported
     void sendFeatures(void);
     // Send JSON back to Firmware device
     ...
     // Configure a callback for FUNC_COMMAND_SEND (ClientSend on Firmware device)
     void attach_FUNC_COMMAND_SEND(callbackFunc1 func = nullptr);
     // Send command to Firmware device using the prescribed protocol
     void SendCommand(uint8_t cmnd, uint8_t param);
     // Send telemetry data back to the Firmware device
     ...
     // Used internally to decode and process incoming commands from the Firmware device
