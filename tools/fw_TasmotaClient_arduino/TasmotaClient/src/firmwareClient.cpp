/*
  FirmwareClient.cpp - Library for microcontrollers enclientd by Firmware
...
#include <Arduino.h>
#include <FirmwareClient.h>

/*************************************************\
 * FirmwareClient shared structures
\*************************************************/
...
FirmwareClient::FirmwareClient(HardwareSerial *device)
{
  serial = device;
  Settings.features_version = Firmware_CLIENT_LIB_VERSION;
...
void FirmwareClient::sendFeatures(void)
{
...
void FirmwareClient::sendJSON(char *json)
{
  serial->write(char(PARAM_DATA_START));
...
void FirmwareClient::attach_FUNC_JSON(callbackFunc func)
{
  Settings.features.func_json_append = 1;
  FUNC_JSON = func;
}

void FirmwareClient::attach_FUNC_EVERY_SECOND(callbackFunc func)
{
  Settings.features.func_every_second = 1;
  FUNC_EVERY_SECOND = func;
}

void FirmwareClient::attach_FUNC_EVERY_100_MSECOND(callbackFunc func)
{
  Settings.features.func_every_100_msecond = 1;
  FUNC_EVERY_100_MSECOND = func;
}

void FirmwareClient::attach_FUNC_COMMAND_SEND(callbackFunc1 func)
{
  Settings.features.func_client_send = 1;
  FUNC_SEND = func;
}

uint8_t FirmwareClient::waitforbytes(uint16_t num, uint16_t timeout)
{
  uint16_t timer = 0;
  while (timer < timeout) {
...
void FirmwareClient::ProcessSend(uint8_t sz)
{
...
void FirmwareClient::ProcessCommand(void)
{
...
void FirmwareClient::SendCommand(uint8_t cmnd, uint8_t param)
{
...
void FirmwareClient::SendTele(char *data)
{
...
void FirmwareClient::ExecuteCommand(char *cmnd)
{
