/*
  ClientSendCommand.ino - Example for FirmwareClient to respond to ClientSend ON and
                         ClientSend OFF commands via console or telemetry.
                         In this example the ON and OFF is case sensitive so needs
                         to be sent in capital letters from the Firmware device.
                         Upon receiving ON/OFF the client will turn the LED on/off
                         respectively.
                         
                         A second function callback is also attached which will
                         respond with a JSON message on Firmware Teleperiod when
                         requested from the Firmware device.
...
FirmwareClient client(&Serial);

/*******************************************************************\
 * Normal setup() function for Arduino to configure the serial port
 * speed (which should match what was configured in Firmware) and
 * attach any callback functions associated with specific requests
 * or commands that are sent by Firmware.
\*******************************************************************/
...
void setup() {
  ...
  Serial.begin(57600);
  // Attach the function which will respond to the JSON request from Firmware
  client.attach_FUNC_JSON(user_FUNC_JSON);
  // Attach the function which will be called when the Firmware device sends data using ClientSend command
  client.attach_FUNC_COMMAND_SEND(user_FUNC_RECEIVE);
}

/*******************************************************************\
 * Function which will be called when this client device receives a
 * ClientSend command from the Firmware device.
\*******************************************************************/
 ...
 * Firmware device upon receiving a request to do so
\*******************************************************************/
