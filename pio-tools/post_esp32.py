...
# -  0x8000 | ~\Firmware\.pio\build\<env name>\partitions.bin
# -  0xe000 | ~\.platformio\packages\framework-arduinoespressif32\tools\partitions\boot_app0.bin
# - 0x10000 | ~\.platformio/packages/framework-arduinoespressif32/variants/Firmware/\<env name>-safeboot.bin
# - 0xe0000 | ~\Firmware\.pio\build\<env name>/firmware.bin
# - 0x3b0000| ~\Firmware\.pio\build\<env name>/littlefs.bin
...
if "CORE32SOLO1" in extra_flags or "FRAMEWORK_ARDUINO_SOLO1" in build_flags:
    ...
    if github_actions and os.path.exists("./firmware/firmware"):
        shutil.copytree("./firmware/firmware", "/home/runner/.platformio/packages/framework-arduino-solo1/variants/Firmware")
elif "CORE32ITEAD" in extra_flags or "FRAMEWORK_ARDUINO_ITEAD" in build_flags:
    FRAMEWORK_DIR = platform.get_package_dir("framework-arduino-ITEAD")
    if github_actions and os.path.exists("./firmware/firmware"):
        shutil.copytree("./firmware/firmware", "/home/runner/.platformio/packages/framework-arduino-ITEAD/variants/Firmware")
else:
    FRAMEWORK_DIR = platform.get_package_dir("framework-arduinoespressif32")
    if github_actions and os.path.exists("./firmware/firmware"):
        shutil.copytree("./firmware/firmware", "/home/runner/.platformio/packages/framework-arduinoespressif32/variants/Firmware")

variants_dir = join(FRAMEWORK_DIR, "variants", "Firmware")

def esp32_create_chip_string(chip):
    Firmware_platform = env.subst("$BUILD_DIR").split(os.path.sep)[-1]
    Firmware_platform = Firmware_platform.split('-')[0]
    if 'Firmware' + chip[3:] not in Firmware_platform: # quick check for a valid name like 'Firmware' + '32c3'
        print('Unexpected naming conventions in this build environment -> Undefined behavior for further build process!!')
        print("Expected build environment name like 'Firmware32-whatever-you-want'")
    return Firmware_platform
...
def esp32_fetch_safeboot_bin(Firmware_platform):
    safeboot_fw_url = "http://ota.Firmware.com/Firmware32/release/" + Firmware_platform + "-safeboot.bin"
    safeboot_fw_name = join(variants_dir, Firmware_platform + "-safeboot.bin")
...
def esp32_copy_new_safeboot_bin(Firmware_platform,new_local_safeboot_fw):
...
def esp32_create_combined_bin(source, target, env):
    ...
    Firmware_platform = esp32_create_chip_string(chip)

    if "-DUSE_USB_CDC_CONSOLE" in env.BoardConfig().get("build.extra_flags") and "cdc" not in Firmware_platform:
        Firmware_platform += "cdc" 
        print("WARNING: board definition uses CDC configuration, but environment name does not -> changing Firmware safeboot binary to:", Firmware_platform + "-safeboot.bin")
 
    if not os.path.exists(variants_dir):
        os.makedirs(variants_dir)
    if("safeboot" in firmware_name):
        esp32_copy_new_safeboot_bin(Firmware_platform,firmware_name)
    else:
        esp32_fetch_safeboot_bin(Firmware_platform)
...
#    if(fs_offset != -1):
    ...
    if("upload-Firmware.php" not in upload_port) and (fs_offset != -1):
