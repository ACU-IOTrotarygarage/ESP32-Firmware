...
def map_gzip(source, target, env):
    ...
    map_file = Firmwarepiolib.get_final_map_path(env)
...
if not Firmwarepiolib.is_env_set(Firmwarepiolib.DISABLE_MAP_GZ, env):
    env.AddPostAction("$BUILD_DIR/${PROGNAME}.bin", [map_gzip])

if Firmwarepiolib.is_env_set(Firmwarepiolib.ENABLE_ESP32_GZ, env) or env["PIOPLATFORM"] != "espressif32":
    ...
    def bin_gzip(source, target, env):
        ...
        if ORG_FIRMWARE_SIZE > 995326 and env["PIOPLATFORM"] != "espressif32":
            print(
                "\u001b[31;1m!!! Firmware firmware size is too big with {} bytes. Max size is 995326 bytes !!! \u001b[0m".format(
                    ORG_FIRMWARE_SIZE
                )
            )
        else:
    ...
    if not Firmwarepiolib.is_env_set(Firmwarepiolib.DISABLE_BIN_GZ, env):
