...
To set a value using an environment variable, prefix the value with "Firmware_" and
ensure the entire value is UPPER CASE, for example in bash, it would be:

      export Firmware_DISABLE_MAP_GZ=1

To set a value in your .ini file, such as in platformio_override.ini, create a
[Firmware] section, and put the key ensuring it is all lower case, for example:
...
def get_Firmware_override_option(name: str, env):
    """Gets a set override option from a .ini or env variable, None if no match"""
    config = env.GetProjectConfig()
    override = config.get("Firmware", name.lower(), None)
    if override is not None:
        return override
    # Return env if available
    return os.environ.get("Firmware_" + name.upper())
...
def get_override_path(pathtype: str, env) -> pathlib.Path:
    ...
    """
    override = get_Firmware_override_option(pathtype, env)
    if override:
...
def is_env_set(name: str, env):
    ...
    val = get_Firmware_override_option(name, env)
    if val:
        val = val.strip()
        return val == "1"
    return False
