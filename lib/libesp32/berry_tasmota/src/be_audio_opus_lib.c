...
// Firmware Logging
extern void Firmware_log_C(uint32_t loglevel, const char * berry_buf, ...);
...
void *be_audio_opus_decoder_init_ntv(int freq, int channels) {
  ...
  Firmware_log_C(LOG_LEVEL_DEBUG, "AUD: allocated %i bytes for Opus decoder", opus_size);
...
int32_t be_audio_opus_decoder_decode(struct bvm *vm) {
  ...
  // Firmware_log_C(LOG_LEVEL_DEBUG, "AUD: frame contains %i samples", samples);
