...
autoconf_module.init = def (m)
  ...
  class Autoconf
    ...
    def init()
      ...
      if entry == nil
        Firmware.log("CFG: No '*.autoconf' file found", 3)
        return nil
      end
    ...
    def load_templates()
      ...
      import json
      try 
        var url = string.format("https://raw.githubusercontent.com/Firmware/autoconf/main/%s_manifest.json", Firmware.arch())
        Firmware.log(string.format("CFG: loading '%s'", url), 3)
        # load the template
        var cl = webclient()
        cl.begin(url)
        var r = cl.GET()
        if r != 200
          Firmware.log(string.format("CFG: return_code=%i", r), 2)
          return nil
        end
        var s = cl.get_string()
        cl.close()
        # convert to json
        var j = json.load(s)
        Firmware.log(string.format("CFG: loaded '%s'", str(j)), 3)
        ...
        Firmware.log(string.format("CFG: exception '%s' - '%s'", e, m), 2)
        return nil
      end
    end
    ...
    def page_autoconf_ctl()
      ...
      try
        if webserver.has_arg("reapply")
          Firmware.log("CFG: removing first time marker", 2);
          ...
          Firmware.log("CFG: removing autoconf files", 2);
          ...
          if arch_name != "reset"
            var url = string.format("https://raw.githubusercontent.com/Firmware/autoconf/main/%s/%s.autoconf", Firmware.arch(), arch_name)
            Firmware.log(string.format("CFG: downloading '%s'", url), 2);
    ...
    def preinit()
      ...
      if path.exists(fname)
        Firmware.log("CFG: loading "+fname, 3)
        load(fname)
        Firmware.log("CFG: loaded  "+fname, 3)
      end
    end
    ...
    def run_bat(fname)    # read a '*.bat' file and run each command
      ...
      try
        ...
        while true
          ...
          if size(line) > 0
            Firmware.cmd(line)             # run the command
          end
        end
    ...
    def autoexec()
      ...
      if self.is_first_time() && path.exists(fname)
        ...
        self.run_bat(fname)
        Firmware.log("CFG: 'init.bat' done, restarting", 2)
      ...
      if gpio.pin_used(gpio.OPTION_A, 2) && path.exists(fname)
        if path.exists("display.ini")
          Firmware.log("CFG: skipping 'display.ini' because already present in file-system", 2)
        else
          import display
          var f = open(fname,"r")
          var desc = f.read()
      ...
      if path.exists(fname)
        Firmware.log("CFG: running "+fname, 3)
        self.run_bat(fname)
        Firmware.log("CFG: ran  "+fname, 3)
      end
