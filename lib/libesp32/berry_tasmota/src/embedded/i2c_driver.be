#-------------------------------------------------------------
 - IMPORTANT
 - THIS CLASS IS ALREADY BAKED IN Firmware
 -
...
class I2C_Driver
  ...
  #- Init and look for device
   ...
   -   i2c_index : Firmware I2C index, see `I2CDEVICES.md` (int)
   --#
  def init(name_or_detect, addr, i2c_index)
    #- check if the i2c index is disabled by Firmware configuration -#
    if i2c_index != nil && !Firmware.i2c_enabled(i2c_index) return end

    self.addr = addr                            #- address for AXP192 -#
    self.wire = Firmware.wire_scan(self.addr)    #- get the right I2C bus -#
