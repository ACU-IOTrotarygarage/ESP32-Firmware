...
class Animate_engine
  ...
  # cur_time:int (opt) current timestamp in ms, defaults to `Firmware.millis()`
  # val:int (opt) starting value, default to `nil`
  def run(cur_time, val)
    if cur_time == nil   cur_time = Firmware.millis() end
  ...
  # runs autonomously in the Firmware event loop
  ...
  def animate(cur_time)     # time in milliseconds, optional, defaults to `Firmware.millis()`
    ...
    while true
      ...
      if   isinstance(ins, animate.ins_ramp)
        ...
        if sub_index < ins.duration
          ...
          self.value = Firmware.scale_uint(sub_index, 0, ins.duration, ins.a, ins.b)
          # call closure
