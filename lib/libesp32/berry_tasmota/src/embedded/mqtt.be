...
class MQTT : MQTT_ntv
  ...
  def lazy_init()
    if self.topics == nil
      ...
      Firmware.add_rule("Mqtt#Connected", / -> self.mqtt_connect())    # call mqtt_connect() when the connection occurs
    end
  end
  ...
  def subscribe(topic, closure)
    ...
    if type(closure) == 'function'
      Firmware.check_not_method(closure)
  ...
  def mqtt_data(topic, idx, payload_s, payload_b)
    ...
    return ret    # return true to stop propagation as a Firmware cmd
  end

  def mqtt_connect()    # called when MQTT connects or re-connects
    Firmware.log("BRY: mqtt subscribe all registered topics", 3)
    for m : self.topics
