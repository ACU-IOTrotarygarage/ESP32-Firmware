...
class Firmware
  ...
  var global          # mapping to FirmwareGlobal
  var settings
  var wd              # when load() is called, `Firmware.wd` contains the name of the archive. Ex: `/M5StickC.autoconf#`
  var _debug_present  # is `import debug` present?

  def init()
    # instanciate the mapping object to FirmwareGlobal
  ...
  #-
    assert(Firmware.find_op("Dimmer>50") == ['Dimmer', '>', '50'])
    assert(Firmware.find_op("Dimmer") == ['Dimmer', nil, nil])
    assert(Firmware.find_op("Status!==Connected") == ['Status', '!==', 'Connected'])
  -#
  def find_op(item)
  ...
  def exec_rules(ev_json, exec_rule)
    ...
    if self._rules || save_cmd_res != nil  # if there is a rule handler, or we record rule results
      ...
      # record the rule payload for Firmware.cmd()
      if save_cmd_res != nil
        self.cmd_res = ev
      end
  ...
  # Firmware.wire_scan(addr:int [, index:int]) -> wire1 or wire2 or nil
  ...
  # fast_loop() is a trimmed down version of event() called at every Firmware loop iteration
  ...
  def add_fast_loop(cl)
    ...
    self.global.fast_loop_enabled = 1      # enable fast_loop at global level: `FirmwareGlobal.fast_loop_enabled = true`
    self._fl.push(cl)
  end
  ...
  def cmd(command, mute)
    ...
    var seriallog_level = Firmware.global.seriallog_level
    var mqttlog_level   = Firmware.settings.mqttlog_level
    var weblog_level    = Firmware.settings.weblog_level

    if mute                 # mute logging
      if seriallog_level >= 2       Firmware.global.seriallog_level = 1    end
      if mqttlog_level   >= 2       Firmware.settings.mqttlog_level = 1    end
      if weblog_level    >= 2       Firmware.settings.weblog_level  = 1    end
    end
    ...
    # restore log_level
    if mute
      Firmware.global.seriallog_level = seriallog_level
      Firmware.settings.mqttlog_level = mqttlog_level
      Firmware.settings.weblog_level = weblog_level
    end
    return ret
  end
  ...
  def get_light(l)
    print('Firmware.get_light() is deprecated, use light.get()')
    import light
    if l != nil
      return light.get(l)
    else
  ...
  def set_light(v,l)
    print('Firmware.set_light() is deprecated, use light.set()')
    import light
    if l != nil
  ...
  def hs2rgb(hue,sat)
    ...
    if sat > 0
      ...
      var f = hue % 60    # 0..59
      var p = 255 - sat
      var q = Firmware.scale_uint(f, 0, 60, 255, p)    # 0..59
      var t = Firmware.scale_uint(f, 0, 60, p, 255)
  ...
  # urlfetch - add a Firmware command to download a file from a URL to file-system
