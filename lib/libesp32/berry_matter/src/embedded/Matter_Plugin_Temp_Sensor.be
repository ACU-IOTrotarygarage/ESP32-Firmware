...
class Matter_Plugin_Temp_Sensor : Matter_Plugin_Device
  ...
  var Firmware_sensor_filter                         # Rule-type filter to the value, like "ESP32#Temperature"
  var Firmware_sensor_matcher                        # Actual matcher object
  ...
  def init(device, endpoint, sensor_filter)
    ...
    self.Firmware_sensor_filter = sensor_filter
    self.Firmware_sensor_matcher = Firmware.Rule_Matcher.parse(sensor_filter)
  end

  #############################################################
  # parse sensor
  #
  # The device calls regularly `Firmware.read_sensors()` and converts
  # it to json.
  def parse_sensors(payload)
    if self.Firmware_sensor_matcher
      var val = real(self.Firmware_sensor_matcher.match(payload))
      if val != nil
        # import string
        # Firmware.log(string.format("MTR: update temperature for endpoint %i - %.1f C", self.endpoint,), 3)
