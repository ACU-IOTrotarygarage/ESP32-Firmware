...
class Matter_Session : Matter_Expirable
  ...
  def before_remove()
    import string
    Firmware.log(string.format("MTR: -Session   (%6i) (removed)", self.local_session_id), 3)
  end
  ...
  def counter_snd_next()
    ...
    Firmware.log(string.format("MTR: .          Counter_snd=%i", next), 3)
  ...
  def update()
    self.last_used = Firmware.rtc()['utc']
  end
