...
class Matter_Control_Message
  ...
  def process_incoming_control_message(msg)
    ...
    Firmware.log("MTR: received control message " + matter.inspect(msg), 2)
    if   msg.opcode == 0x00
      ...
      import string
      Firmware.log(string.format("MTR: >????????? Unknown OpCode (control message) %02X", msg.opcode), 2)
      return false
    end
  ...
  def parse_MsgCounterSyncReq(msg)
    ...
    Firmware.log(string.format("MTR: >MCSyncReq * Not implemented %s", msg.raw[msg.app_payload_idx..].tohex()), 2)
  ...
  def parse_MsgCounterSyncRsp(msg)
    import string
    var session = msg.session
    Firmware.log(string.format("MTR: >MCSyncRsp * Not implemented %s", msg.raw[msg.app_payload_idx..].tohex()), 2)
    return false      # we don't explicitly ack the message
  end
