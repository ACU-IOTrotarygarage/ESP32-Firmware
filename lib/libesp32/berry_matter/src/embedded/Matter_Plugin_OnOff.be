...
class Matter_Plugin_OnOff : Matter_Plugin
  ...
  var Firmware_relay_index             # Relay number in Firmware (zero based)
  var shadow_onoff                           # fake status for now # TODO

  #############################################################
  # Constructor
  def init(device, endpoint, Firmware_relay_index)
    super(self).init(device, endpoint)
    self.get_onoff()                        # read actual value
    if Firmware_relay_index == nil     Firmware_relay_index = 0   end
    self.Firmware_relay_index = Firmware_relay_index
  end
  ...
  #
  def set_onoff(v)
    Firmware.set_power(self.Firmware_relay_index, bool(v))
    self.get_onoff()
  end
  #############################################################
  # get_onoff
  ...
  def get_onoff()
    var state = Firmware.get_power(self.Firmware_relay_index)
    if state != nil
