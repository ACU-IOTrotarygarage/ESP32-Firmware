...
class Matter_Session_Store
  ...
  def find_session_by_resumption_id(resumption_id)
    ...
    while i < size(sessions)
      var session = sessions[i]
      Firmware.log(string.format("MTR: session.resumption_id=%s vs %s", str(session.resumption_id), str(resumption_id)), 3)
      if session.resumption_id == resumption_id && session.shared_secret != nil
        Firmware.log(string.format("MTR: session.shared_secret=%s", str(session.shared_secret)), 3)
        session.update()
        return session
      end
      i += 1
  ...
  def save_fabrics()
    ...
    try
      ...
      f.write(fabs)
      f.close()
      Firmware.log(string.format("MTR: =Saved     %i fabric(s) and %i session(s)", fabs_size, sessions_saved), 2)
    except .. as e, m
  ...
  def load_fabrics()
    ...
    try
      ...
      file_content = nil
      Firmware.gc()      # clean-up a potential long string
      ...
      Firmware.log(string.format("MTR: Loaded %i fabric(s)", size(self.fabrics)), 2)
    except .. as e, m
      if e != "io_error"
