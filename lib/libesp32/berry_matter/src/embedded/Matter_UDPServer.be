...
class Matter_UDPPacket_sent
  ...
  def init(msg)
    ...
    # other information
    self.retries = 0
    self.next_try = Firmware.millis() + matter.UDPServer._backoff_time(self.retries)
  end
...
class Matter_UDPServer
  ...
  #############################################################
  # Starts the server.
  # Registers as device handler to Firmware
  #
  ...
  def every_50ms()
    ...
    while packet != nil
      ...
      Firmware.log(string.format("MTR: UDP received from [%s]:%i", from_addr, from_port), 3)
      if self.dispatch_cb
  ...
  def send(packet)
    ...
    if ok
      Firmware.log(string.format("MTR: sending packet to '[%s]:%i'", packet.addr, packet.port), 4)
    else
      Firmware.log(string.format("MTR: error sending packet to '[%s]:%i'", packet.addr, packet.port), 2)
    end
    return ok
  end
  ...
  def _resend_packets()
    ...
    while idx < size(self.packets_sent)
      ...
      if Firmware.time_reached(packet.next_try)
        if packet.retries <= self.RETRIES
          Firmware.log("MTR: .          Resending packet id=" + str(packet.msg_id), 3)
          self.send(packet)
          packet.next_try = Firmware.millis() + self._backoff_time(packet.retries)
          packet.retries += 1
          idx += 1
        else
          import string
          self.packets_sent.remove(idx)
          Firmware.log(string.format("MTR: .          (%6i) Unacked packet '[%s]:%i' msg_id=%i", packet.session_id, packet.addr, packet.port, packet.msg_id), 2)
        end
      else
        idx += 1
      end
  ...
  def received_ack(msg)
    ...
    Firmware.log("MTR: receveived ACK id="+str(id), 3)
    var idx = 0
    while idx < size(self.packets_sent)
      var packet = self.packets_sent[idx]
      if packet.msg_id == id && packet.exchange_id == exch
        self.packets_sent.remove(idx)
        Firmware.log("MTR: .          Removed packet from sending list id=" + str(id), 3)
      else
        idx += 1
      end
    end
