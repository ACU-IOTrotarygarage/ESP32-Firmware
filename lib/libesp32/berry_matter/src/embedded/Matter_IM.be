...
class Matter_IM
  ...
  def process_incoming(msg)
    ...
    # Firmware.log("MTR: received IM message " + matter.inspect(msg), 3)

    var val = matter.TLV.parse(msg.raw, msg.app_payload_idx)

    # Firmware.log("MTR: IM TLV: " + str(val), 3)

    var InteractionModelRevision = val.findsubval(0xFF)
    # Firmware.log("MTR: InteractionModelRevision=" + (InteractionModelRevision != nil ? str(InteractionModelRevision) : "nil"), 4)
  ...
  def process_incoming_ack(msg)
    ...
    Firmware.log(string.format("MTR: process_incoming_ack exch=%i message=%i", msg.exchange_id, message != nil ? 1 : 0), 3)
    if message
  ...
  def send_enqueued(responder)
    ...
    while idx < size(self.send_queue)
      ...
      if message.finish
        Firmware.log("MTR: remove IM message exch="+str(message.resp.exchange_id), 3)
  ...
  def expire_sendqueue()
    ...
    while idx < size(self.send_queue)
      var message = self.send_queue[idx]
      if Firmware.time_reached(message.expiration)
  ...
  def process_status_response(msg, val)
    ...
    if status == matter.SUCCESS
      if message
        ...
        Firmware.log(string.format("MTR: >OK        (%6i) exch=%i not found", msg.session.local_session_id, msg.exchange_id), 3)      # don't show 'SUCCESS' to not overflow logs with non-information
      end
    else
      # error
      Firmware.log(string.format("MTR: >Status    ERROR = 0x%02X", status), 2)
      if message
  ...
  def _inner_process_read_request(session, query, no_log)
    ...
    def read_single_attribute(ret, pi, ctx, direct)
      ...
      if res != nil
        ...
        if !no_log
          Firmware.log(string.format("MTR: >Read_Attr (%6i) %s%s - %s", session.local_session_id, str(ctx), attr_name, str(res)), 2)
        end
        ...
        if direct
          ...
          ret.attribute_reports.push(a2)
          Firmware.log(string.format("MTR: >Read_Attr (%6i) %s%s - STATUS: 0x%02X %s", session.local_session_id, str(ctx), attr_name, ctx.status, ctx.status == matter.UNSUPPORTED_ATTRIBUTE ? "UNSUPPORTED_ATTRIBUTE" : ""), 2)
          return true
        end
      else
        Firmware.log(string.format("MTR: >Read_Attr (%6i) %s%s - IGNORED", session.local_session_id, str(ctx), attr_name), 2)
    ...
    for q:query.attributes_requests
      ...
      if ctx.endpoint == nil || ctx.cluster == nil || ctx.attribute == nil
        ...
        if ctx.cluster != nil && ctx.attribute != nil
          var attr_name = matter.get_attribute_name(ctx.cluster, ctx.attribute)
          Firmware.log(string.format("MTR: >Read_Attr (%6i) %s", session.local_session_id, str(ctx) + (attr_name ? " (" + attr_name + ")" : "")), 2)
        else
          Firmware.log(string.format("MTR: >Read_Attr (%6i) %s", session.local_session_id, str(ctx)), 2)
        end
    ...
    # Firmware.log("MTR: ReportDataMessage=" + str(ret), 3)
    # Firmware.log("MTR: ReportDataMessageTLV=" + str(ret.to_TLV()), 3)
  ...
  def subscribe_request(msg, val)
    ...
    # Firmware.log("MTR: received SubscribeRequestMessage=" + str(query), 3)
    ...
    Firmware.log(string.format("MTR: >Subscribe (%6i) %s (min=%i, max=%i, keep=%i) sub=%i",
  ...
  def process_invoke_request(msg, val)
    import string
    # structure is `ReadRequestMessage` 10.6.2 p.558
    # Firmware.log("MTR: IM:invoke_request processing start", 4)
    var ctx = matter.Path()
    ...
    if query.invoke_requests != nil
      ...
      for q:query.invoke_requests
        ...
        Firmware.log(string.format("MTR: >Command   (%6i) %s %s %s", msg.session.local_session_id, str(ctx), cmd_name ? cmd_name : "", params_log), 2)
        ctx.log = nil
        ...
        if res == true || ctx.status == matter.SUCCESS      # special case, just respond ok
          ...
          Firmware.log(string.format("MTR: <Replied   (%6i) OK exch=%i", msg.session.local_session_id, msg.exchange_id), 2)
        elif res != nil
          ...
          Firmware.log(string.format("MTR: <Replied   (%6i) %s %s", msg.session.local_session_id, str(ctx), cmd_name ? cmd_name : ""), 2)
        elif ctx.status != nil
          a1.status = matter.CommandStatusIB()
          a1.status.command_path = matter.CommandPathIB()
          a1.status.command_path.endpoint = ctx.endpoint
          ...
          ret.invoke_responses.push(a1)
          Firmware.log(string.format("MTR: <Replied   (%6i) Status=0x%02X exch=%i", msg.session.local_session_id, ctx.status, msg.exchange_id), 2)
        else
          Firmware.log(string.format("MTR: _Ignore    (%6i) exch=%i", msg.session.local_session_id, msg.exchange_id), 2)
          # ignore if content is nil and status is undefined
        end
      end

      # Firmware.log("MTR: invoke_responses="+str(ret.invoke_responses), 4)
      if size(ret.invoke_responses) > 0
        # Firmware.log("MTR: InvokeResponse=" + str(ret), 4)
        # Firmware.log("MTR: InvokeResponseTLV=" + str(ret.to_TLV()), 3)
  ...
  def subscribe_response(msg, val)
    ...
    Firmware.log("MTR: received SubscribeResponsetMessage=" + str(query), 2)
    return false
  end
  ...
  def report_data(msg, val)
    ...
    Firmware.log("MTR: received ReportDataMessage=" + str(query), 2)
    return false
  end
  ...
  def process_write_request(msg, val)
    ...
    # Firmware.log("MTR: received WriteRequestMessage=" + str(query), 3)
    ...
    def write_single_attribute(ret, pi, ctx, write_data, direct)
      ...
      var attr_name = matter.get_attribute_name(ctx.cluster, ctx.attribute)
      attr_name = attr_name ? " (" + attr_name + ")" : ""
      # Firmware.log(string.format("MTR: Read Attribute " + str(ctx) + (attr_name ? " (" + attr_name + ")" : ""), 2)
      # Special case to report unsupported item, if pi==nil
      ...
      if ctx.status != nil
        if direct
          ...
          Firmware.log(string.format("MTR: Write_Attr %s%s - STATUS: 0x%02X %s", str(ctx), attr_name, ctx.status, ctx.status == matter.SUCCESS ? "SUCCESS" : ""), 2)
          return true
        end
      else
        Firmware.log(string.format("MTR: Write_Attr %s%s - IGNORED", str(ctx), attr_name), 2)
        # ignore if content is nil and status is undefined
      end
    end

    # structure is `ReadRequestMessage` 10.6.2 p.558
    # Firmware.log("MTR: IM:write_request processing start", 4)
    var ctx = matter.Path()
    ...
    if query.write_requests != nil
      ...
      for q:query.write_requests      # q is AttributeDataIB
        ...
        if ctx.endpoint == nil
          # we need expansion, log first
          var attr_name = matter.get_attribute_name(ctx.cluster, ctx.attribute)
          Firmware.log("MTR: Write_Attr " + str(ctx) + (attr_name ? " (" + attr_name + ")" : ""), 2)
        end
      ...
      # Firmware.log("MTR: ReportWriteMessage=" + str(ret), 4)
      # Firmware.log("MTR: ReportWriteMessageTLV=" + str(ret.to_TLV()), 3)
  ...
  def process_write_response(msg, val)
    ...
    Firmware.log("MTR: received WriteResponseMessage=" + str(query), 2)
    return false
  end
  ...
  def process_invoke_response(msg, val)
    ...
    Firmware.log("MTR: received InvokeResponseMessage=" + str(query), 2)
    return false
  end
  ...
  def process_timed_request(msg, val)
    ...
    # Firmware.log("MTR: received TimedRequestMessage=" + str(query), 3)

    Firmware.log(string.format("MTR: >Command   (%6i) TimedRequest=%i", msg.session.local_session_id, query.timeout), 2)
  ...
  def send_subscribe_update(sub)
    ...
    Firmware.log(string.format("MTR: <Sub_Data  (%6i) sub=%i", session.local_session_id, sub.subscription_id), 2)
  ...
  def send_subscribe_heartbeat(sub)
    ...
    Firmware.log(string.format("MTR: <Sub_Alive (%6i) sub=%i", session.local_session_id, sub.subscription_id), 2)
