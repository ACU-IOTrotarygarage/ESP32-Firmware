...
class Matter_MessageHandler
  ...
  def send_simple_ack(frame, reliable)
    ...
    if frame.x_flag_r                   # nothing to respond, check if we need a standalone ack
      ...
      resp.encode_frame()
      Firmware.log(string.format("MTR: <Ack       (%6i) ack=%i id=%i %s", resp.session.local_session_id, resp.ack_message_counter, resp.message_counter, reliable ? '{reliable}' : ''), 3)
  ...
  def msg_received(raw, addr, port)
    ...
    try
      # Firmware.log("MTR: MessageHandler::msg_received raw="+raw.tohex(), 4)
      ...
      if frame.sec_p
        # Control message
        Firmware.log("MTR: CONTROL MESSAGE=" + matter.inspect(frame), 4)
        var session = self.device.sessions.find_session_source_id_unsecure(frame.source_node_id, 90)    # 90 seconds max
        Firmware.log("MTR: find session by source_node_id = " + str(frame.source_node_id) + " session_id = " + str(session.local_session_id), 4)
        ...
        if !session._counter_insecure_rcv.validate(frame.message_counter, false)
          Firmware.log(string.format("MTR: .          Duplicate unencrypted message = %i ref = %i", frame.message_counter, session._counter_insecure_rcv.val()), 3)
        ...
        if frame.opcode != 0x10                                 # don't show `MRP_Standalone_Acknowledgement`
          ...
          Firmware.log(string.format("MTR: >Received  (%6i) %s rid=%i exch=%i from [%s]:%i", session.local_session_id, op_name, frame.message_counter, frame.exchange_id, addr, port), 2)
        else
          Firmware.log(string.format("MTR: >rcv Ack   (%6i) rid=%i exch=%i ack=%s %sfrom [%s]:%i", session.local_session_id, frame.message_counter, frame.x_flag_r ? "{reliable} " : "", frame.exchange_id, str(frame.ack_message_counter), addr, port), 3)
        end
        ...
        #############################################################
        # encrypted message
        Firmware.log(string.format("MTR: decode header: local_session_id=%i message_counter=%i", frame.local_session_id, frame.message_counter), 3)

        var session = self.device.sessions.get_session_by_local_session_id(frame.local_session_id)
        if session == nil
          Firmware.log("MTR: unknown local_session_id="+str(frame.local_session_id), 2)
          # Firmware.log("MTR: frame="+matter.inspect(frame), 3)
          return false
        end
        if addr     session._ip = addr     end
        if port     session._port = port   end
        ...
        if !session.counter_rcv_validate(frame.message_counter, true)
          Firmware.log("MTR: .          Duplicate encrypted message = " + str(frame.message_counter) + " counter=" + str(session.counter_rcv), 3)
        ...
        # continue decoding
        Firmware.log(string.format("MTR: idx=%i clear=%s", frame.payload_idx, frame.raw.tohex()), 4)
        frame.decode_payload()
        Firmware.log("MTR: >          Decrypted message: protocol_id:"+str(frame.protocol_id)+" opcode="+str(frame.opcode)+" exchange_id="+str(frame.exchange_id & 0xFFFF), 3)

        Firmware.log(string.format("MTR: >rcv       (%6i) [%02X/%02X] rid=%i exch=%i ack=%s %sfrom [%s]:%i", session.local_session_id, frame.protocol_id, frame.opcode, frame.message_counter, frame.exchange_id, str(frame.ack_message_counter), frame.x_flag_r ? "{reliable} " : "", addr, port), 3)
        ...
        if protocol_id == 0x0000    # PROTOCOL_ID_SECURE_CHANNEL
          ...
          # Firmware.log("MTR: PROTOCOL_ID_SECURE_CHANNEL " + matter.inspect(frame), 3)
        ...
        # -- PROTOCOL_ID_BDX is used for file transfer between devices, not used in Firmware
        # elif protocol_id == 0x0002  # PROTOCOL_ID_BDX -- BDX not handled at all in Firmware
        #   Firmware.log("MTR: PROTOCOL_ID_BDX not yet handled", 2)
        ...
        #   Firmware.log("MTR: PROTOCOL_ID_USER_DIRECTED_COMMISSIONING not yet handled", 2)
        #   return false # ignore for now TODO
        else
          Firmware.log("MTR: ignoring unhandled protocol_id:"+str(protocol_id), 3)
        end
      ...
      Firmware.log("MTR: MessageHandler::msg_received exception: "+str(e)+";"+str(m))
      import debug
      debug.traceback()
      return false
    end
