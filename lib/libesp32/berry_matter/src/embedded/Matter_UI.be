#
# Matter_UI.be - WebUI for Matter configuration in Firmware
#
...
class Matter_UI
  ...
  def show_enable(p)
    import webserver
    import string
    var matter_enabled = Firmware.get_option(matter.MATTER_OPTION)
  ...
  def show_commissioning_info()
    ...
    var seconds_left = (self.device.commissioning_open - Firmware.millis()) / 1000
  ...
  def page_part_ctl()
    ...
    try
      ...
      if webserver.has_arg("passcode") || webserver.has_arg("discriminator")
        ...
        Firmware.cmd("SetOption" + str(matter.MATTER_OPTION) + " 1")
        #- and force restart -#
        webserver.redirect("/?rst=")

      elif webserver.has_arg("disable")
        Firmware.cmd("SetOption" + str(matter.MATTER_OPTION) + " 0")
        #- and force restart -#
        webserver.redirect("/?rst=")
