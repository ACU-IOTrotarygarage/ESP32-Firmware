  ...
  def read_attribute(session, ctx)
    ...
    # ====================================================================================================
      ...
      if   attribute == 0x0000          #  ---------- NetworkInterfaces ----------
        ...
        return TLV.create_TLV(TLV.U2, Firmware.cmd("Status 1", true)['StatusPRM']['BootCount'])
      elif attribute == 0x0002          #  ---------- UpTime u16 ----------
        return TLV.create_TLV(TLV.U4, Firmware.cmd("Status 11", true)['StatusSTS']['UptimeSec'])
    ...
    # ====================================================================================================
      ...
      if   attribute == 0x0000          #  ---------- UTCTime / epoch_us ----------
        var epoch_us = int64(Firmware.rtc()['utc']) * int64(1000000)
      ...
      # TODO add some missing args
        ...
        var epoch_us = int64(Firmware.rtc()['local']) * int64(1000000)
        return TLV.create_TLV(TLV.U8, epoch_us)     # TODO test the conversion of int64()
      end
    ...
    # ====================================================================================================
      ...
      if   attribute == 0x0000          #  ---------- DataModelRevision / CommissioningWindowStatus ----------
        ...
        return TLV.create_TLV(TLV.UTF1, "Firmware")
        ...
        return TLV.create_TLV(TLV.UTF1, Firmware.cmd("DeviceName", true)['DeviceName'])
        ...
        return TLV.create_TLV(TLV.UTF1, Firmware.cmd("FriendlyName", true)['FriendlyName1'])
        ...
        return TLV.create_TLV(TLV.UTF1, Firmware.cmd("Status 2", true)['StatusFWR']['Hardware'])
      elif attribute == 0x0009          #  ---------- SoftwareVersion / u32 ----------
        return TLV.create_TLV(TLV.U2, 1)
      elif attribute == 0x000A          #  ---------- SoftwareVersionString / string ----------
        var version_full = Firmware.cmd("Status 2", true)['StatusFWR']['Version']
        ...
        return TLV.create_TLV(TLV.UTF1, Firmware.wifi().find("mac", ""))
    ...
    # ====================================================================================================
      ...
      if   attribute == 0x0000          #  ---------- DefaultOTAProviders / list[ProviderLocationStruct] ----------
        ...
        return TLV.create_TLV(TLV.BOOL, 0)  # we claim that update is not possible, would require to go to Firmware UI
    ...
    # ====================================================================================================
      ...
      if   attribute == 0x0000          #  ---------- ActiveLocale / string ----------
        return TLV.create_TLV(TLV.UTF1, Firmware.locale())
      elif attribute == 0x0001          #  ---------- SupportedLocales / list[string] ----------
        var locl = TLV.Matter_TLV_array()
        locl.add_TLV(nil, TLV.UTF1, Firmware.locale())
        return locl
      end
  ...
  def invoke_request(session, val, ctx)
    ...
    if   cluster == 0x0030              # ========== GeneralCommissioning cluster 11.9 p.627 ==========
      ...
      if   command == 0x0002            # ---------- CertificateChainRequest ----------
        ...
        att_elts.add_TLV(3, TLV.U4, Firmware.rtc()['utc'])     # timestamp in epoch-s
        var attestation_message = att_elts.tlv2raw()

        var ac = session.get_ac()
        var attestation_tbs = attestation_message + ac
        Firmware.log("MTR: attestation_tbs=" + attestation_tbs.tohex(), 3)
        ...
        Firmware.log("MTR: nocsr_tbs=" + nocsr_tbs.tohex(), 3)
        ...
        Firmware.log("MTR: received ca_root="+RootCACertificate.tohex(), 3)
        ...
        if session.get_ca() == nil
          Firmware.log("MTR: Error: AdNOC without CA", 2)
          return nil
        end
        ...
        if !fabric_id || !deviceid
          Firmware.log("MTR: Error: no fabricid nor deviceid in NOC certificate", 2)
          return false
        end
        # convert fo bytes(8)
        ...
        Firmware.log("MTR: ------------------------------------------", 3)
        Firmware.log("MTR: fabric=" + matter.inspect(session._fabric), 3)
        Firmware.log("MTR: ------------------------------------------", 3)
        ...
        Firmware.log(string.format("MTR: .          Update fabric '%s' label='%s'", session._fabric.get_fabric_id().copy().reverse().tohex(), str(label)), 2)
        ctx.status = matter.SUCCESS                  # OK
        return nil                      # trigger a standalone ack
        ...
        for fab: self.device.sessions.active_fabrics()
          if fab.get_fabric_index() == index
            Firmware.log("MTR: removing fabric " + fab.get_fabric_id().copy().reverse().tohex(), 2)
            # defer actual removal to send a response
            Firmware.set_timer(2000, def () self.device.remove_fabric(fab) end)
            return true                 # Ok
          end
        end
        Firmware.log("MTR: RemoveFabric fabric("+str(index)+") not found", 2)
    ...
    # ====================================================================================================
      ...
      if   command == 0x0000          #  ---------- OpenCommissioningWindow  ----------
        ...
        Firmware.log(string.format("MTR: OpenCommissioningWindow(timeout=%i, passcode=%s, discriminator=%i, iterations=%i, salt=%s)",
        ...
        if size(passcode_verifier) != 32+65 || size(salt) < 16 || size(salt) > 32
          Firmware.log("MTR: wrong size for PAKE parameters")
        ...
        Firmware.log("MTR: OpenBasicCommissioningWindow commissioning_timeout="+str(commissioning_timeout), 2)
