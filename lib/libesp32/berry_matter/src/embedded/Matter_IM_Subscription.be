...
class Matter_IM_Subscription
  ...
  def init(subs_shop, id, session, req)
    ...
    # Firmware.log("MTR: new subsctiption " + matter.inspect(self), 3)
  end
  
  # remove self from subs_shop list
  def remove_self()
    Firmware.log("MTR: -Sub_Del   (      ) sub=" + str(self.subscription_id), 2)
  ...
  def re_arm()
    ...
    if !self.is_keep_alive
      Firmware.log(string.format("MTR: .Sub_Done  (      ) sub=%i", self.subscription_id), 2)
    end
  end
...
class Matter_IM_Subscription_Shop
  ...
  def every_250ms()
    ...
    while idx < size(self.subs)
      ...
      if !sub.wait_status && size(sub.updates) > 0 && Firmware.time_reached(sub.not_before)
    ...
    while idx < size(self.subs)
      var sub = self.subs[idx]
      if !sub.wait_status && Firmware.time_reached(sub.expiration)
