...
#
# Copyright (C) 2023  Stephan Hadinger & Theo Arends and Mahmoud Shams El-Din
#
...
# static Matter_Context_Prefix = "Matter PAKE V1 Commissioning" # spec is wrong
  ...
  def add_session(local_session_id, initiator_session_id, i2r, r2i, ac)
    ...
    Firmware.log(string.format("MTR: add_session local_session_id=%i initiator_session_id=%i", local_session_id, initiator_session_id), 3)
  ...
  def process_incoming(msg)
    ...
    if !self.device.is_commissioning_open() && msg.opcode >= 0x20 && msg.opcode <= 0x24
      Firmware.log("MTR: commissioning not open", 2)
      return false
    end

    Firmware.log("MTR: received message " + matter.inspect(msg), 3)
    if   msg.opcode == 0x10
      ...
      import string
      Firmware.log(string.format("MTR: >????????? Unknown OpCode (secure channel) %02X", msg.opcode), 2)
      return false
    end
  ...
  def parse_PBKDFParamRequest(msg)
    ...
    if msg.opcode != 0x20 || msg.local_session_id != 0 || msg.protocol_id != 0
      Firmware.log("MTR: invalid PBKDFParamRequest message", 2)
      Firmware.log("MTR: StatusReport(General Code: FAILURE, ProtocolId: SECURE_CHANNEL, ProtocolCode: INVALID_PARAMETER)", 2)
    ...
    if pbkdfparamreq.passcodeId != 0
      Firmware.log("MTR: non-zero passcode id", 2)
      Firmware.log("MTR: StatusReport(General Code: FAILURE, ProtocolId: SECURE_CHANNEL, ProtocolCode: INVALID_PARAMETER)", 2)
      self.send_status_report(msg, 0x01, 0x0000, 0x0002, false)
      return false
    end
    ...
    Firmware.log(string.format("MTR: +Session   (%6i) from '[%s]:%i'", session.__future_local_session_id, msg.remote_ip, msg.remote_port), 2)
    ...
    # Firmware.log("MTR: pbkdfparamresp: " + str(matter.inspect(pbkdfparamresp)), 4)
    var pbkdfparamresp_raw = pbkdfparamresp.tlv2raw()
    # Firmware.log("MTR: pbkdfparamresp_raw: " + pbkdfparamresp_raw.tohex(), 4)
  ...
  def parse_Pake1(msg)
    ...
    if msg.opcode != 0x22 || msg.local_session_id != 0 || msg.protocol_id != 0
      Firmware.log("MTR: invalid Pake1 message", 2)
      Firmware.log("MTR: StatusReport(General Code: FAILURE, ProtocolId: SECURE_CHANNEL, ProtocolCode: INVALID_PARAMETER)", 2)
      self.send_status_report(msg, 0x01, 0x0000, 0x0002, false)
      return false
    end
    ...
    var pA = pake1.pA
    # Firmware.log("MTR: received pA=" + pA.tohex(), 4)
    ...
    # compute pB
    spake.compute_pB(y)
    # Firmware.log("MTR: y=" + y.tohex(), 4)
    # Firmware.log("MTR: pb=" + spake.pB.tohex(), 4)

    # compute ZV
    spake.compute_ZV_verifier(pA)
    # Firmware.log("MTR: Z=" + spake.Z.tohex(), 4)
    # Firmware.log("MTR: V=" + spake.V.tohex(), 4)
    ...
    # Firmware.log("MTR: Context=" + context_hash.tohex(), 4)
    ...
    # Firmware.log("MTR: ------------------------------", 4)
    # Firmware.log("MTR: Context = " + spake.Context.tohex(), 4)
    # Firmware.log("MTR: M       = " + spake.M.tohex(), 4)
    # Firmware.log("MTR: N       = " + spake.N.tohex(), 4)
    # Firmware.log("MTR: pA      = " + spake.pA.tohex(), 4)
    # Firmware.log("MTR: pB      = " + spake.pB.tohex(), 4)
    # Firmware.log("MTR: Z       = " + spake.Z.tohex(), 4)
    # Firmware.log("MTR: V       = " + spake.V.tohex(), 4)
    # Firmware.log("MTR: w0      = " + spake.w0.tohex(), 4)
    # Firmware.log("MTR: ------------------------------", 4)

    # Firmware.log("MTR: Kmain   =" + spake.Kmain.tohex(), 4)

    # Firmware.log("MTR: KcA     =" + spake.KcA.tohex(), 4)
    # Firmware.log("MTR: KcB     =" + spake.KcB.tohex(), 4)
    # Firmware.log("MTR: K_shared=" + spake.K_shared.tohex(), 4)
    # Firmware.log("MTR: Ke      =" + spake.Ke.tohex(), 4)
    # Firmware.log("MTR: cB=" + spake.cB.tohex(), 4)

    var pake2 = matter.Pake2()
    pake2.pB = spake.pB
    pake2.cB = spake.cB
    # Firmware.log("MTR: pake2: " + matter.inspect(pake2), 4)
    var pake2_raw = pake2.tlv2raw()
    # Firmware.log("MTR: pake2_raw: " + pake2_raw.tohex(), 4)
  ...
  def parse_Pake3(msg)
    ...
    if msg.opcode != 0x24 || msg.local_session_id != 0 || msg.protocol_id != 0
      Firmware.log("MTR: invalid Pake3 message", 2)
      Firmware.log("MTR: StatusReport(General Code: FAILURE, ProtocolId: SECURE_CHANNEL, ProtocolCode: INVALID_PARAMETER)", 2)
      self.send_status_report(msg, 0x01, 0x0000, 0x0002, false)
      return false
    end
    ...
    var cA = pake3.cA
    # Firmware.log("MTR: received cA=" + cA.tohex(), 4)

    # check the value against computed
    if cA != session.__spake_cA
      Firmware.log("MTR: invalid cA received", 2)
      Firmware.log("MTR: StatusReport(General Code: FAILURE, ProtocolId: SECURE_CHANNEL, ProtocolCode: INVALID_PARAMETER)", 2)
      self.send_status_report(msg, 0x01, 0x0000, 0x0002, false)
      return false
    end
    ...
    # Firmware.log("MTR: ******************************", 4)
    # Firmware.log("MTR: session_keys=" + session_keys.tohex(), 4)
    # Firmware.log("MTR: I2RKey      =" + I2RKey.tohex(), 4)
    # Firmware.log("MTR: R2IKey      =" + R2IKey.tohex(), 4)
    # Firmware.log("MTR: AC          =" + AttestationChallenge.tohex(), 4)
    # Firmware.log("MTR: ******************************", 4)
  ...
  def find_fabric_by_destination_id(destinationId, initiatorRandom)
    ...
    Firmware.log("MTR: SEARCHING: destinationId=" + destinationId.tohex(), 3)
    for fabric : self.device.sessions.fabrics
      ...
      Firmware.log("MTR: SIGMA1: destinationMessage=" + destinationMessage.tohex(), 3)
      Firmware.log("MTR: SIGMA1: key_ipk=" + key.tohex(), 4)
      var h = crypto.HMAC_SHA256(key)
      h.update(destinationMessage)
      var candidateDestinationId = h.out()
      Firmware.log("MTR: SIGMA1: candidateDestinationId=" + candidateDestinationId.tohex(), 3)
  ...
  def parse_Sigma1(msg)
    ...
    if is_resumption
      ...
      Firmware.log(string.format("MTR: session_resumption found session=%s session_resumption=%s", matter.inspect(session), matter.inspect(session_resumption)), 4)
    ...
    if   is_resumption
      ...
      Firmware.log("****************************************", 4)
      Firmware.log("MTR: * s1rk              = " + s1rk.tohex(), 4)
      Firmware.log("MTR: * tag               = " + tag.tohex(), 4)
      Firmware.log("MTR: * Resume1MICPayload = " + Resume1MICPayload.tohex(), 4)
      Firmware.log("MTR: * decrypted_tag     = " + decrypted_tag.tohex(), 4)
      Firmware.log("****************************************", 4)
      if tag == decrypted_tag
        ...
        Firmware.log("****************************************", 4)
        Firmware.log("MTR: * s2rk              = " + s2rk.tohex(), 4)
        Firmware.log("MTR: * s2rk_salt         = " + s2rk_salt.tohex(), 4)
        Firmware.log("MTR: * new_resumption_id = " + session.resumption_id.tohex(), 4)
        Firmware.log("MTR: * responderSessionID= " + str(session.__future_local_session_id), 4)
        Firmware.log("MTR: * sigma2ResumeMIC   = " + Resume2MIC.tohex(), 4)
        Firmware.log("****************************************", 4)
        ...
        Firmware.log("MTR: ******************************", 4)
        Firmware.log("MTR: I2RKey      =" + i2r.tohex(), 4)
        Firmware.log("MTR: R2IKey      =" + r2i.tohex(), 4)
        Firmware.log("MTR: AC          =" + ac.tohex(), 4)
        Firmware.log("MTR: ******************************", 4)

        var sigma2resume_raw = sigma2resume.tlv2raw()
        session.__Msg1 = nil
        Firmware.log("MTR: sigma2resume: " + matter.inspect(sigma2resume), 4)
        Firmware.log("MTR: sigma2resume_raw: " + sigma2resume_raw.tohex(), 4)
    ...
    if !is_resumption
      ...
      if session == nil || session._fabric == nil
        Firmware.log("MTR: StatusReport(GeneralCode: FAILURE, ProtocolId: SECURE_CHANNEL, ProtocolCode: NO_SHARED_TRUST_ROOTS)", 2)
      ...
      Firmware.log("MTR: fabric="+matter.inspect(session._fabric), 4)
      Firmware.log("MTR: no_private_key="+session._fabric.no_private_key.tohex(), 4)
      Firmware.log("MTR: noc           ="+session._fabric.noc.tohex(), 4)
      if session._fabric.get_icac()
        Firmware.log("MTR: icac          ="+session._fabric.get_icac().tohex(), 4)
      end
      Firmware.log("MTR: root_ca_cert  ="+session._fabric.root_ca_certificate.tohex(), 4)
      ...
      Firmware.log("MTR: ResponderEph_priv  ="+session.__responder_priv.tohex(), 4)
      Firmware.log("MTR: ResponderEph_pub  ="+session.__responder_pub.tohex(), 4)
      ...
      Firmware.log("MTR: * resumptionid  = " + session.resumption_id.tohex(), 4)
      Firmware.log("MTR: * MSG1          = " + session.__Msg1.tohex(), 4)
      var TranscriptHash = crypto.SHA256().update(session.__Msg1).out()
      Firmware.log("MTR: TranscriptHash =" + TranscriptHash.tohex(), 4)
      ...
      Firmware.log("MTR: * SharedSecret  = " + session.shared_secret.tohex(), 4)
      Firmware.log("MTR: * s2k_salt      = " + s2k_salt.tohex(), 4)
      Firmware.log("MTR: * s2k           = " + s2k.tohex(), 4)

      var sigma2_tbedata_raw = sigma2_tbedata.tlv2raw()
      Firmware.log("MTR: * TBEData2Raw   = " + sigma2_tbedata_raw.tohex(), 4)
      ...
      Firmware.log("MTR: * TBEData2Enc   = " + TBEData2Encrypted.tohex(), 4)
      Firmware.log("****************************************", 4)
      ...
      Firmware.log("MTR: sigma2: " + matter.inspect(sigma2), 4)
      var sigma2_raw = sigma2.tlv2raw()
      session.__Msg2 = sigma2_raw
      Firmware.log("MTR: sigma2_raw: " + sigma2_raw.tohex(), 4)
  ...
  def parse_Sigma3(msg)
    ...
    Firmware.log("MTR: * session       = " + str(session), 4)
    Firmware.log("MTR:   .ipk_epoch_key=" + str(session.get_ipk_epoch_key()), 4)
    Firmware.log("MTR:   .fabric_compr = " + str(session.get_fabric_compressed()), 4)
    Firmware.log("MTR: * ipk_group_key = " + session.get_ipk_group_key().tohex(), 4)
    Firmware.log("MTR: * TranscriptHash= " + TranscriptHash.tohex(), 4)
    ...
    Firmware.log("****************************************", 4)
    Firmware.log("MTR: * s3k_salt      = " + (session.get_ipk_group_key() + TranscriptHash).tohex(), 4)
    Firmware.log("MTR: * s3k           = " + s3k.tohex(), 4)
    Firmware.log("****************************************", 4)
    ...
    var TBETag3 = ec.tag()
    Firmware.log("MTR: * TBEData3      = " + TBEData3.tohex(), 4)
    Firmware.log("MTR: * TBETag3       = " + TBETag3.tohex(), 4)
    Firmware.log("MTR: * tag_sent      = " + tag.tohex(), 4)
    Firmware.log("****************************************", 4)

    if TBETag3 != tag
      Firmware.log("MTR: Tag don't match", 2)
      Firmware.log("MTR: StatusReport(General Code: FAILURE, ProtocolId: SECURE_CHANNEL, ProtocolCode: INVALID_PARAMETER)", 2)
      self.send_status_report(msg, 0x01, 0x0000, 0x0002, false)
      return false
    end
    ...
    Firmware.log("MTR: * TBEData3TLV   = " + str(TBEData3TLV), 4)
    ...
    Firmware.log("MTR: * initiatorNOC  = " + str(initiatorNOC), 4)
    Firmware.log("MTR: * initiatorICAC = " + str(initiatorICAC), 4)
    Firmware.log("MTR: * ec_signature  = " + str(ec_signature), 4)
    ...
    Firmware.log("MTR: initiatorNOCTLV = " + str(initiatorNOCTLV), 3)
    ...
    Firmware.log("MTR: initiatorFabricId="+str(session.peer_node_id), 3)
    ...
    Firmware.log("MTR: * sigma3_tbs    = " + str(sigma3_tbs), 4)
    var sigma3_tbs_raw = sigma3_tbs.tlv2raw()
    Firmware.log("MTR: * sigma3_tbs_raw= " + sigma3_tbs_raw.tohex(), 4)

    Firmware.log("MTR: * initiatorNOCPubKey= " + initiatorNOCPubKey.tohex(), 4)
    Firmware.log("MTR: * ec_signature      = " + ec_signature.tohex(), 4)
    Firmware.log("****************************************", 4)
    ...
    if !sigma3_tbs_valid
      Firmware.log("MTR: sigma3_tbs does not have a valid signature", 2)
      Firmware.log("MTR: ******************* Invalid signature, trying anyways", 2)
      # Firmware.log("MTR: StatusReport(General Code: FAILURE, ProtocolId: SECURE_CHANNEL, ProtocolCode: INVALID_PARAMETER)", 2)
      # self.send_status_report(msg, 0x01, 0x0000, 0x0002, false)
      # return false
    else
      # All good, compute new keys
      Firmware.log("MTR: Sigma3 verified, computing new keys", 3)
    end

    TranscriptHash = crypto.SHA256().update(session.__Msg1).update(session.__Msg2).update(sigma3.Msg3).out()
    Firmware.log("MTR: * __Msg1            = " + session.__Msg1.tohex(), 4)
    Firmware.log("MTR: * __Msg2            = " + session.__Msg2.tohex(), 4)
    Firmware.log("MTR: * __Msg3            = " + sigma3.Msg3.tohex(), 4)
    Firmware.log("MTR: * TranscriptHash    = " + TranscriptHash.tohex(), 4)
    ...
    Firmware.log("MTR: ******************************", 4)
    Firmware.log("MTR: shared_secret =" + session.shared_secret.tohex(), 4)
    Firmware.log("MTR: ipk + hash    =" + (session.get_ipk_group_key() + TranscriptHash).tohex(), 4)
    # compute session key
    var session_keys = crypto.HKDF_SHA256().derive(session.shared_secret #- input key -#,
  ...
  def parse_StatusReport(msg)
    var session = msg.session
    Firmware.log("MTR: >Status    "+msg.raw[msg.app_payload_idx..].tohex(), 2)
