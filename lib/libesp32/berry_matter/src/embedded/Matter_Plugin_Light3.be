...
class Matter_Plugin_Light3 : Matter_Plugin_Light1
  ...
  def update_shadow()
    ...
    if hue != nil     hue = Firmware.scale_uint(hue, 0, 360, 0, 254)   else hue = self.shadow_hue      end
    if sat != nil     sat = Firmware.scale_uint(sat, 0, 255, 0, 254)   else sat = self.shadow_sat      end
  ...
  def invoke_request(session, val, ctx)
    ...
    if   cluster == 0x0300              # ========== Color Control 3.2 p.111 ==========
      if   command == 0x0000            # ---------- MoveToHue ----------
        ...
        var hue = Firmware.scale_uint(hue_in, 0, 254, 0, 360)
        light.set({'hue': hue})
        self.update_shadow()
        ...
        var sat = Firmware.scale_uint(sat_in, 0, 254, 0, 255)
        light.set({'sat': sat})
        self.update_shadow()
