...
class Matter_IM_Message
  ...
  def init(msg, opcode, reliable)
    ...
    self.expiration = Firmware.millis() + self.MSG_TIMEOUT
  ...
  def ack_received(msg)
    # Firmware.log("MTR: IM_Message ack_received exch="+str(self.resp.exchange_id), 3)
    self.expiration = Firmware.millis() + self.MSG_TIMEOUT     # give more time
    return false
  end
  ...
  def status_ok_received(msg)
    import string
    # Firmware.log(string.format("MTR: IM_Message status_ok_received exch=%i", self.resp.exchange_id), 3)
    self.expiration = Firmware.millis() + self.MSG_TIMEOUT     # give more time
    if msg
  ...
  def send_im(responder)
    import string
    # Firmware.log(string.format("MTR: IM_Message send_im exch=%i ready=%i", self.resp.exchange_id, self.ready ? 1 : 0), 3)
    if !self.ready   return false  end
    var resp = self.resp
    resp.encode_frame(self.data.to_TLV().tlv2raw())    # payload in cleartext
    resp.encrypt()
    Firmware.log(string.format("MTR: <snd       (%6i) id=%i exch=%i rack=%s", resp.session.local_session_id, resp.message_counter, resp.exchange_id, resp.ack_message_counter),3)
...
class Matter_IM_ReportData : Matter_IM_Message
  ...
  def send_im(responder)
    import string
    # Firmware.log(string.format("MTR: IM_ReportData send_im exch=%i ready=%i", self.resp.exchange_id, self.ready ? 1 : 0), 3)
    if !self.ready   return false  end
    ...
    # Firmware.log(string.format("MTR: exch=%i elements=%i msg_sz=%i total=%i", self.get_exchangeid(), elements, msg_sz, sz_attribute_reports), 3)
    var next_elemnts = []
    ...
    if was_chunked
      Firmware.log(string.format("MTR: .Read_Attr next_chunk exch=%i", self.get_exchangeid()), 3)
    end
    if data.more_chunked_messages
      if !was_chunked
        Firmware.log(string.format("MTR: .Read_Attr first_chunk exch=%i", self.get_exchangeid()), 3)
      end
      # Firmware.log("MTR: sending TLV" + str(data), 4)
    end
    ...
    if size(next_elemnts) > 0
      ...
      Firmware.log(string.format("MTR: to_be_sent_later size=%i exch=%i", size(data.attribute_reports), resp.exchange_id), 3)
...
class Matter_IM_ReportDataSubscribed : Matter_IM_ReportData
  ...
  def ack_received(msg)
    # import string
    # Firmware.log(string.format("MTR: IM_ReportDataSubscribed ack_received sub=%i", self.sub.subscription_id), 3)
  ...
  def status_error_received(msg)
    # import string
    # Firmware.log(string.format("MTR: IM_ReportDataSubscribed status_error_received sub=%i exch=%i", self.sub.subscription_id, self.resp.exchange_id), 3)
    self.sub.remove_self()
  end
  ...
  def status_ok_received(msg)
    # import string
    # Firmware.log(string.format("MTR: IM_ReportDataSubscribed status_ok_received sub=%i exch=%i", self.sub.subscription_id, self.resp.exchange_id), 3)
    if self.report_data_phase
  ...
  def send_im(responder)
    import string
    # Firmware.log(string.format("MTR: IM_ReportDataSubscribed send sub=%i exch=%i ready=%i", self.sub.subscription_id, self.resp.exchange_id, self.ready ? 1 : 0), 3)
    # Firmware.log(string.format("MTR: ReportDataSubscribed::send_im size(self.data.attribute_reports)=%i ready=%s report_data_phase=%s", size(self.data.attribute_reports), str(self.ready), str(self.report_data_phase)), 3)
    if !self.ready   return false  end
    if size(self.data.attribute_reports) > 0      # do we have still attributes to send
      if self.report_data_phase
        super(self).send_im(responder)
        # Firmware.log(string.format("MTR: ReportDataSubscribed::send_im called super finish=%i", self.finish), 3)
        ...
        resp.encode_frame()
        resp.encrypt()
        Firmware.log(string.format("MTR: <Ack       (%6i) ack=%i id=%i", resp.session.local_session_id, resp.ack_message_counter, resp.message_counter), 3)
        responder.send_response_frame(resp)
        self.last_counter = resp.message_counter
        self.finish = true
      end
...
class Matter_IM_SubscribedHeartbeat : Matter_IM_ReportData
  ...
  def ack_received(msg)
    # import string
    # Firmware.log(string.format("MTR: Matter_IM_SubscribedHeartbeat ack_received sub=%i", self.sub.subscription_id), 3)
    super(self).ack_received(msg)
    self.finish = true
    return true                       # proceed to calling send() which removes the message
  end
  ...
  def status_error_received(msg)
    # import string
    # Firmware.log(string.format("MTR: Matter_IM_SubscribedHeartbeat status_error_received sub=%i exch=%i", self.sub.subscription_id, self.resp.exchange_id), 3)
    self.sub.remove_self()
    return false                            # let the caller to the ack
  end
  ...
  def status_ok_received(msg)
    # import string
    # Firmware.log(string.format("MTR: Matter_IM_SubscribedHeartbeat status_ok_received sub=%i exch=%i", self.sub.subscription_id, self.resp.exchange_id), 3)
    return false                            # let the caller to the ack
  end
  ...
  def send_im(responder)
    # import string
    # Firmware.log(string.format("MTR: Matter_IM_SubscribedHeartbeat send sub=%i exch=%i ready=%i", self.sub.subscription_id, self.resp.exchange_id, self.ready ? 1 : 0), 3)
    if !self.ready   return false  end
...
class Matter_IM_SubscribeResponse : Matter_IM_ReportData
  ...
  def send_im(responder)
    # import string
    # Firmware.log(string.format("MTR: Matter_IM_SubscribeResponse send sub=%i ready=%i", self.sub.subscription_id, self.ready ? 1 : 0), 3)
    if !self.ready   return false  end
    if self.report_data_phase
      super(self).send_im(responder)
      if self.finish
      ...
      responder.send_response_frame(resp)
      self.last_counter = resp.message_counter
      # Firmware.log(string.format("MTR: Send SubscribeResponseMessage sub=%i id=%i", self.sub.subscription_id, resp.message_counter), 3)
      self.sub.re_arm()
  ...
  def status_ok_received(msg)
    import string
    # Firmware.log(string.format("MTR: IM_SubscribeResponse status_ok_received sub=%i exch=%i ack=%i last_counter=%i", self.sub.subscription_id, self.resp.exchange_id, msg.ack_message_counter ? msg.ack_message_counter : 0 , self.last_counter), 3)
    # once we receive ack, open flow for subscriptions
    Firmware.log(string.format("MTR: >Sub_OK    (%6i) sub=%i", msg.session.local_session_id, self.sub.subscription_id), 2)
    return super(self).status_ok_received(msg)
  end
