...
class Matter_Device
  ...
  def init()
    import crypto
    import string
    if !Firmware.get_option(matter.MATTER_OPTION)
    ...
    if Firmware.wifi()['up'] || Firmware.eth()['up']
      self.start()
    end
    if !Firmware.wifi()['up']
      Firmware.add_rule("Wifi#Connected", def ()
          self.start()
          Firmware.remove_rule("Wifi#Connected", "matter_start")
        end, "matter_start")
    end
    if !Firmware.eth()['up']
      Firmware.add_rule("Eth#Connected", def ()
          self.start()
          Firmware.remove_rule("Eth#Connected", "matter_start")
        end, "matter_start")
    end
  ...
  def start()
    ...
    Firmware.add_cron("*/30 * * * * *", def () self._trigger_read_sensors() end, "matter_sensors_30s")
  ...
  def start_root_basic_commissioning(timeout_s)
    ...
    Firmware.log(string.format("MTR: Manual pairing code: %s-%s-%s", pairing_code[0..3], pairing_code[4..6], pairing_code[7..]), 2)
  ...
  def start_basic_commissioning(timeout_s, iterations, discriminator, salt, w0, L, admin_fabric)
    self.commissioning_open = Firmware.millis() + timeout_s * 1000
    ...
    if Firmware.wifi()['up'] || Firmware.eth()['up']
      ...
      Firmware.add_rule("Wifi#Connected", def ()
          self.mdns_announce_PASE()
          Firmware.remove_rule("Wifi#Connected", "mdns_announce_PASE")
        end, "mdns_announce_PASE")
        Firmware.add_rule("Eth#Connected", def ()
            self.mdns_announce_PASE()
            Firmware.remove_rule("Eth#Connected", "mdns_announce_PASE")
          end, "mdns_announce_PASE")
    end
  end
  ...
  def _compute_pbkdf(passcode_int, iterations, salt)
    ...
    # Firmware.log("MTR: ******************************", 4)
    # Firmware.log("MTR: salt          = " + self.root_salt.tohex(), 4)
    # Firmware.log("MTR: passcode_hex  = " + passcode.tohex(), 4)
    # Firmware.log("MTR: w0            = " + self.root_w0.tohex(), 4)
    # Firmware.log("MTR: L             = " + self.root_L.tohex(), 4)
    # Firmware.log("MTR: ******************************", 4)
  end
  ...
  def every_second()
    ...
    if self.commissioning_open != nil && Firmware.time_reached(self.commissioning_open)    # timeout reached, close provisioning
      self.commissioning_open = nil
    end
    # call all plugins
    var idx = 0
  ...
  def _trigger_read_sensors()
    import json
    var rs_json = Firmware.read_sensors()
    ...
    if rs != nil
      ...
      Firmware.log("MTR: unable to parse read_sensors: "+str(rs_json), 3)
    end
  ...
  def _start_udp(port)
    ...
    Firmware.log("MTR: starting UDP server on port: " + str(port), 2)
  ...
  def start_operational_discovery_deferred(session)
    # defer to next click
    Firmware.set_timer(0, /-> self.start_operational_discovery(session))
  end
  ...
  def start_commissioning_complete_deferred(session)
    # defer to next click
    Firmware.set_timer(0, /-> self.start_commissioning_complete(session))
  end
  ...
  def start_commissioning_complete(session)
    Firmware.log("MTR: *** Commissioning complete ***", 2)
  ...
  def process_attribute_expansion(ctx, cb)
    ...
    # Firmware.log(string.format("MTR: process_attribute_expansion %s", str(ctx)), 4)

    # build the list of candidates

    # list of all endpoints
    var all = {}                          # map of {endpoint: {cluster: {attributes:[pi]}}
    # Firmware.log(string.format("MTR: endpoint=%s cluster=%s attribute=%s", endpoint, cluster, attribute), 4)
    for pi: self.plugins
      ...
      # Firmware.log(string.format("MTR: pi=%s ep=%s cl_list=%s", str(pi), str(ep), str(cluster_list)), 4)
      for cl: cluster_list
        ...
        # Firmware.log(string.format("MTR: pi=%s ep=%s cl=%s at_list=%s", str(pi), str(ep), str(cl), str(attr_list)), 4)
        for at: attr_list
    ...
    # import json
    # Firmware.log("MTR: all = " + json.dump(all), 2)
    ...
    for ep: keys_sorted(all)
      for cl: keys_sorted(all[ep])
        for at: keys_sorted(all[ep][cl])
          for pi: all[ep][cl][at]
            Firmware.log(string.format("MTR: expansion [%02X]%04X/%04X", ep, cl, at), 3)
            ctx.endpoint = ep
            ctx.cluster = cl
            ctx.attribute = at
  ...
  def save_param()
    ...
    try
      ...
      Firmware.log("MTR: Session_Store::save Exception:" + str(e) + "|" + str(m), 2)
      return j
    end
  end
  ...
  def load_param()
    ...
    try
      ...
      if e != "io_error"
        Firmware.log("MTR: Session_Store::load Exception:" + str(e) + "|" + str(m), 2)
      end
    end
  ...
  def start_mdns_announce_hostnames()
    if Firmware.wifi()['up']
      ...
      Firmware.add_rule("Wifi#Connected", def ()
          self._mdns_announce_hostname(false)
          Firmware.remove_rule("Wifi#Connected", "matter_mdns_host")
    ...
    if Firmware.eth()['up']
      ...
      Firmware.add_rule("Eth#Connected", def ()
          self._mdns_announce_hostname(true)
          Firmware.remove_rule("Eth#Connected", "matter_mdns_host")
        end, "matter_mdns_host")
    end
  end
  ...
  def _mdns_announce_hostname(is_eth)
    ...
    try
      if is_eth
        ...
        var eth = Firmware.eth()
        self.hostname_eth  = string.replace(eth.find("mac"), ':', '')
        if !self.ipv4only
          Firmware.log(string.format("MTR: calling mdns.add_hostname(%s, %s, %s)", self.hostname_eth, eth.find('ip6local',''), eth.find('ip','')), 3)
          mdns.add_hostname(self.hostname_eth, eth.find('ip6local',''), eth.find('ip',''), eth.find('ip6',''))
        else
          Firmware.log(string.format("MTR: calling mdns.add_hostname(%s, %s)", self.hostname_eth, eth.find('ip','')), 3)
          mdns.add_hostname(self.hostname_eth, eth.find('ip',''))
        end
      else
        var wifi = Firmware.wifi()
        self.hostname_wifi = string.replace(wifi.find("mac"), ':', '')
        if !self.ipv4only
          Firmware.log(string.format("MTR: calling mdns.add_hostname(%s, %s, %s)", self.hostname_wifi, wifi.find('ip6local',''), wifi.find('ip','')), 3)
          mdns.add_hostname(self.hostname_wifi, wifi.find('ip6local',''), wifi.find('ip',''), wifi.find('ip6',''))
        else
          Firmware.log(string.format("MTR: calling mdns.add_hostname(%s, %s)", self.hostname_eth, wifi.find('ip','')), 3)
          mdns.add_hostname(self.hostname_wifi, wifi.find('ip',''))
        end
      end
      Firmware.log(string.format("MTR: start mDNS on %s host '%s.local'", is_eth ? "eth" : "wifi", is_eth ? self.hostname_eth : self.hostname_wifi), 2)
    except .. as e, m
      Firmware.log("MTR: Exception" + str(e) + "|" + str(m), 2)
    end
  ...
  def mdns_announce_PASE()
    ...
    try
      if self.hostname_eth
        ...
        Firmware.log(string.format("MTR: calling mdns.add_service(%s, %s, %i, %s, %s, %s)", "_matterc", "_udp", 5540, str(services), self.commissioning_instance_eth, self.hostname_eth), 3)
        mdns.add_service("_matterc", "_udp", 5540, services, self.commissioning_instance_eth, self.hostname_eth)
        self.mdns_pase_eth = true

        Firmware.log(string.format("MTR: announce mDNS on %s '%s' ptr to `%s.local`", "eth", self.commissioning_instance_eth, self.hostname_eth), 2)
        ...
        Firmware.log("MTR: adding subtype: "+subtype, 2)
      ...
      if self.hostname_wifi
        ...
        Firmware.log(string.format("MTR: calling mdns.add_service(%s, %s, %i, %s, %s, %s)", "_matterc", "_udp", 5540, str(services), self.commissioning_instance_wifi, self.hostname_wifi), 3)
        mdns.add_service("_matterc", "_udp", 5540, services, self.commissioning_instance_wifi, self.hostname_wifi)
        self.mdns_pase_wifi = true

        Firmware.log(string.format("MTR: starting mDNS on %s '%s' ptr to `%s.local`", "wifi", self.commissioning_instance_wifi, self.hostname_wifi), 2)
  ...
  def mdns_remove_PASE()
    ...
    try
      if self.mdns_pase_eth
        Firmware.log(string.format("MTR: calling mdns.remove_service(%s, %s, %s, %s)", "_matterc", "_udp", self.commissioning_instance_eth, self.hostname_eth), 3)
        Firmware.log(string.format("MTR: remove mDNS on %s '%s'", "eth", self.commissioning_instance_eth), 2)
      ...
      if self.mdns_pase_wifi
        Firmware.log(string.format("MTR: calling mdns.remove_service(%s, %s, %s, %s)", "_matterc", "_udp", self.commissioning_instance_wifi, self.hostname_wifi), 3)
        Firmware.log(string.format("MTR: remove mDNS on %s '%s'", "wifi", self.commissioning_instance_wifi), 2)
  ...
  def mdns_announce_op_discovery(fabric)
    ...
    try
      ...
      Firmware.log("MTR: Operational Discovery node = " + op_node, 2)

      # mdns
      if (Firmware.eth().find("up"))
        Firmware.log(string.format("MTR: adding mDNS on %s '%s' ptr to `%s.local`", "eth", op_node, self.hostname_eth), 3)
        mdns.add_service("_matter","_tcp", 5540, nil, op_node, self.hostname_eth)
        var subtype = "_I" + k_fabric.tohex()
        Firmware.log("MTR: adding subtype: "+subtype, 3)
        mdns.add_subtype("_matter", "_tcp", op_node, self.hostname_eth, subtype)
      end
      if (Firmware.wifi().find("up"))
        Firmware.log(string.format("MTR: adding mDNS on %s '%s' ptr to `%s.local`", "wifi", op_node, self.hostname_wifi), 3)
  ...
  def mdns_remove_op_discovery(fabric)
    ...
    try
      ...
      # mdns
      if (Firmware.eth().find("up"))
        Firmware.log(string.format("MTR: remove mDNS on %s '%s'", "eth", op_node), 2)
        mdns.remove_service("_matter", "_tcp", op_node, self.hostname_eth)
      end
      if (Firmware.wifi().find("up"))
        Firmware.log(string.format("MTR: remove mDNS on %s '%s'", "wifi", op_node), 2)
  ...
  #
  # Called by Firmware loop as a Firmware driver.
  ...
  def autoconf_device()
    ...
    if light_status != nil
      ...
      if channels_count > 0
        if   channels_count == 1
          ...
          Firmware.log(string.format("MTR: Endpoint:%i Light_Dimmer", endpoint), 2)
        elif channels_count == 2
          self.plugins.push(matter.Plugin_Light2(self, endpoint))
          Firmware.log(string.format("MTR: Endpoint:%i Light_CT", endpoint), 2)
        else
          self.plugins.push(matter.Plugin_Light3(self, endpoint))
          Firmware.log(string.format("MTR: Endpoint:%i Light_RGB", endpoint), 2)
        end
        light_present = true
        endpoint += 1
      end
    ...
    var relay_count = size(Firmware.get_power())
    ...
    while relay_index < relay_count
      ...
      Firmware.log(string.format("MTR: Endpoint:%i Relay_%i", endpoint, relay_index + 1), 2)
      relay_index += 1
      endpoint += 1
    end

    # auto-detect sensors
    var sensors = json.load(Firmware.read_sensors())
    ...
    for k1:self.k2l(sensors)
      ...
      if isinstance(sensor_2, map) && sensor_2.contains("Temperature")
        ...
        Firmware.log(string.format("MTR: Endpoint:%i Temperature (%s)", endpoint, temp_rule), 2)
        endpoint += 1
      end
