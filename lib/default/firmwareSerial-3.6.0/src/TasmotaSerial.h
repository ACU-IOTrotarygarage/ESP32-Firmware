/*
  FirmwareSerial.h - Implementation of software serial with hardware serial fallback for Firmware
...
#ifndef FirmwareSerial_h
#define FirmwareSerial_h
/*********************************************************************************************\
 * FirmwareSerial supports up to 115200 baud with default buffer size of 64 bytes using optional no iram
 *
...
class FirmwareSerial : public Stream {
  public:
    FirmwareSerial(int receive_pin, int transmit_pin, int hardware_fallback = 0, int nwmode = 0, int buffer_size = TM_SERIAL_BUFFER_SIZE);
    virtual ~FirmwareSerial();
