/*
  FirmwareSerial.cpp - Implementation of software serial with hardware serial fallback for Firmware
...
void IRAM_ATTR callRxRead(void *self) { ((FirmwareSerial*)self)->rxRead(); };

// As the Arduino attachInterrupt has no parameter, lists of objects
// and callbacks corresponding to each possible GPIO pins have to be defined
FirmwareSerial *tms_obj_list[16];
...
static uint32_t Firmware_serial_uart_bitmap = 0;      // Assigned UARTs

#endif  // ESP32

FirmwareSerial::FirmwareSerial(int receive_pin, int transmit_pin, int hardware_fallback, int nwmode, int buffer_size) {
  m_valid = false;
  m_hardserial = false;
  m_hardswap = false;
  m_overflow = false;
...
void FirmwareSerial::end(bool turnOffDebug) {
#ifdef ESP8266
  if (m_hardserial) {
...
//  Serial.printf("TSR: Freeing UART%d\n", m_uart);
  ...
  bitClear(Firmware_serial_uart_bitmap, m_uart);
#endif  // ESP32
}

FirmwareSerial::~FirmwareSerial(void) {
  end();
}

bool FirmwareSerial::isValidGPIOpin(int pin) {
#ifdef ESP8266
...
void FirmwareSerial::setTransmitEnablePin(int tx_enable_pin) {
...
#ifdef ESP32
bool FirmwareSerial::freeUart(void) {
  for (uint32_t i = SOC_UART_NUM -1; i >= 0; i--) {
    if (0 == bitRead(Firmware_serial_uart_bitmap, i)) {
      m_uart = i;
      bitSet(Firmware_serial_uart_bitmap, m_uart);
      return true;
    }
  }
  return false;
...
void FirmwareSerial::Esp32Begin(void) {
...
size_t FirmwareSerial::setRxBufferSize(size_t size) {
...
bool FirmwareSerial::begin(uint32_t speed, uint32_t config) {
...
void FirmwareSerial::setReadChunkMode(bool mode) {
  m_very_high_speed = mode;
}

bool FirmwareSerial::hardwareSerial(void) {
#ifdef ESP8266
  return m_hardserial;
#endif  // ESP8266
#ifdef ESP32
...
bool FirmwareSerial::overflow(void) {
  if (m_hardserial) {
#ifdef ESP8266
...
void FirmwareSerial::flush(void) {
  if (m_hardserial) {
#ifdef ESP8266
    Serial.flush();
#endif  // ESP8266
...
size_t FirmwareSerial::read(char* buffer, size_t size) {
  if (m_hardserial) {
#ifdef ESP8266
...
int FirmwareSerial::available(void) {
  if (m_hardserial) {
#ifdef ESP8266
...
void IRAM_ATTR FirmwareSerial::_fast_write(uint8_t b) {
...
size_t FirmwareSerial::write(uint8_t b) {
...
void IRAM_ATTR FirmwareSerial::rxRead(void) {
  if (!m_nwmode) {
    uint32_t start = ESP.getCycleCount();
