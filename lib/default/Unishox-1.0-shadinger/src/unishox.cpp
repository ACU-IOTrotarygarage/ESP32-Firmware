...
static const uint16_t RPT_CODE_Firmware = 0x3780;
static const uint16_t RPT_CODE_Firmware_LEN = 10;
...
static const uint16_t BIN_CODE_Firmware = 0x8000;
static const uint16_t BIN_CODE_Firmware_LEN = 3;
  ...
  for (l=0; l<len; l++) {
    ...
    if (l && l < len - 4) {
      if (c_in == in[l - 1] && c_in == in[l + 1] && c_in == in[l + 2] && c_in == in[l + 3]) {   // check for repeat
        ...
        append_bits(RPT_CODE_Firmware, RPT_CODE_Firmware_LEN);     // reusing CRLF for RPT
...
// Serial.printf("Encode %c %d\n", c_in + 32, c_in);
      ...
      append_bits(BIN_CODE_Firmware, BIN_CODE_Firmware_LEN);       // Binary, we reuse the Unicode marker which 3 bits instead of 9
