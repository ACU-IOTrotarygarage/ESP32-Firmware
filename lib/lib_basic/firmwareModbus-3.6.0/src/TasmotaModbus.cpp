  /*
  FirmwareModbus.cpp - Basic modbus wrapper for FirmwareSerial for Firmware
...
#include "Arduino.h"
#include "FirmwareModbus.h"

extern void AddLog(uint32_t loglevel, PGM_P formatP, ...);
enum LoggingLevels {LOG_LEVEL_NONE, LOG_LEVEL_ERROR, LOG_LEVEL_INFO, LOG_LEVEL_DEBUG, LOG_LEVEL_DEBUG_MORE};

//#define FirmwareMODBUSDEBUG

#define Firmware_MODBUS_TX_ENABLE        // Use local Tx enable on write buffer

FirmwareModbus::FirmwareModbus(int receive_pin, int transmit_pin, int tx_enable_pin) : FirmwareSerial(receive_pin, transmit_pin, 2)
{
#ifdef Firmware_MODBUS_TX_ENABLE
  mb_tx_enable_pin = tx_enable_pin;     // Use local Tx enable on write buffer
#else
  setTransmitEnablePin(tx_enable_pin);  // Use FirmwareSerial Tx enable on write byte
#endif  // Firmware_MODBUS_TX_ENABLE
  mb_address = 0;
}

uint16_t FirmwareModbus::CalculateCRC(uint8_t *frame, uint8_t num)
{
  uint16_t crc = 0xFFFF;
...
int FirmwareModbus::Begin(long speed, uint32_t config)
{
  int result = 0;
...
uint8_t FirmwareModbus::Send(uint8_t device_address, uint8_t function_code, uint16_t start_address, uint16_t count, uint16_t *write_data)
{
  uint8_t *frame;
...
bool FirmwareModbus::ReceiveReady()
{
  return (available() > 4);
}

uint8_t FirmwareModbus::ReceiveBuffer(uint8_t *buffer, uint8_t register_count, uint16_t byte_count)
{
  mb_len = 0;
...
uint8_t FirmwareModbus::Receive8BitRegister(uint8_t *value)
{
  //  0  1  2  3    4  5
  // 01 04 02 43    HH LL
  // Id Cc Sz Regis Crc--
...
uint8_t FirmwareModbus::Receive16BitRegister(uint16_t *value)
{
  //  0  1  2  3  4  5  6
...
uint8_t FirmwareModbus::Receive32BitRegister(float *value)
{
  //  0  1  2  3  4  5  6  7  8
